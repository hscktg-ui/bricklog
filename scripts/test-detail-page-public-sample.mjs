...
assert.ok(rice.documentHtml.includes("<table"));
assert.ok(rice.documentHtml.includes("<button"));
assert.ok(rice.documentHtml.includes("<dl"));
assert.ok(rice.documentHtml.includes("[자료 필요"));
assert.ok(rice.documentHtml.includes("<img "));
assert.equal(rice.documentHtml.includes('data-deliverable="image-stack"'), false);
assert.ok(rice.html.includes('data-layout="hero-banner"'));
assert.ok(rice.html.includes("산지"));
assert.ok(rice.html.includes("원재료"));
assert.ok(rice.html.includes('data-category-flow="grocery"'));
assert.ok(rice.html.includes("원물 매크로") || rice.html.includes("쌀알"));
...
assert.equal(rice.html.includes("/detail-sample/open-rice-observe.png"), false);
