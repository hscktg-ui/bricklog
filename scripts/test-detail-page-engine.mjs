...
assert.ok(DETAIL_PAGE_PRODUCT.headline.includes("웹페이지가 아니라"));
...
assert.ok(packToPlainText(pack).includes("산지"));
assert.ok(packToPlainText(pack).includes("원재료"));
assert.equal(packToPlainText(pack).includes("기준 1"), false);
assert.equal(pack._meta.categoryFlow.id, "grocery");
...
assert.equal(packToPlainText(pack).includes("크게 외치지 않습니다"), false);
assert.equal(packToPlainText(pack).includes("기준만 챙기면 됩니다"), false);
assert.ok(packToPlainText(pack).includes("[자료 필요"));
...
assert.ok(html.includes('data-category-flow="grocery"'));
...
assert.ok(html.includes('data-ui="ad-canvas"'));
assert.ok(html.includes('data-photo-slot="hero"'));
assert.ok(html.includes("data-photo-direction"));
...
assert.ok(html.includes('data-layout="ad-canvas"'));
assert.ok(html.includes('data-renderer="detail-canvas"'));
assert.ok(html.includes('data-ranking="naver-shop-rank"'));
assert.ok(html.includes('data-list-sample="creazy"'));
assert.ok(html.includes('data-composition="specification"'));
assert.ok(html.includes("<h1"));
assert.ok(html.includes('data-cta="close"'));
assert.equal(html.includes("<button"), false);
assert.equal(html.includes('data-layout="faq"'), false);
assert.ok(html.includes("[자료 필요"));
assert.equal(html.includes("크게 외치지"), false);

const doc = wrapSmartstoreHtml(html, pack);
assert.ok(doc.startsWith("<!DOCTYPE html>"));
assert.ok(doc.includes("<title>여주 햅쌀 10kg</title>") || doc.includes("브릭로그 상세"));
assert.ok(doc.includes('data-mall="smartstore"'));
