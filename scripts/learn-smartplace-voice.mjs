/**
 * 스마트플레이스 공지 톤 학습 — 네이버 API (web+blog)
 * Run: npm run learn:smartplace-voice
 */
import { mkdirSync, writeFileSync, readFileSync, appendFileSync } from "node:fs";
import { join, dirname } from "node:path";
import { fileURLToPath } from "node:url";
import { learnFromChannelVoiceSearch } from "../lib/channel/channelVoiceLearner.js";
import { buildSmartPlaceLearnQueries } from "../lib/channel/channelVoiceQueryCatalog.js";
...
const OUT_DIR = join(root, "artifacts", "smartplace-voice-learning");
...
async function main() {
  ...
  const queries = buildSmartPlaceLearnQueries(Math.ceil(args.target / args.perQuery) + 8);
  writeFileSync(SAMPLES_JSONL, "", "utf8");

  console.log(`smartplace-voice-learn: target=${args.target} queries=${queries.length}`);

  const result = await learnFromChannelVoiceSearch("smartplace", {
  ...
  console.log("examples:", profile.promptExamples?.length || 0);
