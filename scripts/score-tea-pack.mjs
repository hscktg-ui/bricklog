...
관광 동선 중 짧게 앉아 쉬어 가도, 혼자 조용히 시간을 내도 자연스럽게 어울리는 공간입니다. 가을 바깥 풍경을 바라보며 차를 우려내는 시간, 그 자체가 경주를 걷는 또 다른 방법이 됩니다.`,
    },
  ],
...
const g = assessGoldenQualityGate(pack, INPUT);
console.log(
  JSON.stringify(
    { chars: getBlogFullText(pack).replace(/\s/g, "").length, score: g.score, verdict: g.verdict },
    null,
    2
  )
);
