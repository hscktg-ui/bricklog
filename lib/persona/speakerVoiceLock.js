...
function topicEchoVariants(input = {}) {
  ...
  if (resolveBriclogIndustryKey(input) === "furniture") {
    variants.add(`${core} 전시`);
    variants.add(`${core} 쇼룸`);
  }
...
function nonFieldFacetPhrase(input = {}, topic = "") {
  const facet = resolveIndustryTopicPronouns(input)[0] || "안내";
  if (topic) return `${topic} 관련 ${facet}`;
  return facet;
}

function nonFieldFollowLead(input = {}) {
  const brand = String(input.brandName || "").trim();
  const key = resolveBriclogIndustryKey(input);
  const byKey = {
    furniture: "일정·대상 모델·프로모션 조건",
    salon: "시술 메뉴·예약·케어 조건",
    education: "수업 일정·대상·수강 조건",
    construction: "시공 범위·견적·일정",
    craft: "체험 프로그램·수강·준비물",
    flower: "꽃 구성·예약·픽업 조건",
    cafe: "메뉴·영업·좌석 조건",
    hospital: "진료·예약·준비 사항",
    pension: "객실·예약·이용 조건",
    marketing: "캠페인·일정·문의 조건",
  };
  const scope = byKey[key] || "일정·구성·프로모션 조건";
  if (brand) {
    return `${brand} 공식 안내 기준으로 ${koreanObjectParticle(scope)} 차례로 확인할 수 있습니다.`;
  }
  return `공식 안내 기준으로 ${koreanObjectParticle(scope)} 차례로 확인할 수 있습니다.`;
}

function bridgeLinesForIndustry(input = {}, topic = "") {
  ...
  const sets = {
    ...
    craft: [
      `${region} ${brand} ${topic} 프로그램·수강 조건은 공식 안내를 확인하면 됩니다.`,
      `체험 시간·준비물·인원 제한은 회차마다 다를 수 있어 예약 전에 확인하는 편이 좋습니다.`,
      `당일 예약·취소 규정은 시즌에 따라 달라질 수 있습니다.`,
      `${brand} 안내 기준으로 확인 가능한 범위만 정리했습니다.`,
    ],
    education: [
      `${region} ${brand} ${topic} 일정·대상은 공식 안내를 확인하면 됩니다.`,
      `수강 인원·교재·레벨 구분은 기수마다 다를 수 있어 상담 시 확인하는 편이 좋습니다.`,
      `등록·환불 규정은 개강 시점에 따라 달라질 수 있습니다.`,
      `${brand} 안내 기준으로 확인 가능한 범위만 정리했습니다.`,
    ],
    construction: [
      `${region} ${brand} ${topic} 시공 범위·견적은 공식 상담을 통해 확인하면 됩니다.`,
      `자재·공정·일정은 현장 조건에 따라 달라질 수 있어 상담 시 범위를 확인하는 편이 좋습니다.`,
      `A/S·하자 보수 조건은 계약서 기준으로 확인할 수 있습니다.`,
      `${brand} 안내 기준으로 확인 가능한 범위만 정리했습니다.`,
    ],
    salon: [
      `${region} ${brand} ${topic} 시술 메뉴·예약은 공식 안내를 확인하면 됩니다.`,
      `담당 디자이너·시술 시간은 매장마다 다를 수 있어 예약 전에 확인하는 편이 좋습니다.`,
      `케어·시술 후 관리 안내는 메뉴마다 달라질 수 있습니다.`,
      `${brand} 안내 기준으로 확인 가능한 범위만 정리했습니다.`,
    ],
    flower: [
      `${region} ${brand} ${topic} 구성·예약은 공식 안내를 확인하면 됩니다.`,
      `꽃 종류·포장·픽업 시간은 시즌에 따라 달라질 수 있어 주문 전에 확인하는 편이 좋습니다.`,
      `배달·픽업 조건은 주문 시점에 따라 달라질 수 있습니다.`,
      `${brand} 안내 기준으로 확인 가능한 범위만 정리했습니다.`,
    ],
    cafe: [
      `${region} ${brand} ${topic} 메뉴·좌석은 공식 안내를 확인하면 됩니다.`,
      `시즌 음료·브런치 구성은 매장마다 달라질 수 있어 방문 전에 확인하는 편이 좋습니다.`,
      `영업 시간·테이크아웃 조건은 시즌에 따라 달라질 수 있습니다.`,
      `${brand} 안내 기준으로 확인 가능한 범위만 정리했습니다.`,
    ],
    default: [
      `${region} ${brand} ${topic} 안내·조건은 공식 채널을 확인하면 됩니다.`,
      `메뉴·서비스·이용 조건은 시즌에 따라 달라질 수 있어 방문·예약 전에 확인하는 편이 좋습니다.`,
      `가격·혜택·일정은 매장·행사 조건에 따라 달라질 수 있습니다.`,
      `${brand} 안내 기준으로 확인 가능한 범위만 정리했습니다.`,
    ],
  };
...
function buildNonFieldOpeningLead(input = {}, archetype = "brand_editor") {
  ...
  const facet = nonFieldFacetPhrase(input, topic);
  const lead = [
    region && brand ? `${region} ${brand}` : brand || region,
    facet,
    "을 미리 정리해 두면 비교가 수월합니다.",
  ]
    .filter(Boolean)
    .join(" ");
  return `${lead} ${nonFieldFollowLead(input)}`;
}

function buildNonFieldSectionBridge(input = {}, sectionIndex = 1) {
  const topic = topicFacet(input).replace(/전시소식/g, "전시");
  const lines = bridgeLinesForIndustry(input, topic).filter(
    (l) => l.replace(/\s/g, "").length >= 12
  );
  const primary = lines[sectionIndex % lines.length] || lines[0];
  const secondary = lines[(sectionIndex + 2) % lines.length] || lines[1] || primary;
  return `${primary} ${secondary}`;
}

/** voice lock 후 outline_body(80자 미만) 방지 — 얇은 섹션만 보강 */
export function repairThinSectionsAfterVoiceLock(pack, input = {}) {
  if (!pack?.sections?.length || isFieldReviewSpeaker(input)) return pack;
  if (pack?._meta?.researchGroundedHumanPack) return pack;
  const sections = pack.sections.map((sec, idx) => {
    const body = String(sec?.body || "").trim();
    if (body.replace(/\s/g, "").length >= 80) return sec;
    const bridge = buildNonFieldSectionBridge(input, idx);
    const addon = buildNonFieldSectionBridge(input, idx + 3);
    const merged =
      body.replace(/\s/g, "").length >= 24
        ? `${body}\n\n${addon}`
        : `${bridge}\n\n${addon}`;
    return { ...sec, body: merged.trim() };
  });
  return {
    ...pack,
    sections,
...
function sanitizeNonFieldSectionBody(
  ...
  const prevLen = text.replace(/\s/g, "").length;
  if (!cleaned || cleaned.replace(/\s/g, "").length < 40) {
    cleaned = buildNonFieldSectionBridge(input, sectionIndex);
  } else if (bodyStillContaminated(cleaned, input)) {
    if (prevLen >= 120) {
      const sentenceScrub = filterContaminatedSentences(cleaned, input);
      cleaned =
        sentenceScrub && sentenceScrub.replace(/\s/g, "").length >= 80
          ? sentenceScrub
          : buildNonFieldSectionBridge(input, sectionIndex);
    } else {
      cleaned = buildNonFieldSectionBridge(input, sectionIndex);
