...
export function scrubSpeakerMismatchTitleOpening(pack, input = {}) {
  ...
  if (openingNeedsScrub(firstBody, title, input)) {
    let sanitized = sanitizeNonFieldOpeningParagraph(
      firstBody,
      input,
      profile.archetype,
      title
    );
    if (openingStillContaminated(sanitized, title, input)) {
      const paras = firstBody.split(/\n\n+/);
      const rest = paras.slice(1).join("\n\n").trim();
      const lead = buildNonFieldOpeningLead(input, profile.archetype);
      sanitized = rest ? `${lead}\n\n${rest}`.trim() : lead;
    }
    if (sanitized !== firstBody) {
