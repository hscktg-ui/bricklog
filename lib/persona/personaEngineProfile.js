...
export function scorePersonaEngineAlignment(pack, input = {}, profile = null) {
  ...
  const forbiddenHits = (prof.forbiddenVoice || []).filter((re) => re.test(full));
  ...
  if (prof.introMarkers && open.length > 30 && !prof.introMarkers.test(open)) {
  ...
  const arcHit = (prof.narrativeEmphasis || []).map((id) => {
    ...
    return step ? { id, ok: step.markers.test(full) } : { id, ok: false };
  });
