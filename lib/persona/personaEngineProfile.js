...
import { detectFieldReviewSurfaceLeak, detectOpeningContamination, detectSpeakerBodyContamination } from "@/lib/persona/speakerSurfacePatterns";
...
export function scorePersonaEngineAlignment(pack, input = {}, profile = null) {
  ...
  if (prof.archetype !== "field_review") {
    ...
    const bodySample = (pack?.sections || [])
      .map((s) => `${s.heading || ""} ${s.body || ""}`)
      .join(" ")
      .slice(0, 4000);
    if (detectSpeakerBodyContamination(bodySample).leak) {
      issues.push({ type: "speaker_body_visit_leak", persona: prof.id });
    }
  }
  ...
  if (issues.some((i) => i.type === "speaker_body_visit_leak")) score -= 18;
  ...
  const ok =
    ...
    !issues.some((i) =>
      i.type === "speaker_title_visit_leak" ||
      i.type === "speaker_opening_visit_leak" ||
      i.type === "speaker_body_visit_leak"
    );
...
export function buildPersonaEngineRegenNote(pack, input = {}) {
  ...
  for (const issue of alignment.issues || []) {
    ...
    } else if (issue.type === "speaker_body_visit_leak") {
      lines.push("본문 전체에서 방문·체험·현장 후기 톤과 네이버 스니펫 잔재를 제거하고 선택 화자 톤을 유지하세요.");
    }
  }
