...
export const OPENING_CONTAMINATION_RES = [
  ...
  /산뜻한\s*침대추천/,
  /신혼침대-/,
  /[가-힣A-Za-z0-9]{2,20}후기\s*:/,
  /알려드릴(?:게요|드)/,
  /구매한게/,
  /견적\s*후기/,
  /선택하게\s*되었는데\s*오늘은/,
  /우리\s*모두\s*잠이\s*중요/,
  /.{0,20}관련해서\s*.{0,32}(?:막히|분위기|고민)/,
  /혼수리스트/,
  /에이스스퀘어\.{2,}/i,
  /\.{3,}\s*:/,
  /\(LUCE/i,
...
export const NON_FIELD_SOFT_EXPERIENCE_RES = [
  ...
  /바로\s*결정하지\s*않고/,
  /현장(?:에서|)\s*.{0,32}(?:들|메모|본|이야기)/,
  /쇼룸\s*이번/,
  /직원분(?:이|께서)\s*.{0,24}(?:짚|설명)/,
];
...
/** @deprecated alias — 본문 전역 scrub에서도 동일 규칙 */
export function isSpeakerBodySentenceContaminated(sentence = "") {
  return isOpeningSentenceContaminated(sentence);
}

export function detectSpeakerBodyContamination(text = "") {
  return detectOpeningContamination(text);
}
