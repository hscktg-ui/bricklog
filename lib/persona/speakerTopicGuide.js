...
import { isExplicitVisitReviewInput } from "@/lib/content/writingContract";
...
export function resolveSpeakerTopicKind(input = {}) {
  if (isExplicitVisitReviewInput(input)) return "visit";
...
export function assessVisitSpeakerAlignment(input = {}) {
  if (!isExplicitVisitReviewInput(input)) return { ok: true, kind: "visit" };
...
export function getSpeakerTopicGuidance(input = {}) {
  ...
  return {
    ...
    message:
      speaker === "auto"
        ? "주제에 맞는 화자를 추천해 드려요. 어색하면 아래에서 바꿔 보세요."
        : "주제에 맞게 화자를 고르거나 「추천」을 쓸 수 있어요.",
  };
}
