...
/** Landing signature — one quiet confirmation (intro complete only) */
export async function playSignatureSound() {
  const ctx = await readyContext();
  if (!ctx) return;
  dispatchSfxPlay();
  scheduleTone(ctx, { freq: 523.25, start: 0, duration: 0.16, gain: 0.032, type: "sine" });
  scheduleTone(ctx, { freq: 659.25, start: 0.07, duration: 0.2, gain: 0.022, type: "sine" });
}
