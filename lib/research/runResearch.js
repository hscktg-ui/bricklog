...
export async function runResearch({ query, types = [], brandContext = {} }) {
  ...
  const typeLabels = researchTypeLabels(types);
  const system = `You are BRICLOG Research Mode — Korean content research assistant.
Return ONLY one JSON object:
{
  "summary": "3-6 sentences in Korean",
  "keywords": ["..."],
...
브랜드: ${brandContext.brandName || "(미입력)"}
지역: ${brandContext.region || "(미입력)"}
업종: ${brandContext.industry || "(미입력)"}
메인 키워드: ${brandContext.mainKeyword || "-"}
