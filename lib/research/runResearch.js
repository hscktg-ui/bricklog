...
import { getResearchSynthesisMaxTokens, getChannelTokenBudget } from "@/lib/config/channelTokenBudget";
import { isOfficialSourceFirstEnabled } from "@/lib/config/brandEngineFlags";
import { getNaverMaxResultsCap } from "@/lib/config/researchStackA";
import {
...
function isFastResearchMode() {
  if (isBriclogMaxQualityEnabled()) return false;
  return process.env.BRICLOG_FAST_RESEARCH !== "0";
}

function isSlaFastResearch(brandContext = {}) {
  return Boolean(brandContext.slaFastResearch) || isCustomerTwoMinuteSlaMode();
}
...
async function gatherWebLeads(query, brandContext = {}) {
  ...
  const productVariant = productBase
    ...
    .replace(/\s+/g, " ")
    .trim();
  const maxQ = getNaverMaxQueries(brandContext);
  const queries = [
  ...
  try {
    const leads = await fetchWebLeadsForQueries(queries, {
      ...
      brandContext: { officialDomain, brandName: brandContext.brandName },
    });
    ...
    if (!isFastResearchMode() && !useGeminiResearchProvider()) {
...
export async function runResearch({
  ...
  brandContext.knowledgeCoverage =
    brandContext.knowledgeCoverage ||
    (isSlaFastResearch(brandContext)
      ? null
...
네이버 검색 스니펫이 있으면 그 안에서만 확인된 사실을 researchFacts에 반영. 브랜드 작업실·공식자료가 우선.`;
    ...
    if (!parsed && isOpenAIConfigured()) {
      const sla = isSlaFastResearch(brandContext);
      const researchMaxTokens = depthPass
        ? getResearchSynthesisMaxTokens(sla)
        : sla
          ? getChannelTokenBudget("research", "expandSla")
          : getChannelTokenBudget("research", "expandFull");
      const raw = await callOpenAIChat(
        ...
        depthPass
          ? { temperature: 0.4, maxTokens: researchMaxTokens }
          : { temperature: 0.45, maxTokens: researchMaxTokens }
      );
    ...
    if (isSlaFastResearch(brandContext)) {
      parsed.mode = parsed.mode || "sla_fast_research";
      return parsed;
    }
