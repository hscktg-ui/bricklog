...
export function reinterpretSignal(signal, ctx = {}) {
  ...
  if (ctx.nonField) {
    if (/클래스|체험|원데이|공방|도자|공예/.test(s)) {
      return `${s} — 소요 시간·난이도·준비물은 예약 전 안내 확인`;
    }
    if (/특강|학원|수업|내신|방학/.test(s)) {
      return `${s} — 대상 학년·반 편성·등록 일정은 상담 안내 기준`;
    }
    if (/염색|펌|시술|두피/.test(s)) {
      return `${s} — 시술 순서·소요 시간·관리 방법은 상담 안내 기준`;
    }
    return `${s} — 운영·예약 조건은 공식 안내 기준`;
  }
  return `${s} — 현장에서 확인한 운영 포인트`;
}

export function reinterpretSignals(signals = [], ctx = {}) {
  const voices = [];
  const seen = new Set();
  for (const sig of signals) {
    const v = reinterpretSignal(sig, ctx);
    if (v && !seen.has(v)) {
      seen.add(v);
      voices.push(v);
    }
