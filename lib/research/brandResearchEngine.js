...
function buildBrandUniqueness(user, input = {}) {
  const brand = user.brandName || "이 브랜드";
  const region = user.region ? `${user.region} ` : "";
  const { key, flavor } = getIndustryFlavorForInput(input);
  if (key === "craft") return `${brand} — ${region}체험·원데이 클래스 운영 맥락`;
  if (key === "education") return `${brand} — ${region}학원·특강·내신 상담 맥락`;
  if (key === "salon") return `${brand} — ${region}시술·두피·컬러 상담 맥락`;
  if (key === "flower" || key === "unmanned_flower") return `${brand} — ${region}꽃·포장·픽업 맥락`;
  return `${brand} — ${region}${flavor.label || "매장"} 운영·예약 맥락`;
}
...
function isNonFieldResearchVoice(input = {}) {
  const speaker = String(input.v4Speaker || input.speaker || "").trim();
  return speaker && !/^(real_use|field_review)$/i.test(speaker);
}

function buildBrandSummary(user, searchVoices, collection, profile, input = {}) {
  const ctx = { nonField: isNonFieldResearchVoice(input) };
  const traits = mergeUserOverSearch(
    [
      ...
      ...user.includeList.map((s) => reinterpretSignals([s], ctx)[0] || s),
    ].filter(Boolean),
    searchVoices
  );
  ...
  const operationStyle =
    ...
    (user.includeList[0] ? reinterpretSignals([user.includeList[0]], ctx)[0] : null) ||
  ...
  return {
    ...
    uniqueness: profile
      ...
      : user.brandName
        ? buildBrandUniqueness(user, input)
        : "사용자 입력을 우선합니다",
  };
}
...
export function runBrandResearchEngine(input = {}) {
  ...
  const searchVoices = reinterpretSignals(collection.signals, {
    nonField: isNonFieldResearchVoice(input),
  });
  const summary = buildBrandSummary(user, searchVoices, collection, profile, input);
