...
} from "@/lib/config/researchStackA";
import {
...
export async function fetchWebLeadsForQueries(queries = [], opts = {}) {
  const naverOpts = resolveNaverLeadFetchOptions({
    maxQueries: opts.maxQueries ?? 4,
  });
  const maxQueries = naverOpts.maxQueries;
  const perQuery = opts.perQuery ?? naverOpts.perQuery;
  const uniqueQ = [];
  const seenQ = new Set();
  ...
  if (isNaverSearchConfigured()) {
    try {
      const naver = await fetchNaverLeadsForQueries(uniqueQ, {
        maxQueries,
        perQuery: Math.min(perQuery, 10),
        maxResults: opts.maxResults ?? naverOpts.maxResults,
        includeBlog: naverOpts.includeBlog,
        includeWeb: naverOpts.includeWeb,
        includeNews: naverOpts.includeNews,
      });
      ingestSearchBatch(merged, seenUrl, providers, naver);
    } catch {
      /* 브랜드 메모리·로컬 단서로 계속 */
    }
  } else if (isGoogleSearchEnabledInV12()) {
  ...
  if (isGoogleCseSupplementEnabled()) {
    const cseQueries = buildCseSupplementQueries(uniqueQ, opts.brandContext || {});
    for (const q of cseQueries) {
      try {
        const batch = await fetchGoogleCse(q, { num: Math.min(perQuery, 6) });
        ingestSearchBatch(merged, seenUrl, providers, batch);
      } catch {
        /* optional supplement */
      }
...
export function formatWebLeadsForPrompt(leads) {
  ...
  const lines = [
    "【네이버·공식 검색 실마리 — 재료만, 복사 금지 · 브랜드 메모리 관점으로 재해석】",
