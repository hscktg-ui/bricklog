...
import { coalesceBlogGenerationInput } from "@/lib/workspace/brandFormSync";
...
export function useBufferedObjectPatch(values, onChange, options = {}) {
  ...
  useEffect(() => {
    ...
    const axisKeys = ["brandName", "region", "topic", "mainKeyword"];
    const localHasAxis = axisKeys.some((k) => String(local[k] ?? "").trim());
    const next = localHasAxis
      ? { ...incoming, ...coalesceBlogGenerationInput(incoming, local) }
      : incoming;
    setLocal(next);
