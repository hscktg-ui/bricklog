...
import { mergeBrandFormSyncPayload, formAxisSignature } from "@/lib/workspace/brandFormSync";
...
export function useBufferedObjectPatch(values, onChange, options = {}) {
  ...
  useEffect(() => {
    ...
    const next = mergeBrandFormSyncPayload(incoming, local);
    setLocal(next);
    latestRef.current = next;
    if (!deferParentSync) onDraftChange?.(next);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- brand·축 변경 시 부모 values 동기화
  }, [values[syncKey], formAxisSignature(values), deferParentSync, onDraftChange]);
  ...
  const mergeWithLatest = useCallback((partial) => {
  ...
  const applyLocal = useCallback(
    (partial) => {
      const next = mergeWithLatest(partial);
      setLocal(next);
      notifyDraftNow(next);
      if (!deferParentSync) onChangeRef.current(next);
    },
    [mergeWithLatest, notifyDraftNow, deferParentSync]
  );
  ...
  const patch = useCallback(
    (partial) => {
      ...
      if (debounce) {
        // deferParentSync: 상위 Context는 생성 시 flush — draft·검증 UI는 즉시 갱신
    ...
    [bufferedKeys, schedule, flush, notifyDraftNow, deferParentSync, mergeWithLatest]
  );
  ...
  const patchImmediate = useCallback(
    ...
    [deferParentSync, applyLocal, flush, notifyDraftNow, mergeWithLatest]
  );
