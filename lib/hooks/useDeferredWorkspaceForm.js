"use client";

import { useCallback, useEffect, useRef, useState } from "react";
import {
  BRAND_FORM_SYNC_EVENT,
  mergeBrandFormSyncPayload,
  mergeLiveFormWithCommitted,
  formAxisSignature,
} from "@/lib/workspace/brandFormSync";

/**
 * 채널 브리프 — Context(blogInput 등)는 생성 CTA에서만 flush
 * @param {object} committedValues
 * @param {(next: object) => void} setCommitted
 * @param {string} [syncKey]
 */
export function useDeferredWorkspaceForm(
  committedValues,
  setCommitted,
  syncKey = "brandId"
) {
  const [draft, setDraft] = useState(committedValues);
  const formApiRef = useRef(null);
  const committedRef = useRef(committedValues);
  committedRef.current = committedValues;

  const applyExternalForm = useCallback((form) => {
    if (!form) return;
    setDraft(form);
    formApiRef.current?.replaceAll?.(form);
  }, []);

  const committedAxisSig = formAxisSignature(committedValues);

  useEffect(() => {
    const cur =
      formApiRef.current?.getValues?.() ??
      committedRef.current ??
      draft;
    const merged = mergeBrandFormSyncPayload(committedValues, cur);
    applyExternalForm(merged);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- 브랜드·축 변경 시 committed 동기화
  }, [committedValues[syncKey], committedAxisSig, applyExternalForm]);

  useEffect(() => {
    const onBrandSync = (e) => {
      const form = e.detail?.form;
      if (!form) return;
      const live =
        formApiRef.current?.getValues?.() ??
        committedRef.current ??
        draft;
      const merged = mergeBrandFormSyncPayload(form, live);
      applyExternalForm(merged);
    };
    window.addEventListener(BRAND_FORM_SYNC_EVENT, onBrandSync);
    return () => window.removeEventListener(BRAND_FORM_SYNC_EVENT, onBrandSync);
  }, [applyExternalForm]);

  const flushToCommitted = useCallback(() => {
    const live =
      formApiRef.current?.getValues?.() ??
      draft;
    const next = mergeLiveFormWithCommitted(live, committedRef.current);
    formApiRef.current?.replaceAll?.(next);
    setCommitted(next);
    setDraft(next);
    return next;
  }, [draft, setCommitted]);

  const patchDraft = useCallback((partial) => {
    formApiRef.current?.patchImmediate?.(partial);
  }, []);

  return {
    draft,
    setDraft,
    formApiRef,
    flushToCommitted,
    patchDraft,
    applyExternalForm,
  };
}
