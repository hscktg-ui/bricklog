...
import { adaptInputForChannelDerivation } from "@/lib/content/channelDerivationVoice";
...
import { finishChannelPack, finishChannelPackForDelivery } from "@/lib/product/channelQualityStack";
...
export function normalizePipelineInput(formValues) {
  ...
  const base = {
    ...
    _verifiedGenerationAxes: formValues._verifiedGenerationAxes,
  };
  return stampCoreRulesOnInput(
    applyWritingSkillToInput(
      applyPipelineQualityDefaults(
        materializeVerifiedGenerationAxes(stampVerifiedGenerationAxes(base))
      )
    )
  );
}
...
export function buildPipelineContext(input, blog, insights, channel = "place") {
  const mergedInput =
    blog && channel
      ? adaptInputForChannelDerivation(input, blog, channel)
      : input;
  const base = createPromptContext(mergedInput);
  ...
  return {
    ...
    feedbackRegenDirective: mergedInput.feedbackRegenDirective || null,
    feedbackSeed: mergedInput.feedbackSeed ?? 0,
    channelDeriveBrief: mergedInput.channelDeriveBrief || null,
    channelDeriveVoice: mergedInput.channelDeriveVoice || null,
...
export async function generateBlogPipelineAsync(formValues, opts = {}) {
  const normalized = normalizePipelineInput(formValues);
  const merged = materializeVerifiedGenerationAxes({
    ...formValues,
    ...normalized,
  });
  const { slimBlogApiPayload } = await import(
    "@/lib/generation/slimBlogApiPayload"
  );
  const payload = slimBlogApiPayload(merged);
  const { shouldUseAsyncBlogPipeline, generateBlogPipelineAsyncJob } =
    await import("@/lib/generation/fetchBlogAsync");
  const { fetchWithAuth } = await import("@/lib/api/clientAuth");
  const { getBlogGenerationAbortSignal } = await import(
    "@/lib/generation/blogGenerationAbort"
  );
  const { getBlogClientFetchTimeoutMs } = await import(
    "@/lib/config/briclogFastPipeline"
  );

  const signal = getBlogGenerationAbortSignal();

  const onLongWait = opts?.hooks?.setPipelineStep
    ? (label) => opts.hooks.setPipelineStep(label)
    : undefined;

  const data = shouldUseAsyncBlogPipeline()
    ? await generateBlogPipelineAsyncJob(payload, { signal, onLongWait })
    : await fetchWithAuth("/api/content/blog", {
        method: "POST",
        body: JSON.stringify(payload),
        timeoutMs: getBlogClientFetchTimeoutMs(),
        signal,
      });
  if (data && data.ok === false && !data.blogContent?.sections?.length) {
...
export async function generateChannelPipelineAsync(channel, formValues) {
  ...
  const { fetchWithAuth } = await import("@/lib/api/clientAuth");
  const timeoutMs = getChannelClientFetchTimeoutMs(payload);
...
const CHANNEL_CONTENT_KEYS = {
  place: "placeContent",
  instagram: "instagramContent",
  image: "imagePrompts",
};

/** 클라이언트 시그니처 실패 시 서버 /api/content/channel 1회 (로컬 fallback 전) */
export async function rescueChannelViaServerApi(channel, formValues, extras = {}) {
  const key = CHANNEL_CONTENT_KEYS[channel];
  if (!key) return { ok: false };
  try {
    const data = await generateChannelPipelineAsync(channel, {
      ...formValues,
      ...extras,
...
export function runPlacePipeline(formValues, blogContent, baseLabel) {
  const input = normalizePipelineInput(formValues);
  const derived = deriveChannelFromVerifiedBlog("place", blogContent, input);
  if (derived) {
    return stampCoreRulesOnDelivery(
      finishChannelPackForDelivery("place", derived, { input, sourceChannel: "blog" }),
      input,
      "place"
    );
  }
  const insights = extractBlogInsights(blogContent);
  const ctx = buildPipelineContext(input, blogContent, insights, "place");
  const label = baseLabel || buildBaseContentLabel(formValues, blogContent);
  const pack = buildPlaceFromBlog(ctx, insights, label);
  return stampCoreRulesOnDelivery(
    finishChannelPack("place", pack, {
      input,
      ...ctx,
      insights,
      sourceChannel: "blog",
...
export function runInstagramPipeline(
  ...
  const input = normalizePipelineInput(formValues);
  const derived = deriveChannelFromVerifiedBlog("instagram", blogContent, input, instaToneKey);
  if (derived) {
    return stampCoreRulesOnDelivery(
      finishChannelPackForDelivery("instagram", derived, { input, sourceChannel: "blog" }),
      input,
      "instagram"
    );
  }
  const insights = extractBlogInsights(blogContent);
  const ctx = buildPipelineContext(input, blogContent, insights, "instagram");
  const label = baseLabel || buildBaseContentLabel(formValues, blogContent);
  const pack = buildInstagramFromBlog(ctx, insights, instaToneKey, label);
  return stampCoreRulesOnDelivery(
    finishChannelPack("instagram", pack, {
      input,
      ...ctx,
      insights,
      sourceChannel: "blog",
