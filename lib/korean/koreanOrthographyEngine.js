...
const COMPOUND_JOSA_FIXES = [
  ...
  [/프로그램\s+을/g, "프로그램을"],
  [/왜\s+을\s+찾/g, "왜 찾"],
  [/이\s+지역\s+에\s+서/g, "이 지역에서"],
  [/이\s+지역\s+에서/g, "이 지역에서"],
  [/원데가/g, "원데이"],
  [/동성으로/g, "동성로에서"],
  [/([가-힣]{2,12})로으로/g, "$1로에서"],
  [/컬러를에/g, "컬러에"],
...
function shouldSkipJosaNoun(noun = "", whole = "", offset = 0, particleUsed = "") {
  const n = String(noun || "");
  if (n.length < 2) return true;
  if (particleUsed && whole) {
    const afterIdx = offset + n.length + particleUsed.length;
    if (/[가-힣]/.test(whole[afterIdx] || "")) return true;
  }
  if (particleUsed === "로" || particleUsed === "으로") {
    const placeStem = `${n}로`;
    if (whole.includes(placeStem) && offset + n.length + particleUsed.length <= whole.indexOf(placeStem) + placeStem.length) {
      return true;
    }
    if (/(?:로|길|거리)$/.test(`${n}${particleUsed}`)) return true;
  }
...
function fixHangulParticleAgreement(text = "") {
  ...
  for (const rule of WRONG_PARTICLE_PATTERNS) {
    out = out.replace(rule.re, (full, noun, offset, whole) => {
      if (shouldSkipJosaNoun(noun, whole, offset, rule.wrong)) return full;
...
function scanSpellingIssues(text = "") {
  ...
  const checks = [
    ...
    [/것같/, "것_같"],
    [/왠지/, "웬지"],
    [/브랜드은|매장는|이용를|쇼룸를|안내을|분위기을/, "조사_오류"],
  ];
