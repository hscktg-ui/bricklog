...
import {
  FREE_LAUNCH_PAYMENT_NOTE,
  FREE_LAUNCH_USER_MESSAGE,
  isBriclogFreeLaunchMode,
} from "@/lib/billing/freeLaunchMode";
...
export function getBillingPresentation(opts = {}) {
  ...
  const paymentPreparingNote = inicisReview
    ? `결제 시스템 준비 중입니다. ${PG_INICIS_FULL} PG 심사 완료 후 유료 플랜 결제가 연결됩니다.`
    : "결제 시스템 준비 중입니다. 정식 오픈 후 유료 플랜 결제가 연결됩니다.";

  if (isBriclogFreeLaunchMode()) {
    return {
      ...baseMeta,
      freeLaunchActive: true,
      betaActive: true,
      betaUntil: null,
      betaUntilLabel: null,
      checkoutEnabled: false,
      paymentStatus: "free_launch",
      planBadge: "무료 · 전 기능",
      userMessage: FREE_LAUNCH_USER_MESSAGE,
      paymentNote: FREE_LAUNCH_PAYMENT_NOTE,
      upgradeDisabledReason:
        "현재 무료 운영 기간입니다. 결제·업그레이드는 추후 유료화 단계에서 안내드립니다.",
      operatorHint: forOperator
        ? "BRICLOG_FREE_LAUNCH · studio entitlements · bypassQuotas"
        : undefined,
    };
  }
  ...
  if (betaActive) {
    return {
      ...
      paymentNote: paymentPreparingNote,
      upgradeDisabledReason:
        "베타 기간에는 결제가 필요하지 않습니다. 결제 시스템은 준비 중입니다.",
      operatorHint: forOperator
        ? "BETA_FULL_ACCESS_UNTIL · BRICLOG_RESET_PAYMENT_PAUSED · BRICLOG_PG_PROVIDER=inicis"
        : undefined,
    };
  }
  ...
  if (isBriclogResetPaymentPaused()) {
    return {
      ...
      paymentNote: paymentPreparingNote,
      upgradeDisabledReason:
        "결제 시스템 준비 중입니다. 잠시 후 다시 시도해 주세요.",
      operatorHint: forOperator
