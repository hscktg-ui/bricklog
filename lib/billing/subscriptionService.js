...
export function formatSubscriptionState(row, userEmail = null) {
  ...
  if (beta) {
    ...
    const isFreeLaunch = beta.source === "free_launch";
    return {
      ...
      source: beta.source,
      bypassBilling: true,
      betaPeriod: !isFreeLaunch,
      freeLaunch: isFreeLaunch,
      betaUntil: isFreeLaunch ? null : getBetaFullAccessUntil(),
      renewalDate: null,
      periodStart: null,
      periodEnd: null,
      pendingPlan: null,
      ...
      cancelAtPeriodEnd: false,
      canManage: false,
      planLabel: isFreeLaunch ? `${studio.label} (무료)` : `${studio.label} (베타)`,
      pendingPlanLabel: null,
    };
  }
