...
export async function fetchUserPlan(supabase, userId, userEmail = null) {
  ...
  if (
    process.env.BRICLOG_E2E_QUOTA_BYPASS !== "false" &&
    userEmail?.toLowerCase() === E2E_TEST_EMAIL.toLowerCase()
  ) {
    return {
      planId: "studio",
      source: "e2e_test",
...
export async function getUsageSummary(supabase, userId, userEmail = null) {
  ...
  return {
    planId,
    planLabel: source === "free_launch" ? `${plan.label} (무료)` : plan.label,
    displayPrice: plan.displayPrice,
    source,
    freeLaunch: source === "free_launch",
    billingReady,
