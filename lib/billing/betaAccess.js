...
import { freeLaunchPlanOverride, isBriclogFreeLaunchMode } from "@/lib/billing/freeLaunchMode";

export const DEFAULT_BETA_UNTIL = "2026-07-15";
...
export function isBetaFullAccessActive(now = new Date()) {
  if (isBriclogFreeLaunchMode()) return true;
...
export function betaPlanOverride(now = new Date()) {
  const freeLaunch = freeLaunchPlanOverride();
  if (freeLaunch) return freeLaunch;
