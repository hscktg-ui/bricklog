...
const FEEDBACK_RULES = [
  ...
  { re: /담백|간결|짧게\s*(써|해|만들)|본문\s*줄이|분량\s*줄/i, category: "shorter", apply: shortenText },
  { re: /분량|길이|더\s*길|너무\s*짧|length_wrong/i, category: "longer", apply: (t) => t },
...
export function recordRewriteLearning(brandId, channel, feedbackText, intent, brandHint = null) {
  if (!brandId) return;
  learnRewritePreference(brandId, channel, feedbackText, intent?.categories || [], brandHint);
}
