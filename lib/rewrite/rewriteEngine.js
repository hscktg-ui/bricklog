...
import { ANTI_SEO_SPAM_PRONOUNS, softenTokenRepeats } from "@/lib/product/antiSeoSpamEngine";
...
const FEEDBACK_RULES = [
  ...
  { re: /브랜드|brand_weak/i, category: "brand_tone", apply: (t, ctx) => (ctx.brandName ? `${ctx.brandName} — ${t}` : t) },
...
function resolveKeywordSubstitutes(ctx = {}, mainKw = "") {
  ...
  return ANTI_SEO_SPAM_PRONOUNS.topic;
}
...
function applyTextFeedback(text, intent, ctx) {
  let out = scrubMechanicalSeoPhrases(scrubGptToneDeep(text));
  for (const rule of FEEDBACK_RULES) {
    if (intent.categories.includes(rule.category) || intent.categories.includes("general")) {
      if (rule.re.test(intent.raw) || intent.categories.includes(rule.category)) {
...
export function rewriteBlogPack(pack, feedbackText, ctx, scope = "all", tagIds = []) {
  ...
  next = enrichBlogPack(next, ctx, ctx.input || {});
