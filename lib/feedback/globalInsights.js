...
export async function autoApprovePendingInsights(db, metrics = {}) {
  ...
  for (const row of pending) {
    const decision = isFriendBetaLearningMode()
      ...
      : evaluateInsightAutoApply(row, metrics);
    if (!decision.apply) {
      deferred.push({ id: row.id, reason: decision.reason });
      continue;
    }
    try {
      await approveInsight(db, row.id);
      approved += 1;
    } catch {
      /* skip single failure */
    }
