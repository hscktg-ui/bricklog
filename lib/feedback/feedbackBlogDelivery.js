...
/** rewrite 후 delivery SSOT — Safe Edit → Golden Gate → 업종 잠금 */
export function polishFeedbackRewritePack(pack, ctx = {}, input = {}) {
  if (!pack?.sections?.length) return pack;
  const pipelineInput = input || ctx.input || ctx;
  let next = applyGoldenSafeEdit(pack, pipelineInput);
  const gate = assessGoldenQualityGate(next, pipelineInput);
  next = {
    ...next,
    _meta: {
      ...(next._meta || {}),
