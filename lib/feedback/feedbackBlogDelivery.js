...
export function shouldFeedbackFullRegen({
  ...
  if (/주제\s*바꿔|다른\s*주제|처음부터|전체\s*다시|새로\s*써|gpt|ai\s*틱|정보\s*부족|가구|꽃|업종|분량|너무\s*짧|더\s*길|길이\s*부족/i.test(memoText)) {
    return true;
  }
...
/** rewrite 후 delivery SSOT — Safe Edit → EQS → Golden Gate → publishReady */
