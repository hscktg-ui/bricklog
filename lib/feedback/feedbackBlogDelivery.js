 ...
 */
import { applyEditorQualityPack } from "@/lib/content/editorQualityEngine";
...
/** rewrite 후 품질 다듬기 — 이미 salvage된 편집본은 가벼운 보정만 */
export function polishFeedbackRewritePack(pack, ctx = {}, input = {}) {
  if (!pack?.sections?.length) return pack;
  const pipelineInput = input || ctx.input || ctx;
  const alreadyFinal = Boolean(pack._meta?.salvageDeliveryFinalized);
  let next = pack;
  if (!alreadyFinal) {
    next = salvageBlogPackForDelivery(pack, pipelineInput);
    next = ensureBlogDisplayPack(next, pipelineInput);
  } else {
    next = applyEditorQualityPack(next, ctx, pipelineInput);
    if (!next?.sections?.length) next = pack;
  }
  return {
    ...next,
    _meta: {
      ...
      salvageDeliveryFinalized:
        next._meta?.salvageDeliveryFinalized ?? alreadyFinal ?? true,
    },
  };
}
