...
export function runAiEditorAudit(pack, input = {}, ctx = {}) {
  ...
  const contextLockAudit = assertContextLockPostWrite(next, input);
  const directorJudgment = assessDirectorFinalJudgment(next, input, publishAudit);
  const editorReport = runEditorAI("blog", next, evalCtx);

  const reasons = [];
  if (!editorGate.ok) reasons.push(...(editorGate.reasons || ["editor_gate_fail"]));
  if (!contextLockAudit.ok) {
    reasons.push(...(contextLockAudit.reasons || ["context_lock_fail"]));
  }
  if (!directorJudgment.ok) {
    reasons.push(...(directorJudgment.reasons || ["director_judgment_fail"]));
  }
  ...
  return {
    ...
    pack: attachDirectorMeta(next, input, directorJudgment),
    editorGate,
    publishAudit,
    contextLockAudit,
    directorJudgment,
    editorReport,
    publishQuestion: directorJudgment.question || publishAudit.question,
    publishAnswer: ok ? "YES" : "NO",
    publishReady: ok && publishAudit.publishReady && directorJudgment.publishReady,
    userMessage: ok
      ? null
