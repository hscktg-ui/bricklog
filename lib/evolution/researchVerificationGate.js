...
export function computePreGenerationMetrics(input = {}) {
  ...
  return {
    ...
    brandWikiOk: wiki.ok,
    brandWikiTopicExplainable: wiki.topicExplainable,
    brandWikiVerifiedFacts: wiki.verifiedFactCount,
    brandWikiExplainGaps: wiki.explainGaps,
    verifiedBrandCount,
...
export function assertResearchVerificationGate(input = {}) {
  ...
  if (metrics.repetitionRate > MAX_PREWRITE_REPETITION_RATE) {
    if (!(hasCompletedResearchStep(input) && metrics.infoCount >= metrics.minInfoCount)) {
  ...
  if (metrics.brandWikiTopicExplainable === false && metrics.infoCount >= metrics.minInfoCount) {
    reasons.push("brand_wiki_topic_not_explainable");
  }
