...
import { matchFaqReply } from "./faqIndex";
...
export function matchQuickReply(message, ctx = {}) {
  ...
  if (
    ...
    return {
      reply:
        "BRICLOG은 조사·맥락 점검 후 브랜드 운영 글 세 채널을 맞춥니다.\n\n" +
        `· 한 주제 → ${formatAutoPipelineOrder()} (필요하면 프롬프트)\n` +
        "· 무료 샘플: 이야기·플레이스·인스타 미리보기 + 발행 준비도\n" +
        "· 이야기 폼 「더 맞추기」에서 자료조사·화자·문체\n" +
        "· 게시·업로드는 직접, 여기서는 채널별 초안만 정리합니다\n\n" +
        formatFreePlanChannelNote() +
  ...
  if (
    ...
    return {
      reply:
        ...
        `· ${CHANNEL_PRODUCTS.plan.menuLabel}: ${CHANNEL_PRODUCTS.plan.desc}\n\n` +
        `이야기 연동: ${formatAutoPipelineOrder()}.\n` +
        formatFreePlanChannelNote() +
  ...
  if (
    ...
    return {
      reply:
        `Brief 섹션 「${CHANNEL_PRODUCTS.plan.menuLabel}」·「${CHANNEL_PRODUCTS.growth.menuLabel}」에서 이번 주·이번 달 주제와 톤을 잡습니다.\n\n` +
        "1) Today에서 오늘 주제 확인\n" +
        "2) Plans·Briefs에서 주별·월별 리듬 확인\n" +
        "3) Create에서 이야기·플레이스·인스타로 이어가기",
      topic: "채널",
    };
  }
  ...
  if (
    ...
    return {
      reply:
        ...
        `· 더보기: Today, ${CHANNEL_PRODUCTS.growth.menuLabel}, ${CHANNEL_PRODUCTS.plan.menuLabel}, ${CHANNEL_PRODUCTS.history.menuLabel}, 설정·${PLAN_UI_LABEL}\n` +
        "· 오른쪽 아래 ? : 도움말\n\n" +
        "PC에서는 왼쪽 메뉴가 Today → Brief → Create → Review · Library 순입니다.",
      topic: "시작하기",
    };
  }
  ...
  if (
    ...
    return {
      reply:
        `「${CHANNEL_PRODUCTS.plan.menuLabel}」는 보관·운영 메뉴에 있습니다. 브랜드·습관에 맞춰 이번 주·이번 달 주제와 채널 리듬을 잡습니다.\n\n` +
        "· 주제 확인 후 이야기·플레이스·인스타로 이어가기\n" +
        "· 새 글은 콘텐츠 만들기 메뉴에서 생성",
      topic: "검수",
    };
  }
  ...
  if (includesAny(t, ["사용법", "처음", "시작", "순서", "첫", "도움", "어떻게 써"])) {
    return {
      reply:
        ...
        `4) Today에서 오늘 주제 확인 → Create 「${CHANNEL_PRODUCTS.blog.generateLabel}」\n5) Review 점검 후 복사·게시\n6) Library·${CHANNEL_PRODUCTS.growth.menuLabel}에 보관\n\n` +
        `모바일은 하단 ${formatMainChannelTabs()}·더보기(Today·Briefs·Plans·Library). 아래 버튼으로 주제를 골라 보세요.`,
      topic: "시작하기",
    };
  }
  ...
  const faqHit = matchFaqReply(message);
  if (faqHit) {
    const topicMap = {
      research: "자료조사",
      quality: "검수",
    ...
    return {
      reply: faqHit.reply,
      topic: topicMap[faqHit.faqId] || faqHit.topic || "도움말AI",
    };
  }
...
export function getWelcomeMessage(ctx = {}) {
  if (ctx.loggedIn) {
    return (
      `안녕하세요. 조사·채널·발행 준비도·${PLAN_UI_LABEL}·한도 안내는 여기서 받을 수 있어요. 아래 추천 질문 또는 직접 입력해 보세요.`
    );
  }
  return (
    "안녕하세요. BRICLOG — 한 주제로 이야기·플레이스·인스타를 같은 맥락으로 이어 주는 서비스입니다.\n" +
    "무료 샘플·처음 순서·요금·채널 FAQ는 아래 추천 질문에서 골라 보세요."
  );
}
