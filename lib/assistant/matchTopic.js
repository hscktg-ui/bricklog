...
export function matchQuickReply(message, ctx = {}) {
  ...
  if (
    ...
    return {
      reply:
        "BRICLOG은 조사·맥락 점검 후 브랜드 운영 글과 상품 화면을 맞춥니다.\n\n" +
        `· 한 주제 → ${formatAutoPipelineOrder()} (필요하면 프롬프트)\n` +
        "· 상품 화면: 브릭로그 상세(briclog.ai/detail) — 붙일 860px HTML\n" +
        "· 무료 샘플: 이야기·플레이스·인스타 미리보기 + 발행 준비도\n" +
        "· 이야기 폼 「더 맞추기」에서 자료조사·화자·문체\n" +
        "· 게시·업로드는 직접, 여기서는 글과 화면만 정리합니다\n\n" +
        formatFreePlanChannelNote() +
  ...
  if (
    ...
    return {
      reply:
        ...
        `· 더보기: ${CHANNEL_PRODUCTS.plan.menuLabel}, ${CHANNEL_PRODUCTS.growth.menuLabel}, ${CHANNEL_PRODUCTS.history.menuLabel}, 설정·${PLAN_UI_LABEL}\n` +
        "· 오른쪽 아래 ? : 도움말\n\n" +
        "PC에서는 왼쪽 메뉴가 고정됩니다.",
      topic: "시작하기",
    };
...
export function getWelcomeMessage(ctx = {}) {
  ...
  return (
    "안녕하세요. BRICLOG — 한 주제로 이야기·플레이스·인스타, 상품은 브릭로그 상세에서 고르는 화면입니다.\n" +
