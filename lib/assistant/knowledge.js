...
import { matchFaqReply, buildFaqKnowledgeBlock } from "./faqIndex";
...
export const QUICK_PROMPTS = [
  ...
  { id: "schedule", label: "Plans·스케줄", category: "create" },
  ...
  { id: "publish_ready", label: "발행 준비도·맥락 점수", category: "create" },
  { id: "place_insta", label: "플레이스·인스타 차이", category: "create" },
  { id: "help_ai", label: "AI 도움말 사용법", category: "product" },
  { id: "search_find", label: "검색에서 찾기", category: "product" },
];
...
export const QUICK_TEXT = {
  ...
  schedule: "Plans·Briefs에서 이번 주 운영은 어디서 보나요?",
  ...
  login: "회원가입이랑 로그인 방법",
  publish_ready: "발행 준비도·맥락 점수는 어떻게 보나요?",
  place_insta: "플레이스 공지와 인스타 캡션은 어떻게 다른가요?",
  help_ai: "AI 도움말은 어디서 쓰나요?",
  search_find: "네이버·구글에서 브릭로그는 어떻게 찾나요?",
};
...
export function buildAssistantContext(ctx = {}) {
  ...
  const lines = [
    ...
    `채널: ${kb.channels.join(", ")}`,
    `흐름: ${kb.workflow.join(" → ")}`,
    `이야기 연동 순서: ${formatAutoPipelineOrder()}.`,
    `운영 계획: 사이드바 Brief 「${CHANNEL_PRODUCTS.plan.menuLabel}」 — 이번 주·이번 달 주제·채널 리듬.`,
    `자료조사: 이야기 폼 「${FORM_ADVANCED_SECTION}」 → 「자료조사 사용」. 조사 유형 예: ${RESEARCH_TYPE_LABELS}. 팩트 부족 시 생성이 멈출 수 있음.`,
    "화자: 같은 「더 맞추기」에서 선택. 방문 후기는 담백한 후기형·실사용 후기형 권장.",
    `모바일: 하단 탭 ${formatMainChannelTabs()} + 더보기(Today · Briefs · Plans · Library · 플랜)`,
    "말투·이모지: 이야기 폼 더 맞추기 또는 Briefs·브랜드 설정",
    ...
    "발행 준비도: 브랜드·지역·주제·근거 축별 점수. 무료 테스트·Today·Create에서 확인.",
    "무료 테스트: 랜딩에서 발행 샘플 — 이야기·플레이스·인스타 미리보기 + 맥락 점수.",
    "도움말 AI: 오른쪽 아래 ? — 추천 질문 또는 직접 입력. FAQ 전체: briclog.ai/help",
    `FAQ (${knowledgeBase.faqCount || 15}항):\n${buildFaqKnowledgeBlock().slice(0, 6000)}`,
    `안내: ${IN_APP_HELP_HINT}`,
