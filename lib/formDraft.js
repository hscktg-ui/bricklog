...
export function loadFormDraft(userId, brandId) {
  ...
  try {
    if (brandId) {
      const scoped = localStorage.getItem(draftKey(userId, brandId));
      if (scoped) return JSON.parse(scoped);
    }
    const defaultScoped = localStorage.getItem(draftKey(userId, "default"));
    if (defaultScoped) {
      const parsed = JSON.parse(defaultScoped);
      if (brandId && parsed && typeof parsed === "object") {
        try {
          localStorage.setItem(
            draftKey(userId, brandId),
            JSON.stringify({ ...parsed, brandId })
          );
          localStorage.removeItem(draftKey(userId, "default"));
        } catch {
          /* ignore migration */
        }
      }
    ...
    if (!brandId) {
      const defaultScoped = localStorage.getItem(draftKey(userId, "default"));
      if (defaultScoped) return JSON.parse(defaultScoped);
