...
} from "@/lib/config/channelTokenBudget";
...
const DERIVED_CHANNEL_MAX_ATTEMPTS = 1;
const DERIVED_CHANNEL_FAST_BUDGET_MS = 8_000;

function remainingLoopBudgetMs(loopStartedAt, loopBudgetMs) {
  return loopBudgetMs - (Date.now() - loopStartedAt);
}

async function callOpenAIChatWithinLoopBudget(
  loopStartedAt,
  loopBudgetMs,
  messages,
  options = {}
) {
  const remaining = remainingLoopBudgetMs(loopStartedAt, loopBudgetMs);
  if (remaining <= 400) {
    throw new Error("channel_time_budget_exceeded");
  }
  return Promise.race([
    callOpenAIChat(messages, options),
    new Promise((_, reject) =>
      setTimeout(
        () => reject(new Error("channel_time_budget_exceeded")),
        remaining
      )
    ),
  ]);
...
export async function generateChannelWithLLMFirst(channel, input = {}) {
  ...
  if (mode === "brief_only" && !isOpenAIConfigured()) {
    ...
    if (pack) {
      const polished = finishChannelFallbackHumanProse(pack, channel, normalized, {
        deliveryFinalize: true,
      });
      return gateOrchestratorChannelPack(normalized, polished, channel, {
        mode: "draft_fallback",
  ...
  const channelFast = isChannelStandaloneFastInput({
    ...normalized,
    contentChannel: channel,
  });
  const loopBudgetMs = channelFast
    ? getChannelLlmLoopBudgetMs()
    : isDerivedFromBlog
      ? DERIVED_CHANNEL_FAST_BUDGET_MS
      : Number.MAX_SAFE_INTEGER;
  const attemptLimit = isDerivedFromBlog
    ? Math.min(MAX_ATTEMPTS, DERIVED_CHANNEL_MAX_ATTEMPTS)
    : MAX_ATTEMPTS;
  const softPassFloor = channelFast ? getChannelSoftPassFloor() : CORE_TARGET_SCORE;

  for (let attempt = 0; attempt < attemptLimit; attempt++) {
    try {
      if (Date.now() - loopStartedAt > loopBudgetMs) {
        lastFailures = [
          ...new Set([...(lastFailures || []), "channel_time_budget_exceeded"]),
        ];
        break;
      }
      ...
      const channelMaxTokens =
        channel === "place"
          ? getPlaceChannelMaxTokens()
          : channel === "instagram"
            ? getInstagramChannelMaxTokens()
            : getPlaceChannelMaxTokens();
      const raw = await callOpenAIChatWithinLoopBudget(
        loopStartedAt,
        loopBudgetMs,
        messages,
        sensitive.isSensitive
          ? { temperature: 0.65, maxTokens: getBlogOrchestratorWriteMaxTokens("medium", true) }
          : { temperature: 0.7, maxTokens: channelMaxTokens }
      );
  ...
  if (bestProcessed?.pack) {
    ...
    if (deliver.ok && bestScore >= softPassFloor) {
      return successChannelResult(
        channel,
        normalized,
        deliver.pack,
        bestProcessed,
        attemptLimit,
        { softPass: bestScore < CORE_TARGET_SCORE }
      );
    }
    if (channelFast && bestProcessed.pack) {
      return successChannelResult(
        channel,
        normalized,
        bestProcessed.pack,
  ...
  if (
    isChannelStandaloneFastInput({ ...normalized, contentChannel: channel }) &&
    (channel === "place" || channel === "instagram")
  ) {
    const northFallback = buildDeliverableChannelFallback(channel, {
      input: normalized,
      instaTone: input.instaTone,
      failures: [...(lastFailures || []), "standalone_north_star_fallback"],
    }).pack;
    if (northFallback && isPublishableChannelPack(channel, northFallback)) {
      const deliver = assertPostWriteDeliverable(normalized, northFallback);
      if (deliver.ok) {
        return successChannelResult(
          channel,
          normalized,
      ...
      const first = assessChannelFirstDeliveryQuality(
        northFallback,
        channel,
        normalized
      );
      if (first.displayReady) {
        return successChannelResult(
          channel,
          normalized,
          northFallback,
...
function successChannelResult(
  ...
  return {
    ...
    meta: {
      ...
      generationMode: `llm_gpt55_${channel}`,
      passOutput: !softPass,
      contentChannel: channel,
      regenAttempts,
      qualityScore:
...
export function blockUnverifiedChannelApiResponse(channel, result, input = {}) {
  ...
  const channelInput = { ...input, contentChannel: channel, channel };
  const gate = assertPostWriteDeliverable(channelInput, result[key]);
  if (gate.ok) {
    return {
      ...result,
      [key]: gate.pack,
  ...
  console.warn("[channel-api] post-write blocked", {
    channel,
  ...
  const recovered = buildDeliverableChannelFallback(channel, {
    input: channelInput,
    sourceBlog: channelInput._sourceBlogPack || null,
    bestPack: result[key],
    instaTone: channelInput.instaTone,
    failures: gate.reasons || [],
  }).pack;
  if (recovered) {
    const polished = finishChannelFallbackHumanProse(recovered, channel, channelInput, {
      deliveryFinalize: true,
    });
    const retry = assertPostWriteDeliverable(channelInput, polished);
    if (retry.ok) {
      return {
        ...result,
        ok: true,
        ...
        meta: {
          ...
          v2PipelineVerified: true,
          v3PipelineVerified: true,
          generationMode: "channel_fallback_recovered",
          failReasons: gate.reasons,
          channelRecovery: true,
        },
      };
    }
    const first = assessChannelFirstDeliveryQuality(polished, channel, channelInput);
