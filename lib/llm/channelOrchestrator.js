...
import { getLLMMode } from "./llmProvider";
...
export async function generateChannelWithLLMFirst(channel, input = {}) {
  ...
  if (mode === "brief_only") {
    return researchGateBlockedChannelResult(channel, normalized, {
      ok: false,
      userMessage:
        "조사·검증 없이 출력할 수 없습니다. AI 연결이 되면 「바로 만들기」를 이용해 주세요.",
    ...
    try {
      ...
      const coreRegenNote =
        attempt > 0 && lastFailures.length
          ...
          : "";
      const regenNote = [coreRegenNote, v2AxisNote].filter(Boolean).join(" ");
      ...
      if (needsCoreRegen(coreQuality) && attempt < MAX_ATTEMPTS - 1) {
        lastFailures = [...(coreQuality?.failReasons || [])];
        continue;
      }
      if (needsV2AxisRegen(v2Axis) && attempt < MAX_ATTEMPTS - 1) {
  ...
  if (bestProcessed?.pack) {
    ...
    if (deliver.ok && bestScore >= CORE_TARGET_SCORE - 5) {
      return successChannelResult(
        channel,
        normalized,
        deliver.pack,
