  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      const humanTierNote =
        attempt > 0 &&
        lastFailures.some((r) =>
          ["human_tier_under", "length_tier_under", "length_too_short"].includes(r)
        )
          ? buildHumanTierRegenNote(bestProcessed?.pack, normalized)
          : "";
      const coreRegenNote =
      ...
      const tierKey =
        normalized.blogLengthTier || buildCtx.blogLengthTier || DEFAULT_BLOG_LENGTH_TIER;
      if (
        !isHumanTierMet(processed.pack, { blogLengthTier: tierKey }) &&
        ...
        lastError = new Error("HUMAN_TIER_REGEN");
