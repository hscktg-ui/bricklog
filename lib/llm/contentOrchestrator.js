...
import { formatPostVerifyUserMessage } from "@/lib/product/customerOutput";
import {
...
export async function generateBlogWithLLMFirst(input = {}) {
  ...
  if (v2Gate && !preWriteGate.ok) {
    return researchGateBlockedResult(normalized, preWriteGate);
  }
  if (!preWriteGate.ok && preWriteGate.reasons?.includes("missing_axes")) {
    return researchGateBlockedResult(normalized, preWriteGate);
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      if (v2Gate && !firstDeliveryReady && attempt < maxAttempts - 1) {
    ...
    return {
      ...
      meta: {
        ...
        optionApplication: {
          ...
          length: buildLengthApplicationSummary(normalized, bestProcessed.pack),
        },
      },
      userMessage: hint,
...
function finalizeForDelivery(input, pack) {
  ...
  if (hasLengthGateFailure(gate.reasons || [])) return gate;
  if (
    pack?.sections?.length &&
    hasFilledBlogAxes(input) &&
    isSoftInformationGateFailure(gate) &&
    !isCompletionHardFailure(gate, pack, input)
  ) {
    return {
      ok: true,
      pack: {
  ...
  if (shouldWithholdFailedPostVerify(input)) {
    const preview = deliverBlogDespiteGate(input, pack, gate);
    if (preview) {
      return { ok: true, pack: preview.blogContent, softPass: true };
    }
    return {
      ...gate,
      userMessage: formatPostVerifyUserMessage(gate) || gate.userMessage,
    };
  }
  if (shouldApplyStrictBrandGuard(input)) return gate;
  const lengthBlocked = (gate.reasons || []).includes("length_tier_under");
    ...
    return {
      ...
      pack: {
        ...
        _meta: {
          ...
          passOutput: true,
          postVerifySoft: true,
          lengthSoft: lengthBlocked || undefined,
          failReasons: gate.reasons,
        },
      },
      softPass: true,
