 ...
 *
 * V17 Multi-AI: Research·Local·Memory·TopicExpansion → Writer(GPT-5.6 Sol) → DuplicateKiller → Length → Reviewer → 출력
...
import {
  ...
  isTriAiResearchMaxMode,
  shouldSkipHeavyPostLlmExpansion,
} from "@/lib/config/briclogFastPipeline";
import { getBlogOrchestratorWriteMaxTokens } from "@/lib/config/channelTokenBudget";
import { GENERATION_LLM_LOOP_BUDGET_MS, DEFAULT_BLOG_LENGTH_TIER } from "@/lib/constants";
import { PUBLIC_TEST_LLM_BUDGET_MS } from "@/lib/publicTest/publicTestConfig";
...
import { tryWriterFirstOrchestratorDelivery } from "@/lib/product/gpt55LightDelivery";
import {
  buildHumanColumnRegenNote,
  isHumanColumnContractMet,
} from "@/lib/product/humanTierRegen";
import { runBriclogWriterEngine, isWriterEngineEligiblePack } from "@/lib/product/briclogWriterEngine";
import {
...
import { needsWriterEnginePass, needsAGradePass } from "@/lib/product/humanTierRegen";
import {
  applyHumanLikeDeliveryPass,
  needsHumanLikePass,
} from "@/lib/product/humanLikeDeliveryEngine";
import { isBriclogMaxQualityEnabled } from "@/lib/config/briclogMaxQuality";
import { countBlogBodyCharsWithSpaces } from "@/lib/prompts/engine/textUtils";
import { tryDeliverForcedMissionProsePack, tryDeliverResearchHeavyMissionRescue } from "@/lib/product/missionProseRouteEngine";
import {
  ensureServerResearchHydrated,
...
function buildLengthApplicationSummary(input, blog) {
  ...
  return {
    ...
    requestedTier: contract?.tier || input?.blogLengthTier || DEFAULT_BLOG_LENGTH_TIER,
    min,
...
export async function generateBlogWithLLMFirst(input = {}) {
  ...
  if (typeof window === "undefined") {
    try {
      const { hydrateGoldenSamplesForInput } = await import(
        "@/lib/golden/goldenDatasetRuntime.server"
      );
      baseInput = await hydrateGoldenSamplesForInput(input);
    } catch {
      /* server hydrate optional */
    }
  }
  const enriched = applyWritingSkillToInput(
    applyV2PersonaToInput(applyV4SpeakerToInput(enrichMinimalBlogInput(baseInput)))
  );
  const intentNorm = normalizeBlogInputIntent(enriched);
  let normalized = enrichInputForGeneration(intentNorm.input);
  normalized = attachServerTrendSnapshot(normalized);
  normalized = await ensureServerResearchHydrated(normalized);
  ...
  const forcedMission = tryDeliverForcedMissionProsePack(normalized);
  if (forcedMission?.ok && forcedMission?.blogContent?.sections?.length) {
    return forcedMission;
  }

  const researchHeavyRescue = tryDeliverResearchHeavyMissionRescue(normalized);
  if (researchHeavyRescue?.ok && researchHeavyRescue?.blogContent?.sections?.length) {
    return researchHeavyRescue;
  }
  ...
  if (
    ...
    if (isBriclogAlwaysDeliverEnabled() && hasFilledBlogAxes(normalized)) {
      normalized = {
        ...normalized,
        thinPreWriteProceed: true,
        preWriteGateSoft: preWriteGate,
      };
    } else {
  ...
  const buildCtx = { ...prep.ctx, ...normalized, input: normalized };
  let sensitive =
  ...
  const maxAttempts = isLaunchPublishFirstMode()
    ? getLaunchPublishMaxAttempts()
    : normalized.publicTestMode
    ? 2
    : sensitive.isSensitive
    ? MAX_SENSITIVE_LLM_ATTEMPTS
    : MAX_LLM_ATTEMPTS;
  const loopBudgetMs = isLaunchPublishFirstMode()
    ? getLaunchPublishTimeBudgetMs()
    : normalized.publicTestMode
    ? PUBLIC_TEST_LLM_BUDGET_MS
    : FAST_DELIVERY_BUDGET_MS;
  let lastError = null;
  let lastFailures = [];
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    if (Date.now() - loopStartedAt > loopBudgetMs) {
    ...
    try {
      ...
      const humanTierNote =
        ...
        lastFailures.some((r) =>
          [
            ...
            "body_completion_low",
            "engine_spam_draft",
            "column_visit_north_star",
          ].includes(r)
        )
          ? buildHumanColumnRegenNote(bestProcessed?.pack, normalized)
          : "";
      const coreRegenNote =
      ...
      const writeTier =
        normalized.blogLengthTier || buildCtx.blogLengthTier || DEFAULT_BLOG_LENGTH_TIER;
      const writeMaxTokens = sensitive.isSensitive
        ? getBlogOrchestratorWriteMaxTokens(writeTier, true)
        : getBlogOrchestratorWriteMaxTokens(writeTier, false);
      ...
      const tierKey =
        normalized.blogLengthTier || buildCtx.blogLengthTier || DEFAULT_BLOG_LENGTH_TIER;
      const tierInput = { ...normalized, blogLengthTier: tierKey };
      if (
        !isHumanColumnContractMet(processed.pack, tierInput) &&
        attempt < maxAttempts - 1
      ) {
        const contractReasons =
          processed.pack?._meta?.humanColumnContract?.reasons ||
          [];
        lastFailures = [
          ...new Set([
            ...(lastFailures || []),
            ...(contractReasons.length
              ? contractReasons
              : ["length_tier_under", "human_tier_under"]),
          ]),
        ];
        lastError = new Error("HUMAN_COLUMN_REGEN");
        const coreQualityEarly = processed.pack?._meta?.coreQuality;
        const scoreTotalEarly =
          coreQualityEarly?.total ??
          processed.pack?._meta?.qualityScore?.total ??
          processed.pack?._meta?.humanityScore ??
          0;
        const humanityScoreEarly =
          processed.pack?._meta?.humanBeliefScore ??
          processed.pack?._meta?.humanBelief?.score ??
          processed.pack?._meta?.v4Core?.humanityScore ??
          0;
        const compositeEarly = Math.round(
          scoreTotalEarly * 0.52 + humanityScoreEarly * 0.48
        );
        if (compositeEarly > bestScore) {
          bestScore = compositeEarly;
      ...
      if (hardPass) {
        ...
        if (!deliver.ok) {
          const writerFirstPack = tryWriterFirstOrchestratorDelivery(
            reviewed.pack,
            normalized
          );
          if (writerFirstPack?.sections?.length) {
            return successResult(
              input,
              writerFirstPack,
              { ...reviewed, pack: writerFirstPack },
              attempt + 1,
              sensitive
            );
          }
  ...
  if (bestProcessed?.pack?.sections?.length) {
    ...
    if (v2Gate) {
      ...
      if (!deliver.ok) {
        ...
        if (writerFirstPack?.sections?.length) {
          bestProcessed.pack = writerFirstPack;
    ...
    if (isBriclogMissionEnforced()) {
      if (
        hasSubstantiveLlmBody(bestProcessed.pack, normalized) &&
        isLlmOriginatedPack(bestProcessed.pack)
      ) {
      ...
      if (
        isBriclogAlwaysDeliverEnabled() &&
        bestProcessed.pack?.sections?.length
      ) {
        const salvaged = ensureBlogDisplayPack(
          salvageBlogPackForDelivery(bestProcessed.pack, normalized),
          normalized
        );
        const ship = salvaged?.sections?.length
          ? salvaged
          : finalizeContentQualityForDelivery(bestProcessed.pack, normalized, "blog");
        if (ship?.sections?.length) {
          return successResult(
            input,
            ship,
            { ...bestProcessed, pack: ship },
            maxAttempts,
            sensitive
          );
        }
      ...
      if (!isBriclogAlwaysDeliverEnabled()) {
        return researchGateBlockedResult(
          normalized,
          {
            ...
            userMessage:
              hint ||
              "품질 기준에 맞추지 못해 보류했습니다. 입력을 구체화하거나 「다시 받기」를 눌러 주세요.",
            reasons: [
              ...new Set([
                ...(bestProcessed.pack._meta?.failReasons || lastFailures),
...
async function attachQualityReview(processed, buildCtx, normalized) {
  ...
  let pack = applyHumanLikeDeliveryPass(processed.pack, normalized);
  pack = applyAGradeQualityPass(pack, normalized);
  let sqv = computeContentQualityValue(pack, normalized);
  const humanLikeNeeded = needsHumanLikePass(pack, normalized);
  const maxQuality = isBriclogMaxQualityEnabled();
  const skipHeavy = shouldSkipHeavyPostLlmExpansion();

  if (
    !skipHeavy &&
    (maxQuality ||
      (sqv.score ?? 0) < A_GRADE_MIN_SCORE ||
      humanLikeNeeded) &&
    isOpenAIConfigured()
  ) {
    try {
      const reviewed = await runContentQualityReviewPipeline(pack, buildCtx, {
        input: normalized,
      });
      if (reviewed?.pack?.sections?.length) {
        pack = applyHumanLikeDeliveryPass(reviewed.pack, normalized);
        pack = applyAGradeQualityPass(pack, normalized);
        sqv = computeContentQualityValue(pack, normalized);
      }
    } catch {
      /* optional GPT quality review */
    }
  }
  ...
  if (
    !skipHeavy &&
    isWriterEngineExpansionEnabled() &&
    isWriterEngineEligiblePack(pack, normalized) &&
    (maxQuality || needsWriterEnginePass(pack, normalized))
  ) {
    try {
      const engine = await runBriclogWriterEngine(pack, buildCtx, normalized);
      if (engine?.pack?.sections?.length) {
        pack = applyHumanLikeDeliveryPass(engine.pack, normalized);
        pack = applyAGradeQualityPass(pack, normalized);
        sqv = computeContentQualityValue(pack, normalized);
      }
    } catch {
      /* optional writer engine */
    }
  }

  if (needsAGradePass(pack, normalized)) {
    pack = applyAGradeQualityPass(pack, normalized);
  }

  if (!pack?.sections?.length) return processed;
  return {
    ...processed,
    pack,
    charCount: countBlogBodyCharsWithSpaces(pack),
    passOutput: processed.passOutput,
  };
}
...
function finalizeForDelivery(input, pack) {
  ...
  const salvagedRaw = salvageBlogPackForDelivery(pack, input);
  const salvaged = ensureBlogDisplayPack(
    isLlmOriginatedPack(pack)
      ? salvagedRaw
      : ensureMissionProseTierLength(salvagedRaw, { input }),
    input
  );

  const preview = deliverBlogDespiteGate(input, salvaged, gate);
  if (preview?.blogContent?.sections?.length) {
    return { ok: true, pack: preview.blogContent, softPass: false };
  }

  const stamped = stampCompleteCustomerDelivery(salvaged, input, {
...
function successResult(input, blog, processed, regenAttempts, sensitive) {
  const tier = input.blogLengthTier || DEFAULT_BLOG_LENGTH_TIER;
  const blogWithMeta = {
    ...blog,
  ...
  return alignBlogApiDeliveryResponse(
    {
      ok: true,
      mode: "llm",
      llmAvailable: true,
...
export function getLlmServiceStatus() {
  ...
  const dominant = isGpt55WriterDominant();
  return {
    llmAvailable: available,
    mode: getLLMMode(),
    writerModel: getOpenAIModel(),
    writerFamily: "gpt-5.6",
    writerTier: "sol",
    gpt55Dominant: dominant,
    writerDominant: dominant,
    writerEngineExpansion: dominant && isWriterEngineExpansionEnabled(),
    triAiResearchMax: isTriAiResearchMaxMode(),
