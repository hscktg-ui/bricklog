...
async function attachQualityReview(processed, buildCtx, normalized) {
  ...
  if (!needsWriterEnginePass(processed.pack, normalized)) return processed;
  if (!isWriterEngineEligiblePack(processed.pack, normalized)) return processed;
  try {
    const engine = await runBriclogWriterEngine(
      processed.pack,
      buildCtx,
      normalized
    );
    if (!engine?.pack?.sections?.length) return processed;
    return {
      ...processed,
      pack: engine.pack,
      charCount: countBlogBodyCharsWithSpaces(engine.pack),
      passOutput:
        engine.grade?.tierMet && processed.passOutput !== false
          ? processed.passOutput
          : processed.passOutput,
    };
  } catch {
