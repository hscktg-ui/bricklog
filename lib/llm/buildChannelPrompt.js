...
import { buildChannelUnifiedProsePromptBlock } from "@/lib/content/channelUnifiedProseEngine";
import { buildChannelWriterFirstOrchestratorHint } from "@/lib/product/writerFirstDelivery";
import {
...
No "블로그", SEO lecture, keyword stuffing.`,
  ...
  instagram: {
    label: "인스타그램 캡션",
    jsonKey: "instagram",
    rules: `Save-worthy caption: topic-specific hook (≤56 chars) + line-broken body (220–420 chars medium) + soft ending. Hashtags 5–12.
Topic/mainKeyword must appear in hook or first 2 body lines. Emoji 3–6 (one per line, industry-relevant).
...
function systemPrompt(channel, ctx) {
  ...
  const writerFirstHint = buildChannelWriterFirstOrchestratorHint(channel, promptCtx.input || promptCtx);
  const northStarRef = buildNorthStarReferencePromptBlock(channel);
  const brandGrowth = buildNorthStarBrandGrowthBrief();
...
- ${channel === "place" || channel === "instagram" ? buildChannelUnifiedProsePromptBlock(channel).replace(/\n/g, " ") : ""}
...
${writerFirstHint ? `${writerFirstHint}\n` : `${brandGrowth}\n${northStarRef}\n`}
