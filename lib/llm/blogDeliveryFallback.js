...
import { topicRaw as resolveTopicRaw } from "@/lib/content/topicFacetEngine";
import { resolveChannelBrandName } from "@/lib/content/channelBrandResolve";
import {
...
import { hasUsableResearchFacts, buildResearchGroundedHumanPack } from "@/lib/content/researchGroundedHumanPack";
...
function isAcceptableLlmDraftPack(pack, input = {}) {
  if (!hasBlogSections(pack) || !isPublishableBlogPack(pack)) return false;
  if (isCoverageSlotDumpPack(pack)) return false;
  ...
  const minChars = Math.max(480, Math.min(720, Math.round(target * 0.35)));
  if (sections < 3 || chars < minChars) return false;
  if (isBriclogResetQualityEnforced() && hasUsableResearchFacts(input)) {
...
function buildResearchGroundedDraftFallback(enriched, failures = []) {
  if (!hasUsableResearchFacts(enriched)) return null;
  const raw = buildResearchGroundedHumanPack(enriched);
  if (!raw?.sections?.length) return null;
  const pack = finishBlogFallbackHumanProse(
    stripIndustryContaminationFromPack(raw, enriched),
    enriched
  );
  return markDraftPack(pack, "research_grounded_human", failures, enriched);
}

function markDraftPack(pack, source, failures = [], input = {}) {
  const skipV17Polish =
    source === "research_grounded_human" ||
    pack?._meta?.researchGroundedHumanPack === true ||
...
function buildProseFallbackBodies(input) {
  ...
  const topic = resolveTopicRaw(input) || input.mainKeyword?.trim() || "이야기";
  const trendNote =
...
export function buildDeliverableBlogFallback({
  ...
  if (isBriclogMissionEnforced() && isGpt55WriterDominant()) {
    if (isAcceptableLlmDraftPack(bestPack, enriched)) {
      return {
        pack: markDraftPack(bestPack, "llm_draft", failures, enriched),
        source: "llm_draft",
      };
    }
    const researchFirst = buildResearchGroundedDraftFallback(enriched, failures);
    if (researchFirst) {
      return { pack: researchFirst, source: "research_grounded_human" };
    }
    const missionPack = buildMissionProseFallbackPack(enriched);
    if (!hasBlogSections(missionPack) || missionPack?._meta?.researchFirstWithheld) {
      return {
        pack: missionPack,
        source: missionPack?._meta?.generationMode || "research_first_withheld",
      };
    }
    return {
      pack: markDraftPack(
        missionPack,
        "mission_prose_fallback",
        failures,
        enriched
      ),
      source: "mission_prose_fallback",
  ...
  if (isBriclogMissionEnforced()) {
    ...
    if (!isIndustryHumanColumnEditorialPack(missionPack) && !missionPack._meta?.researchGroundedHumanPack) {
