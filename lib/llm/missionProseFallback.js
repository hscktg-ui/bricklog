...
function buildFlowerRecommendationSectionSets(p, input, sectionCount) {
...
function buildInformationalSectionSets(p, input, sectionCount) {
  if (isFlowerRecommendationTopic(input)) {
    return buildFlowerRecommendationSectionSets(p, input, sectionCount);
  }
...
export function buildMissionProseFallbackPack(input = {}) {
  if (shouldForceMissionProseOnlyPath(input)) {
    /* Experience+Opinion catalog only — editorial·조사 스니펫 경로 금지 */
  } else if (shouldUseEditorialQualityPath(input)) {
    return buildEditorialQualityPack(input);
  ...
  const title =
    ...
    ? `${p.regionBit}${p.brand} ${topicWritingFacet(input) || displayTopic}`.trim()
