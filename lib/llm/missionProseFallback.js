...
export function buildMissionProseFallbackPack(input = {}) {
  ...
  if (!hasResearch && !forcedMissionOnly) {
  ...
  if (hasResearch && !forcedMissionOnly && !researchHeavyRescue) {
    return buildResearchFirstWithheldPack(input, "research_grounded_failed");
  }

  if (!forcedMissionOnly && !researchHeavyRescue && !shouldUseResearchGroundedPack) {
    return buildResearchFirstWithheldPack(input);
  }
  const p = deriveTopicWritingContext(input);
  const forcedMission = forcedMissionOnly || researchHeavyRescue;
