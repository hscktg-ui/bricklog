...
export function buildMissionProseFallbackPack(input = {}) {
  ...
  if (isLocalServicePromoTopic(input)) {
    const pack = buildIndustryHumanColumnEditorialPack(input);
    if (pack?.sections?.length) return pack;
  }
  ...
  if (shouldUseResearchGroundedPack) {
    ...
    const editorial = buildIndustryHumanColumnEditorialPack(input);
    if (editorial?.sections?.length) {
      return finishBlogFallbackHumanProse(
        weaveResearchFactsIntoPack(editorial, input),
        input
      );
    }
  }
  ...
  if (
    ...
    const editorial = buildIndustryHumanColumnEditorialPack(input);
    if (editorial?.sections?.length) return editorial;
  }
  if (
  ...
  const p = deriveTopicWritingContext(input);
  const forcedMission =
    shouldForceMissionProseOnlyPath(input) || researchHeavyRescue;
