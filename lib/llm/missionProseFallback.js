...
import { resolveCustomerDeliveryBlogTargetChars } from "@/lib/content/missionProseGate";
import {
...
import { shouldForceMissionProseOnlyPath, shouldResearchHeavyMissionRescue } from "@/lib/product/missionProseRouteFlags";
import { isGpt55WriterDominant, isOpenAIConfigured } from "@/lib/llm/llmProvider";
import {
...
import { RESEARCH_FIRST_WITHHOLD_MESSAGE } from "@/lib/config/researchFirstFlags";

import { BRICLOG_BASELINE_V2 } from "@/lib/product/briclogUltimateV20";

const TIER_MIN_SECTIONS = BRICLOG_BASELINE_V2.missionSectionGoal;
...
function poolByArcRole(catalog, role, input = {}) {
  const tests = {
  ...
  const cleanCatalog = filterMissionExperienceParagraphs(catalog, input);
...
function buildInformationalSectionSets(p, input, sectionCount) {
  ...
  const industryRolePads = {
    education: {
      gi: [
        ...leadMissionGiParagraphs(input),
        `방학 특강을 알아보면 과목·시간·반 편성부터 막히는 날이 있습니다.`,
      ],
      seung: [
        `${p.brand} 상담실에서 ${subject} 대상 학년·특강 기간을 확인했습니다.`,
        `수업 시간표·커리큘럼 설명을 들으며 메모해 두었습니다.`,
      ],
      jeon: [
        `${subject} 비교 시 반 편성·수업 시간·등록 절차를 나눠 보면 기준이 분명해집니다.`,
        `방학 일정과 기존 학원 시간이 겹치는지 상담에서 짚어 봤습니다.`,
      ],
      gyeol: [
        `${p.regionBit}${p.brand} ${subject} — 대상 학년·특강 일정·등록 안내를 함께 보면 비교가 수월합니다.`,
        `궁금한 점은 상담으로 확인하시면 됩니다.`,
      ],
    },
    craft: {
      gi: [
        ...leadMissionGiParagraphs(input),
        `원데이 클래스를 찾다 보면 소요 시간·난이도부터 막히는 날이 있습니다.`,
      ],
      seung: [
        `${p.brand} 체험실에서 ${subject} 소요 시간·난이도·인원 제한을 확인했습니다.`,
        `완성품 사진과 작업 과정 설명을 함께 들었습니다.`,
      ],
      jeon: [
        `${subject} 비교 시 준비물·취소 규정·주차를 나눠 보면 기준이 분명해집니다.`,
        `예약금·취소 규칙은 예약 전에 확인하는 편이 좋습니다.`,
      ],
      gyeol: [
        `${p.regionBit}${p.brand} ${subject} — 소요 시간·예약·준비물을 함께 보면 선택이 수월합니다.`,
        `궁금한 점은 매장 문의로 확인하시면 됩니다.`,
      ],
    },
    flower: {
      gi: [...leadMissionGiParagraphs(input), `꽃을 고를 때 색감·포장·픽업 시간부터 확인하는 편이 좋습니다.`],
      seung: [
        `${p.brand}에서 ${subject} 관련 진열·포장 안내를 정리해 봤습니다.`,
        `색감·보관은 매장에서 직접 보면서 비교했습니다.`,
      ],
      jeon: [
        `선물 목적별로 ${subject} 구성이 달라질 수 있습니다.`,
        `비슷한 구성을 비교할 때는 포장·픽업 시간을 나란히 보면 수월합니다.`,
      ],
      gyeol: [
        `${p.regionBit}${p.brand} ${subject} — 종류·포장·픽업 시간을 함께 보면 선택이 수월합니다.`,
        `궁금한 점은 매장 문의로 확인하시면 됩니다.`,
      ],
    },
    default: {
      ...
      seung: [
        ...
        `${p.regionBit}${p.brand} ${flavor.spaceWord}에서 ${m0}·${m1} 안내를 들었습니다.`,
      ],
      jeon: [
        `${subject} 비교 시 ${m0}·${m1}·이용 목적을 함께 보면 기준이 분명해집니다.`,
        `두 가지 안을 놓고 보면 ${subject} 선택 기준이 조금씩 달라질 수 있습니다.`,
      ],
      gyeol: [
        `${p.regionBit}${p.brand} ${subject} — ${flavor.visitReason}에 맞는 ${flavor.productWord} 안내를 함께 보면 비교가 수월합니다.`,
        `궁금한 점은 매장 문의로 확인하시면 됩니다.`,
      ],
    },
  };
  const defaultByRole = industryRolePads[key] || industryRolePads.default;
  const sets = [];
  for (let i = 0; i < sectionCount; i += 1) {
    const role = roles[i] || "seung";
    const pool = [...poolByArcRole(catalog, role, input), ...defaultByRole[role]];
...
function buildGenericSectionSets(p, input, sectionCount) {
  ...
  const defaultByRole = {
    ...
    seung: [
      ...
      `현장에서 ${flavor?.productWord || "안내"}·조건을 하나씩 확인했어요.`,
    ],
    jeon: [
  ...
  for (let i = 0; i < sectionCount; i += 1) {
    const role = roles[i] || "seung";
    const pool = [...poolByArcRole(catalog, role, input), ...defaultByRole[role]];
    let paras = pickUniqueParagraphs(pool, usedGlobal, i, Math.max(5, 7));
    if (i === 0 && role === "gi") {
...
export function deepenMissionProseToMin(pack, minChars, input = {}) {
  ...
  if (shouldSuppressLengthTopoff(pack, input)) {
    ...
    let best = pack;
    let bestChars = before;
    for (let round = 0; round < 72 && bestChars < minChars; round += 1) {
      const next = dedupeMissionPackGlobally(
        deepenDensityFirstPack(best, minChars, input, {
          polishAfter: false,
          seedOffset: round,
          researchLines,
        })
      ...
      if (chars > bestChars && (next.sections?.length || 0) >= 3) {
        best = next;
        bestChars = chars;
      }
    }
    return bestChars >= before ? best : pack;
  }
...
function buildSectionBodies(input, sectionCount) {
  ...
  if (isVisitReviewTopicInput(input)) {
    return buildVisitIndustrySectionSets(p, input, sectionCount).map((paras) =>
      paras.join("\n\n")
    );
  }
...
export function dedupeMissionPackGlobally(pack) {
  const seen = new Set();
  const seenNorms = [];
  const isNearDup = (text) => {
    for (const prev of seenNorms) {
      if (wordOverlapRatio(prev, text) >= 0.72) return true;
    }
    return false;
  };
  const tryKeep = (t, force = false) => {
    const trimmed = String(t || "").trim();
    if (trimmed.replace(/\s/g, "").length < 12) return null;
    const key = paragraphKey(trimmed);
    if (!force && (seen.has(key) || isNearDup(trimmed))) return null;
    if (!seen.has(key)) seen.add(key);
    seenNorms.push(trimmed);
    return trimmed;
  };
  ...
  const sections = (pack.sections || []).map((sec) => {
    ...
    for (const para of raw) {
      const keptPara = tryKeep(para);
      if (keptPara) kept.push(keptPara);
    }
    if (!kept.length && raw.length) {
      const fallback = tryKeep(raw[0], true);
      if (fallback) kept.push(fallback);
    }
    return { ...sec, body: kept.join("\n\n").trim() };
  });

  const minKeep = pack?._meta?.researchGroundedHumanPack
    ? Math.min(4, (pack.sections || []).length)
    : Math.min(3, (pack.sections || []).length);
  let safeSections = sections.filter((s) => s.body.replace(/\s/g, "").length >= 40);
  if (safeSections.length < minKeep) {
    safeSections = sections.filter((s) => s.body.replace(/\s/g, "").length >= 20);
  }
  if (!safeSections.length) {
    safeSections = (pack.sections || []).filter(
      (s) => String(s.body || "").replace(/\s/g, "").length >= 20
    );
  }

  let conclusion = String(pack.conclusion || "").trim();
  if (conclusion.replace(/\s/g, "").length >= 12) {
    const concKey = paragraphKey(conclusion);
    if (seen.has(concKey) || isNearDup(conclusion)) {
      conclusion = "";
    } else {
      seen.add(concKey);
      seenNorms.push(conclusion);
    }
  }

  return { ...pack, sections: safeSections, conclusion };
}
...
function buildResearchFirstWithheldPack(input = {}, reason = "research_first_blocked") {
  return {
    sections: [],
    title: "",
    representativeTitle: "",
    _meta: {
      researchFirstWithheld: true,
      researchFirstBlocked: true,
      draftFallback: true,
      generationMode: "research_first_withheld",
      withholdReason: reason,
      userMessage: RESEARCH_FIRST_WITHHOLD_MESSAGE,
...
export function buildMissionProseFallbackPack(input = {}) {
  const researchHeavyRescue = shouldResearchHeavyMissionRescue(input);
  const forcedMissionOnly = shouldForceMissionProseOnlyPath(input);
  const hasResearch = hasUsableResearchFacts(input);
  ...
  if (
    ...
    return buildResearchFirstWithheldPack(input, "columnist_sovereign_required");
  }
  ...
  if (!hasResearch && !forcedMissionOnly && !input.batchLocalFinish) {
    return buildResearchFirstWithheldPack(input);
  }

  const shouldUseResearchGroundedPack =
    hasResearch &&
    !isFurnitureChairProductTopic(input) &&
    !isFlowerRecommendationTopic(input) &&
    !(isFurnitureIndustry(input) && isExhibitionTopic(input));

  if (hasResearch && !forcedMissionOnly && !input.batchLocalFinish) {
    if (
      isVisitReviewTopicInput(input) &&
      isOpenAIConfigured() &&
      process.env.BRICLOG_VISIT_REVIEW_SOVEREIGN !== "false"
    ) {
      return buildResearchFirstWithheldPack(input, "visit_review_sovereign_required");
    }
    const researchPack = buildResearchGroundedHumanPack(input);
    if (researchPack?.sections?.length >= 1) {
      return finishBlogFallbackHumanProse(
        stripIndustryContaminationFromPack(researchPack, input),
        input
      );
    }
  }

  const researchGroundedFailed = shouldUseResearchGroundedPack && hasResearch;
  const missionProseFallback =
    forcedMissionOnly ||
    researchHeavyRescue ||
    researchGroundedFailed ||
    !shouldUseResearchGroundedPack ||
    input.batchLocalFinish === true;

  if (
    isGpt55WriterDominant() &&
    !researchHeavyRescue &&
    !forcedMissionOnly &&
    !missionProseFallback
  ) {
    return buildResearchFirstWithheldPack(input, "gpt55_research_required");
  }

  if (!missionProseFallback && !hasResearch && !forcedMissionOnly) {
    return buildResearchFirstWithheldPack(input);
  }

  const p = deriveTopicWritingContext(input);
  const forcedMission =
    forcedMissionOnly || researchHeavyRescue || researchGroundedFailed || missionProseFallback;
  ...
  if (missionProseFallback) {
    ...
    const tierFloor =
      tierKey === "long" ? 6 : tierKey === "medium" ? 5 : 4;
    const catalogCap = Math.max(tierFloor, Math.ceil(Math.max(catalogSize, 1) / 2));
    sectionCount = Math.max(tierFloor, Math.min(sectionCount, catalogCap));
  }
  ...
  if (!forcedMission) {
    ...
    if (profile.archetype === "field_review" && isVisitReviewTopicInput(input)) {
  ...
  const deliveryTarget = resolveCustomerDeliveryBlogTargetChars(tierKey, tier);
  const lengthTarget = forcedMission ? deliveryTarget : tier.min;
  if (!forcedMission) {
    pack = applyNarrativeArcShape(pack, input, { force: true });
  }
  pack = dedupeMissionPackGlobally(pack);
  ...
  if (countBlogBodyCharsWithSpaces(pack) < lengthTarget) {
    let refill = 0;
    while (countBlogBodyCharsWithSpaces(pack) < lengthTarget && refill < 12) {
      pack = deepenMissionProseToMin(pack, lengthTarget, input);
      refill += 1;
    }
  }

  if (hasUsableResearchFacts(input)) {
    pack = weaveResearchFactsIntoPack(pack, input);
    pack = stripIndustryContaminationFromPack(pack, input);
    if (!pack._meta?.researchGroundedHumanPack) {
      pack = finishBlogFallbackHumanProse(pack, input);
    }
  } else {
    pack = stripIndustryContaminationFromPack(pack, input);
  }
  ...
  return {
    ...
    _meta: {
      ...
      customerDeliveryTarget: deliveryTarget,
      lengthTierMet: chars >= lengthTarget,
      lengthTierTarget: lengthTarget,
    },
  };
}
