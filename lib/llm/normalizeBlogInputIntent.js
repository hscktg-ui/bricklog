...
function buildIntentContract({
  ...
  return {
    ...
    purpose: purpose || "",
    objective: objective || "",
    blogLengthTier: blogLengthTier || "medium",
    businessFirst,
...
function buildLengthContract(blogLengthTier = "medium") {
  const tierKey = blogLengthTier || "medium";
...
export function normalizeBlogInputIntent(input = {}) {
  const lengthContract = buildLengthContract(input.blogLengthTier || "medium");
