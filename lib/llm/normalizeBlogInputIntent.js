...
export function normalizeBlogInputIntent(input = {}) {
  ...
  let mustInclude = mergeMustInclude(includeList, input.mainKeyword, primaryStory, []);
  for (const part of splitContextBeatParts(input.storeFeatures)) {
    if (part.length >= 4 && !mustInclude.some((m) => m.includes(part))) {
      mustInclude.push(part);
    }
  }
  ...
  const writingContract = resolveWritingContract({
    ...speakerMapped,
  ...
  const correctedInput = {
    ...
    _writingContract: writingContract,
    writingContract,
  };
