...
export function postProcessLlmChannel(channel, pack, ctx = {}, input = {}) {
  ...
  const writerFirst = shouldUseWriterFirstChannelPostProcess(input, next, channel);
  if (!writerFirst) {
  ...
  const outputPack = writerFirst
    ? finalizeWriterFirstChannelDelivery(graded, channel, input)
    : graded;

  return {
    pack: outputPack,
    passOutput: outputPack._meta?.passOutput ?? passOutput,
    charCount: outputPack._meta?.charCount ?? next._meta.charCount,
    fullText: getChannelFullText(outputPack, channel),
  };
}
