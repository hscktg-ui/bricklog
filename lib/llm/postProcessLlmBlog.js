...
import { getStrictLengthMaxAttempts } from "@/lib/config/briclogFastPipeline";
...
import {
  applyGpt55HumanityLightFinish,
  applyGpt55PostWriteLightPass,
  shouldUseGpt55LightDelivery,
} from "@/lib/product/gpt55LightDelivery";
import { shouldUseWriterFirstPostProcess } from "@/lib/product/writerFirstDelivery";
...
export function postProcessLlmBlog(pack, ctx, input = {}) {
  ...
  const writerFirst = shouldUseWriterFirstPostProcess(input, next);
  ...
  if (!writerFirst) {
    ...
    next = sanitizeBlogPackPlannerLeak(lengthNormalized.pack || next);
  } else {
  ...
  const pruned = { pack: next, removedCount: 0, removedSamples: [] };
  next = pruned.pack;
  if (!writerFirst) {
    next = applyBrandContentEngine(next, ctx, input);
    next = applyPerspectiveEngine(next, ctx, input);
  ...
  let v3Post = null;
  if (!writerFirst && (input.v3EngineEnforced || input.v3PreWriteVerified)) {
  ...
  if (shouldUseGpt55LightDelivery(next, input)) {
    next = applyGpt55PostWriteLightPass(next, input);
  } else {
  ...
  next._meta = finalizePipelineMeta(next, ctx, {
    charCount,
    blogLengthTier: input.blogLengthTier || ctx.blogLengthTier || DEFAULT_BLOG_LENGTH_TIER,
    qualityScore,
    hardValidation,
    noCopy,
    finalAudit,
  ...
  if (isGpt55WriterDominant() || isBriclogMissionEnforced()) {
