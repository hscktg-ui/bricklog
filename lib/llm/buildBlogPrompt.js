...
import { buildPersonaNarrativeFlowBrief } from "@/lib/persona/speakerVoiceLock";
import {
...
import { buildSpeakerPurposeExplainBrief } from "@/lib/product/briclogContentDoctrine";
...
import { buildHumanColumnNarrativeBrief } from "@/lib/product/humanColumnProseEngine";
import { buildEditorialQualityPromptBlock } from "@/lib/product/editorialQualityStandard";
import { buildGoldenReferencePromptBlock } from "@/lib/golden/goldenPromptReference";
import {
  buildHaeshinDnaPromptBlock,
  buildHaeshinPreGenerationChecklist,
} from "@/lib/golden/haeshinPromptBlocks";
...
import { buildWriterFirstOrchestratorHint } from "@/lib/product/writerFirstDelivery";

function writerFirstPromptAddon(ctx = {}) {
  const hint = buildWriterFirstOrchestratorHint(ctx.input || ctx);
  return hint ? `\n${hint.replace(/\n/g, " ")}` : "";
}
...
function blogLengthForCtx(ctx) {
  const tier =
    ctx.input?.blogLengthTier || ctx.blogLengthTier || DEFAULT_BLOG_LENGTH_TIER;
  return resolveBlogLengthTier(tier);
}

function slimEngineRulesBlock(ctx) {
  return [
    BRICLOG_FAST_PIPELINE_BRIEF.replace(/\n/g, " "),
    HUMAN_DELIVERY_PROMPT_RULES.replace(/\n/g, " "),
    `Banned clichés: ${CONSTITUTION_V2_AI_CLICHES.slice(0, 6).join(", ")}.`,
    `Outline forbidden: ${OUTLINE_PLAN_HEADING_PHRASES.slice(0, 5).join(" / ")}.`,
    ...
    buildSensitiveSystemAddon(ctx.sensitiveCompliance),
...
function blogOnlySystemPrompt(ctx) {
  ...
  const tier = ctx.input?.blogLengthTier || ctx.blogLengthTier || DEFAULT_BLOG_LENGTH_TIER;
  ...
  if (isSlimWriterPromptEnabled()) {
    return `${buildConstitutionSystemAddon("blog", ctx)}

You are BRICLOG — professional brand columnist. Write publish-ready Korean blog JSON only.
...
Core rules:
- Follow the user brief (facts, brand, region, topic). No filler, no FAQ/checklist tone.
- Never paste the user's topic string verbatim into title/body.
- Scene-first column (기→승→전→결). SEO is an outcome, not the goal.
- Region: ${ctx.region || "(omit)"} | Brand: ${ctx.brandName || "(omit)"} | Industry: ${ctx.industryLabel || "(omit)"}
- LENGTH: ${blogMin}–${blogMax} chars (spaces), target ~${blogTarget}. Under ${blogMin} is REJECTED. GPT-5.6 default brevity does not apply — hit the target with scenes, reasons, and experience, never generic padding.
- sections: ${longForm ? "6–7" : "5–6"} with substantial bodies; titles: exactly 5.
- STRICT EXCLUDE: ${forbidden}
- Unverified specs/prices forbidden. Korean only.
- ${slimEngineRulesBlock(ctx)}${writerFirstPromptAddon(ctx)}`;
  }
...
- LENGTH TIER (customer promise): ${blogMin}–${blogMax} chars (spaces included), target ~${blogTarget}. GPT-5.6 default brevity does not apply — hit the target with scenes, reasons, and experience; never pad with filler; trim if over max.
- ${HUMAN_DELIVERY_PROMPT_RULES.replace(/\n/g, " ")}
...
- ${buildHumanColumnNarrativeBrief(ctx.input || ctx).replace(/\n/g, " ")}
- ${buildEditorialQualityPromptBlock(ctx.input || ctx).replace(/\n/g, " ")}
- ${buildGoldenReferencePromptBlock(ctx.input || ctx).replace(/\n/g, " ")}
- ${buildHaeshinDnaPromptBlock(ctx.input || ctx).replace(/\n/g, " ")}
- ${buildHaeshinPreGenerationChecklist(ctx.input || ctx).replace(/\n/g, " ")}
- ${buildExperienceVoicePromptBlock().replace(/\n/g, " ")}
- ${buildVisitReviewUnifiedProsePromptBlock().replace(/\n/g, " ")}
- ${buildHumanBeliefPromptBlock(ctx.input || ctx).replace(/\n/g, " ")}
...
User input overrides conflicting inference.${buildSensitiveSystemAddon(ctx.sensitiveCompliance)}${ctx.input?.evolutionLabBrief || getEvolutionPromptAddon(ctx.input || ctx)}${writerFirstPromptAddon(ctx)}`;
}

function buildStackResearchBrief(ctx = {}) {
  const input = ctx.input || ctx;
  const outline = input.geminiWriterOutline || ctx.geminiWriterOutline;
  const parts = [
    ctx.researchFirstBrief || input.researchFirstBrief,
    ctx.geminiWriterBrief || input.geminiWriterBrief,
    input.researchBrief,
    Array.isArray(outline) && outline.length
      ? `【조사 기반 작성 아웃라인】\n${outline.slice(0, 8).join("\n")}`
      : "",
  ].filter(Boolean);
  return parts.join("\n\n").slice(0, 3600);
}
...
export function buildBlogUserPrompt(ctx) {
  ...
  const authoritativeBrief = [
    ctx.canonicalBrief ||
      ...
      null,
    ctx.brandDepthAuthoritativeBrief ||
      ctx.input?.brandDepthAuthoritativeBrief ||
      ctx.input?._brandDepthBrief ||
      null,
  ]
    .filter(Boolean)
    .join("\n\n") || null;
  ...
  const stackResearchBrief = buildStackResearchBrief(ctx);

  const directorBrief =
    ctx.directorMasterBrief || ctx.input?.directorMasterBrief || "";
  const slim = isSlimWriterPromptEnabled();
  const directorCap = slim ? 1400 : 6200;
  const personaNarrative = buildPersonaNarrativeFlowBrief(ctx.input || ctx);

  if (slim) {
    return `${buildConstitutionUserAddon(ctx)}
${directorBrief ? `\n${directorBrief.slice(0, directorCap)}\n\n` : ""}아래 브랜드 컨텍스트만 사용해 위 JSON 형식의 blog 객체만 작성하세요.
추정·업종 일반론·검색어 나열·AI 소제목 금지. 주제 문장 그대로 복사 금지.
서사: ${personaNarrative}. FAQ·체크리스트·홍보 문장 금지. 구성안 출력 금지.
${buildBrandMemoryUserSection(ctx)}
${authoritativeBrief ? `\n${authoritativeBrief}\n\n` : ""}${ctx.brandWikiBrief || ctx.input?.brandWikiBrief ? `\n${(ctx.brandWikiBrief || ctx.input?.brandWikiBrief || "").slice(0, 1600)}\n\n` : ""}${buildSpeakerPurposeExplainBrief(ctx.input || ctx) ? `\n${buildSpeakerPurposeExplainBrief(ctx.input || ctx).slice(0, 1400)}\n\n` : ""}
【수집 실마리 · 조사】
${factsPrompt}
${stackResearchBrief ? `\n${stackResearchBrief}\n` : ""}

${ctx.v2AxisBrief || ctx.input?.v2AxisBrief ? `【조사 요약】\n${(ctx.v2AxisBrief || ctx.input?.v2AxisBrief || "").slice(0, 1200)}\n\n` : ""}${ctx.customerQuestionBrief || ctx.input?.customerQuestionBrief ? `${(ctx.customerQuestionBrief || ctx.input?.customerQuestionBrief || "").slice(0, 1200)}\n\n` : ""}【주제】 ${ctx.informationUnits?.topicFacet || ctx.contentThesis || ctx.writingSubject || ctx.topic}
【META】 유형=${ctx.metaStrategy?.contentType || "-"} | 목적=${ctx.metaStrategy?.purpose || "-"} | 독자=${ctx.metaStrategy?.audience || "-"} | 톤=${ctx.metaStrategy?.tone || "-"}
【길이】 tier=${tierLabel} | ${blogMin}~${blogMax}자 (목표 ${blogTarget})
【화자】 ${ctx.contentPersonaLabel || ctx.contentPersona}
...
【브랜드】 ${profile.brandName || ctx.brandName || "-"} | ${profile.region || ctx.region || "-"} | ${profile.industry || ctx.industryLabel || "-"}
...
서사: ${personaNarrative}. FAQ·체크리스트·홍보 문장(알려드립니다·많은 분들께·지금 바로)은 쓰지 마세요.
...
${(() => {
  const inp = ctx.input || ctx;
  const regen =
    (inp.rewriteCount || 0) > 0 ||
    inp.regenVariation ||
    inp.feedbackRegenDirective === "regen_variation_required" ||
    (Array.isArray(inp.feedbackHints) && inp.feedbackHints.length > 0);
  if (!regen) return "";
  const brief = String(inp.brandFeedbackBrief || inp.feedbackRegenDirective || "").slice(0, 480);
  return `\n【다시 받기·피드백】 이전 결과와 다른 구성·도입·사례 각도로 작성. 동일 문장·소제목 반복 금지. ${brief}`;
})()}
JSON만 반환. blog 키만 포함.${buildSensitiveUserAddon(
    ctx.sensitiveCompliance,
    ctx.pipeline?.complianceRegenNote || ctx.pipeline?.regenNote
