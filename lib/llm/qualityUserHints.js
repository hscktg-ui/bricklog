...
const HINTS = {
  ...
  customer_questions_missing:
    "검색자가 왜 찾는지 분석한 뒤, 왜·누가·언제·비교·자주 묻는 질문·구매 전 실수에 답해 주세요.",
  topic_description_only:
    "주제 설명만 하지 말고, 검색한 사람의 질문에 답하는 본문으로 써 주세요.",
  ...
  human_belief_low:
    "광고·확인하세요 반복 대신, 직접 가 본 사람이 쓴 것처럼 장면과 비교를 담아 주세요.",
  brand_feature: "브랜드 특징이 본문에 더 녹아야 합니다.",
  first_delivery_regen:
    "첫 편집본 기준에 맞게 장면·비교·에디터 톤을 한 번 더 다듬는 중입니다.",
  first_delivery_human_editor:
    "사람 에디터가 쓴 것처럼 장면과 비교를 더 담아 주세요.",
  first_delivery_editor_v95:
    "도입·리듬·반복 표현을 에디터 기준에 맞게 다듬어 주세요.",
  first_delivery_persona: "선택한 화자 톤을 처음부터 끝까지 유지해 주세요.",
  first_delivery_human_belief:
    "광고 문장 대신 직접 본 것처럼 쓰는 톤을 살려 주세요.",
  visit_speaker_mismatch:
    "방문 후기 주제에는 담백한 후기형·실사용 후기형 화자가 잘 맞아요.",
  info_speaker_mismatch:
    "정보·가이드 주제에는 전문 정보형·매거진·칼럼형 화자가 잘 맞아요.",
  information_yield_low:
    "주제에 대한 구체 정보·비교·기준이 더 필요해요.",
};

export function buildQualityUserHint(failures = []) {
  const list = [...new Set(failures)].filter(Boolean);
  if (!list.length) {
    return `편집본을 준비했습니다. 확인 후 필요하면 「${RETRY.cta}」를 눌러 주세요.`;
  }
  const tips = list
    .slice(0, 3)
  ...
  return `편집본을 준비했습니다. ${detail} 필요하면 「${RETRY.cta}」를 눌러 주세요.`;
}
