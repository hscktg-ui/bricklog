...
export function buildBlogExpansionMessages(pack, ctx = {}, input = {}) {
  const tierKey =
 ...
 * @param {object} [input]
 */
export async function expandPackToHumanTierViaLlm(pack, ctx = {}, input = {}) {
  if (!pack?.sections?.length) return pack;
  if (isHumanTierMet(pack, input)) return pack;

  try {
    const messages = buildBlogExpansionMessages(pack, ctx, input);
