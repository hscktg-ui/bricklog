...
function buildSectionPlanHint(input = {}) {
  const plan = buildSectionPlan({}, input);
  return (plan.sections || [])
    .slice(0, 8)
    .map((s, i) => `${i + 1}) ${s.infoUnit}(${s.headingSuffix})`)
    .join(" · ");
}

function buildDensityRules(contract = null, input = {}) {
  const gaps = (contract?.reasons || [])
    ...
    .join(", ");
  const planHint = buildSectionPlanHint(input);
  const arcRule =
    "Sections follow gi→seung→jeon→gyeol; each section ONE distinct fact/scene, no repeated 「관련 안내를 들었어요」 pads.";
  if (!gaps) {
    return `${arcRule} Plan units: ${planHint || "visit scene, compare, personal wrap"}.`;
  }
  return `Fix quality gaps (${gaps}): ${arcRule} Assign one plan unit per section: ${planHint}. No template boilerplate, no duplicate sentences.`;
}
...
function stampUpgradedColumnMeta(original, next, input = {}, kind = "expansion") {
  ...
  return {
    ...
    _meta: {
      ...meta,
      llmGenerated: true,
      generationMode: "llm_gpt55_human_column",
...
export function buildBlogRewriteMessages(pack, ctx = {}, input = {}, contract = null) {
  ...
  const density = buildDensityRules(contract, input);

  const arcBrief = buildNarrativeArcWriterBrief(input);
  const magazineBrief = buildMagazineArcPromptBlock(input);
  const columnBrief = buildHumanColumnPromptAddon(input);
...
Rules:
- Write FRESH from research — discard template phrases like 「헷갈리는 포인트」「정리했습니다」「관련 조건은 제품·시즌에 따라」「확인된 범위에서」.
- Target ${tier.min}–${tier.max} chars WITH SPACES; ${HUMAN_MIN_SECTIONS}+ sections; gi(why)→seung(visit/scene)→jeon(compare/criteria)→gyeol(personal wrap) — each section ONE new fact or scene.
- ${density}
- Scene-first opening; 솔직히·직접·다녀왔어요·생각보다 3+ times total.
- Korean only; 해요체 경험 구어; 습니다체·FAQ·체크리스트·브로슈어체 금지.
- 업종 밖 단어(카페 글에 침대·꽃집, 꽃집 글에 매트리스 등) 금지. 검색 스니펫·블로그 잡담(ㅎㅎ·~·찾아보시면) 복사 금지. 섹션마다 브랜드·지역·주제 중 하나는 반드시 보이게.
...
- ${buildHumanVoiceWriterBrief().replace(/\n/g, " ")}
- ${buildDensityRules(contract, input)}`,
    },
    {
      role: "user",
      content: `Brand: ${input.brandName || ctx.brandName || "—"}
