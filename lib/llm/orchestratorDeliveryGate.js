/**
 * 오케스트레이터 fallback — V2/V3·베타 가드 통과 시에만 사용자 출력
 */
import {
  assertPostWriteDeliverable,
  requiresV2ResearchGate,
  researchGateBlockedResult,
} from "@/lib/content/v2PipelineGate";
import {
  buildDeliveryQualityHint,
  formatPostVerifyUserMessage,
} from "@/lib/product/customerOutput";
import { attachDeliveryTelemetry } from "@/lib/product/deliveryTelemetry";
import { deliverBlogDespiteGate } from "@/lib/product/deliverySoftPass";
import { LLM_USER_MESSAGES } from "@/lib/llm/messages";
import {
  buildDeliverableBlogFallback,
  enrichMinimalBlogInput,
} from "@/lib/llm/blogDeliveryFallback";
import { ensureV17MissionPolish, ensureNaverChannelClean } from "@/lib/content/v17PostProcess";
import {
  assertCompleteBlogPackForDelivery,
} from "@/lib/product/completeDeliveryGate";
import { isBriclogMissionEnforced } from "@/lib/product/missionFlags";
import { buildMissionRescueApiDelivery } from "@/lib/generation/missionRescueDelivery";
import { tryDeliverResearchHeavyMissionRescue } from "@/lib/product/missionProseRouteEngine";
import {
  isWriterFirstRescueBlocked,
  buildWriterFirstWithholdMessage,
} from "@/lib/product/writerFirstDelivery";
import {
  isLaunchPublishFirstMode,
  finalizeLaunchPublishBlogPack,
} from "@/lib/config/launchPublishMode";

function withholdWriterFirstOrchestrator(evalInput, partial = {}) {
  if (!isWriterFirstRescueBlocked(evalInput)) return null;
  return {
    ok: false,
    blogContent: null,
    withheld: true,
    softPass: false,
    mode: partial.mode || "writer_first_withheld",
    userMessage:
      partial.userMessage || buildWriterFirstWithholdMessage(evalInput),
    userDetail: partial.userDetail ?? null,
    llmAvailable: partial.llmAvailable ?? true,
    baseContentLabel: partial.baseContentLabel ?? null,
    meta: {
      ...(partial.meta || {}),
      passOutput: false,
      writerFirstRescueBlocked: true,
      failReasons: partial.meta?.failReasons || ["writer_first_rescue_blocked"],
    },
  };
}

/**
 * @param {object} input
 * @param {object|null} pack
 * @param {object} [partial]
 */
export function gateOrchestratorBlogPack(input, pack, partial = {}) {
  const evalInput = { ...input, contentChannel: "blog" };

  if (pack?.sections?.length && isLaunchPublishFirstMode()) {
    const delivered = finalizeLaunchPublishBlogPack(pack, evalInput);
    return {
      ok: true,
      blogContent: delivered,
      withheld: false,
      softPass: false,
      mode: "llm",
      userMessage: null,
      userDetail: partial.userDetail ?? null,
      llmAvailable: partial.llmAvailable ?? true,
      baseContentLabel: partial.baseContentLabel ?? null,
      meta: {
        ...(partial.meta || {}),
        passOutput: true,
        v2PipelineVerified: true,
        launchPublishFirst: true,
        generationMode: "llm",
      },
    };
  }

  pack = ensureV17MissionPolish(pack, evalInput, "blog");
  pack = ensureNaverChannelClean(pack, evalInput);

  if (!pack?.sections?.length) {
    const enriched = enrichMinimalBlogInput(evalInput);
    const { pack: fallback } = buildDeliverableBlogFallback({
      input: enriched,
      failures: ["empty_pack"],
    });
    pack = fallback;
    if (!pack?.sections?.length) {
      const blocked = withholdWriterFirstOrchestrator(evalInput, {
        ...partial,
        mode: "empty_pack",
        meta: { ...(partial.meta || {}), failReasons: ["empty_pack"] },
      });
      if (blocked) return blocked;
      const researchHeavy = tryDeliverResearchHeavyMissionRescue(evalInput);
      if (researchHeavy?.ok && researchHeavy?.blogContent?.sections?.length) {
        return researchHeavy;
      }
      const rescued = buildMissionRescueApiDelivery(
        evalInput,
        { reasons: ["empty_pack"] },
        { ...partial, mode: partial.mode || "draft_fallback" }
      );
      if (rescued) return rescued;
      return {
        ok: false,
        blogContent: null,
        withheld: true,
        softPass: false,
        mode: partial.mode || "empty_pack",
        userMessage: partial.userMessage || LLM_USER_MESSAGES.qualityWithheld,
        userDetail: partial.userDetail ?? null,
        llmAvailable: partial.llmAvailable ?? false,
        baseContentLabel: partial.baseContentLabel ?? null,
        meta: {
          ...(partial.meta || {}),
          passOutput: false,
        },
      };
    }
  }

  if (!requiresV2ResearchGate(evalInput)) {
    if (isBriclogMissionEnforced()) {
      const complete = assertCompleteBlogPackForDelivery(pack, evalInput);
      if (!complete.ok) {
        return {
          ok: false,
          blogContent: null,
          withheld: true,
          softPass: false,
          mode: partial.mode || "draft_fallback_withheld",
          userMessage: complete.userMessage || LLM_USER_MESSAGES.qualityWithheld,
          userDetail: partial.userDetail ?? null,
          llmAvailable: partial.llmAvailable ?? false,
          baseContentLabel: partial.baseContentLabel ?? null,
          meta: {
            ...(partial.meta || {}),
            passOutput: false,
            failReasons: complete.reasons,
          },
        };
      }
    }
    const meta = attachDeliveryTelemetry(
      {
        ...(partial.meta || {}),
        passOutput: partial.meta?.passOutput ?? false,
        softPass: partial.softPass !== false,
        draftFallback: true,
        generationMode:
          pack._meta?.industryHumanColumnEditorial ||
          pack._meta?.missionProseFallback
            ? "mission_rescue_delivery"
            : partial.meta?.generationMode || "draft_fallback",
      },
      pack
    );
    return {
      ok: true,
      blogContent: pack,
      withheld: false,
      softPass: partial.softPass !== false,
      mode:
        pack._meta?.industryHumanColumnEditorial ||
        pack._meta?.missionProseFallback
          ? "mission_rescue_delivery"
          : partial.mode || "draft_fallback",
      userMessage:
        buildDeliveryQualityHint(meta, pack) ||
        partial.userMessage ||
        null,
      userDetail: partial.userDetail ?? LLM_USER_MESSAGES.draftFallbackDetail,
      llmAvailable: partial.llmAvailable ?? false,
      baseContentLabel: partial.baseContentLabel ?? null,
      meta,
    };
  }

  const gate = assertPostWriteDeliverable(evalInput, pack);
  if (!gate.ok) {
    const preview = deliverBlogDespiteGate(evalInput, pack, gate, {
      ...partial,
      mode: partial.mode || "draft_fallback_preview",
      llmAvailable: partial.llmAvailable ?? false,
      userMessage: null,
      userDetail: partial.userDetail ?? null,
      baseContentLabel: partial.baseContentLabel ?? null,
    });
    if (preview) {
      return {
        ...preview,
        meta: {
          ...(preview.meta || {}),
          ...(partial.meta || {}),
          draftFallback: partial.meta?.draftFallback ?? true,
        },
      };
    }
    const blocked = withholdWriterFirstOrchestrator(evalInput, {
      ...partial,
      mode: partial.mode || "draft_fallback",
      meta: {
        ...(partial.meta || {}),
        failReasons: gate.reasons || gate.failReasons || ["post_write_gate"],
      },
    });
    if (blocked) return blocked;
    const rescued = buildMissionRescueApiDelivery(evalInput, gate, {
      ...partial,
      mode: partial.mode || "draft_fallback",
    });
    if (rescued) return rescued;
    return researchGateBlockedResult(
      evalInput,
      {
        ok: false,
        userMessage:
          formatPostVerifyUserMessage(gate) ||
          partial.userMessage ||
          LLM_USER_MESSAGES.qualityWithheld,
        reasons: gate.reasons || gate.failReasons,
        stage: gate.stage,
        betaTestGuard: gate.betaTestGuard,
      },
      pack
    );
  }

  if (gate.ok && isBriclogMissionEnforced()) {
    const complete = assertCompleteBlogPackForDelivery(gate.pack || pack, evalInput);
    if (!complete.ok) {
      const rescued = buildMissionRescueApiDelivery(evalInput, complete, {
        ...partial,
        mode: partial.mode || "draft_fallback",
      });
      if (rescued) return rescued;
      const blocked = withholdWriterFirstOrchestrator(evalInput, {
        ...partial,
        mode: partial.mode || "draft_fallback",
        meta: {
          ...(partial.meta || {}),
          failReasons: complete.reasons || ["complete_delivery_gate"],
        },
      });
      if (blocked) return blocked;
      return researchGateBlockedResult(
        evalInput,
        {
          ok: false,
          userMessage: complete.userMessage || LLM_USER_MESSAGES.qualityWithheld,
          reasons: complete.reasons,
        },
        gate.pack || pack
      );
    }
  }

  const rescueMode =
    (gate.pack || pack)?._meta?.industryHumanColumnEditorial ||
    (gate.pack || pack)?._meta?.missionProseFallback
      ? "mission_rescue_delivery"
      : partial.mode || "draft_fallback";

  return {
    ok: true,
    blogContent: gate.pack,
    withheld: false,
    softPass: false,
    mode: rescueMode,
    userMessage: null,
    userDetail: partial.userDetail ?? null,
    llmAvailable: partial.llmAvailable ?? true,
    baseContentLabel: partial.baseContentLabel ?? null,
    meta: {
      ...(partial.meta || {}),
      v2PipelineVerified: true,
      v3PipelineVerified: true,
      passOutput: true,
      softPass: false,
      draftFallback: partial.meta?.draftFallback ?? true,
      generationMode: rescueMode,
      betaTestGuard: gate.betaTestGuard,
    },
  };
}

/**
 * @param {object} input
 * @param {object|null} pack
 * @param {string} channel
 * @param {object} [partial]
 */
export function gateOrchestratorChannelPack(input, pack, channel, partial = {}) {
  const evalInput = { ...input, contentChannel: channel };
  const key =
    channel === "place"
      ? "placeContent"
      : channel === "instagram"
        ? "instagramContent"
        : "imagePrompts";

  if (!pack) {
    return {
      ok: false,
      [key]: null,
      withheld: true,
      softPass: false,
      userMessage: LLM_USER_MESSAGES.unavailable,
      mode: "empty_channel",
    };
  }

  if (!requiresV2ResearchGate(evalInput)) {
    return {
      ok: true,
      [key]: pack,
      withheld: false,
      softPass: true,
      ...partial,
    };
  }

  const gate = assertPostWriteDeliverable(evalInput, pack);
  if (!gate.ok) {
    return {
      ok: false,
      [key]: null,
      withheld: true,
      softPass: false,
      userMessage:
        formatPostVerifyUserMessage(gate) || LLM_USER_MESSAGES.qualityWithheld,
      mode: gate.stage || "output_verification_failed",
      meta: { failReasons: gate.reasons },
    };
  }

  return {
    ok: true,
    [key]: gate.pack,
    withheld: false,
    softPass: false,
    meta: {
      ...(partial.meta || {}),
      v2PipelineVerified: true,
      passOutput: true,
    },
    ...partial,
  };
}
