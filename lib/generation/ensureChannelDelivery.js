...
} from "@/lib/product/brandContentCustomerGate";
import { shouldUseDerivedChannelLocalOnly } from "@/lib/config/briclogFastPipeline";
import { finalizeWriterFirstChannelDelivery } from "@/lib/product/channelWriterFirst";
import { finishLocalChannelPackForBatch } from "@/lib/product/localBatchFinish";
...
function finalizeChannelForDelivery(channel, content, pipelineInput) {
  ...
  if (
    ...
    next = finalizeWriterFirstChannelDelivery(next, channel, pipelineInput);
    next = applyProdChannelBeliefBoost(next, channel, pipelineInput);
    try {
      return stampCoreEngineDeliveryMeta(next, pipelineInput, channel);
    } catch {
      return next;
    }
  }
  if (channel === "place" || channel === "instagram") {
    next = finishChannelPackForDelivery(channel, next, {
      input: pipelineInput,
      sourceChannel: pipelineInput.sourceChannel,
      ...pipelineInput,
    });
    next = applyProdChannelBeliefBoost(next, channel, pipelineInput);
  }
  next = applyAGradeChannelPass(next, channel, pipelineInput);
  try {
    return stampCoreEngineDeliveryMeta(next, pipelineInput, channel);
  } catch {
    return next;
...
function packChannelForCustomerDelivery(channel, pack, pipelineInput, partial = {}) {
  const key = CONTENT_KEYS[channel];
  if (!pack || !hasDeliverable(channel, pack)) return null;

  let content = applyCustomerChannelHygienePass(pack, channel, pipelineInput, {
    fromVerifiedBlog: pack?._meta?.derivedFromVerifiedBlog,
  });
  ...
  content = finalizeChannelForDelivery(channel, content, pipelineInput);
  const enforced = enforceCustomerChannelOutput(content, channel, pipelineInput);
  if (!enforced.ok) {
    return {
      ok: false,
      [key]: null,
      withheld: true,
      userMessage: enforced.userMessage,
      mode: enforced.reason || "channel_customer_gate",
      meta: {
        contentChannel: channel,
        channelNorthStar: enforced.northStar,
      },
    };
  }
...
function finishDerivedChannelPack(channel, pack, pipelineInput) {
  if (!pack) return pack;
  const batchInput = {
    ...pipelineInput,
  ...
  if (northStar) {
    const north = assessChannelVisitNorthStar(pack, channel, batchInput);
    if (north.publishOk && isChannelPackDeliverable(channel, pack)) {
      return stampCoreRulesOnDelivery(pack, batchInput, channel);
    }
  }
  return finishLocalChannelPackForBatch(pack, channel, batchInput);
}

function verifyDerivedChannelPostWrite(channel, pipelineInput, pack) {
  const channelInput = { ...pipelineInput, contentChannel: channel, sourceChannel: "blog" };
  const finished = finishDerivedChannelPack(channel, pack, pipelineInput);
  const first = assessChannelFirstDeliveryQuality(finished, channel, channelInput);
  if (first.displayReady || first.northStarFastPass) {
    return {
      ok: true,
      pack: {
        ...finished,
        _meta: {
          ...(finished._meta || {}),
          channelNorthStarFastPass: Boolean(first.northStarFastPass),
          passOutput: true,
          writtenFromVerifiedResearch: true,
        },
      },
      channelFirstDelivery: first,
    };
  }
  return assertPostWriteDeliverable(channelInput, finished);
}

export function deliverDerivedChannelFromBlog(channel, pipelineInput, sourceBlog, hooks = {}) {
  const { setPipelineStep } = hooks;
  const v2Gate = requiresV2ResearchGate(pipelineInput);

  let pack =
    tryFastChannelFromVerifiedBlog(channel, sourceBlog, pipelineInput) || null;
  if (!pack || !hasDeliverable(channel, pack)) {
    const { pack: fallbackPack } = buildDeliverableChannelFallback(channel, {
      input: pipelineInput,
      sourceBlog,
      instaTone: pipelineInput.instaTone,
      failures: ["sla_local_derive"],
    });
    pack = fallbackPack;
  }
  ...
  if (!pack || !hasDeliverable(channel, pack)) {
    ...
    return {
      ok: false,
      [key]: null,
      userMessage: "채널 편집본을 만들지 못했습니다.",
      mode: "derived_local_empty",
    };
  }

  if (v2Gate) {
    setPipelineStep?.(CUSTOMER_PIPELINE_STEP_LABELS.review);
    let post = verifyDerivedChannelPostWrite(channel, pipelineInput, pack);
    if (!post.ok) {
      pack = ensurePublishableChannelPack(channel, pack, pipelineInput, {
        sourceBlog,
        instaTone: pipelineInput.instaTone,
      });
      post = verifyDerivedChannelPostWrite(channel, pipelineInput, pack);
    }
    if (!post.ok) {
      const key = CONTENT_KEYS[channel];
      return {
        ...
        meta: { failReasons: post.reasons || [], contentChannel: channel },
      };
    }
    pack = post.pack;
  }
  ...
  const key = CONTENT_KEYS[channel];
  const delivered = packChannelForCustomerDelivery(channel, pack, pipelineInput, {
    mode: "verified_blog_derive",
    meta: { derivedLocalOnly: true, slaPack: true },
  });
  if (delivered?.ok && delivered[key]) {
    delivered[key] = {
      ...delivered[key],
      _meta: {
        ...(delivered[key]._meta || {}),
        derivedLocalOnly: true,
      },
    };
  }
  return (
    delivered || {
      ok: false,
      [CONTENT_KEYS[channel]]: null,
      userMessage: "채널 편집본을 화면에 올리지 못했습니다.",
      mode: "derived_local_gate",
    }
  );
}
...
export async function ensureChannelDelivery(channel, pipelineInput, hooks = {}) {
  ...
  if (shouldUseDerivedChannelLocalOnly(pipelineInput, channel, sourceBlog)) {
    return deliverDerivedChannelFromBlog(channel, pipelineInput, sourceBlog, hooks);
  }
  ...
  if (
    ...
    const fast = tryFastChannelFromVerifiedBlog(channel, sourceBlog, pipelineInput);
    if (fast && hasDeliverable(channel, fast)) {
      if (v2Gate) {
        setPipelineStep?.(CUSTOMER_PIPELINE_STEP_LABELS.review);
        const post = verifyDerivedChannelPostWrite(channel, pipelineInput, fast);
        if (post.ok) {
          const delivered = packChannelForCustomerDelivery(
            channel,
            post.pack,
            pipelineInput,
            { mode: "verified_blog_derive" }
  ...
  try {
    ...
    if (hasDeliverable(channel, content)) {
      if (v2Gate) {
        ...
        return packChannelForCustomerDelivery(
          channel,
          post.pack,
          pipelineInput,
          partial
      ...
      return packChannelForCustomerDelivery(channel, content, pipelineInput, {
        ...partial,
        ok: partial.ok !== false,
      });
    }
