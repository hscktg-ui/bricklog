...
function deliverDerivedChannelFromBlog(channel, pipelineInput, sourceBlog, hooks = {}) {
  ...
  if (v2Gate) {
    ...
    let post = assertPostWriteDeliverable(pipelineInput, pack);
    if (!post.ok) {
      pack = ensurePublishableChannelPack(channel, pack, pipelineInput, {
        sourceBlog,
        instaTone: pipelineInput.instaTone,
      });
      post = assertPostWriteDeliverable(pipelineInput, pack);
    }
    if (!post.ok) {
      const key = CONTENT_KEYS[channel];
      return {
...
export async function ensureChannelDelivery(channel, pipelineInput, hooks = {}) {
  ...
  if (
    ...
    if (fast && hasDeliverable(channel, fast)) {
      if (v2Gate) {
        setPipelineStep?.(CUSTOMER_PIPELINE_STEP_LABELS.review);
        const post = assertPostWriteDeliverable(pipelineInput, fast);
        if (post.ok) {
