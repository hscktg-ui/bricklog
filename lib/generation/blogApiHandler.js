  ...
  try {
    ...
    if (columnistFirst) {
      ...
      if (!rawResult) {
        ...
        if (!throttle) {
          const { attemptSovereignQualityLeapDeliver } = await import(
            "@/lib/product/sovereignAlwaysDeliver"
          );
          const leap = attemptSovereignQualityLeapDeliver(hydratedInput, columnistFailDiagnostic);
          if (leap?.pack?.sections?.length) {
            rawResult = {
              ok: true,
              withheld: false,
              softPass: Boolean(leap.pack._meta?.qualityLeapSalvage),
              userMessage: null,
              blogContent: leap.pack,
              mode: leap.mode,
              meta: {
                columnistFirstFastPath: true,
                columnistSlowFallback,
                columnistFailDiagnostic,
                sovereignQualityLeap: true,
                qualityLeapSource: leap.source,
                generationMode: leap.mode,
                v2PipelineVerified: true,
                v3PipelineVerified: true,
                passOutput: true,
                serverVerifiedSkipClientReverify: true,
                fastPipelineDelivery: true,
                unifiedDelivery: leap.unified,
              },
            };
          }
        }
