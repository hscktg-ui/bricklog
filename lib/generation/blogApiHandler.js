...
import { UNEDITED_PUBLISH_WITHHOLD_MESSAGE } from "@/lib/product/uneditedPublishGradeGate";
...
export async function runBlogApiGeneration(auth, rawInput, opts = {}) {
  ...
  try {
    ...
    if (columnistFirst) {
        ...
        if (isCustomerTwoMinuteSlaMode()) {
          ...
          for (let attempt = 0; attempt < slaRetries && !sovereignPack?.sections?.length; attempt += 1) {
            sovereignPack = await generateColumnistSovereignPack({
              ...
              columnistMaxLlmRounds: 1,
            });
          }
          if (!sovereignPack?.sections?.length && shouldUseColumnistSlaSlowFallback()) {
      ...
      const columnistFailDiagnostic = takeColumnistLastFailure();
      if (sovereignPack?.sections?.length) {
        rawResult = {
          ok: true,
          withheld: false,
