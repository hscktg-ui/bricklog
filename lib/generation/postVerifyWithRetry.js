...
export function runPostVerifyWithAutoRetry(pipelineInput, blogPack, hooks = {}) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    if (attempt < maxAttempts - 1) {
      if (!isLlmOriginatedPack(current)) {
        current = salvageBlogPackForDelivery(current, pipelineInput);
      }
      continue;
    }

    const salvaged = isLlmOriginatedPack(current)
      ? current
      : salvageBlogPackForDelivery(current, pipelineInput);
    for (const candidate of [salvaged, current]) {
      ...
      if (
        ...
        isCustomerPreviewDeliverablePack(candidate, { mode: "post_verify_preview" })
      ) {
        const stamped = stampCompleteCustomerDelivery(candidate, pipelineInput, {
          postVerifyStamped: true,
