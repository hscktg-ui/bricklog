...
export function normalizeBlogGenerationFailure(err) {
  ...
  if (status === 401 || status === 403) {
    return {
      message: raw || `로그인이 필요합니다. 다시 로그인한 뒤 ${BLOG_GENERATE_CTA_QUOTED}를 눌러 주세요.`,
  ...
  if (status === 422) {
    const mode = err?.payload?.mode || err?.code;
    if (mode === "axis_align_blocked") {
      const hint =
        err?.payload?.meta?.axisAlignHints?.[1] ||
        err?.payload?.meta?.axisAlignHints?.[0];
      return {
        message:
          raw ||
          hint ||
          "브랜드와 주제가 맞지 않아요. 주제를 이 브랜드에 맞게 수정해 주세요.",
        panelTitle: "브랜드·주제 확인",
        toastType: "info",
        soft: true,
        retryable: false,
      };
    ...
    return {
      message: raw || "입력을 확인한 뒤 다시 시도해 주세요.",
      panelTitle: "입력 확인",
      toastType: "info",
      soft: true,
      retryable: false,
  ...
  if (isGenerationTimeoutError(err)) {
    return {
      message: `이번에는 연결이 끊겼을 수 있어요. ${BLOG_GENERATE_CTA_QUOTED}를 한 번 더 눌러 주세요.`,
  ...
  if (/Failed to fetch|네트워크|인터넷/i.test(raw)) {
    return {
      message: `인터넷 연결을 확인한 뒤 다시 ${BLOG_GENERATE_CTA_QUOTED}를 눌러 주세요.`,
  ...
  if (isTechnicalErrorMessage(raw)) {
    return {
      message: blogGenerateCtaInlineRetry(),
