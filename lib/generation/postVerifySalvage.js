...
function writerFirstWithholdResult(pipelineInput, gate, result = {}) {
  return {
    ok: false,
    withheld: true,
...
export function resolveBlogUiDelivery(blog, pipelineInput = {}, result = {}) {
  ...
  if (shouldWithholdCustomerMissionPack(blog, pipelineInput)) {
    return writerFirstWithholdResult(pipelineInput, null, result);
  }
  ...
  if (!gate.ok) {
    if (isWriterFirstRescueBlocked(pipelineInput)) {
      ...
      return writerFirstWithholdResult(pipelineInput, gate, result);
    }
