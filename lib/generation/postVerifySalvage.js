...
function writerFirstWithholdResult(pipelineInput, gate, result = {}, pack = null) {
  if (pack?.sections?.length && isLaunchPublishFirstMode()) {
    const delivered = finalizeLaunchPublishBlogPack(pack, pipelineInput);
    return {
      ok: true,
      pack: delivered,
      preview: false,
  ...
  return {
    ...
    meta: { ...(result.meta || {}), writerFirstWithheld: true },
  };
}
...
function mergeResearchGroundedWhenShort(pack, input, tier) {
  ...
  const preserved = expandLlmPackPreservingBody(pack, input, tier);
  if (preserved) {
    return {
      ...preserved,
      title: pack.title || preserved.title,
      representativeTitle: pack.representativeTitle || preserved.representativeTitle,
      titles: pack.titles?.length ? pack.titles : preserved.titles,
      _meta: {
        ...(preserved._meta || {}),
        ...pack._meta,
        salvageLlmPreserve: true,
        postVerifySalvaged: true,
      },
    };
  }
  ...
  if (isLlmOriginatedPack(pack)) {
    ...
    if (countBlogBodyCharsWithSpaces(next) > before) {
      return {
        ...next,
        title: pack.title || next.title,
        representativeTitle: pack.representativeTitle || next.representativeTitle,
        _meta: {
          ...(next._meta || {}),
          ...pack._meta,
          salvageResearchWeave: true,
          postVerifySalvaged: true,
        },
      };
    }
...
export function salvageBlogPackForDelivery(pack, pipelineInput = {}) {
  ...
  if (isLlmOriginatedPack(pack)) {
    ...
    next = applyHumanColumnProsePass(next, pipelineInput, { force: true });
    return {
      ...next,
      _meta: {
        ...(next._meta || {}),
...
export function resolveBlogUiDelivery(blog, pipelineInput = {}, result = {}) {
  ...
  if (shouldWithholdCustomerMissionPack(blog, pipelineInput)) {
    return writerFirstWithholdResult(pipelineInput, null, result, blog);
  }
  ...
  const serverVerified =
    (!result.withheld ||
      (blog._meta?.writerFirstOrchestratorEscape &&
        blog._meta?.publishReady)) &&
  ...
  if (!gate.ok) {
    if (isWriterFirstRescueBlocked(pipelineInput)) {
      if (isLlmOriginatedPack(blog, pipelineInput)) {
        const enforced = enforceCustomerBlogOutput(blog, pipelineInput);
        if (enforced.ok) {
          return {
            ok: true,
            pack: ensureBlogDisplayPack(enforced.pack, pipelineInput),
            preview: false,
            userMessage: null,
            gate,
          };
      ...
      return writerFirstWithholdResult(pipelineInput, gate, result, blog);
    }
