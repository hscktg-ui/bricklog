...
import { applyEditorV95Pass, scoreEditorV95 } from "@/lib/product/briclogEditorEngineV95";
import { applyHumanConversationalVoice } from "@/lib/content/humanConversationalVoice";
...
function dedupeTitleEchoes(pack, input = {}) {
  ...
  const gi = sections[0];
  const sentences = String(gi.body || "")
    .split(/(?<=[.!?。])\s+|\n+/)
    .map((s) => s.trim())
...
function finalizeSalvageDeliveryQuality(pack, ctx, input, tier) {
  ...
  if (chars < tier.min || !dup.ok) {
    next = deepenPackBodiesToMin(next, tier.min, { ...ctx, input, _salvageForce: true }, input);
    next = dedupeSectionHeadings(next, ctx, input);
    next = weaveTopicDominanceIntoPack(next, { ...ctx, input });
    next = applyDuplicateKiller(next, ctx, "blog");
    next = stripGlobalExactDuplicateSentences(next);
  ...
  next = ensureVerbatimTopicCompliance(next, input, "blog");
  if (!scoreEditorV95(next, ctx, input).editorPass) {
