...
async function triggerAsyncRun(runUrl, opts = {}) {
  try {
    return await fetchJson(runUrl, {
      method: "POST",
      body: JSON.stringify({}),
      timeoutMs: opts.timeoutMs ?? getAsyncRunAwaitTimeoutMs(),
      signal: opts.signal,
    });
  } catch {
    return null;
...
function maybeLongWaitHint(polls, onLongWait) {
  ...
  if (polls === 1) onLongWait("백그라운드에서 조사·집필 중…");
  else if (polls === 8) onLongWait("품질 기준에 맞춰 작성 중…");
  else if (polls === 30) onLongWait("조금 더 걸리고 있어요…");
...
export async function generateBlogPipelineAsyncJob(payload, opts = {}) {
  ...
  onLongWait?.("글을 받을 준비 중…");

  void triggerAsyncRun(runUrl, {
    timeoutMs: getAsyncRunAwaitTimeoutMs(),
    signal,
  });
