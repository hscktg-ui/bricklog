...
async function applyWriterEngineIfNeeded(pack, pipelineInput, hooks = {}) {
  if (
    !pack?.sections?.length ||
    !isLlmOriginatedPack(pack) ||
    !isWriterEngineExpansionEnabled() ||
    !needsWriterEnginePass(pack, pipelineInput)
  ) {
    return pack;
  }
  hooks.setPipelineStep?.("편집본 분량 맞추는 중…");
  const ctx = createPromptContext(pipelineInput);
  const engine = await runBriclogWriterEngine(pack, ctx, pipelineInput);
...
function finalizeRescuePackForUi(pack, input = {}) {
  if (!pack?.sections?.length) return pack;
  const enriched = enrichMinimalBlogInput(input);
  let next = sanitizeVerbatimTopicInPack(pack, enriched, "blog");
  next = ensureNaverChannelClean(next, enriched);
  next = stripBlogPackHashtags(next);
  next = scrubCustomerForbiddenSurfaceInPack(next);
...
function buildGuaranteedMissionDelivery(pipelineInput) {
  ...
  try {
    ...
    return {
      ok: true,
      withheld: false,
      softPass: !isHumanDeliveryGrade(uiPack, pipelineInput),
      userMessage: isHumanDeliveryGrade(uiPack, pipelineInput)
        ? null
        : null,
      mode: isHumanDeliveryGrade(uiPack, pipelineInput)
      ...
      meta: {
        ...
        passOutput: uiPack._meta?.deliveryGrade === "publish",
      },
    };
  } catch {
    return null;
...
export function forceLocalBlogPreviewDelivery(pipelineInput, partial = null) {
  ...
  if (partial?.blogContent?.sections?.length) {
    return {
      ...
      ok: true,
      withheld: false,
      blogContent: finalizeBlogPackForUi(partial.blogContent, pipelineInput),
      mode: partial.mode || "client_partial_rescue",
    };
  }
  try {
    const guaranteed = buildGuaranteedMissionDelivery(pipelineInput);
    if (guaranteed?.blogContent?.sections?.length) return guaranteed;
  } catch {
    /* ignore */
  }
...
function guaranteeBlogDeliveryResult(pipelineInput, result, partial = null) {
  if (hasCustomerVisibleBody(result?.blogContent)) {
    const isRescue =
      result?.blogContent?._meta?.deliveryRescue ||
  ...
  try {
    ...
    if (delivered) {
      ...
      if (delivered.blogContent?.sections?.length) {
        delivered.blogContent = await applyWriterEngineIfNeeded(
