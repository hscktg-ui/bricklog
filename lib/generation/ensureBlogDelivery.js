...
function refreshBlogPackAfterWriterEngine(pack, input = {}) {
  if (!pack?.sections?.length) return pack;
  const enriched = enrichMinimalBlogInput(input);
  return ensureBlogDisplayPack(pack, enriched);
}
        ...
        if (rescued?.sections?.length) {
          ...
          return guaranteeBlogDeliveryResult(
            pipelineInput,
            {
              ...delivered,
              ok: true,
              blogContent: refreshBlogPackAfterWriterEngine(writerPack, pipelineInput),
              withheld: false,
              softPass: false,
              userMessage: null,
