...
import { normalizeBlogLengthAndStructure, ensureMinBlogSections } from "@/lib/content/blogLengthControl";
...
import { resolveDraftFallbackMinChars } from "@/lib/product/draftFallbackLength";
...
import { applyEditorWriterDeliveryPass, applyEditorWriterLengthPass } from "@/lib/product/editorWriterDeliveryPass";
...
import { shouldResearchHeavyMissionRescue } from "@/lib/product/missionProseRouteFlags";
import {
...
import {
  shouldFastPipelineSkipClientPostVerify,
  shouldSkipClientWriterEnginePass,
...
function resolveDraftMinChars(tier, tierKey = "short") {
  return resolveDraftFallbackMinChars(tierKey, tier);
}

function isRescueOrMissionPack(pack, enriched = {}) {
  return (
    pack?._meta?.missionProseFallback ||
    pack?._meta?.draftFallback ||
    pack?._meta?.deliveryRescue ||
    isMissionFallbackPack(pack, enriched)
  );
}

function padPackToDraftMin(pack, enriched, tier = null) {
  if (!pack?.sections?.length) return pack;
  if (isRescueOrMissionPack(pack, enriched)) {
    if (pack._meta?.batchVerifiedBlog) {
      const finalChars = countBlogBodyCharsWithSpaces(pack);
      return {
        ...pack,
        _meta: {
          ...(pack._meta || {}),
          charCount: finalChars,
          blogCharCount: finalChars,
          editorWriterLengthChars: finalChars,
        },
      };
    }
    return finishCustomerRescueBlogPack(pack, enriched);
  }
  const resolvedTier =
    tier || resolveBlogLengthTier(enriched.blogLengthTier || DEFAULT_BLOG_LENGTH_TIER);
  const draftMin = resolveDraftMinChars(resolvedTier, enriched.blogLengthTier);
  let next = pack;
  const humanityFinished = Boolean(pack._meta?.humanityFinishPass);
  if (countBlogBodyCharsWithSpaces(next) < draftMin) {
    next = ensureMissionProseTierLength(next, { input: enriched, ...enriched });
    if (countBlogBodyCharsWithSpaces(next) < draftMin) {
      next = expandPackByInformation(
        next,
        { input: enriched, ...enriched },
        enriched,
        {
          minChars: Math.max(draftMin, Math.round(resolvedTier.min * 0.4)),
          channel: "blog",
          salvageForce: true,
        }
      );
    }
    if ((next.sections?.length || 0) < 2) {
      next = ensureMinBlogSections(next, { input: enriched }, enriched, 3);
    }
    let padGuard = 0;
    while (countBlogBodyCharsWithSpaces(next) < draftMin && padGuard < 12) {
      const chunk = buildLengthExpansionChunk(enriched, padGuard);
      const idx = Math.max(0, (next.sections?.length || 1) - 1);
    ...
    if (!humanityFinished && countBlogBodyCharsWithSpaces(next) < draftMin) {
      next = deepenMissionProseToMin(next, draftMin, enriched);
    }
    if (!humanityFinished && countBlogBodyCharsWithSpaces(next) < draftMin) {
      next = applyEditorWriterLengthPass(next, enriched, { input: enriched });
    }
    if (countBlogBodyCharsWithSpaces(next) < draftMin) {
      const expanded = expandRescuePackToHumanTier(next, enriched);
      if (countBlogBodyCharsWithSpaces(expanded) >= draftMin) {
        next = expanded;
      }
    }
  }
...
function finalizeBlogPackForUi(pack, input = {}) {
  ...
  return padPackToDraftMin(next, enriched, tier);
}
...
function finalizeRescuePackForUi(pack, input = {}) {
  ...
  let result = finishCustomerRescueBlogPack(next, enriched);
  result = applyProdBlogBeliefBoost(result, enriched);
  return result;
}
...
function expandRescuePackToHumanTier(pack, input = {}) {
  ...
  if (
    ...
    } else if (!isLlmOriginatedPack(next) && !isWriterFirstRescueBlocked(enriched)) {
...
function buildGuaranteedMissionDelivery(pipelineInput) {
  if (isGpt55WriterDominant() && !shouldResearchHeavyMissionRescue(pipelineInput)) {
    return null;
  }
  ...
  try {
    ...
    if (!pack?.sections?.length) return null;
    const uiPack = finalizeBlogPackForUi(pack, enriched);
...
export function deliverWithOptionalPostVerify(pipelineInput, partial, v2Gate, hooks = {}) {
  ...
  if (shouldFastPipelineSkipClientPostVerify(partial, blog)) {
    const uiPack = finalizeBlogPackForUi(blog, pipelineInput);
    return {
      ...partial,
      ok: true,
      blogContent: uiPack,
      ...
      withheld: false,
      userMessage: null,
      mode: partial?.mode || "llm_fast",
      meta: {
        ...(partial.meta || {}),
        v2PipelineVerified: true,
        v3PipelineVerified: true,
        passOutput: true,
        clientPostVerifySkipped: true,
...
export function forceLocalBlogPreviewDelivery(pipelineInput, partial = null) {
  ...
  if (local?.blogContent?.sections?.length) {
    const blogContent = local.blogContent?._meta?.contentQualityDelivered
      ? local.blogContent
      : finalizeRescuePackForUi(local.blogContent, pipelineInput);
    return {
      ...local,
      ok: true,
      withheld: false,
  ...
  if ((rescuePack?.sections?.length || 0) >= 2) {
    ...
    uiPack = finalizeBlogPackForUi(uiPack, enriched);
...
function guaranteeBlogDeliveryResult(pipelineInput, result, partial = null) {
  if (shouldWithholdCustomerMissionPack(result?.blogContent, pipelineInput)) {
    if (
      isLaunchPublishFirstMode() &&
      hasCustomerVisibleBody(result?.blogContent)
    ) {
      const uiPack = finalizeLaunchPublishBlogPack(
      ...
      return {
        ...
        blogContent: uiPack,
        userMessage: null,
        mode: result.mode || "launch_publish_mission_deliver",
        meta: {
          ...(result?.meta || {}),
          launchPublishDelivered: true,
          passOutput: true,
        },
      };
    }
    return {
      ...
      withheld: true,
      blogContent: null,
      userMessage: buildWriterFirstWithholdMessage(pipelineInput),
      mode: "writer_first_mission_withheld",
      meta: {
        ...(result?.meta || {}),
        failReasons: ["mission_fallback_blocked"],
        generationMode: "writer_first_mission_withheld",
      },
    };
  }
  if (hasCustomerVisibleBody(result?.blogContent)) {
    ...
    if (uiPack?._meta?.outputWithheld || uiPack?._meta?.writerFirstMissionBlocked) {
      return {
        ...result,
        ok: false,
        withheld: true,
        blogContent: null,
        userMessage: buildWriterFirstWithholdMessage(pipelineInput),
        mode: "writer_first_quality_withheld",
        meta: {
          ...(result.meta || {}),
          failReasons: [
            ...new Set([
              ...(result.meta?.failReasons || []),
              uiPack._meta?.withholdReason || "writer_first_quality",
            ]),
          ],
          generationMode: "writer_first_quality_withheld",
        },
      };
    }
    return {
  ...
  if (!isWriterFirstRescueBlocked(pipelineInput)) {
    const researchHeavyRescue = tryDeliverResearchHeavyMissionRescue(pipelineInput);
    if (hasCustomerVisibleBody(researchHeavyRescue?.blogContent)) {
...
export async function ensureBlogDelivery(pipelineInput, hooks = {}) {
  const { setPipelineStep } = hooks;
  Object.assign(pipelineInput, ensureGenerationAxesOnInput(pipelineInput));
  ...
  const researchHeavyRescue = isWriterFirstRescueBlocked(pipelineInput)
    ? null
    : tryDeliverResearchHeavyMissionRescue(pipelineInput);
  ...
  try {
    ...
    if (delivered) {
      if (
        !shouldBlockClientBlogPromotion(partial, pipelineInput, inboundPack) &&
        ...
        if (rescued?.sections?.length) {
          const writerPack = shouldSkipClientWriterEnginePass(partial, rescued)
            ? rescued
      ...
      if (delivered.blogContent?.sections?.length) {
        const pack = shouldSkipClientWriterEnginePass(
          { ...delivered, ...partial },
    ...
    if (
      partial?.userMessage &&
      !partial?.blogContent?.sections?.length &&
      !isTechnicalErrorMessage(partial.userMessage)
    ) {
      return {
        ok: false,
        blogContent: null,
        ...
        mode: partial.mode || err?.code || "api_failed",
        meta: partial.meta || {},
        usage: partial.usage,
        usageWarning: partial.usageWarning,
