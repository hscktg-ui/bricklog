...
function resolveDraftMinChars(tier) {
  return tier.key === "long"
    ? Math.max(800, Math.round(tier.min * 0.22))
    : tier.key === "medium"
      ? Math.max(400, Math.round(tier.min * 0.36))
      : Math.max(400, Math.round(tier.min * 0.55));
}
...
function padPackToDraftMin(pack, enriched, tier = null) {
  ...
  const draftMin = resolveDraftMinChars(resolvedTier);
  let next = pack;
  if (countBlogBodyCharsWithSpaces(next) < draftMin) {
    ...
    let padGuard = 0;
    while (countBlogBodyCharsWithSpaces(next) < draftMin && padGuard < 8) {
