...
function ensureBlogDisplayPackStampDelivered(blog, input = {}) {
  ...
  let pack = humanizeBlogProsePack(blog, input);
  ...
  pack = applyKoreanOrthographyToBlogPack(pack, input);
...
function ensureBlogDisplayPackLight(blog, input = {}) {
  ...
  if (blog?._meta?.flowerRecommendationEditorial === true) {
    return ensureBlogDisplayPackStampDelivered(blog, input);
  }
  if (blog?._meta?.industryHumanColumnEditorial === true) {
    return ensureBlogDisplayPackStampDelivered(blog, input);
  }
  if (blog?._meta?.forcedMissionProseRoute === true) {
    return ensureBlogDisplayPackStampDelivered(blog, input);
  }
...
function pruneResolvedFailReasons(pack, input = {}, reasons = []) {
  ...
  let next = [...reasons];
  const sectionCount = (pack?.sections || []).filter(
    (s) => String(s?.body || "").replace(/\s/g, "").length >= 40
  ).length;
  if (sectionCount >= 3 && full.replace(/\s/g, "").length >= 400) {
    next = next.filter(
      (r) =>
        r !== "empty_pack" &&
  ...
  if (editorPre.ok) {
    ...
    const v95Score = Number(editor.editorV95?.score || 0);
    next = next.filter((r) => {
      if (r === "editor_korean_orthography") return false;
      if (r === "editor_editor_v95" && v95Score >= 64) return false;
      if (r === "first_delivery_editor_v95" && v95Score >= 64) return false;
      return true;
    });
  }
...
export function ensureBlogDisplayPack(blog, input = {}) {
  ...
  if (isQualityDeliveredDisplayPack(blog)) {
    ...
    const scrubbed = applyKoreanOrthographyToBlogPack(
      scrubPlaceholderFromPack(stamped),
      input
    );
    if (assertNoPlaceholderContamination(scrubbed, input).ok) {
      return {
        ...scrubbed,
        _meta: {
          ...(scrubbed._meta || {}),
          placeholderScrubOnStamp: true,
        },
      };
    }
  }
