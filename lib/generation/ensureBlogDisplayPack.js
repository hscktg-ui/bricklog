...
function ensureBlogDisplayPackLight(blog, input = {}) {
  let pack = stripSearchSnippetLeakFromPack(blog, input);
...
export function ensureBlogDisplayPack(blog, input = {}) {
  ...
  if (isLlmOriginatedPack(blog) && hasSubstantiveLlmBody(blog)) {
    const light = ensureBlogDisplayPackLight(blog, input);
    if (light?.sections?.length && hasSubstantiveLlmBody(light)) {
      return light;
    }
  }
  ...
  const meta = {
    ...
    publishReady: completeDraft ? true : pack._meta?.publishReady === true,
    sqv: pack._meta?.sqv,
