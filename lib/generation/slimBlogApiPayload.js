...
export function slimBlogApiPayload(input = {}) {
  ...
  for (const key of ["brandName", "region", "topic", "mainKeyword"]) {
    ...
    if (raw && !String(next[key] ?? "").trim()) next[key] = input[key];
  }

  if (input._verifiedGenerationAxes && typeof input._verifiedGenerationAxes === "object") {
    next._verifiedGenerationAxes = input._verifiedGenerationAxes;
  }
