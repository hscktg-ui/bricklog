 ...
 */
import { materializeVerifiedGenerationAxes, stampVerifiedGenerationAxes } from "@/lib/product/deliverySoftPass";
import { sanitizeResearchFactsList } from "@/lib/content/researchFactSanitize";
...
function slimFacts(facts = [], input = {}) {
  const out = [];
  const seen = new Set();
  for (const row of sanitizeResearchFactsList(facts, input)) {
...
export function slimBlogApiPayload(input = {}) {
  ...
  const seeded = materializeVerifiedGenerationAxes(
    stampVerifiedGenerationAxes(input)
  );
  const next = {};
  ...
  if (Array.isArray(next.researchFacts)) {
    next.researchFacts = slimFacts(next.researchFacts, seeded);
  ...
  for (const key of ["brandName", "region", "topic", "mainKeyword"]) {
    const raw = String(seeded[key] ?? "").trim();
    if (raw) next[key] = seeded[key];
  }

  if (seeded._verifiedGenerationAxes && typeof seeded._verifiedGenerationAxes === "object") {
    next._verifiedGenerationAxes = seeded._verifiedGenerationAxes;
  }
  ...
  return materializeVerifiedGenerationAxes(next);
}
