...
import { getResearchClientTimeoutMs, getLaunchResearchTimeoutMs, getCustomerResearchBudgetMs, isCustomerTwoMinuteSlaMode } from "@/lib/config/briclogFastPipeline";
...
async function serverGenerateResearchAsync(input = {}) {
  ...
  const timeoutMs = isLaunchPublishFirstMode()
    ...
    : getCustomerResearchBudgetMs(input);
  const research = await Promise.race([
    runResearch({
      ...
      brandContext: {
        ...input,
        slaFastResearch: isCustomerTwoMinuteSlaMode(),
      },
