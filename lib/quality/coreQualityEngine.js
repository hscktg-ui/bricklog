...
import { BLOG_MIN_BODY_CHARS, resolveBlogLengthTier, DEFAULT_BLOG_LENGTH_TIER } from "@/lib/constants";
...
import { countPlaceholderContamination, detectEmptyInputVars } from "@/lib/content/placeholderContaminationEngine";
...
export function detectPlaceholder(full) {
  if (hasTemplatePlaceholder(full) || /\{\{|\}\}/.test(String(full || ""))) {
    return true;
  }
  return countPlaceholderContamination(full).total >= 3;
}
...
function scoreInformationDensity(full, ctx = {}, channel = "blog") {
  ...
  if (regionRaw && !/^전국$/.test(regionRaw)) {
    ...
    const minHits = isBriclogMissionEnforced() ? 1 : 2;
    if (regionHits < minHits) {
...
const REGEN_HINTS = {
  ...
  human_tier_under:
    "사람이 쓴 블로그 분량(최소 tier min·3섹션)에 맞춰 섹션별 2~4문단으로 다시 작성하세요. outline·bullet 금지.",
  length_too_short: `선택한 분량 tier 최소 ${BLOG_LENGTH_TIERS.short.min}자(공백 포함)에 맞춰 보강하되, 빈 수식·반복으로 채우지 마세요.`,
  length_under_min:
...
export function buildRegenPromptForFailures(failReasons = [], ctx = {}) {
  const tier = resolveBlogLengthTier(
    ctx.blogLengthTier || ctx.input?.blogLengthTier || DEFAULT_BLOG_LENGTH_TIER
  );
...
/** 재작성 없이 배달 가능 — 정보·밀도·분량(soft) 등 */
export const CORE_SOFT_REGEN_REASONS = new Set([
  "topic_dominance_low",
...
export function partitionCoreRegenReasons(failReasons = []) {
  const reasons = [...new Set(failReasons.filter(Boolean))];
  const soft = reasons.filter((r) => CORE_SOFT_REGEN_REASONS.has(r));
  const critical = reasons.filter((r) => !CORE_SOFT_REGEN_REASONS.has(r));
  return { reasons, soft, critical };
}

export function needsCoreRegen(coreResult) {
  if (!coreResult) return false;
  const { critical, soft, reasons } = partitionCoreRegenReasons(
    coreResult.failReasons || []
  );
  if (critical.length > 0) return true;
  if (!reasons.length) {
    return coreResult.total < getQualityTarget();
  }
  const softFloor = getQualityTarget() - 3;
  return soft.length > 0 && coreResult.total < softFloor;
}
