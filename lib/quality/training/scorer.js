...
import { hasHardPlaceholderSignal } from "@/lib/quality/hardPlaceholderSignal";
...
export function scoreTrainingContent(pack, ctx = {}, channel = "blog") {
  ...
  if (channel === "blog") {
    ...
    const hardPlaceholder = hasHardPlaceholderSignal(fullText);

    if (!deliveryTrusted && (!coreEngine.pass || charCount < trainingMin)) {
      total = Math.min(total, Math.max(0, target - 6));
    }

    const pass =
      deliveryTrusted && !hardPlaceholder
        ? true
        : coreEngine.pass &&
