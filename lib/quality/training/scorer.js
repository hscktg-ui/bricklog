...
export function scoreTrainingContent(pack, ctx = {}, channel = "blog") {
  ...
  if (channel === "blog") {
    ...
    const coreMeetsTarget = coreEngine.total >= target;
    ...
    if (pack._meta?.generationMode?.includes("llm") && coreMeetsTarget) {
      total = Math.max(total, target);
    }
    if (charCount < TRAINING_BLOG_MIN_CHARS && !coreMeetsTarget) {
      total = Math.min(total, 78);
    }

    const pass =
      !blockers.includes("placeholder") && coreMeetsTarget;

    return {
      total,
      blockers: pass ? blockers.filter((b) => b === "placeholder") : blockers,
      pass,
