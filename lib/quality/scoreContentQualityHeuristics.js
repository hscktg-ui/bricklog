...
function scoreNaverBlogFit(text, constitution, ctx = {}) {
  let s = 72;
  const industry = ctx.industry || ctx.industryLabel || "";
  const targets = getNaverCategoryTargets(industry);

  if (scoreNaverVoiceDensity(text) >= Math.max(1, Math.round(targets.voiceRate / 40))) {
    s += 12;
  }
  if (/다녀|느꼈|직접|체험|방문|가봤|먹어\s*보|솔직|내돈내산/.test(text)) s += 6;
  if (/확인하세요|권합니다|체크리스트|알아보시다|검색하시는|체험 전 알아둘/.test(text)) {
    s -= 16;
  }
  if (constitution.checks?.sceneMoments) s += 6;
  if (constitution.checks?.noForbiddenOpen) s += 6;
  const sections = (text.match(/\n\n/g) || []).length;
  if (sections >= 8 && sections <= 24) s += 4;
  return clamp(s);
}
...
function scoreInstagramFit(insta) {
  ...
  let s = 75;
  if (body.length >= 40 && body.length <= 4400) s += 8;
  if (insta.hook || /^.+\n/.test(body)) s += 6;
...
export function scoreContentQualityHeuristics(pack, ctx = {}, extras = {}) {
  ...
  const scores = {
    ...
    naverBlogFit: scoreNaverBlogFit(text, constitution, ctx),
