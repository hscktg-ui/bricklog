...
import { detectFailureArticlePatterns } from "@/lib/golden/goldenFailureDetection";
...
function scoreProseConsistency(full) {
  const hasSeupnida = /(?:습니다|습니까|됩니다)(?![^.\n]{0,8}어요)/.test(full);
  const hasCasualHaeyo = /(?:해요|예요|이에요|거예요|했어요|봤어요|됐어요|였어요|더라구요|더라고요)/.test(
    full
  );
  const modes = [hasSeupnida, hasCasualHaeyo].filter(Boolean).length;
...
function scoreAiClicheRemoval(full, input = {}) {
  const hits = countPhraseHits(full, getAiClichePhrases(input));
  const score = Math.max(0, 100 - hits * 18);
  return { score, ok: hits === 0, hits };
}

function scoreForbiddenPlaceholder(full, input = {}) {
  const forbidden = getForbiddenGlobalPhrases(input);
  const hits = countPhraseHits(full, forbidden);
...
export function assessHaeshinQualityScore(pack, input = {}) {
  ...
  const aiCliche = scoreAiClicheRemoval(full, input);
  const placeholder = scoreForbiddenPlaceholder(full, input);
  ...
  if (failure.hits.length) total -= Math.min(25, failure.hits.length * 8);
  if (failure.criticalFail) total = Math.min(total, 65);
  const score = Math.round(Math.max(0, Math.min(100, total)));

  const reasons = [];
  if (failure.criticalFail) reasons.push("haeshin_failure_pattern");
  if (failure.hits.length) reasons.push(...failure.hits.map((h) => `haeshin_fail_${h}`));
  ...
  let verdict = "pass";
  if (score < 80 || failure.criticalFail) verdict = "fail";
  ...
  return {
    ...
    ok: score >= 90,
    verdict,
    shouldBlock: score < 80 || failure.criticalFail,
    shouldRevise: score >= 80 && score < 90 && !failure.criticalFail,
    components,
    maxPoints: HAESHIN_SCORE_WEIGHTS,
    reasons: [...new Set(reasons)],
    checks: { topic, intent, brand, industry, prose, speaker, repetition, aiCliche, placeholder, info, failure },
  };
}
