...
export const GOLDEN_INDUSTRY_OPTIONS = [
  ...
  { value: "craft_workshop", label: "공방·체험" },
...
const ALIAS_TO_GOLDEN = [
  ...
  [/도자|공방|체험\s*공|원데이|workshop|craft/i, "craft_workshop"],
  [/반려|애견|펫|pet/i, "pet"],
];

/** @param {object} input */
export function resolveGoldenIndustryKey(input = {}) {
  const directKeys = [input.goldenIndustry, input.golden_industry, input.industry]
    .map((v) => String(v || "").trim())
    .filter(Boolean);
  for (const key of directKeys) {
    if (GOLDEN_INDUSTRY_OPTIONS.some((o) => o.value === key)) return key;
  }
