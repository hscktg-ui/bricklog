...
function applyEditorialGoldenBoost(score, pack, haeshin, compare, goldenSimilarity = 0) {
  ...
  if (
    pack?._meta?.koreanHumanPioneerPass &&
    pack?._meta?.researchGroundedTrustPolish &&
    haeshin.score >= 84 &&
    compare.repetition_score >= 88 &&
    !haeshin.checks?.failure?.criticalFail
  ) {
    boosted = Math.max(
      boosted,
      Math.min(
        94,
        Math.round(haeshin.score * 0.86 + compare.repetition_score * 0.1 + goldenSimilarity * 0.04)
      )
    );
  }
  return boosted;
...
export function assessGoldenQualityGate(pack, input = {}, goldenSamples = null) {
  ...
  if (
    ...
    !haeshin.checks?.failure?.criticalFail &&
    haeshin.score >= 68 &&
    industryFit.foreignHits.length === 0
  ) {
    score = Math.max(score, Math.min(88, haeshin.score + 12));
  }
  if (!industryFit.ok) {
