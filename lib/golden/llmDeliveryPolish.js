...
export function llmDeliveryTargetChars(input = {}, pack = {}) {
  ...
  const expandFloor = current + 400;
  return Math.max(tier.min, current, expandFloor);
}

/** 송출 회귀·draft 확장 하한 — stripped no-space + 350, tier.min cap */
export function llmDeliveryExpandFloor(input = {}, pack = {}) {
  const tier = resolveBlogLengthTier(input.blogLengthTier || DEFAULT_BLOG_LENGTH_TIER);
  const noSpace = getBlogFullText(pack).replace(/\s/g, "").length;
  return Math.min(tier.min, noSpace + 350);
}
...
function buildLlmDnaAnchorLine(input = {}, anchors = {}) {
  ...
  return `${subject}에서는 ${productClause}${anchorClause}구성을 함께 살펴보면 기준이 잡혀요.`
    .replace(/\s{2,}/g, " ")
    .trim();
}
...
export function deepenLlmPackWithResearch(pack, input = {}) {
  if (!pack?.sections?.length) return pack;
  const expandFloor = llmDeliveryExpandFloor(input, pack);
  const target = Math.max(llmDeliveryTargetChars(input, pack), expandFloor);
...
/** LLM 송출 회귀 하한(stripped +350)까지 조사·DNA로 보강 */
export function ensureLlmPackExpandFloor(pack, input = {}, referencePack = pack) {
  if (!pack?.sections?.length) return pack;
  const ref = stripLlmPackSurfaceNoise(referencePack || pack);
  const floor = llmDeliveryExpandFloor(input, ref);
  let next = pack;
  let chars = llmPackCharCount(next);
  if (chars >= floor) return next;

  if (hasUsableResearchFacts(input)) {
    next = weaveResearchFactsIntoPack(next, input);
    chars = llmPackCharCount(next);
  }

  const lines = hasUsableResearchFacts(input)
    ? buildResearchFactLines(input, 10).filter(Boolean)
    : [];
  if (chars < floor && lines.length) {
    const sections = [...next.sections];
    let li = 0;
    while (chars < floor && li < lines.length * 3) {
      const line = lines[li % lines.length];
      const idx = Math.min(
        sections.length - 1,
        1 + (li % Math.max(1, sections.length - 1))
      );
      const body = String(sections[idx]?.body || "").trim();
      if (line && !body.includes(line.slice(0, Math.min(12, line.length)))) {
        sections[idx] = {
          ...sections[idx],
          body: `${body}\n\n${line}`.trim(),
        };
        chars = countBlogBodyCharsWithSpaces({ ...next, sections });
      }
      li += 1;
    }
    next = { ...next, sections };
  ...
  return {
    ...
    _meta: {
      ...(next._meta || {}),
      llmExpandFloor: floor,
      llmExpandFloorMet: chars >= floor,
      llmExpandFloorTopoff: chars >= floor,
    },
  };
}
...
export function polishLlmPackForDelivery(pack, input = {}) {
  ...
  const preserveGpt55Body = isGpt55WriterDominant();
  let next = normalizeLlmVoiceForDelivery(pack, input);
  if (!preserveGpt55Body) {
    next = enrichLlmPackDnaAnchors(next, input);
    next = deepenLlmPackWithResearch(next, input);
  }
  next = applyGoldenSafeEdit(next, input, { forceVoice: "seupnida", forceApply: true });
  if (!preserveGpt55Body && hasUsableResearchFacts(input)) {
    const expandFloor = Math.min(
      llmDeliveryTargetChars(input, pack),
      llmPackCharCount(pack) + 400
    );
    if (llmPackCharCount(next) < expandFloor && next.sections?.length) {
      const full = getBlogFullText(next);
      const extra = buildResearchFactLines(input, 6)
        .filter((line) => line && !full.includes(line.slice(0, Math.min(14, line.length))))
        .slice(0, 2)
        .join("\n\n");
      if (extra) {
        const sections = [...next.sections];
        ...
        sections[idx] = {
          ...sections[idx],
          body: `${String(sections[idx].body || "").trim()}\n\n${extra}`.trim(),
        };
        next = {
          ...next,
          sections,
          _meta: { ...(next._meta || {}), llmResearchTailTopoff: true },
        };
      }
    }
  }
  if (inboundChars >= 200 && llmPackCharCount(next) < Math.max(180, inboundChars * 0.55)) {
    const rescueBase = preserveGpt55Body
      ? normalizeLlmVoiceForDelivery(pack, input)
      : enrichLlmPackDnaAnchors(normalizeLlmVoiceForDelivery(pack, input), input);
    next = applyGoldenSafeEdit(rescueBase, input, {
      forceVoice: "seupnida",
      forceApply: true,
    });
    next = {
  ...
  return {
    ...
    _meta: {
      ...
      ...(preserveGpt55Body ? { gpt55LlmPolishPreserved: true } : {}),
    },
  };
}
