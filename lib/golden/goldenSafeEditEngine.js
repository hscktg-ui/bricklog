...
function stripForbiddenInline(text = "") {
  let t = String(text || "");
...
function cleanSentence(sentence = "", voiceMode = "seupnida", aggressive = false) {
  let t = String(sentence || "").trim();
  if (!t) return "";
  ...
  for (const re of DROP_SENTENCE_RES) {
    if (re.test(t)) return aggressive ? "" : normalizeSentenceVoice(stripForbiddenInline(t), voiceMode);
  }

  t = stripForbiddenInline(t);
...
function editParagraph(text = "", voiceMode = "seupnida", aggressive = false) {
  const parts = splitKoreanSentences(text)
    .map((s) => cleanSentence(s, voiceMode, aggressive))
...
function buildEditedPack(pack, voiceMode, aggressive = false) {
  const sections = pack.sections.map((sec) => {
    const body = editParagraph(sec.body || "", voiceMode, aggressive);
    return { ...sec, body: body || sec.body };
  });
  let conclusion = pack.conclusion ? editParagraph(pack.conclusion, voiceMode, aggressive) : pack.conclusion;
...
export function applyGoldenSafeEdit(pack, input = {}) {
  ...
  let next = buildEditedPack(pack, voiceMode, false);
  ...
  if (afterScore < beforeScore || ratio < SAFE_EDIT_MIN_PRESERVE_RATIO) {
    next = buildEditedPack(pack, voiceMode, true);
