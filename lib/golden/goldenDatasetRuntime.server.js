/**
 * Golden Dataset — 서버 생성 시 DB·파일 우수글 + DNA 오버라이드 hydrate
 */
import { loadAllGoldenSamples } from "@/lib/golden/goldenDatasetStore.server";
import { getHaeshinDnaOverrides } from "@/lib/golden/haeshinDnaOverrides.server";
...
export async function hydrateGoldenSamplesForInput(input = {}) {
  ...
  const failureSamples = samples.filter((s) => s.sample_kind === "failure");
  return {
    ...input,
    goldenSamples: samples,
    failureSamples,
    _haeshinDnaOverrides: overrides,
