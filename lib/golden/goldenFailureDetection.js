...
function detectVoiceMix(full = "") {
  const hasSeupnida = /(?:습니다|습니까|됩니다|입니다)(?![^.\n]{0,8}어요)/.test(full);
  const hasCasual = /(?:해요|예요|이에요|거예요|했어요|봤어요|뒀어요|됐어요|였어요|더라구요|더라고요)/.test(
    full
  );
  const modes = [hasSeupnida, hasCasual].filter(Boolean).length;
  return { modes, mixed: modes >= 2, severe: hasSeupnida && hasCasual };
}
...
export function detectFailureArticlePatterns(full = "", input = {}) {
  ...
  const formalHits = (text.match(/(?:습니다|습니까|됩니다|입니다)/g) || []).length;
  if (voice.severe && formalHits >= 2) {
    hits.push("voice_mix");
    reasons.push("말투 혼합(합니다·해요)");
  } else if (voice.mixed && formalHits >= 2 && /메모해\s*뒀|확인해\s*뒀|들으며/.test(text)) {
    hits.push("voice_mix");
  ...
  const criticalFail =
    ...
    (unique.includes("voice_mix") && voice.severe && formalHits >= 2) ||
    placeholderHits >= 3;
