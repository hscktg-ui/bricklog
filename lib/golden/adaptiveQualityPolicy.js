...
import { resolveMissionTemplateGoldenMin } from "@/lib/product/missionCatalogDelivery";

export const ADAPTIVE_QUALITY_VERSION = "v1";

function isIndustryHumanColumnPack(pack = {}) {
  return Boolean(
...
function isLlmOriginatedDeliveryPack(pack = {}) {
  return Boolean(
    pack?._meta?.llmGenerated === true ||
      pack?._meta?.generationMode === "llm_openai" ||
      pack?._meta?.llmDeliveryLightPath === true ||
...
export function resolveGoldenPublishOk(goldenGate = {}, pack = {}) {
  ...
  const evalScore = pack?._meta?.contentEvaluation?.score ?? 0;
  const industryColumn = isIndustryHumanColumnPack(pack);

  if (industryColumn && !critical && evalScore >= 88 && goldenGate.score >= 78 && haeshin >= 78) {
    return true;
  }
  if (
    industryColumn &&
  ...
  if (industryColumn && !critical && evalScore >= 90 && goldenGate.score >= 72 && haeshin >= 72) {
    return true;
  }
  if (industryColumn && !critical && evalScore >= 88 && goldenGate.score >= 74 && haeshin >= 74) {
    return true;
  }
  if (industryColumn && !critical && evalScore >= 90 && goldenGate.score >= 84 && haeshin >= 84) {
    return true;
  }
  ...
  if (
    ...
    if (goldenGate.score >= goldenMin && haeshin >= 68) {
      return true;
    }
  }
  ...
  if (isLlmOriginatedDeliveryPack(pack) && !critical) {
    if (evalScore >= 88 && goldenGate.score >= 78 && haeshin >= 78) return true;
    if (evalScore >= 90 && goldenGate.score >= 80 && haeshin >= 80) return true;
  }
