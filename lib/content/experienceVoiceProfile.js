...
const CROSS_CATEGORY_BLOCK = {
  가구점: /(?:카페|맛집|디저트|약국|병원|헬스장|꽃집|펜션|피시방|학원|횟집|커피)/,
...
function buildContextualExperienceLine(role, p, input = {}) {
  ...
  const templates = {
    ...
    reflection: isInformationalTopicInput(input)
      ? [
          clean(`처음엔 어디부터 볼지 막막했는데, 기준만 정리해 두니까 훨씬 수월했어요.`),
          clean(`검색만 하다 보면 기준이 많아서 헷갈리는데, 목적별로 나눠 보니까 감이 왔어요.`),
          pc.in_deut ? clean(`전체적으로는 참고할 만한 편인 듯해요.`) : null,
        ]
      : [
          clean(`처음엔 어디부터 볼지 막막했는데, 누워보니 감이 왔어요.`),
          clean(`솔직히 예상보다 체감 차이가 있더라구요.`),
          pc.in_deut ? clean(`전체적으로는 괜찮은 편인 듯해요.`) : null,
        ],
    punch: [
  ...
  const pool = (templates[role] || []).filter(Boolean);
...
export function buildExperienceVoiceLines(p, input = {}) {
  ...
  for (const role of ROLE_ORDER) {
    ...
    const key = String(line || "")
      ...
      .slice(0, 40);
    if (used.has(key)) {
      line = buildContextualExperienceLine(role, p, input);
    }
    if (line) {
      used.add(
        String(line)
