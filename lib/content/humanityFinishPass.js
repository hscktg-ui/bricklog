...
import { guardPackAgainstShrink, snapshotPackForShrink } from "@/lib/product/packShrinkGuard";
...
export function applyHumanityFinishPass(pack, ctx = {}, channel = "blog") {
  ...
  const inboundPack = snapshotPackForShrink(pack);
  const fullCtx = { ...ctx, input };
  const explainGate = assessContentExplainabilityForPublish(input);
  const mayLengthRefill = explainGate.ok && !ctx.batchLocalFinish && !ctx.skipLengthRefill;
  ...
  next = guardPackAgainstShrink(inboundPack, next, { stage: "humanityFinishPass" });
  if (!ctx.skipDeliveryFinalize) {
    const preFinalize = snapshotPackForShrink(next);
    next = finalizeContentQualityForDelivery(next, input, "blog");
    next = guardPackAgainstShrink(preFinalize, next, { stage: "humanityFinishFinalize" });
  }
