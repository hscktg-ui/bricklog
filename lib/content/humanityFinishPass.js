...
import { finalizeMissionProsePack } from "@/lib/product/missionProseEngine";
...
export function applyHumanityFinishPass(pack, ctx = {}, channel = "blog") {
  ...
  if (isBriclogMissionEnforced() && next?.sections?.length) {
    ...
    while (bodyChars < tier.min && postSanitizeRefill < 5) {
      next = finalizeMissionProsePack(next, input, tier);
      next = applyMissionOutputSanitizerPack(
        {
          ...next,
    ...
    if (bodyChars < tier.min && next.sections?.length) {
      const scenes = buildStoryTargetSceneLines(input, 8);
      const sections = [...next.sections];
      for (let i = 0; i < scenes.length && countBlogBodyCharsWithSpaces(next) < tier.min; i += 1) {
      ...
      next = applyMissionOutputSanitizerPack(
        { ...next, _meta: { ...(next._meta || {}), missionOutputSanitizerForce: true } },
        input,
        { dedupe: false }
      );
      bodyChars = countBlogBodyCharsWithSpaces(next);
