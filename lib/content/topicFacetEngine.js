...
/** 제품코드·스펙·쇼룸 비교 — 조사·정보형 우선 (스트레스리스·다이닝체어 등) */
const RESEARCH_HEAVY_MODEL_RE =
  /\b[A-Z]{2,}(?:[\s-][A-Z0-9]{2,}){1,}\b|STRESSLESS|스트레스리스|다이닝\s*체어|제로지|리클라이닝|모션\s*베드|좌판|등받이|쇼룸\s*비교|모델\s*비교|라인업|시리즈\s*비교/i;
const RESEARCH_HEAVY_INDUSTRY_RE = /가구|침대|매트리스|furniture|bed|mattress|쇼룸/i;
...
export function isResearchHeavyTopicInput(input = {}) {
  ...
  if (
    ...
    /비교|선택|스펙|모델|체어|침대|쇼룸|제로지|리클라이닝|좌판|등받이/.test(blob)
  ) {
    return true;
  }
  if (
