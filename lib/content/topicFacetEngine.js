...
/** 제목·대표제목을 포함한 방문 후기 의도 판별용 입력 (생성된 pack.title 반영) */
export function resolveVisitReviewIntentInput(input = {}, pack = null) {
  const title = String(
    input.title || input.representativeTitle || pack?.title || pack?.representativeTitle || ""
  ).trim();
  const representativeTitle = String(
    input.representativeTitle || pack?.representativeTitle || pack?.title || input.title || ""
  ).trim();
  return {
    ...input,
    ...(title ? { title } : {}),
...
export function isVisitReviewTopicInput(input = {}, pack = null) {
  ...
  const blob = [
    ...
    .join(" ")
    .toLowerCase();
  const rawTopic = topicRaw(ctx).toLowerCase();
  ...
  if (
    ...
    /오픈|수영|체험|목장|승마|워터|풀장|메뉴|돈까스|식사|카페|맛집|브런치/.test(blob)
  ) {
    return true;
  }
  return false;
