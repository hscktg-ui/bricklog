...
import { getResearchDepthMaxRounds, resolveResearchDepthMaxRounds } from "@/lib/config/briclogFastPipeline";
import {
...
import { stampCoreEngineOnInput } from "@/lib/product/briclogCoreEngine";
...
export async function applySignatureResearchServer(input = {}, channel = "blog") {
  ...
  const researchConfig = resolveResearchQueryAndTypes(enriched);
  const { query, types, querySource, typesSource } = researchConfig;
  enriched.researchQuery = query;
  enriched.researchTypes = types;
  enriched.researchQuerySource = querySource;
  enriched.researchTypesSource = typesSource;
  ...
  const depthRun = await runResearchDepthCascade({
    ...
    brandResearch,
    runResearch,
    maxRounds: enriched.publicTestMode
      ? 0
      : resolveResearchDepthMaxRounds(enriched, false),
  });
  ...
  if (isBriclogResearchFirstEnforced()) {
    ...
    out = stampCoreEngineOnInput(stampBrandContentOSOnInput(stampResearchFirstOnInput(out, dossier)));
    if (!dossier.writable && !enriched.publicTestMode) {
      return {
        ok: false,
        insufficient: true,
        input: out,
    ...
    out = stampCoreEngineOnInput(stampBrandContentOSOnInput(out));
  }
