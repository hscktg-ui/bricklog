...
import { enrichChannelInput } from "@/lib/content/channelBrandResolve";
...
function resolveIndustryKind(input = {}) {
  ...
  const topicBlob = `${ind} ${input.topic || ""} ${input.mainKeyword || ""}`;
  if (/가구|침대|매트리스|쇼룸|소파/.test(topicBlob)) return "furniture";
  if (/꽃|플로리스트|플라워/.test(topicBlob)) return "flower";
  if (/반려|펫|애견|고양이/.test(topicBlob)) return "pet";
  if (/인테리어|리모델|홈스타일링/.test(topicBlob)) return "interior";
  if (/펜션|숙박|호텔|게스트하우스/.test(topicBlob)) return "lodging";
  if (/공방|원데이|체험\s*클래스|핸드메이드/.test(topicBlob)) return "workshop";
  if (/약국/.test(topicBlob)) return "pharmacy";
  if (/부동산|중개|임대/.test(topicBlob)) return "realestate";
  if (/법률|세무|노무|변호/.test(topicBlob)) return "legal";
  if (/패션|리테일|의류|뷰티\s*샵/.test(topicBlob)) return "retail";
  if (/쇼핑몰|이커머스|온라인/.test(topicBlob)) return "ecommerce";
  if (/카페|커피|디저트/.test(topicBlob)) return "cafe";
  if (/음식|맛집|식당|한식|일식|중식/.test(topicBlob)) return "food";
  if (/요가|피트니스|헬스|필라|운동/.test(topicBlob)) return "fitness";
  if (/수영|물놀이|워터파크|풀장|스파/.test(topicBlob)) return "leisure";
  if (/치과|병원|의료|한의|피부/.test(topicBlob)) return "medical";
  if (/미용|헤어|네일|피부과/.test(topicBlob)) return "beauty";
  if (/학원|교육|과외|입시/.test(topicBlob)) return "academy";
  return "general";
}
...
function buildPlaceScheduleLine(input = {}) {
  ...
  if (kind === "leisure") {
    return `이용 시간·입장 조건은 시즌·요일에 따라 달라질 수 있어요.`;
  }
...
function buildInstaSceneLines(input, ctx) {
  ...
  if (kind === "furniture") {
    return [
      ...
      `쇼룸 무드가 달랐더라구요.`,
      `순간 마음이 편해졌어요.`,
      brand ? `${brand} · 프로필·플레이스에 더 올려둘게요.` : "프로필·플레이스 링크에서 보면 돼요.",
    ];
  }
  if (kind === "flower") {
    return [
...
function scrubInstaBlogLeak(text) {
  return String(text || "")
    ...
    .replace(/예약·문의는\s*프로필·플레이스에서\s*이어서\s*보면\s*됩니다\.?/g, "프로필·플레이스 링크에서 보면 돼요.")
...
function buildInstaBody(pack, ctx, input) {
  ...
  if (lines.join("").replace(/\s/g, "").length < 150) {
    lines.push(
      `${String(ctx.region || input.region || "").trim() ? `${ctx.region || input.region}, ` : ""}직접 들러 보고 정리한 내용을 짧게 남겨봤어요. 플레이스에서 확인해 주세요.`
    );
  }
  return toInstaLineBreaks(scrubInstaBlogLeak(lines.join("\n"))).slice(0, 520);
}
...
export function applyPlaceMarketerPack(pack, ctx = {}, input = {}) {
  ...
  ctx = { ...ctx, brandName: input.brandName, region: input.region };
