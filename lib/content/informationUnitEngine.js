...
import { isResearchProperNounTopic } from "@/lib/product/researchProperNounProfile";
...
export function detectVerbatimTopicUsage(pack, input = {}) {
  ...
  const researchRelief = Boolean(pack?._meta?.researchGroundedTrustPolish || pack?._meta?.draftFallback);
  ...
  let maxOpenHits = isProductNounTopic ? Math.min(3, Math.ceil(sectionCount / 4)) : 1;
  if (researchRelief) {
    maxOpenHits = Math.min(5, Math.max(2, Math.ceil(sectionCount / 2)));
  }
