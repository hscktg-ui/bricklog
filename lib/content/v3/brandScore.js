...
export function evaluateV3BrandScore(pack, ctx = {}, input = {}) {
  ...
  const v2Axis = evaluateV2Axis(pack, ctx, { ...input, contentChannel: channel });
  ...
  const depthTier = input.researchDepthTier || "direct";
  const target = v3TargetForDepth(depthTier);
  const factN = input.researchFactCount ?? 0;
  const infoScore =
    depthTier === "contextual"
      ? Math.min(92, 72 + Math.min(20, factN * 4))
      : depthTier === "expanded"
        ? Math.min(95, 78 + Math.min(17, factN * 3))
        : Math.min(100, factN >= 20 ? 95 : 55 + Math.min(35, factN * 2));
  const seoScore = v2Axis.scores?.seo ?? 80;
  const trustScore =
    input.v3FactCheck?.pass !== false ? 92 : 55;

  const title = String(
    pack.representativeTitle || pack.title || pack.hook || ""
  );
  ...
  const failReasons = [];
  if (total < target) failReasons.push("v3_below_95");
...
export function buildV3RegenNote(score) {
  ...
  if (score?.total < (score?.target ?? V3_TARGET_SCORE)) {
    parts.push(`V3 브랜드 점수 ${score?.target ?? V3_TARGET_SCORE}점 이상`);
  }
  return parts.join(". ");
}
