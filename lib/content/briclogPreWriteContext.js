...
export function prepareBriclogPreWriteContext(input = {}) {
  const expansion = runKnowledgeExpansionPipeline(input);
  const customerQuestionMap = buildCustomerQuestionAnalysis(input, {
    researchFacts: input.researchFacts,
  });
  const customerSituations = inferCustomerSituations(input);
  const enriched = {
    ...input,
    ...
    customerSituations,
    deepLearningBrief: buildDeepLearningPreWriteBrief(input),
  };
  return enriched;
}
