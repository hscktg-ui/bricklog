...
import { mergeRegexProfile, normalizeRegexList } from "@/lib/utils/safeRegex";
...
export function loadColumnMagazineProfile() {
  ...
  if (learnedColumnMagazineProfile && learnedColumnMagazineProfile.sampleCount) {
    cachedProfile = mergeRegexProfile(DEFAULT_PROFILE, learnedColumnMagazineProfile);
    return cachedProfile;
  }
  cachedProfile = mergeRegexProfile(DEFAULT_PROFILE, {});
  return cachedProfile;
}

function markerHit(text, markers) {
  return normalizeRegexList(markers).some((re) => re.test(String(text || "")));
}
...
function detectVoiceEndingStyle(text) {
  ...
  let hamnida = 0;
  let banmal = 0;
  const endings = profile.voiceEndings || {};
  for (const s of sentences) {
    const t = s.trim();
    if (endings.haeyo?.test?.(t)) haeyo += 1;
    else if (endings.hamnida?.test?.(t)) hamnida += 1;
    else if (endings.banmal?.test?.(t)) banmal += 1;
  }
...
export function scoreToneBookends(pack) {
  ...
  for (const re of normalizeRegexList(profile.openerClosersBad)) {
...
export function applyMagazineArcPolish(pack, input = {}) {
  ...
  const skipHeavyBridge = Boolean(pack._meta?.narrativeArcShape);
  const arcPolished = sections.map((sec, i) => {
    let body = String(sec.body || "").trim();
    const needsBridge = !skipHeavyBridge && (i === 1 || i === 3);
    if (needsBridge && body && !/(그래서|직접|비교|정리|솔직히)/.test(body.slice(0, 48))) {
