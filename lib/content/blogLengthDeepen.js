...
function consumerTopicChunks(ctx, input, slot, count = 8) {
  ...
  return buildTopicAwareConsumerPads(merged, slot, count).map((line) =>
    cleanPara(line)
  );
}
...
export function deepenPackBodiesToMin(pack, min, ctx = {}, input = {}) {
  ...
  if (isCoverageExpansionForbidden() && !merged._salvageForce) {
  ...
  const targetMin = merged._salvageForce ? min + 380 : min;
