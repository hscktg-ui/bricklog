...
export const HUMAN_DELIVERY_PROMPT_RULES = `【고객 약속 · 인간 글 · GPT/제미나이보다 읽히는 칼럼】
- 정보 나열·스펙 리스트·소제목→한 줄→소제목 구조 금지. 한 주제로 이어지는 칼럼처럼 쓸 것.
- 기(왜 찾게 됐는지) → 승(현장·매장) → 전(비교·기준) → 결(정리) 순서. 같은 문장·안내 패턴 반복 금지.
- 조사에서 나온 팩트는 각각 한 번만, 해당 arc 구간에 자연스럽게 녹일 것. 이미 쓴 팩트 재진술 금지.
- 모든 팩트는 왜/언제/어떻게 중 하나로 설명하고, 관찰·경험·의견 중 하나를 붙일 것.
- AI가 쓴 듯한 말투·관용구·체크리스트 금지. 사람이 키보드로 직접 쓴 느낌.
- 감정·공감은 '감동을 선사' 같은 말이 아니라 장면·순간·몸의 반응으로 전달.
- 문단마다 앞 문장과 자연스럽게 이어지게 연결어·시점을 맞출 것.
- 선택한 분량 tier(min~max)를 반드시 지킬 것. 부족하면 조사 팩트·장면으로 보강하고, 초과하면 군더더기를 자르기. 분량 맞추기용 반복·일반론·빈 문단 금지.
...
export function scoreLengthTierCompliance(pack, ctx = {}) {
  ...
  const effectiveMin = resolveEffectiveBlogLengthMin(pack, input, tierKey, tier);
  const chars = countBlogBodyCharsWithSpaces(pack);
  const reasons = [];
  let score = 10;

  if (chars < effectiveMin) {
    reasons.push("length_tier_under");
    score -= Math.min(8, Math.round(((effectiveMin - chars) / effectiveMin) * 10));
  ...
  const inBand = chars >= effectiveMin && chars <= tier.max;
  return {
    ok: inBand,
    ...
    target: tier.target,
    max: tier.max,
    researchGroundedLength: effectiveMin < tier.min,
    reasons,
  };
}
