...
import { verifyPreWriteResearch, isClientAxisResearchComplete } from "@/lib/content/v2PipelineGate";
...
import { mergeWorkspaceBrandIntoInput } from "@/lib/workspace/brandFormSync";
import { hasFilledBlogAxes, materializeVerifiedGenerationAxes } from "@/lib/product/deliverySoftPass";
...
import { injectBrandContextResearchFacts } from "@/lib/content/researchFactSanitize";
...
/** 2분 SLA — 조사 API 1회 + 축 브리프만 (V3·보강·연쇄 조사 생략) */
async function applySlaFastAxisResearch({
  pipelineInput,
  input,
  ...
  const { query, types, querySource, typesSource } = researchConfig;
  pipelineInput.researchQuery = query;
  pipelineInput.researchTypes = types;
  pipelineInput.researchQuerySource = querySource;
  pipelineInput.researchTypesSource = typesSource;
  ...
  const factRows = augmentFactsFromLocalContext(
  ...
  const depthRun = {
    research,
    facts: factRows,
    depth: assessResearchDepth(factRows.length, input),
  };

  research = depthRun.research || {
    summary: `${input.brandName || ""} · ${input.region || ""} · ${input.topic || input.mainKeyword || ""} 맥락으로 글을 구성합니다.`,
    sources: [],
    v2Axis: { researchStatus: "ok", insufficient: false },
  };

  const parsed = buildContextualParsed(input, brandResearch, research, depthRun);
  const enrichedFacts = injectBrandContextResearchFacts({
    ...input,
    researchFacts: parsed.facts,
  });
  parsed.facts = enrichedFacts;
  parsed.factCount = enrichedFacts.length;
  ...
  pipelineInput.slaFastAxisResearch = true;
...
export async function applyV2AxisResearch({
  ...
  const input = mergeWorkspaceBrandIntoInput(pipelineInput);
  Object.assign(pipelineInput, materializeVerifiedGenerationAxes(input));
  ...
  if (isClientAxisResearchComplete(input) && hasUsableResearchFacts(input)) {
    onStep?.("조사 반영 — 작성 준비");
    ...
    return {
      ...
      insufficient: false,
      skipped: true,
      brief: pipelineInput.v2AxisBrief || pipelineInput.researchBrief || "",
      storage: pipelineInput.researchPayload || null,
      pipelineInput,
    };
  }

  const channelFast = isChannelStandaloneFastInput(pipelineInput);
  if (channelFast) {
    pipelineInput.channelStandaloneFast = true;
    pipelineInput.v3EngineEnforced = false;
  }

  if (isCustomerTwoMinuteSlaMode() && !channelFast) {
    return applySlaFastAxisResearch({
      pipelineInput,
      input,
      generateResearchAsync,
      setResearchResult,
  ...
  if (!channelFast) {
    ...
    pipelineInput.channelStandaloneResearchSkipped = true;
    input.channelStandaloneResearchSkipped = true;
  }

  onStep?.("관련 정보 탐색 중…");
  const skipDepthCascade =
    channelFast ||
    (shouldSkipResearchDepthCascadeForSla() && research != null);
  const depthRun = skipDepthCascade
    ? {
        research,
        ...
        depth: assessResearchDepth(
          collectResearchFacts(research, input, brandResearch).length,
          input
        ),
      }
    : await runResearchDepthCascade({
        input,
        types,
        regionKeywordHints: regionHints,
        ...
        onStage: onStep,
        runResearch,
        maxRounds: resolveResearchDepthMaxRounds(input, isModelLikeTopic(topic)),
      });
  ...
  if (
    !channelFast &&
    !shouldSkipModelTopicExtraResearch() &&
    !isClientResearchPreVerified(input) &&
  ...
  onStep?.("조사 검증 중…");
  if (
    !(shouldSkipChannelSupplementalResearch(pipelineInput) && channelFast) &&
    !shouldSkipSupplementalResearchForSla({
      ...input,
      researchFacts: parsed.facts,
  ...
  let preWriteVerification = verifyPreWriteResearch(parsed, research);
  const materializedInput = materializeVerifiedGenerationAxes(input);
  Object.assign(pipelineInput, materializedInput);
  Object.assign(input, materializedInput);
  const sufficiencyBlock = researchGateBlockedForInsufficient(
    materializedInput,
    parsed,
    research
  );
  if (sufficiencyBlock) {
    const missingAxes = (sufficiencyBlock.reasons || []).some((r) =>
      /missing_axes|missing_brand|missing_region|missing_topic/.test(String(r))
    );
    if (missingAxes && hasFilledBlogAxes(materializedInput)) {
      pipelineInput.researchThinMode = true;
      ...
      pipelineInput.researchSufficiencyWarning = sufficiencyBlock.userMessage;
      pipelineInput.researchThinMode = true;
      input.researchThinMode = true;
    }
  }
  ...
  onStep?.("콘텐츠 전략 수립 중…");
  let v3 = channelFast || shouldSkipV3PreWriteForSla()
    ? {
        ok: true,
        v3MasterBrief: [brandBrief, regionBrief, parsed.brief]
          .filter(Boolean)
