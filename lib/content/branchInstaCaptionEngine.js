...
export function resolveIndustryKindForBranch(input = {}) {
  ...
  if (/수영|물놀이|워터파크|풀장|스파/.test(blob)) return "leisure";
...
export function resolveBranchInstaArchetype(input = {}) {
  ...
  if (/오픈|신규|호점|입점|그랜드\s*오픈|오픈했|오픈해|신규\s*매장|개장|n호점|\d+\s*호점/.test(blob)) {
    return "store_open";
  }
  if (
...
function buildStoreOpenCaption(input = {}, ctx = {}) {
  ...
  if (kind === "leisure") {
    body.push(
      "날씨 걱정 없이 즐길 수 있는 수영·물놀이 공간을 준비했습니다.",
      "가족 나들이·휴식을 함께 즐기기 좋도록 이용 안내를 플레이스에 올려두었습니다."
    );
  }
