...
function stripCatalogFromConclusion(conclusion = "") {
  return stripCatalogContaminationSentences(String(conclusion || "").trim());
}
...
export function buildVisitReviewParagraphs(input = {}, need = 6) {
  ...
  const subject = topicWritingFacet(input) || defaultTopicFacet(input);
  ...
  const topicBlob = [
    ...
    .join(" ");
  const pool = [
    `${p.regionBit}${p.brand}에 새로 ${subject} 소식이 있어 현장을 직접 둘러봤어요.`,
    `기존에도 ${flavor.spaceWord || "공간"}과 식사·카페가 함께 운영되고 있었는데, 이번에는 물놀이까지 이어지는 구성으로 한곳에서 하루를 보내기 편해졌어요.`,
    `처음 들어서니 가족 단위 방문객이 편하게 이용할 수 있도록 동선과 좌석 배치가 잘 정돈되어 있었어요.`,
    `${flavor.productWord || "이용"}은 현장에서 본 구성과 안내를 기준으로 비교했어요.`,
    `수영 후 바로 식사나 휴식으로 이어갈 수 있어 하루 일정이 끊기지 않았어요.`,
    `메뉴·이용 시간·주차 안내는 매장에서 확인한 범위만 메모해 두었어요.`,
    `혼잡한 시간대와 한산한 시간대 분위기 차이도 같이 봤어요.`,
    `방문 전 운영 시간·이용 요금·예약 여부를 미리 확인하는 편이 좋았어요.`,
    `궁금한 점은 ${p.regionBit}${p.brand} 문의로 다시 확인하는 편이 정확해요.`,
  ];

  if (/수영|물놀이|워터|풀장|수영장/.test(topicBlob)) {
    pool.unshift(
      `식사·카페·휴식과 물놀이가 한 공간 안에서 자연스럽게 이어지는 점이 먼저 눈에 들어왔어요.`,
      `아이들과 함께 방문할 때 수영 뒤 바로 식사·휴식으로 넘어갈 수 있다는 점이 장점으로 느껴졌어요.`,
      `단순 규모보다 이용 환경과 편의시설이 만족도에 더 크게 작용한다는 걸 현장에서 다시 확인했어요.`
    );
  }
...
export function detectVisitReviewTemplateContamination(pack, input = {}) {
  ...
  const violations = [];
  const key = resolveBriclogIndustryKey(intentInput);
  ...
  for (const re of engineSpam) {
    if (re.test(full)) {
      violations.push({ type: "engine_template_spam", pattern: re.source });
    }
  }

  if (!isVisitReviewTopicInput(intentInput, pack)) {
  ...
  const rawTopic = topicRaw(intentInput);
...
export function rebuildVisitReviewAccuratePack(pack, input = {}) {
  ...
  return {
    ...
    sections,
    conclusion: shouldSkipMissionCatalogConclusion(pack, input)
      ? stripCatalogFromConclusion(pack.conclusion)
      : buildMissionConclusionLine(p, input, subject, pack),
    hashtags: [],
    _meta: {
      ...(pack._meta || {}),
...
export function applyVisitReviewTopicPackGate(pack, input = {}) {
  ...
  if (
    !shouldSkipMissionCatalogConclusion(pack, input) &&
    (!conclusion || PRODUCT_INFO_PAD_RES.some((re) => re.test(conclusion)))
  ) {
    conclusion = buildMissionConclusionLine(p, input, subject, pack);
  } else if (shouldSkipMissionCatalogConclusion(pack, input)) {
    conclusion = stripCatalogFromConclusion(conclusion);
  }
