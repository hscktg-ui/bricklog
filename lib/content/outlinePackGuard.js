...
/** 플레이스 공지 — · 불릿 본문은 정상 포맷 (구성안 마커·PLAN 문구만 차단) */
function placeBodyLooksLikeOutline(body) {
  const t = String(body || "").trim();
  if (!t) return true;
  if (OUTLINE_BODY_MARKERS.some((m) => m && t.includes(m))) return true;
  if (/구성안입니다|구성안 —|구성안만|체크포인트\s*확인/.test(t)) return true;
  const lines = t.split(/\n+/).map((l) => l.trim()).filter(Boolean);
  const sceneHits = lines.filter((l) =>
    OUTLINE_PLAN_HEADING_PHRASES.some((p) => l.includes(p))
  ).length;
...
function detectPlaceOutlineLeak(pack) {
  ...
  if (placeBodyLooksLikeOutline(shortNotice)) reasons.push("outline_short_notice");
  if (
    detailBody.replace(/\s/g, "").length < 80 ||
    placeBodyLooksLikeOutline(detailBody)
  ) {
...
function detectInstagramOutlineLeak(pack) {
  ...
  if (hook && bodyLooksLikeOutlinePlan(hook)) reasons.push("outline_hook");
...
export function buildBrandFocusedSectionHeadings(input = {}, count = 4) {
  if (isVisitReviewTopicInput(input)) {
    return buildTopicArcSectionHeadings(input, Math.max(1, count));
  }
