...
export function injectBrandFactsIntoPack(pack, input = {}, opts = {}) {
  ...
  const effectiveMinHits =
    input._brandDepthBriefUsed ? Math.min(minHits, 2) : minHits;
  const probe = assessBrandFactPresence(pack, input);
  if (probe.ok || probe.available < 1) return pack;

  const brand = String(input.brandName || "").trim();
  const full = getBlogFullText(pack);
  const brandInBody = brand.length >= 2 && full.includes(brand);
  if (input._brandDepthBriefUsed && probe.hits.length >= effectiveMinHits && brandInBody) {
    return {
      ...pack,
      _meta: {
        ...(pack._meta || {}),
        brandFactInjection: {
          skipped: "brand_depth_brief_satisfied",
  ...
  const region = String(input.region || "").trim();
  const lines = missing
    .slice(0, Math.max(effectiveMinHits - probe.hits.length, 1))
