...
function isLowYieldMetaResearchFact(text = "", input = {}) {
  ...
  if (/방문·시즌\s*맥락/.test(text)) return true;
  if (/매장\s*체험·행사\s*조건/.test(text)) return true;
  if (/이번\s*글의\s*핵심\s*주제/.test(text)) return true;
  return false;
}
...
export function humanizeResearchFact(fact, p, input, slot = 0, profile = {}) {
  ...
  if (profile.archetype === "essay" && core.length >= 8) {
    ...
    const pool = [
      `${core} — 예약 전에 다시 읽어 보면 도움이 돼요.`,
      `안내에서 ${core} 쪽이 눈에 들어왔어요.`,
      `${p.brand} ${facet} 준비할 때 ${core}를 메모해 뒀어요.`,
      `손에 잡히는 디테일로 보면 ${core}.`,
    ];
