...
/** 플레이스·인스타는 매장 화자 — 방문후기 톤은 고객 시점·field_review일 때만 */
...
export function hasUsableResearchFacts(input = {}) {
  const facts = collectMergedResearchFacts(input).filter((f) => !isBlockedResearchFact(f, input));
...
function humanizeResearchFact(fact, p, input, slot = 0, profile = {}) {
  ...
  if (isField) {
    if (tailMeta) {
      return `${p.regionBit}${p.brand} ${p.topicFacet} 관련해 ${cleanFactText(text)}를 확인했습니다.`;
    }
    const pool = [
  ...
  const pool = [
    `${p.brand} 안내 기준으로 ${tail}를 정리해 봤어요.`,
    `현장에서 ${tail} 이야기를 들으며 메모해 뒀어요.`,
    `${tail} — 당일 매장 설명과 맞춰 메모해 뒀어요.`,
  ];
  return pool[slot % pool.length];
}
...
function buildTitle(input = {}, p) {
  ...
  return `${p.regionBit}${p.brand} ${facet} — ${flavor.visitReason || "방문"} 정리`
    .replace(/\s+/g, " ")
    .trim();
}
...
export function buildResearchGroundedHumanPack(input = {}) {
  ...
  for (let i = 0; i < sectionCount; i += 1) {
    ...
    if (paras.length < 2 && i > 0) {
      paras.push(
        `${p.regionBit}${p.brand}에서는 ${p.topicFacet}를 볼 때 계절·목적에 맞는 구성부터 살펴봤어요.`
      );
    }
    sections.push({
  ...
  if (bodyChars < tier.min && factLinePool.length) {
    let refillRound = 0;
    while (bodyChars < tier.min && refillRound < 8) {
      pack = deepenDensityFirstPack(pack, tier.min, input, {
        polishAfter: true,
        seedOffset: refillRound,
        researchLines: factLinePool,
      });
...
export function buildResearchGroundedPlacePack(input = {}) {
  ...
  const facet = topicWritingFacet(input) || p.topicFacet || defaultTopicFacet(input);
  const visitTone = useVisitReviewToneForChannel(input, "place");
  const title = visitTone
    ? `${p.regionBit}${brand} ${facet} 방문 후기`.replace(/\s+/g, " ").trim()
    : `${p.regionBit}${brand} ${facet}`.replace(/\s+/g, " ").trim();

  const shortNotice = (
    visitTone
      ? `${brand} ${facet} — ${region ? `${region} ` : ""}매장 소식`
      : lines[0] || `${brand} ${facet} 안내`
  ).slice(0, 120);
  const detailBody = [
    visitTone
    ...
    ...lines,
    `${brand} 예약·문의는 매장 안내를 먼저 확인해 주세요.`,
  ]
    .filter(Boolean)
    .join("\n\n")
    .slice(0, 520);
