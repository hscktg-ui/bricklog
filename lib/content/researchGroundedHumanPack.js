...
export function humanizeResearchFact(fact, p, input, slot = 0, profile = {}) {
  ...
  if (profile.archetype === "expert_column" && core.length >= 10) {
    ...
    const coreBit = core.replace(/(?:은|는|이|가)\s*$/u, "").trim();
    return pickHumanizedFactLine(
      [
        `${p.brand} ${topicBit} — ${coreBit}은 상담·등록 전에 확인하는 편이에요.`,
        `${coreBit}. ${p.regionBit}${p.brand} ${topicBit} 관련 포인트예요.`,
        `${p.brand} ${topicBit} 관련해 ${coreBit}.`,
        `${coreBit} — ${p.brand} 안내 기준으로 정리했어요.`,
        `${p.regionBit}${p.brand} ${topicBit}는 ${coreBit}.`,
      ],
      slot
    );
  }
