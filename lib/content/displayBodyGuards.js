...
export const GENERIC_EXPERIENCE_VOICE_RES = [
  ...
  /검색만\s*하다\s*보면\s*기준이\s*많아서\s*어디서부터\s*볼지\s*막히/,
];

/** 에이전트·개발 메타 지시문 — 본문 노출 금지 */
export const AGENT_META_LEAK_RES = [
  /엔진의\s*구조/,
  /마케팅\s*에디터/,
  /엔진을\s*개선/,
  /분석하고\s*생각해서/,
...
export function isGenericDisplayPadSentence(text = "", input = {}) {
  ...
  if (AGENT_META_LEAK_RES.some((re) => re.test(t))) return true;
...
function normalizeDisplayHeading(raw = "") {
  return String(raw || "")
    .replace(/(?:\s*[—–-]\s*이어서)+$/gi, "")
    .replace(/\s+이어서\s*$/gi, "")
    .trim();
...
function dedupeSectionHeadings(sections = []) {
  const seen = new Set();
  return sections.map((sec, index) => {
    let heading = normalizeDisplayHeading(sec.heading);
    const key = heading.toLowerCase().replace(/\s+/g, "");
    if (heading && seen.has(key)) {
      heading = heading ? `${heading} (${index + 1})` : `이야기 ${index + 1}`;
    }
    if (heading) seen.add(key);
    return { ...sec, heading };
  });
}
...
export function applyDisplayBodyGuardPack(pack, input = {}) {
  ...
  let sections = (pack.sections || [])
    .map((sec) => ({
      ...sec,
      heading: normalizeDisplayHeading(stripForbiddenSentences(sec.heading, input)),
      body: stripForbiddenSentences(sec.body, input),
    }))
    .filter((sec) => String(sec.body || "").replace(/\s/g, "").length >= 20);
  sections = dedupeSectionHeadings(sections);
  if (sections.length < minKeep) {
    sections = (pack.sections || [])
      ...
      .filter((sec) => String(sec.body || "").replace(/\s/g, "").length >= 12);
  }
  if (sections.length < minKeep) {
    sections = (pack.sections || [])
      .map((sec) => ({
