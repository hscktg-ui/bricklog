...
function pruneDuplicateParagraphsGlobally(pack) {
...
function weaveSectionPlanGaps(pack, input = {}) {
...
function ensureTierMinConclusion(pack, input = {}) {
  ...
  const tail = buildMissionConclusionLine(p, input, facet);
...
function scrubVerbatimUntilOk(pack, input = {}) {
  let next = ensureVerbatimTopicCompliance(pack, input, "blog");
