...
const INDUSTRY_ANCHOR_RES = {
  ...
  education: /특강|학년|반\s*편성|커리큘럼|등록|내신|수업|상담실|교실/,
  craft: /원데이|클래스|체험|도자기|완성품|난이도|소요\s*시간|공방/,
  pension: /숙박|객실|예약|체크인|펜션/,
  construction: /시공|견적|현장|공사|리모델/,
...
export function scoreGiSeungJeonGyeol(pack) {
  ...
  const gi = /(왜|검색|찾|고민|상황|배경|궁금|솔직히|막혔)/.test(
...
export function applyEditorQualityPack(pack, ctx = {}, input = {}) {
  ...
  next = applyKoreanOrthographyToBlogPack(next, { ...ctx, ...input });
  return {
    ...next,
    _meta: {
      ...(next._meta || {}),
      editorQualityEngine: true,
      koreanOrthographyApplied: true,
    },
  };
}
...
export function detectEditorQualityIssues(pack, ctx = {}, input = {}) {
  ...
  const orthography = detectKoreanOrthographyIssues(full, { ...ctx, ...input });
  if (!orthography.pass) {
    issues.push({
      type: "korean_orthography",
      score: orthography.score,
      hits: orthography.hits?.slice(0, 6),
    });
  }
  ...
  const researchRelief =
    isResearchGroundedDeliveryPack(pack, input) &&
    Boolean(pack?._meta?.researchGroundedTrustPolish || pack?._meta?.draftFallback);
  const salvageRelief = Boolean(
    pack?._meta?.salvageDeliveryFinalized ||
      pack?._meta?.contentQualityDelivered ||
      /columnist|llm_/i.test(String(pack?._meta?.generationMode || ""))
  );
  const filtered = researchRelief
    ? issues.filter(
        (i) =>
          !["verbatim_topic_dump", "editor_v95", "missing_region"].includes(i.type)
      )
    : issues;

  const softFiltered = filtered.filter((i) => {
    if (i.type === "korean_orthography" && salvageRelief && Number(i.score || 0) >= 84) {
      return false;
    }
    if (
      i.type === "editor_v95" &&
      ...
      Number(i.score || 0) >= 64 &&
      !(i.issues || []).some((x) =>
        ["forbidden_intro", "ad_smell", "same_meaning_repeat"].includes(x?.type)
      )
    ) {
      return false;
    }
  ...
  return { ok: softFiltered.length === 0, issues: softFiltered, gi, tone };
}
...
export function applyProfessionalEditorDeliveryPass(pack, ctx = {}, input = {}) {
  ...
  if (researchGrounded) {
    ...
    next = applyKoreanOrthographyToBlogPack(next, { ...input, ...ctx });
