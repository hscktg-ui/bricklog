...
function scoreResearchFactCoverage(full, input = {}) {
  ...
  for (const f of sample) {
    ...
    const core = String(f).replace(/\s*—\s*.+$/, "").trim();
    const coreAnchor = core.length > 12 ? core.slice(0, 10) : core;
    if (
      full.includes(f) ||
      (anchor.length >= 6 && full.includes(anchor)) ||
      (coreAnchor.length >= 6 && full.includes(coreAnchor))
    ) {
      covered += 1;
    }
  }
