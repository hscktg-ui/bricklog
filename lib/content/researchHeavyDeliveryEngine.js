...
export function scoreResearchHeavyDelivery(pack, input = {}) {
  ...
  const infoOk = info.score >= 58 || (factsWoven && info.score >= 48);
  const lengthOk = sections >= HUMAN_MIN_SECTIONS && chars >= minChars;
  const factsOk = factsWoven || !hasUsableResearchFacts(input);
  const sqvOk = typeof sqvScore === "number" && sqvScore >= B_GRADE_MIN_SCORE;

  return {
    ok: lengthOk && factsOk && (infoOk || sqvOk),
    chars,
    sections,
    minChars,
...
export function applyResearchHeavyDeliveryPass(pack, input = {}) {
  ...
  if (isFurnitureChairProductTopic(input)) {
    let next = scrubFurnitureChairPack(pack, input);
    ...
    next = scrubFurnitureChairPack(next, input);
    let refill = 0;
    while (countBlogBodyCharsWithSpaces(next) < minChars && refill < 8) {
      next = finishLocalBlogPackForBatch(next, input, {
        customerDelivery: true,
        targetChars: minChars,
      });
    ...
    let stamped = stampContentQualityValue(next, input);
    let score = scoreResearchHeavyDelivery(
      {
        ...stamped,
        _meta: {
          ...stamped._meta,
          researchFactsWoven:
            stamped._meta?.researchFactsWoven ?? hasUsableResearchFacts(input),
        },
      },
      input
    );
    if (
      !score.ok &&
      typeof (stamped._meta?.sqv?.score ?? stamped._meta?.contentQualityValue) ===
        "number" &&
      (stamped._meta?.sqv?.score ?? stamped._meta?.contentQualityValue) >=
        B_GRADE_MIN_SCORE &&
      score.chars >= score.minChars
    ) {
      score = { ...score, ok: true, sqvCalibrated: true };
    }
    return {
      ...stamped,
      _meta: {
  ...
  if (isInformationalTopicInput(input) && !isFurnitureChairProductTopic(input)) {
  ...
  if (countBlogBodyCharsWithSpaces(next) < minChars) {
    ...
    if (countBlogBodyCharsWithSpaces(next) < minChars) {
      next = ensureCustomerDeliveryBlogLength(next, {
        ...input,
        _salvageForce: true,
      });
    }
  ...
  let stamped = stampContentQualityValue(next, input);
  let score = scoreResearchHeavyDelivery(stamped, input);
  const sqvScore = stamped._meta?.sqv?.score ?? stamped._meta?.contentQualityValue;
  if (
    !score.ok &&
    typeof sqvScore === "number" &&
    sqvScore >= B_GRADE_MIN_SCORE &&
    score.chars >= score.minChars &&
    score.sections >= HUMAN_MIN_SECTIONS
  ) {
    score = { ...score, ok: true, sqvCalibrated: true };
  }
  ...
  if (isFurnitureChairProductTopic(input)) {
    let refill = 0;
    while (
      countBlogBodyCharsWithSpaces(stamped) < minChars &&
      refill < 6
    ) {
      stamped = finishLocalBlogPackForBatch(stamped, input, {
        customerDelivery: true,
        targetChars: minChars,
      });
      stamped = ensureCustomerDeliveryBlogLength(stamped, input);
      refill += 1;
    }
    score = scoreResearchHeavyDelivery(stamped, input);
  }
...
/** SQV — 조사형 송출본 B등급(76+) 하한 (실제 분량·정보량·고유명 반영 충족 시) */
export function calibrateResearchHeavySqMinimum(sqv, pack, input = {}) {
  ...
  const proper = isResearchProperNounTopic(input)
    ? scoreResearchProperNounAnchoring(getBlogFullText(pack), input)
    : null;
  const properOk =
    proper &&
    proper.score >= 78 &&
    proper.productRatio >= 0.4 &&
    score.chars >= score.minChars &&
    score.sections >= HUMAN_MIN_SECTIONS;

  if (!score.ok && !properOk) return sqv;
