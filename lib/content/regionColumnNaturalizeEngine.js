...
import {
  applyRegionBrandMashRepairToPack,
  assessPackRegionBrandMash,
  isRegionEmbeddedInBrand,
...
function capRegionInText(text, region, max, state, substitutes, brand = "") {
  if (!region || region.length < 2) return text;
  const re = new RegExp(escapeRegExp(region), "gi");
  return String(text || "").replace(re, (match, offset) => {
    if (isRegionEmbeddedInBrand(region, brand, text, offset, match.length)) {
      return match;
    }
    state.n += 1;
...
export function capRegionMentionsOnPack(pack, input = {}, maxTotal) {
  ...
  const cap = (t) => capRegionInText(t, region, max, state, subs, brand);
    ...
    next = {
      ...
      sections: (next.sections || []).map((sec) => ({
        ...
        body: softenTokenRepeats(
          ...
          ANTI_SEO_SPAM_MAX_REGION_REPEAT,
          { brandName: input.brandName, region: input.region }
        ),
