...
export const TEMPLATE_PHRASE_LIMITS = [
  ...
  { re: /로컬\s*매장\s*운영·예약\s*맥락/g, max: 1, label: "로컬 매장 운영·예약 맥락" },
];
...
export function buildExplainAxisLine(input = {}, slot = 0) {
  ...
  const generic = brand
    ? [
        `${brand}에서 ${topic} 안내를 받을 때 운영 시간·예약 조건부터 확인해 보세요.`,
        `현장에서 직접 보면 ${topic} 구성이 사진과 다를 수 있어요.`,
        `${brand} 기준으로 ${topic} 일정·비용은 당일 안내를 따르는 편이 안전해요.`,
        `방문 전에 ${topic} 관련 문의 사항을 메모해 가면 상담이 빨라져요.`,
      ]
    : [
