...
import { detectEmptyInputVars } from "@/lib/content/placeholderContaminationEngine";
import {
...
import { shouldAllowNonLlmTemplateFallback } from "@/lib/llm/llmProvider";
...
import { tryDeliverResearchHeavyMissionRescue } from "@/lib/product/missionProseRouteEngine";
...
/** 클라이언트에서 축 조사·검증을 이미 마친 경우 — research_first 재검사 생략 */
...
export function assertPreWriteVerified(input = {}) {
  Object.assign(input, materializeVerifiedGenerationAxes(input));
  const emptyVars = detectEmptyInputVars(input);
  if (!emptyVars.ok) {
    return {
      ok: false,
      stage: "input_vars",
      reasons: ["empty_input_vars", ...emptyVars.empty.map((k) => `empty_${k}`)],
      userMessage: "브랜드·지역·주제를 입력해 주세요.",
  ...
  const hasAxes = hasFilledBlogAxes(input);
    ...
    if (!topicProof.ok && !input.publicTestMode) {
      ...
      const topicProofBypass =
        ...
        (hasFilledBlogAxes(input) &&
          (factN >= 1 ||
            String(input.brandDescription || "").trim().length >= 12 ||
  ...
  if (isBriclogResearchFirstEnforced()) {
    const researchFirst = assertResearchFirstWritable(input);
    if (!researchFirst.ok && !researchFirst.skipped) {
      return {
        ok: false,
        stage: "research_first",
        reasons: researchFirst.reasons || ["research_first_blocked"],
        userMessage: sanitizeCustomerResearchMessage(
          researchFirst.userMessage,
          input,
          researchFirst.reasons
        ),
        researchFirstDossier: researchFirst.dossier,
        pipelineStages: KNOWLEDGE_EXPANSION_STAGES,
      };
    }
    if (researchFirst.dossier) {
      input.researchFirstDossier = researchFirst.dossier;
      input.researchFirstBrief = formatResearchFirstBrief(researchFirst.dossier);
      input.researchFirstWritable = researchFirst.dossier.writable;
    }
  }
...
export function assertPostWriteDeliverable(input = {}, pack) {
      ...
      const v17SoftOnly =
        ...
        ((v17.reasons || []).every((r) => SOFT_INFORMATION_REASONS.has(r)) ||
          (isLlmOriginatedPack(pack) &&
            isChannelPackDeliverable("blog", pack) &&
            !isHardWithholdFailure({ reasons: v17.reasons }, pack)));
      if (!v17SoftOnly) {
        return {
          ok: false,
          stage: "output_verify",
  ...
  if (contentChannel === "place" || contentChannel === "instagram") {
    const channelFirst = assessChannelFirstDeliveryQuality(
      pack,
      contentChannel,
      {
        ...input,
        contentChannel,
        sourceChannel: input.sourceChannel || pack._meta?.sourceChannel || "form",
      }
    );
    if (channelFirst.displayReady || channelFirst.northStarFastPass) {
      return {
        ok: true,
        stage: "output_verify",
        pack: {
          ...
          _meta: {
            ...(pack._meta || {}),
            channelNorthStarFastPass: Boolean(channelFirst.northStarFastPass),
            channelFirstDeliveryPass: true,
            passOutput: true,
          },
        },
        channelFirstDelivery: channelFirst,
      };
    }
  }
  ...
  if (contentChannel === "blog") {
    let blogPack = pack;
    const llmPack = isLlmOriginatedPack(blogPack);
    let corrected = { pack: blogPack, gate: null };

    if (!llmPack) {
      corrected = applyEditorPreOutputCorrection(blogPack, input, input);
      blogPack = corrected.pack;
    }

    const lengthGate = assertBlogLengthTier(input, blogPack);
    const lengthSoftLlm =
      llmPack && !lengthGate.ok && isChannelPackDeliverable("blog", blogPack);
    if (!lengthGate.ok && !lengthSoftLlm) {
      return {
        ok: false,
        stage: "output_verify",
    ...
    if (!topicPostProof.ok) {
      ...
      const topicPostSoft =
        ...
        infoOnlyFail &&
        !brandFail &&
        (input.publicTestMode || isLlmOriginatedPack(blogPack));
      if (!topicPostSoft) {
        return {
          ok: false,
    ...
    if (isTopicLockEnforced()) {
      ...
      if (!topicLockPost.ok && !topicLockPost.skipped) {
        ...
        if (!recheck.ok) {
          if (
            isLlmOriginatedPack(blogPack) &&
            hasSubstantiveLlmBody(blogPack, input)
          ) {
            blogPack = {
              ...blogPack,
              _meta: {
                ...blogPack._meta,
                topicLockSoftPassLlm: true,
                topicLockReasons: recheck.reasons,
              },
            };
          } else {
            return {
    ...
    const editorGate = llmPack
      ? { ok: true, skipped: true, reasons: [] }
      : corrected.gate || assertEditorPreOutput(blogPack, input, input);
    if (!editorGate.ok) {
      return {
        ok: false,
        stage: "output_verify",
    ...
    pack = {
      ...
      _meta: {
        ...
        lengthSoftPassLlm: lengthSoftLlm || undefined,
        editorPreOutput: editorGate,
        passOutput: true,
        softPass: lengthSoftLlm || false,
      },
    };
  }
...
function withholdResetQualityApiResponse(result, input = {}, pack = null) {
  const body = pack || result?.blogContent;
  if (!body?.sections?.length || !isBriclogResetQualityEnforced()) return null;
  if (
    body._meta?.writerFirstOrchestratorEscape ||
    (body._meta?.publishReady === true && body._meta?.columnistDeliveryOk)
  ) {
    return null;
  }
  const reset = assertBriclogResetQualityGate(body, input);
  if (!reset.shouldWithhold) return null;
  return researchGateBlockedResult(
    input,
    {
      ok: false,
      userMessage: reset.userMessage || RESET_QUALITY_WITHHOLD_MESSAGE,
...
export function blockUnverifiedBlogApiResponse(result, input = {}) {
  ...
  const resetBlocked = withholdResetQualityApiResponse(result, input);
  if (resetBlocked) return resetBlocked;
  ...
  if (gate.ok) {
    if (isBriclogMissionEnforced()) {
      ...
      if (!complete.ok) {
        if (
          hasSubstantiveLlmBody(gate.pack, input) &&
          isLlmOriginatedPack(gate.pack, result)
        ) {
          ...
          const displayed =
            polished?.sections?.length
              ? ensureBlogDisplayPack(
                  {
                    ...polished,
                    _meta: { ...polished._meta, deliveryRescue: true },
                  },
                  input
                )
              : null;
          if (displayed?.sections?.length && hasSubstantiveLlmBody(displayed, input)) {
            return {
              ...result,
              ok: true,
              ...
              meta: attachContentQualityToApiMeta(
                {
                  ...(result.meta || {}),
                  passOutput: displayed._meta?.publishReady ?? true,
                  v2PipelineVerified: false,
                  llmCompleteGateSoft: true,
    ...
    const resetAfterGate = withholdResetQualityApiResponse(result, input, gate.pack);
    if (resetAfterGate) return resetAfterGate;
    return {
      ...result,
  ...
  if (
    hasSubstantiveLlmBody(result.blogContent, input) &&
    ...
    const displayed =
      polished?.sections?.length
        ...
        : null;
    if (displayed?.sections?.length && hasSubstantiveLlmBody(displayed, input)) {
      const resetAfterRescue = withholdResetQualityApiResponse(result, input, displayed);
      if (resetAfterRescue) return resetAfterRescue;
      return {
        ...result,
        ok: true,
        blogContent: displayed,
        ...
        softPass: false,
        userMessage: null,
        mode: result.mode || "llm_api_preserved",
        meta: attachContentQualityToApiMeta(
          {
            ...(result.meta || {}),
            passOutput: displayed._meta?.publishReady ?? true,
            v2PipelineVerified: false,
            llmApiPreserved: true,
...
export function researchGateBlockedResult(input, gate, pack = null) {
  ...
  if (isWriterFirstRescueBlocked(input)) {
    const escaped =
      pack?.sections?.length > 0
        ? tryWriterFirstOrchestratorDelivery(pack, input)
        : null;
    if (escaped?.sections?.length) {
      return {
        ok: true,
        withheld: false,
        softPass: false,
        ...
        meta: {
          ...
          generationMode: "llm_gpt55",
          passOutput: true,
          v2PipelineVerified: true,
          v3PipelineVerified: true,
          gateBypassReasons: gate?.reasons || gate?.failReasons || [],
        },
      };
    }
    return {
      ...
      softPass: false,
      userMessage:
        gate?.userMessage || buildWriterFirstWithholdMessage(input),
      mode: "writer_first_withheld",
      meta: {
        failReasons: gate?.reasons || gate?.failReasons || ["writer_first_rescue_blocked"],
        writerFirstRescueBlocked: true,
      },
    };
  }
  ...
  if (body?.sections?.length && hasFilledBlogAxes(input)) {
    const salvaged = salvageBlogPackForDelivery(body, input);
    const rescueMeta = { ...salvaged._meta, deliveryRescue: true };
    body = isLlmOriginatedPack(body)
      ? ensureBlogDisplayPack({ ...salvaged, _meta: rescueMeta }, input)
      : ensureBlogDisplayPack(
          {
            ...ensureMissionProseTierLength(salvaged, { input }),
            _meta: rescueMeta,
          },
          input
        );
  ...
  if (!body?.sections?.length && hasFilledBlogAxes(input) && shouldAllowNonLlmTemplateFallback()) {
  ...
  if (
    isLlmOriginatedPack(pack) &&
    hasFilledBlogAxes(input) &&
    hasSubstantiveLlmBody(pack, input)
  ) {
    const polished = finalizeContentQualityForDelivery(pack, input, "blog");
    const displayed =
      polished?.sections?.length
        ? ensureBlogDisplayPack(
            {
  ...
  const rescued = buildMissionRescueApiDelivery(input, gate, {
    mode: "draft_fallback",
  });
  if (rescued) return rescued;

  const researchHeavyRescue = tryDeliverResearchHeavyMissionRescue(input);
  if (researchHeavyRescue?.ok && researchHeavyRescue?.blogContent?.sections?.length) {
    return {
      ...researchHeavyRescue,
      userMessage: null,
      withheld: false,
      softPass: researchHeavyRescue.softPass ?? false,
    };
  }

  if (isBriclogAlwaysDeliverEnabled() && hasFilledBlogAxes(input)) {
    const enriched = enrichMinimalBlogInput(input);
    let fallback = buildMissionProseFallbackPack(enriched);
    if (!fallback?.sections?.length) {
      ({ pack: fallback } = buildDeliverableBlogFallback({
    ...
    if (fallback?.sections?.length) {
      ...
      if (displayed?.sections?.length) {
        return {
          ...
          softPass: false,
          userMessage: null,
          mode: "always_deliver_fallback",
          meta: attachContentQualityToApiMeta(
            { failReasons: gate?.reasons || [], alwaysDeliver: true },
            displayed
          ),
        };
      }
