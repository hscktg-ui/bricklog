...
export function buildTopicAwareConsumerPads(input = {}, slot = 0, count = 12) {
  ...
  const variants = [
    ...
    `${label}를 비교할 때 ${brand} 기준으로 가격·구성·예약 방법을 함께 적어 두면 선택이 빨라집니다.`,
    ...
    `${regionBit}생활권에서 ${brand} ${kw || label} 방문 시 주차·대기·픽업 옵션을 함께 보면 동선이 편합니다.`,
    `${brand} ${label} — 확인되지 않은 할인·재고는 단정하지 말고 매장·공식 문의로 확인하세요.`,
    ...
    `${regionBit}${brand} ${label} 방문 전 혼잡 시간·좌석·주문 대기를 미리 보면 편합니다.`,
    `${label} 관련 ${brand} 공지·이벤트는 공식 채널 기준으로 다시 확인하는 것이 안전합니다.`,
  ];
