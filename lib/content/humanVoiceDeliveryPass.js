 ...
 * @param {object} [input]
 */
export function applyHumanVoiceDeliveryPass(pack, input = {}) {
  if (!pack?.sections?.length) return pack;
  if (!isBriclogMissionEnforced()) return pack;

  let next = applyNarrativeArcShape(pack, input);
  next = applyHumanColumnPolish(next, input, { lightAnchors: true });
