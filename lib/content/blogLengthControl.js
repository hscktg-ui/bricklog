...
import { buildVisitReviewParagraphs } from "@/lib/content/visitReviewTopicGate";
import {
...
import { buildSplitContinuationHeading } from "@/lib/product/humanColumnProseEngine";
...
const TIER_STRUCTURE_RULES = BRICLOG_BASELINE_V2.tierStructure;

const TIER_MAX_SECTIONS = { short: 14, medium: 18, long: 22 };
...
function resolveSectionHeadings(ctx = {}, input = {}, count = 6) {
  const contract =
    input._writingContract ||
    input.writingContract ||
    resolveWritingContract({ ...input, ...ctx });

  if (isBriclogContext(ctx, input)) {
    const mode = resolveBriclogSectionBlueprint(input, contract);
    if (mode === "philosophy") return BRICLOG_BLUEPRINT.slice(0, count);
    return BRICLOG_PRODUCT_BLUEPRINT.slice(0, count);
  }
  if (contract.type === "visit_review" && contract.visitToneAllowed) {
    return buildTopicArcSectionHeadings(input, count);
  }
  if (
    contract.density === "segmented" &&
    (contract.type === "product_guide" ||
      contract.type === "info_guide" ||
      contract.type === "info_compare")
  ) {
...
function addSectionsFromPlan(pack, minSectionCount, ctx, input) {
  ...
  if (sections.length < minSectionCount) {
    const headings = resolveSectionHeadings(ctx, input, minSectionCount + 2);
    const visitPads = isVisitReviewTopicInput(input)
      ? buildVisitReviewParagraphs(input, minSectionCount + 4)
      : null;
    let cursor = 0;
    while (sections.length < minSectionCount) {
      const heading =
        ...
        headings[headings.length - 1] ||
        `${String(ctx.brandName || input.brandName || "브랜드")} 안내`;
      const body = visitPads
        ? sanitizeForbidden(
            visitPads[cursor % visitPads.length] ||
              visitPads[visitPads.length - 1] ||
              ""
          )
        : sanitizeForbidden(
...
function deepenResearchGroundedPackToMin(pack, minChars, input = {}) {
  const researchLines = buildResearchFactLines(input, 14);
  let next = pack;
  for (let round = 0; round < 8 && countBlogBodyCharsWithSpaces(next) < minChars; round += 1) {
...
export function normalizeBlogLengthAndStructure(pack, ctx = {}, input = {}) {
  ...
  if (isResearchGroundedDeliveryPack(pack, input)) {
    const lengthMin = resolveEffectiveBlogLengthMin(pack, input, tierKey, tier);
    let next = {
      ...pack,
      sections: (pack.sections || []).map((s) => ({
        ...s,
    ...
    next = dedupeSectionsByMeaning(next);
    if (countBlogBodyCharsWithSpaces(next) < lengthMin && hasUsableResearchFacts(input)) {
      next = deepenResearchGroundedPackToMin(next, lengthMin, input);
    }
    next = capSectionsForTier(next, tierKey);
    const after = countBlogBodyCharsWithSpaces(next);
    const inBand = after >= lengthMin && after <= tier.max;
    return {
      pack: {
        ...next,
        _meta: {
          ...
          researchGroundedLengthNormalize: true,
          lengthControl: {
            current: after,
            target: tier.target,
            min: lengthMin,
            tierCatalogMin: tier.min,
            max: tier.max,
            inBand,
          },
      ...
      adjusted: after !== countBlogBodyCharsWithSpaces(pack),
      charCount: after,
      lengthOk: inBand,
    };
  }
  if (isIndustryHumanColumnEditorialPack(pack)) {
    ...
    return {
      ...
      adjusted: false,
      charCount: chars,
      lengthOk: chars >= tier.min * 0.45,
    };
  }
  const sourceSections = (pack.sections || []).filter((s) =>
    String(s.body || "").replace(/\s/g, "").length >= 40
  );
  let next = {
    ...pack,
    sections: (pack.sections || []).map((s) => ({
  ...
  if (!(next.sections || []).length && sourceSections.length) {
    next = {
      ...next,
      sections: sourceSections.map((sec, i) => ({
        ...sec,
...
const GISEUNG_MIN_SECTIONS = BRICLOG_BASELINE_V2.giseungMinSections;

/** 긴 섹션을 문단 단위로 분할 — 섹션 수 회귀(2개 fat section) 복구 */
export function splitPackSectionsForStructure(
  pack,
  minSections = GISEUNG_MIN_SECTIONS,
  input = {}
) {
  if (!pack?.sections?.length) return pack;
  if (pack.sections.length >= minSections) {
    const totalChars = pack.sections.reduce(
      (n, s) => n + String(s.body || "").replace(/\s/g, "").length,
      0
    );
    if (totalChars / pack.sections.length < 180) return pack;
  }
  ...
  while (sections.length < minSections && guard < 12) {
    ...
    sections.splice(
      ...
      {
        heading: buildSplitContinuationHeading(baseHeading, 1, input),
...
export function ensureMinBlogSections(pack, ctx = {}, input = {}, minOverride) {
  ...
  let next = splitPackSectionsForStructure(pack, target, input);
  if ((next.sections || []).length >= target) return next;
  if (isWriterFirstDeliveryEnabled()) return next;
