...
export function buildTopicArcSectionHeadings(input = {}, count = 6) {
  ...
  if (isInformationalTopicInput(input) && !prefersVisitExperienceTone(input)) {
    ...
    return [
      `${shortSubject}, 처음 찾게 된 계기`,
      `${place}에서 직접 본 ${shortSubject}`,
      `${shortSubject} — 비교하면서 본 차이`,
      `목적별로 나눠 보면`,
      `다시 가 본다면`,
      `정리 — ${shortSubject} 선택`,
    ].slice(0, Math.max(1, count));
...
function buildArcAnchor(role, p, input) {
  ...
  if (isInformationalTopicInput(input) && !prefersVisitExperienceTone(input)) {
    const infoAnchors = {
      ...
      seung: `${p.regionBit}${p.brand}에 직접 가서 ${subject}를 눈으로 확인했어요.`,
      jeon: `비교해 보니 ${subject} 고를 때 기준이 조금씩 보였어요.`,
      gyeol: `정리하면 ${subject}는 본인 용도에 맞춰 고르면 될 것 같아요.`,
    };
