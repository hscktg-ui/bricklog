...
const INSTA_STACCATO_CONNECTORS = ["", " ", " — ", " · "];
...
function staccatoConnectorsFor(channel) {
  return channel === "instagram" ? INSTA_STACCATO_CONNECTORS : STACCATO_CONNECTORS;
}

function weaveStaccatoBuffer(parts = [], channel) {
  if (parts.length < 2) return parts.join(". ") + (parts[0]?.endsWith(".") ? "" : ".");
  const connectors = staccatoConnectorsFor(channel);
...
function mergeSectionStaccato(body = "", channel) {
  const paras = splitParagraphs(body);
  if (paras.length < 2) return mergeStaccatoParagraph(body, channel);
  ...
  if (allShort) {
    return weaveStaccatoBuffer(
      paras.map((p) => p.replace(/[.!?…]+$/, "")),
      channel
    );
  }
  ...
  for (const p of paras) {
    ...
    if (isShortPara) {
      ...
      if (buffer.length >= 2) merged.push(weaveStaccatoBuffer(buffer, channel));
      else if (buffer.length === 1) merged.push(`${buffer[0]}.`);
      buffer = [];
      merged.push(mergeStaccatoParagraph(p, channel));
    }
  }
  if (buffer.length >= 2) merged.push(weaveStaccatoBuffer(buffer, channel));
  else if (buffer.length === 1) merged.push(`${buffer[0]}.`);
...
function mergeStaccatoParagraph(text = "", channel) {
...
export function buildVisitReviewThesis(input = {}) {
  ...
  const topicObj =
    p.topicObj ||
    koreanObjectParticle(String(p.topicFacet || p.topic || input.topic || "방문").trim());
  return `${topicObj} 위해 ${p.regionBit}${p.brand} ${flavor.spaceWord || "매장"}에 다녀와 직접 체험하며 느낀 후기`.replace(
    /\s+/g,
    " "
  );
}
...
export function applyUnifiedProseToText(text = "", phraseCounts, opts = {}) {
  ...
  let body = mergeSectionStaccato(String(text || "").trim(), channel);
