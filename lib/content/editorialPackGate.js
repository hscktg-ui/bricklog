...
/** 업종 flavor 기반 내러티브 시드 — 가구(허리·숙면) 템플릿 전 업종 오염 방지 */
function buildIndustryNarrativeSeedSentences(input = {}) {
  const region = String(input.region || "").trim();
  const brand = String(input.brandName || "브랜드").trim();
  ...
  const visitReason = flavor.visitReason || "이용 목적";

  if (key === "furniture" || isFurnitureIndustry(input)) {
    return [
      `${regionBit}${topicObj} 알아보게 된 계기는 보통 허리·숙면 불편, 예산, 체험 가능 여부부터 정리하는 경우가 많습니다.`,
  ...
  if (key === "education") {
    return [
      `${regionBit}${topicObj} 알아보게 된 계기는 보통 대상 학년·특강 기간·반 편성부터 정리하는 경우가 많습니다.`,
      `${brand} 상담실에서 ${topic} 커리큘럼·등록 일정을 먼저 확인한 뒤, 방학 일정과 맞춰 보면 결정이 빨라집니다.`,
      `반별 수업 시간·과목 구성 차이가 달라, 메모해 두고 상담할 때 꺼내면 효율적입니다.`,
    ];
  }
  if (key === "craft") {
    return [
      `${regionBit}${topicObj} 알아보게 된 계기는 보통 소요 시간·난이도·인원 제한부터 정리하는 경우가 많습니다.`,
      `${brand} ${space}에서 ${topic} 준비물·완성품 수령 방법을 확인한 뒤, 예약·취소 규정을 같이 보면 결정이 빨라집니다.`,
      `클래스마다 난이도·작업 과정·소요 시간이 달라, 메모해 두고 상담할 때 꺼내면 효율적입니다.`,
    ];
  }
  if (key === "salon") {
    return [
      `${regionBit}${topicObj} 알아보게 된 계기는 보통 두피·손상·예산, 시술 가능 여부부터 정리하는 경우가 많습니다.`,
      `${brand} ${space}에서 ${topic} 상담·시술 조건을 확인한 뒤, 일정·비용을 같이 보면 결정이 빨라집니다.`,
      `시술마다 톤·케어·소요 시간 차이가 달라, 메모해 두고 상담할 때 꺼내면 효율적입니다.`,
    ];
  }
  return [
    `${regionBit}${topicObj} 알아보게 된 계기는 보통 ${visitReason}, 예산, 예약 조건부터 정리하는 경우가 많습니다.`,
    `${brand} ${space}에서 ${topic} 안내를 확인한 뒤, 일정·비용을 같이 보면 결정이 빨라집니다.`,
    `안내마다 확인 포인트·조건 차이가 달라, 메모해 두고 상담할 때 꺼내면 효율적입니다.`,
  ];
}
...
function buildFlowSections(pack, input) {
  ...
  if (sentences.length < 6) {
    ...
    const seed = opimoExhibit
      ...
      : buildIndustryNarrativeSeedSentences(input);
