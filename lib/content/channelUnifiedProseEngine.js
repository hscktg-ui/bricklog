...
} from "@/lib/content/placeNoticeHumanize";
...
export function buildInstagramCaptionThesis(input = {}) {
  ...
  return `${topic} — ${p.regionBit}${koreanObjectParticle(p.brand)} 피드에서 저장해 두기 좋은 캡션`.replace(/\s+/g, " ");
}
...
export function applyPlaceUnifiedProsePass(pack, input = {}) {
  ...
  return humanizePlaceNoticePack(next, input);
}
...
export function assessChannelUnifiedProse(pack, channel, input = {}) {
  ...
  const hasBridge =
    channel === "place"
      ? /이어서/.test(String(pack.detailBody || "")) || /(?:^|\n)\s*·\s+/m.test(String(pack.detailBody || ""))
      : false;
  const humanTone =
    channel === "instagram"
      ? assessInstaCaptionHumanTone(full)
      : channel === "place"
        ? assessPlaceNoticeHumanTone(full)
        : { ok: true };
  ...
  return {
    ok:
      phraseRepeats <= 1 &&
      (channel === "place"
        ? hasBridge && humanTone.ok && fieldHits >= 1
        : emotionHits >= 1 && fieldHits >= 2 && humanTone.ok),
    phraseRepeats,
    emotionHits,
    fieldHits,
    hasBridge,
