...
export function applyInstagramUnifiedProsePass(pack, input = {}) {
  ...
  let hook = applyUnifiedProseToText(pack.hook || "", phraseCounts);
  ...
  if (hook && body) {
    ...
    lines[0] = bridgeText(lines[0], "이어서 ");
    body = lines.join("\n");
  }
  if (body && ending) {
    ending = bridgeText(ending, "마지막으로 ");
  }
...
export function assessChannelUnifiedProse(pack, channel, input = {}) {
  ...
  return {
    ok: phraseRepeats <= 1 && emotionHits >= 1 && fieldHits >= 2 && hasBridge,
    phraseRepeats,
    emotionHits,
    fieldHits,
    hasBridge,
