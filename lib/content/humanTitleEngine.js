...
const NATURAL_TITLE_MARKERS =
  /에서|라면|있다면|한다면|전에|보다|먼저|찾|기준|포인트|고민|이야기|체험|알아|봐야|정리|읽을|질문|비교|후기|느낀|헷갈|선택|방문|고를|한눈|체크|솔루션|방식|이유|다녀|솔직|FAQ|포인트|맥락|주의|분석|차이|대안|느껴|겪|설명하는|알두|짚|안내|소식|문의|예약|운영|공지|확인/;

const TEMPLATE_TITLE_RES = [
  /바라보는\s*시선/,
  /에\s*담긴\s*이야기$/,
  /체험\s*전\s*알아둘\s*것$/,
  /봐야\s*할\s*기준$/,
...
function topicFallback(input = {}, ctx = {}) {
  return topicWritingFacet({ ...input, ...ctx }) || defaultTopicFacet({ ...input, ...ctx });
}
...
export function titleContext(ctx = {}, input = {}) {
  ...
  const core = topicCore(input, ctx) || topicFallback(input, ctx);
...
/** SEO 템플릿 스팸 — 「바라보는 시선」 등 */
export function titleHasTemplateSpam(title) {
  const t = String(title || "").trim();
  return TEMPLATE_TITLE_RES.some((re) => re.test(t));
}

/** 제목에 주제·브랜드가 기계적으로 두 번 반복 */
export function titleEchoesTopicTwice(title, ctx = {}, input = {}) {
  const t = String(title || "").trim();
  const { brand, topic, topicFull } = titleContext(ctx, input);
  const core = topicFull || topic;
  if (!core || core.length < 8) return false;

  if (titleHasTemplateSpam(t)) return true;

  const coreNorm = core.replace(/\s+/g, "");
  const tNorm = t.replace(/\s+/g, "");
  if (coreNorm.length >= 8) {
    let idx = tNorm.indexOf(coreNorm);
    let hits = 0;
    while (idx !== -1) {
      hits += 1;
      idx = tNorm.indexOf(coreNorm, idx + Math.max(4, coreNorm.length - 2));
    }
    if (hits >= 2) return true;
  }

  if (brand && topic) {
    const pair = `${brand}${topic}`.replace(/\s+/g, "");
    if (pair.length >= 10 && tNorm.includes(pair) && tNorm.indexOf(pair) !== tNorm.lastIndexOf(pair)) {
      return true;
    }
  }
  return false;
...
export function isNaturalDeliveryTitle(title, ctx = {}, input = {}) {
  const t = String(title || "").trim();
  if (!t) return false;
  if (titleHasTemplateSpam(t) || titleEchoesTopicTwice(t, ctx, input)) return false;
  if (isMechanicalListingTitle(t, ctx, input)) return false;
  return titleIncludesAllEntities(t, ctx, input);
}
...
export function isMechanicalListingTitle(title, ctx = {}, input = {}) {
  ...
  if (TEMPLATE_TITLE_RES.some((re) => re.test(t))) return true;
...
export function buildHumanClickTitles(ctx = {}, input = {}, perspective = "brand") {
  ...
  const perspectiveLead =
    perspective === "comparison"
      ...
      : perspective === "brand"
        ? c.region
          ? `${regionBit}${c.brand} ${c.topic}, ${sit0}`
          : `${c.brand} ${c.topic}, ${sit0}`
        : null;
  ...
  const templates = [
    ...
    .filter((s) => !titleHasTemplateSpam(s) && !titleEchoesTopicTwice(s, ctx, input));
...
export function rewriteMechanicalTitle(title, ctx = {}, input = {}, perspective = "brand") {
  const t = String(title || "").trim();
  if (
    isNaturalDeliveryTitle(t, ctx, input)
  ) {
    return t;
  }
  const candidates = buildHumanClickTitles(ctx, input, perspective).filter((c) =>
    isNaturalDeliveryTitle(c, ctx, input)
  );
  return candidates[0] || buildHumanClickTitles(ctx, input, perspective)[0] || t;
}
