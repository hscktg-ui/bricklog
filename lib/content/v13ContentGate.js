...
/** mission topic dominance — 업종·주제 동의어 (염색↔시즌 컬러 등) */
export function expandTopicDominanceKeywords(ctx = {}) {
  const input = ctx.input || ctx;
  const { key } = getIndustryFlavorForInput(input);
  const topicLine = `${input.topic || ""} ${input.mainKeyword || ""} ${ctx.topic || ""}`;
  const syn = [];
  if (key === "salon" || /미용|염색|펌|컬러|두피|네일/.test(topicLine)) {
    syn.push("염색", "펌", "컬러", "시즌", "두피", "톤", "시술");
  }
  if (key === "education" || /학원|특강|내신|방학|수학|영어/.test(topicLine)) {
    syn.push("특강", "방학", "내신", "학년", "등록", "수업", "반 편성");
  }
  if (key === "craft" || /공방|체험|원데이|도자|클래스/.test(topicLine)) {
    syn.push("체험", "클래스", "원데이", "준비물", "난이도", "완성품");
  }
  const facet = topicWritingFacet(input);
  if (facet && facet.length >= 2) syn.push(facet);
  return [...new Set(syn.filter((s) => s && s.length >= 2))];
}

export function inputTopicKeywords(ctx = {}) {
  const anchor = ctx.topicAnchor || ctx.inputGrounding?.topicAnchor;
  if (anchor?.length) {
    return [...new Set([...anchor, ...expandTopicDominanceKeywords(ctx)])];
  }
  ...
  const raw = [
    ...
    .filter(Boolean)
    .join(" ");
  return [...new Set([...tokenizeTopic(raw), ...topicParts, ...expandTopicDominanceKeywords(ctx)])];
}
...
function weaveParagraphDominance(para, kws, brand, region, shortAnchor, facet, regionBit) {
  const topicAnchored =
    ...
    (region.length >= 2 && para.includes(region) && brand.length >= 2 && para.includes(brand));
  if (topicAnchored) return para;

  const legacyAnchored =
    (brand.length >= 2 && para.includes(brand)) ||
    (region.length >= 2 && para.includes(region));
  if (legacyAnchored && !isBriclogMissionEnforced()) return para;
  ...
  if (isBriclogMissionEnforced()) {
    const missingKw = kws.find((k) => k.length >= 2 && !para.includes(k));
    const anchor = facet || shortAnchor || missingKw;
    if (!anchor || para.includes(anchor)) return para;
    if ((para.match(/ — /g) || []).length >= 1) {
      return `${para.replace(/\s+$/, "")} (${anchor} 기준)`.trim();
    }
    if (brand.length >= 2 && para.includes(brand) && missingKw) {
      return `${para.replace(/\s+$/, "")} — ${missingKw} 기준으로 보면 이해가 수월해요.`.trim();
    }
    if (brand.length >= 2 && !para.includes(brand)) {
      return `${brand} — ${para}`.replace(/\s+/g, " ").trim();
    }
...
export function weaveTopicDominanceIntoPack(pack, ctx = {}) {
  ...
  const rawKws = inputTopicKeywords(fullCtx).filter((k) => k.length >= 2);
  const flavorKey = getIndustryFlavorForInput(input).key;
  const kws =
    flavorKey === "cafe" || flavorKey === "restaurant" || flavorKey === "tea_cafe"
      ? rawKws
      : rawKws.filter((k) => !/^메뉴$|^브런치$|^원두$/.test(k));
