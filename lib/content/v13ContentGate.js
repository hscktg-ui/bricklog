...
/** 키워드 미포함 문장에 주제·브랜드 앵커 보강 */
function weaveSentenceDominance(text, kws, brand, region, hooks, startIdx) {
  const sentences = String(text || "")
    .split(/(?<=[.!?。])\s+|\n+/)
    .map((s) => s.trim())
    .filter((s) => s.replace(/\s/g, "").length >= 8);
  if (!sentences.length) return { text: String(text || ""), nextIdx: startIdx };

  let idx = startIdx;
  const patched = sentences.map((sent) => {
    const anchored =
      (brand.length >= 2 && sent.includes(brand)) ||
      (region.length >= 2 && sent.includes(region)) ||
      kws.some((k) => k.length >= 2 && sent.includes(k));
    if (anchored) {
      idx += 1;
      return sent;
    }
    ...
    idx += 1;
    const merged = `${hook} ${sent}`.replace(/\s+/g, " ").trim();
    return merged.length >= 12 ? merged : sent;
  });
  return { text: patched.join(" "), nextIdx: idx };
}
  ...
  for (let round = 0; round < 3; round += 1) {
    ...
    sections = sections.map((sec, secIdx) => {
      ...
      const patched = paras.map((para, pi) => {
        ...
        const hooks = [
          `${regionBit}${brand}에서 ${facet || shortAnchor}를 보면`,
          `솔직히 ${brand} ${shortAnchor} 기준으로`,
          `${brand} ${shortAnchor} —`,
          `${regionBit}방문 후 ${shortAnchor} 쪽에서`,
          `직접 본 ${shortAnchor} 기준으로`,
        ];
        const woven = weaveSentenceDominance(para, kws, brand, region, hooks, paraIdx);
        paraIdx = woven.nextIdx;
        return woven.text.length > 24 ? woven.text : para;
      });
