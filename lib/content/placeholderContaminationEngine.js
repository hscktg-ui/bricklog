...
export const PLACEHOLDER_CONTAMINATION_PATTERNS = [
  ...
  { id: "broken_bomyeon", re: /(?<![가-힣])를\s*보면/g },
  { id: "broken_e_direct", re: /에\s*직접\s*가(?:서|보면)/g },
  /** 템플릿 잔재 — 「이용 관련해서」「관련해서 를」만 (자연 「이와 관련해서」 제외) */
  { id: "empty_related", re: /이용\s*관련해서|관련해서\s*를/g },
  { id: "broken_related_bomyeon", re: /관련해서\s*를\s*보면/ },
  { id: "condition_composition", re: /조건\s*및\s*구성/g },
  { id: "neutral_summary", re: /중립적으로\s*정리/g },
  /** 체크리스트형 비교 클로셰 — 「비교가 수월했어요/습니다」는 허용 */
  { id: "easy_compare", re: /비교가\s*수월해요(?![가-힣])/g },
...
/** 생성 전 필수 축 — null/undefined/빈 문자열 (_verifiedGenerationAxes 폴백) */
export function detectEmptyInputVars(input = {}) {
  const stamped = input._verifiedGenerationAxes;
  const merged = {
    ...input,
    brandName: String(input.brandName || stamped?.brandName || "").trim(),
    region: String(input.region || stamped?.region || "").trim(),
    topic: String(
  ...
  const topic = String(merged.topic || merged.mainKeyword || "").trim();
  if (!topic) {
    empty.push("topic");
  }
