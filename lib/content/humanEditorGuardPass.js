...
export function scoreHumanEditorGuard(fullText = "", input = {}) {
  ...
  if (region && countTokenMentions(text, region) > ANTI_SEO_SPAM_MAX_REGION_REPEAT + 1) {
...
function softenGuardEntities(text, input = {}) {
  ...
  if (region) {
    out = softenTokenRepeats(out, region, ANTI_SEO_SPAM_PRONOUNS.region, ANTI_SEO_SPAM_MAX_REGION_REPEAT);
  }
