...
/** 미션 블로그 — 해요체 유지하며 어미만 살짝 바꿈 */
const MISSION_HAEYO_ROTATIONS = [
  (s) => s.replace(/봤어요([.!?…]?)$/, "확인해 봤어요$1"),
  (s) => s.replace(/갔어요([.!?…]?)$/, "다녀왔어요$1"),
  (s) => s.replace(/했어요([.!?…]?)$/, "해 봤어요$1"),
  (s) => s.replace(/좋았어요([.!?…]?)$/, "괜찮더라구요$1"),
  (s) => s.replace(/됐어요([.!?…]?)$/, "됐더라구요$1"),
  (s) => s.replace(/있어요([.!?…]?)$/, "있더라구요$1"),
  (s) => s.replace(/들었어요([.!?…]?)$/, "들어봤어요$1"),
  (s) => s.replace(/남겼어요([.!?…]?)$/, "적어 뒀어요$1"),
];
...
function polishSentence(sentence, globalState, sectionIdx) {
...
function diversifyEsseoYoInText(text, counter = { n: 0 }) {
  return String(text || "").replace(
    ...
    (m) => {
      ...
      if (isBriclogMissionEnforced()) {
        return MISSION_HAEYO_ROTATIONS[counter.n % MISSION_HAEYO_ROTATIONS.length](m);
      }
      return m.replace(/어요$/, "었다");
    }
  );
}
...
function rewriteSectionBody(body, input, sectionIdx, globalState) {
  ...
  for (const para of paras) {
    ...
    const polished = sentences
      .map((s) => polishSentence(s, globalState, sectionIdx))
