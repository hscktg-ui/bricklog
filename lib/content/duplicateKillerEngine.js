...
/** place·instagram — shortNotice↔detailBody·hook↔body 교차 중복 제거 */
export function stripChannelCrossFieldDuplicates(pack, channel = "place") {
  if (!pack) return pack;
  const overlapCutoff = HUMAN_DUPLICATE_POLICY.similarityPercent / 100;

  if (channel === "place") {
    const short = String(pack.shortNotice || "").trim();
    const detailSentences = splitSentences(pack.detailBody || "");
    if (!short || !detailSentences.length) return pack;
    const shortNorm = normalizeSentence(short);
    const kept = detailSentences.filter((s) => {
      const norm = normalizeSentence(s);
      if (norm === shortNorm) return false;
      if (wordOverlapRatio(short, s) >= overlapCutoff) return false;
      if (shortNorm.length >= 24 && norm.includes(shortNorm.slice(0, Math.min(28, shortNorm.length)))) {
        return false;
      }
      return true;
    });
    if (kept.length === detailSentences.length) return pack;
    return {
      ...pack,
      detailBody: kept.length ? kept.join("\n\n") : pack.detailBody,
    };
  }
  ...
  if (channel === "instagram") {
    ...
    if (!hook || !rawBody) return pack;
    const lines = rawBody.split(/\n+/).map((l) => l.trim()).filter(Boolean);
    if (!lines.length) return pack;
    const hookNorm = normalizeSentence(hook);
    const filtered = lines.filter((line, i) => {
      if (i > 1) return true;
      const norm = normalizeSentence(line);
      if (norm === hookNorm) return false;
      return wordOverlapRatio(hook, line) < overlapCutoff;
    });
    if (filtered.length === lines.length) return pack;
    const nextBody = filtered.join("\n\n");
    return {
      ...pack,
      [bodyKey]: nextBody,
  ...
  if (channel === "place" || channel === "instagram") {
    ...
    for (let round = 0; round < 3; round += 1) {
      const overlapPct = Math.max(
        62,
        (ctx.similarityPercent ?? HUMAN_DUPLICATE_POLICY.similarityPercent) - round * 4
      );
      ...
      next = stripChannelCrossFieldDuplicates(next, channel);
      next = applyRepetitionControl(next, channel);
      dupCheck = detectDuplicateKillerIssues(getChannelFullText(next, channel), {
