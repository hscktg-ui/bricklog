...
export function applyRegionVoiceLockToPack(pack, input = {}) {
  ...
  if ((next.sections || []).length < minKeep) {
    const soft = sections.filter((s) => String(s.body || "").replace(/\s/g, "").length >= 12);
    if (soft.length >= minKeep) next = { ...next, sections: soft };
    else if (sections.length >= minKeep) next = { ...next, sections };
  }
