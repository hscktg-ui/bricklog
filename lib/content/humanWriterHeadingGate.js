...
function isForbiddenHeading(heading, input = {}) {
  ...
  if (
    /찾게\s*되는가|찾게\s*되었는가|왜\s+.+\s*찾게|검색하게\s*되는|왜\s+지금\s+검색/.test(
      h
    )
  ) {
    return true;
...
function rewriteHeading(heading, input) {
  let next;
  if (isFurnitureExhibitionContext(input)) {
    next = rewriteFurnitureExhibitionHeading(heading, input);
  } else if (isSignatureWritingEnforced()) {
    next = rewriteSignatureHeading(heading, input);
  } else {
    next = rewriteHumanWriterHeading(heading, input);
  }
  if (isForbiddenHeading(next, input)) {
    next = rewriteHumanWriterHeading(heading, input);
  }
  if (isForbiddenHeading(next, input)) {
    next = buildNaturalConcernHeading(input);
  }
  return next;
}
