...
export function applyV17PostWritePack(pack, ctx = {}, channel = "blog") {
  ...
  if (
    channel === "blog" &&
    isCoverageSlotDumpPack(seedPack) &&
    !seedPack._meta?.missionProseFallback
  ) {
    seedPack = replaceCoverageDumpWithMissionProse(seedPack, input);
    seedPack = {
      ...seedPack,
      _meta: {
        ...(seedPack._meta || {}),
        missionProseFallback: true,
        coverageDumpReplaced: true,
      },
    };
  }
  if (
    channel === "blog" &&
    shouldUseGpt55LightDelivery(seedPack, input) &&
    !isDeliveryRescuePack(seedPack) &&
    !seedPack._meta?.missionProseFallback
  ) {
    let next = applyDuplicateKiller(seedPack, { ...ctx, input }, channel);
    ...
    return {
      ...
      _meta: {
        ...
        v17Engine: true,
        gpt55V17Light: true,
        gpt55LightDeliveryVersion: "gpt55-light-v1",
      },
    };
  }
  if (channel === "blog" && isDeliveryRescuePack(seedPack)) {
    let next = sanitizeVerbatimTopicInPack(seedPack, input, channel);
  ...
  if (channel === "blog" && seedPack._meta?.missionProseFallback) {
    let next = sanitizeVerbatimTopicInPack(seedPack, input, channel);
    if (isBriclogMissionEnforced() && next?.sections?.length) {
      next = applyNaverBlogChannelGate(next, { ...ctx, input });
      next = dedupeMissionProsePack(next);
  ...
  let next = applyV14PostWritePack(seedPack, { ...ctx, input }, channel);
  ...
  if (channel === "blog" && next?.sections?.length) {
    if (!isDeliveryRescuePack(next)) {
      const beforeSubstantive = next.sections || [];
      next = applyKnowledgeCoverageGate(next, { ...ctx, input }, channel);
      const filtered = (next.sections || []).filter((s) =>
      ...
      next = {
        ...
        sections:
          filtered.length >= 3
            ...
            : beforeSubstantive
                .filter(
                  (s) => String(s.body || "").replace(/\s/g, "").length >= 32
                )
                .slice(0, Math.max(3, beforeSubstantive.length)),
      };
    }
