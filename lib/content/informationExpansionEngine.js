...
export function expandPackByInformation(pack, ctx = {}, input = {}, opts = {}) {
  if (!pack) return pack;
  const depth = Number(opts._expansionDepth ?? 0);
  if (depth >= 3) {
    return {
      ...pack,
      _meta: {
        ...(pack._meta || {}),
        expansionDepthCap: true,
        passOutput: pack._meta?.passOutput ?? false,
      },
    };
  }
  const channel = opts.channel || "blog";
  const minChars = opts.minChars ?? BLOG_MIN_BODY_CHARS;
  const bodyChars =
    channel === "blog" ? countBlogBodyCharsWithSpaces(pack) : 0;
  const batchLocalFinish = Boolean(
    ctx.batchLocalFinish ||
      ctx.batchLocalChannelFinish ||
      input.batchLocalFinish
  );
  const salvageForce =
    (opts.salvageForce === true && channel === "blog" && bodyChars < minChars) ||
    (opts.salvageForce === true &&
      batchLocalFinish &&
  ...
  if (channel === "blog") {
    ...
    next = expandInstagramByInformation(pack, ctx, enrichedInput, Math.min(minChars, 320));
  }
