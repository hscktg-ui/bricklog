...
export function getProductOverviewSnapshot() {
  ...
  return {
    ...
    seo: {
      siteUrl,
      sitemapPaths: ["/", "/help", "/detail", ...guidePaths, "/terms", "/privacy", "/refund"].filter(
        (p, i, arr) => arr.indexOf(p) === i
      ),
      verification: verificationStatus(),
      indexablePages: guidePaths.length + 5,
    ...
    quality: {
      ...
      crossChannelBatch: crossBatch
        ? {
            ...
            pass: crossBatch.pass ?? null,
            fail: crossBatch.fail ?? null,
            at: crossBatch.finishedAt ?? crossBatch.generatedAt ?? crossBatch.startedAt,
            byChannel: crossBatch.byChannel
              ? Object.fromEntries(
                  Object.entries(crossBatch.byChannel).map(([ch, d]) => [
                    ch,
                    {
                      passRate: d.passRate,
                      pass: d.pass,
