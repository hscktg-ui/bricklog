...
} from "@/lib/admin/operatorErrorHints";
import {
  classifyEngineOpsNote,
  hasCriticalNowAction,
} from "@/lib/admin/engineOpsNotePriority";
...
function loadCrossChannelBatch() {
  try {
...
export async function buildOperatorAdvisory(ctx = {}) {
  ...
  const crossBatch = loadCrossChannelBatch();
  ...
  const todayErrors = errors;

  for (const note of engineOps.notes || []) {
    if (!note) continue;
    const tier = classifyEngineOpsNote(note);
    actions.push({
  ...
  if (errorsToday > 0) {
    const topRoute = todayErrors[0]?.route || "unknown";
    const topMessage = todayErrors[0]?.message || "";
    const sameMessageCount = todayErrors.length
      ? todayErrors.filter((e) => e.message === topMessage).length
      : errorsToday;
    const errHint = hintForErrorMessage(topMessage);
    const staleUniform =
      Boolean(topMessage) &&
      isLikelyStaleUniformErrors(todayErrors, topMessage);
    const resolvedStale = staleUniform && errHint?.staleIfUniform;
    const summary =
      errHint?.summary || topMessage.slice(0, 100) || "메시지 없음";
    actions.push({
      id: "errors_today",
      priority: resolvedStale ? "watch" : errorsToday >= 5 ? "now" : "soon",
      critical: !resolvedStale,
      title: resolvedStale
        ? `오늘 오류 ${errorsToday}건 (패치 전 로그 — 신규만 확인)`
        : `오늘 오류 ${errorsToday}건`,
      advice: topRoute !== "unknown"
        ? `최근: ${topRoute} — ${summary}${sameMessageCount > 1 ? ` (동일 ${sameMessageCount}회)` : ""}`
        : `오늘 ${errorsToday}건 — 상세는 오류 로그 탭에서 확인하세요.`,
      action: resolvedStale
        ? errHint?.action ||
          "scripts/resolve-known-error-logs.mjs 로 해결된 로그 정리 가능."
        : errHint?.action ||
          "하단 오류 로그에서 경로·메시지·stack(meta)를 확인하세요.",
    });
  }
  ...
  if (topFails.length > 0) {
    ...
    const reasonKey = top.reason || top.label || "";
    const failHint = hintForFailReason(reasonKey);
    actions.push({
      id: "top_fail_reason",
      priority: "soon",
      title: "생성 실패·품질 이슈",
      advice: `최다 사유: ${reasonKey} (${top.count || "—"}회)${failHint?.summary ? ` — ${failHint.summary}` : ""}`,
      action: failHint?.summary
        ? "npm run test:research-human-pack · test:research-channel-pack · test:human-writing-gate 로 회귀 확인."
  ...
  if (crossBatch?.byChannel?.blog) {
    const blog = crossBatch.byChannel.blog;
    const blogRate = blog.passRate ?? pct(blog.pass, blog.total);
    const blogTarget = 90;
    if (blogRate != null && blogRate < blogTarget) {
      const needed = Math.max(
        0,
        Math.ceil((blogTarget / 100) * (blog.total || 0)) - (blog.pass || 0)
      );
      actions.push({
        id: "cross_channel_blog_gap",
        priority: blogRate < blogTarget - 1 ? "now" : "soon",
        critical: blogRate < blogTarget - 1,
        title: `블로그 배치 ${blogRate}% — 목표 ${blogTarget}%`,
        advice: `${blog.pass}/${blog.total} 통과 · ${needed}건 추가 필요. human_editor·belief 게이트 위주.`,
        action:
          "npm run test:cross-channel-batch · batchBeliefBoost stampBatchBlogFirstDeliveryMeta 확인.",
      });
    }
  }
      ...
      action:
        gap.id === "billing"
          ...
          : gap.id === "multi_channel"
            ? "채널 SLA·test:research-channel-pack. 조사 있으면 place/insta 조사 폴백(c1506d74) 우선 확인."
            : gap.id === "first_use"
              ? "도움말·더 맞추기 안내(58714815) 반영 여부·공개 테스트 게이트 모니터링."
              : null,
    });
  }
  ...
  const staleErrorOnly = actions.some(
    (a) => a.id === "errors_today" && a.priority === "watch"
  );
  if (hasCriticalNowAction(actions)) {
    headline = "지금 손봐야 할 인프라·오류가 있습니다.";
  } else if (staleErrorOnly && errorsToday > 0) {
    headline = `오늘 오류 ${errorsToday}건 — 패치 전 로그 가능, 신규 발생 모니터링.`;
