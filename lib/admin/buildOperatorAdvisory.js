...
export async function buildOperatorAdvisory(ctx = {}) {
  ...
  if (errorsToday > 0) {
    ...
    const sameMessageCount = todayErrors.filter(
      (e) => e.message === topMessage
    ).length;
    const errHint = hintForErrorMessage(topMessage);
    const staleUniform = isLikelyStaleUniformErrors(todayErrors, topMessage);
    const summary =
      errHint?.summary || topMessage.slice(0, 100) || "메시지 없음";
    actions.push({
      id: "errors_today",
      priority:
        staleUniform && errHint?.staleIfUniform
          ? "watch"
          : errorsToday >= 5
            ? "now"
            : "soon",
      title: staleUniform
        ? `오늘 오류 ${errorsToday}건 (동일 메시지 — 패치 전 가능)`
        : `오늘 오류 ${errorsToday}건`,
      advice: `최근: ${topRoute} — ${summary}${sameMessageCount > 1 ? ` (동일 ${sameMessageCount}회)` : ""}`,
      action:
        errHint?.action ||
