...
import { fetchTrendSystemAdminStats } from "@/lib/trends/liveEngine";
...
export async function fetchAdminDashboardExtras({
  ...
  dashboard.trendSystem = await fetchTrendSystemAdminStats(db);
  ...
  if (!dashboard.trendSystem) {
    warnings.push("live trend system tables 없음 — schema-v25-live-hourly-trends.sql 적용 필요");
  }
