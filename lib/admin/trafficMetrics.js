 ...
 */
async function fetchVisitRowsSince(db, sinceIso) {
  ...
  if (error) {
    ...
    if (/utm_|source_channel|column/i.test(error.message)) {
      const fallback = await db
        ...
        .limit(FETCH_LIMIT);
      if (fallback.error) {
        return { rows: [], tableReady: false, truncated: false, error: fallback.error.message };
      }
      return {
        rows: fallback.data || [],
        tableReady: true,
        truncated: (fallback.data || []).length >= FETCH_LIMIT,
        schemaV20: false,
        error: null,
      };
    }
...
export async function fetchAdminTrafficMetrics(db) {
  ...
  const [weekPack, monthPack] = await Promise.all([
    fetchVisitRowsSince(db, since7d),
    fetchVisitRowsSince(db, since30d),
  ]);

  const tableReady = weekPack.tableReady || monthPack.tableReady;
  const schemaV20 = weekPack.schemaV20 !== false && monthPack.schemaV20 !== false;

  return {
    asOf: new Date().toISOString(),
    tableReady,
    schemaV20,
    truncated: weekPack.truncated || monthPack.truncated,
    ...
    error: weekPack.error || monthPack.error,
  };
}
