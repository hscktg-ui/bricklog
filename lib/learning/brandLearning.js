...
export function learnFromEdit(brandId, channel, beforePlain, afterPlain, brandHint = null) {
  if (!brandId || !afterPlain) return null;
  const brand =
    brandHint?.id === brandId
      ? brandHint
...
export function learnEditorAIAction(brandId, actionId, brandHint = null) {
...
export function learnRewritePreference(
  brandId,
  channel,
  feedbackText,
  categories = [],
