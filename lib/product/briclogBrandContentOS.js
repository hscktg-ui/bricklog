...
import { getOperatingPlanPlatformStrategy } from "@/lib/trends/platformTrends2026";
...
export const BRAND_CONTENT_OS_PHASES = [
  { id: "planning", step: 1, weight: 30, label: "콘텐츠 기획", role: "무엇을·왜 쓸지", center: "plan" },
  { id: "research", step: 2, weight: 30, label: "조사", role: "조사 항목·팩트", center: "research" },
  { id: "explain", step: 3, weight: 20, label: "설명", role: "이유·활용·브랜드 연결", center: "write" },
  { id: "writing", step: 4, weight: 10, label: "글쓰기", role: "사람이 읽을 문장", center: "write" },
  { id: "review", step: 5, weight: 10, label: "검수", role: "품질·placeholder 차단", center: "review" },
  { id: "ops", step: 6, weight: 0, label: "운영", role: "채널 리듬·다음 액션", center: "ops" },
];
...
export function buildContentOperatingPlan(input = {}) {
  ...
  const placeTopic = contract.visitToneAllowed
    ? `${brand} ${region} 플레이스·방문 정보`.trim()
    : `${brand} ${topic} — 매장·서비스 안내`.trim();

  const recentTopics = (input.recentTopics || input.topicMemoryRecent || [])
    .map((t) => String(t?.topic || t || "").trim())
    .filter((t) => t.length >= 2);
  const deepenMatch = recentTopics.find(
    (t) => topic && (t.includes(topic.slice(0, 8)) || topic.includes(t.slice(0, 8)))
  );
  const primaryAxis = deepenMatch ? "deepen" : "fresh";
  const primaryReason = deepenMatch
    ? `「${deepenMatch}」 축을 한 층 더 깊게 — 주제 일관성·검색 신뢰`
    : contract.userIntent || intent.userIntent || `${topic} — 독자 검색 의도에 답하기`;
  ...
  const whatToWrite = [
    ...
    },
    {
      ...
      priority: "이번 달",
      axis: "fresh",
      reason: "한 장면으로 분위기·경험 전달",
    },
    {
      id: "support_detail",
      topic: `${String(input.productName || topic || "상품").trim()} — 고르는 기준이 보이는 상세`,
      channel: "detailPage",
      priority: "이번 달",
      axis: "fresh",
      reason: "고르는 기준이 보이는 860px 상세 디자인",
    },
    {
      id: "season_refresh",
      topic: `시즌 새로고침: ${topic}`,
      ...
      priority: "이번 달",
      axis: "refresh",
      reason: "같은 주제를 시즌·시의성으로 업데이트해 신선도 유지",
    },
  ];
  ...
  const whyWrite = [
    {
      topic,
      reason: primaryReason,
      outcome: contract.readerGain || intent.readerOutcome || "방문·구매·문의 전환에 도움이 되는 정보",
      axis: primaryAxis,
    },
    {
      topic: `${brand} 브랜드 신뢰`,
  ...
  return {
    ...
    whyWrite,
    researchMustKnow,
    platformStrategy: getOperatingPlanPlatformStrategy(input),
    operatingHeadline: `${monthLabel()} ${brand} 블로그 운영안 — ${topic}`,
    userValueStatement: BRICLOG_VISION.userValue,
    topicAxis: {
      primary: primaryAxis,
      deepenHint: deepenMatch || null,
      refreshHint: `시즌 새로고침: ${topic}`,
    },
  };
}

const CHANNEL_ACTION_LABELS = {
  blog: "주제만 바꿔 다음 블로그 받기",
  place: "플레이스 탭에서 이어 만들기",
  instagram: "인스타 탭에서 이어 만들기",
  detailPage: "상품 상세에서 860px 디자인 받기",
};
...
/**
 * 블로그 발행 후 — 남은 채널·다음 주제 (기획 30% · 결과 화면)
 * @param {ReturnType<typeof buildContentOperatingPlan>} plan
 * @param {{ hasPlace?: boolean, hasInsta?: boolean, blogTopic?: string }} opts
 */
export function buildPostPublishOperatingSteps(plan = {}, opts = {}) {
  const { hasPlace = false, hasInsta = false, hasDetail = false, blogTopic = "" } = opts;
  if (!plan?.whatToWrite?.length) return [];
  ...
  const done = {
    ...
    instagram: hasInsta,
    insta: hasInsta,
    detailPage: hasDetail,
  };

  for (const item of plan.whatToWrite) {
    const channel = item.channel === "insta" ? "instagram" : item.channel;
    if (channel === "blog") {
      if (blogTopic && item.topic === blogTopic) {
        steps.push({
          ...item,
          channel,
          channelLabel: CHANNEL_DISPLAY.blog,
          actionLabel: CHANNEL_ACTION_LABELS.blog,
          status: "next",
        });
      }
      continue;
    ...
    steps.push({
      ...item,
      channel,
      channelLabel: CHANNEL_DISPLAY[channel] || channel,
      actionLabel: CHANNEL_ACTION_LABELS[channel] || "이어 만들기",
      status: "pending",
    });
  }
...
export function getPublicBrandContentOSVision() {
  return {
    ...
    pioneerEngine: "korean-human-pioneer-v2",
    pioneerKo: "대한민국 브랜드 칼럼 — 조사·설명·관찰·화자 register",
  };
}
