...
export function deliverBlogDespiteGate(input, pack, gate = {}, partial = {}) {
  ...
  const completeDraft = readiness.completeDraft;
  const previewMessage =
    partial.previewMessage ||
    (completeDraft
      ? null
      : isSoftInformationGateFailure(gate)
        ? SOFT_PREVIEW_HINT
        : "아래 편집본을 확인한 뒤 필요하면 「다시 받기」로 더 다듬을 수 있어요.");
  ...
  return {
    ...
    blogContent: {
      ...
      _meta: {
        ...(pack._meta || {}),
        deliveryPreview: !completeDraft,
        deliveryPreviewMessage: completeDraft ? undefined : previewMessage,
        softPass: !completeDraft,
        passOutput: false,
        postVerifySoft: !completeDraft,
        completeDraft,
        displayReady: completeDraft,
    ...
    withheld: false,
    softPass: !completeDraft,
    userMessage: completeDraft ? null : null,
    mode: partial.mode || "output_verify_preview",
