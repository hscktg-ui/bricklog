...
import { assessPlaceStructureSignals } from "@/lib/quality/structureScoreKpi";
...
function scorePlaceShape(pack = {}) {
  ...
  const hasExperience = sentenceExperienceOpinionAxes(detail).length >= 1;
  let score = 40;
...
export function assessChannelVisitNorthStar(pack, channel = "place", input = {}) {
  ...
  if (channel === "place") {
    const structure = assessPlaceStructureSignals(full, input);
    if (!structure.experienceSentence) {
      issues.push({ type: "experience_observation_missing", structure });
    }
  }
...
function isWaterLeisureContext(text = "", industry = "") {
  const blob = `${text} ${industry}`;
  return /수영|물놀이|풀|워터|아쿠아/.test(blob);
}

function isVenueOpenTopic(text = "") {
  return /오픈|신규|리뉴얼|개장/.test(text);
}
...
export function buildNorthStarPlacePack(input = {}, factLines = []) {
  ...
  const context = `${topic} ${facts.join(" ")} ${input.industry || ""}`;
  const waterOpen = isWaterLeisureContext(context) && isVenueOpenTopic(context);
  const genericOpen = !isWaterLeisureContext(context) && isVenueOpenTopic(context);
  ...
  const shortNotice = (
    waterOpen
      ...
      : genericOpen
        ? `${brand} ${facility} 새 소식을 안내드립니다.`
        : facts[0] || `${brand} ${topic} 소식을 안내드립니다.`
  ).slice(0, 120);

  const paragraphs = [];
  if (waterOpen && !shortNotice.includes("가족")) {
    paragraphs.push(
      "가족 나들이, 물놀이, 휴식을 한 번에 즐길 수 있도록 준비했으며 기존 야외시설과 함께 이용 가능합니다."
    );
  }
  if (!waterOpen && !genericOpen) {
  ...
  } else if (facts.some((f) => /식사|카페|식음|커피|휴식/.test(f)) || waterOpen) {
    paragraphs.push(
      `${brand}는 ${facility.includes("수영") ? "수영장" : facility}뿐 아니라 식사, 카페, 휴식공간까지 함께 운영되는 복합 문화공간입니다.`
    );
  }
  if (!paragraphs.some((p) => p.includes(brand)) && !waterOpen && !genericOpen) {
    paragraphs.push(
