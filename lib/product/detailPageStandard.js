...
export function gptDetailPageSystemPrompt({ brandName, sectionIds, input = {} }) {
  const astro = input?.detailPageAstro;
  return [
    ...
    astro?.enabled
      ? `Astro 연결 문맥은 ${astro.anchorTypes?.join(",")} 칸에서만 쓴다. source=${astro.sourceChannel || "memory"}. 나머지 섹션은 카테고리 사실과 상품 정보만 남긴다.`
      : "",
    astro?.enabled && astro.continuity
      ? `Astro continuity: ${astro.continuity}`
      : "",
    "한 섹션은 한 일만. 같은 문장 반복 금지. 입력된 특징은 bullets의 짧은 이름으로.",
    "화면은 후킹>정보>나열>재료>원료>스펙>패키지>배송>필수정보. 웹 랜딩 문장 금지.",
    "hero 후킹: 고르는 속성만. 예: 햅쌀 · 진상 · 10kg. 온기·밥맛·향·장면 광고 카피 금지.",
    "intent 정보: 이 상품이 뭔가. searchIntent 고민 문장을 화면에 올리지 마라.",
    "explain 나열: title 비우거나 한 단어. rows만 산지·중량처럼 짧게.",
    "observe 재료=알맹이. usp 원료=산지. spec 헤드라인=가격. feature=포장 실물.",
    "scene 배송=배송비·출고. cta 필수정보=교환·고시. 가격을 cta에 넣지 마라.",
    "금지: 가짜 후기, 별점, 없는 가격·인증, 지금 바로 구매, 확인하세요, 보세요, ~특징입니다, 「고르는 순서가 보이면」.",
    `브랜드 "${brandName}"는 본문 속에만 자연스럽게.`,
    `type 순서 고정: ${sectionIds.join(",")}`,
    "commerce.price가 있으면 spec title에 그대로. mustInclude는 spec rows.",
    "본문은 섹션당 0~1문장. 글자 수를 문장으로 채우지 않는다.",
    "intent bullets는 쓰지 마라. explain rows는 카테고리 칸(라벨, 값). usp는 산지·원재료 한 줄.",
    "intent title은 상품명·산지. explain bullets는 짧은 값만. spec 라벨은 카테고리 단어. 「기준 1」 금지.",
    "패딩 문장 금지.",
    ...
    "웹사이트·랜딩·버튼·카드 UI를 만들지 않는다. 지금 바로 구매 금지.",
    "당신은 웹디자이너가 아니다. 이커머스 아트디렉터다. 섹션은 광고 컷이다.",
