/**
 * BRICLOG 상세페이지 기준 SSOT
 * 경쟁사(전환 템플릿·가짜 후기·강한 CTA)와 분리.
 * 해신 DNA: 광고처럼 안 보임 · 사실만 · 관찰·의견 · 약한 안내.
 */
...
export const DETAIL_PAGE_STANDARD_VERSION = "briclog-pdp-v1";
...
export function gptDetailPageSystemPrompt({ brandName, sectionIds }) {
  return [
    "BRICLOG 상세페이지. GPT는 섹션 JSON만. 이미지는 그리지 않음.",
    formatDetailPageDesignBrief(),
    "기준: 사실만 / 고르는 사람이 막힌 점부터 / 정보 다음 이유 / 관찰·의견 / 브랜드 은근 / CTA는 안내.",
    "금지: 가짜 후기, 별점, 없는 가격·인증, 지금 바로 구매, 광고 관용구, ~특징입니다.",
    `브랜드 "${brandName}"는 본문 속에만 자연스럽게.`,
    `type 순서 고정: ${sectionIds.join(",")}`,
    "highlights는 제목·본문에 그대로 살린다. mustInclude는 사실로만 녹인다. 사진 캡션은 그리지 않고 문장에만 맞춘다.",
    "길이: 각 body는 2~4문장. hero 70자+, intent·explain·observe 110자 이상(공백 제외).",
    "usp bullets는 완전한 문장 4개. spec만 rows.",
    "표준 구성은 공백 제외 1,000자 이상. 담백하되 한 줄로 끝내지 말 것.",
