...
export function assessBlogApiDeliveryWithhold(result = {}, input = {}) {
  ...
  if (isBriclogResetQualityEnforced()) {
    const unedited = shouldWithholdUneditedPublish(pack, input, result);
    if (unedited.withhold) {
      return {
        withhold: true,
        reasons: unedited.reasons || ["unedited_publish_not_a"],
...
export function alignBlogApiDeliveryResponse(result = {}, input = {}) {
  ...
  return {
    ...
    userMessage:
      ...
      result.userMessage ||
      RESET_QUALITY_WITHHOLD_MESSAGE,
    blogContent: result.blogContent?.sections?.length
      ? {
          sections: [],
          title: "",
          representativeTitle: "",
