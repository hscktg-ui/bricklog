...
import {
  headingHasChecklistForbiddenSnippet,
  sanitizeChecklistForbiddenHeading,
  sanitizeChecklistForbiddenHeadingsOnPack,
...
function rewriteHeadingToNatural(heading, input, index = 0) {
  ...
  if (headingHasChecklistForbiddenSnippet(h)) {
    return sanitizeChecklistForbiddenHeading(h, input, index);
  }
...
export function buildSplitContinuationHeading(baseHeading, splitIndex, input = {}) {
  const raw = stripCatalogContaminationHeading(String(baseHeading || "").trim());
  const base = raw.replace(/(?:\s*—\s*이어서)+$/g, "").trim();
  if (/보러\s*갔을|확인\s*포인트|선택지로/.test(base) || !base) {
    return rewriteHeadingToNatural("", input, splitIndex + 1);
  }
  if (splitIndex === 0) return base;
  if (/—\s*이어서/.test(raw)) return base;
  return `${base} — 이어서`;
}
