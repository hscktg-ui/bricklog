...
import { buildBrandContentOsCenterSnapshot } from "@/lib/product/brandContentOsCenters";

export const DELIVERY_VALUE_EXPOSURE_VERSION = "delivery-value-v2";
...
export function buildDeliveryValueExposure(input = {}, dossier = null, pack = null) {
  ...
  const brandFact = pack?.sections?.length
    ? assessBrandFactPresence(pack, input)
    : { hits: [], ok: false };
  const full = pack?.sections?.length ? getBlogFullText(pack) : "";
  const experienceAxes = full ? sentenceExperienceOpinionAxes(full) : [];
  const osCenters = buildBrandContentOsCenterSnapshot(input, {
    plan,
    pack,
    meta: pack?._meta || input._meta,
  ...
  return {
    ...
    operatingItems: (plan.whatToWrite || []).slice(0, 4).map((item) => ({
    ...
    checks,
    trustSignals: {
      brandHits: brandFact.hits?.length ?? 0,
      researchLineCount: researchLines.length,
      experienceAxes: experienceAxes.slice(0, 3),
      structureScore: pack?.sections?.length
        ? assessStructureScore(pack, input).score
        : null,
    },
    dossierWritable: d.writable !== false,
    researchMustKnow: (plan.researchMustKnow || []).slice(0, 3),
    centers: osCenters.centers,
    timeSavings: osCenters.timeSavings,
    actions: osCenters.actions,
    dna: osCenters.dna,
    week1: osCenters.week1,
  };
}
