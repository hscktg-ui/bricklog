...
export function textContainsUnverifiedSearchLeak(text, input = {}) {
  const t = String(text || "");
  if (!t) return false;
  if (isPollutedResearchFactText(t, input)) return true;
  if (!isExhibitionLeakContext(input)) return false;
...
function sentenceHasSearchSnippetChunk(sentence, input = {}) {
  ...
  for (const r of results) {
    const title = String(r.title || "").trim();
    if (title.length >= SEARCH_SNIPPET_MIN_LEN && s.includes(title)) {
      if (title.replace(/\s/g, "").length / Math.max(sLen, 1) > 0.55) return true;
    }
    const snippet = String(r.snippet || r.description || "").trim();
    if (snippet.length >= 24) {
      const chunk = snippet.replace(/\s+/g, " ").trim().slice(0, 48);
      if (chunk.length >= 20 && s.includes(chunk)) {
        if (chunk.replace(/\s/g, "").length / Math.max(sLen, 1) > 0.55) return true;
      }
    }
  }
  return false;
