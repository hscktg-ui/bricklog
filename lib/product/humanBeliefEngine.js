 ...
 * 규칙 통과 ≠ 사람 글. 광고 smell vs 화자 register 일치.
 */
...
export const HUMAN_BELIEF_VERSION = "v21";
/** 출고·재생성 기준 — 「사람이 썼네」 미달 시 보정·재시도 */
export const HUMAN_BELIEF_MIN_SCORE = 85;

export const HUMAN_BELIEF_SUCCESS =
  '"AI가 잘 썼네"(X) → "선택한 화자가 처음부터 끝까지 썼네"(O)';
...
/** 현장·경험 smell — field_review 화자 전용 */
...
export function buildHumanBeliefPromptBlock(input = {}) {
  ...
  const voiceBlock = isFieldReviewSpeaker(input)
    ? buildExperienceVoicePromptBlock()
    : "";
  const speaker = resolveSpeakerDisplayLabel(input);
  return [
    `【HUMAN BELIEF · ${HUMAN_BELIEF_VERSION}】
${HUMAN_BELIEF_SUCCESS}
화자: ${speaker} (${profile.id}) — 처음부터 끝까지 동일 register.
독자가 "광고 같다"면 실패. 선택 화자가 자연스럽게 썼으면 성공.
...
export function scoreHumanBelief(fullText, ctx = {}, pack = null) {
  ...
  const evalInput = ctx.input || ctx;
  const profile = resolvePersonaEngineProfile(evalInput);
  const fieldReview = profile.archetype === "field_review";
  ...
  const conversational = fieldReview ? scoreExperienceVoice(text) : { ok: true, hits: 0 };
  let score = 72;
  ...
  if (fieldReview) {
    ...
    const voiceHits = countPatternHits(text, profile.requiredVoice || []);
    const forbiddenHits = countPatternHits(text, profile.forbiddenVoice || []);
    score += Math.min(18, voiceHits * 6);
    score -= Math.min(24, forbiddenHits * 8);
    if (voiceHits < 1) {
      issues.push("persona_voice_weak");
      score -= 14;
    }
    if (forbiddenHits > 0) {
      issues.push("persona_voice_forbidden");
      score -= 10;
    }
    if (pack) {
      const align = scorePersonaEngineAlignment(pack, evalInput, profile);
      if (!align.ok) {
        issues.push("persona_misaligned");
        score -= Math.min(20, (align.issues?.length || 1) * 6);
      } else {
        score += 6;
      }
    }
  ...
  if (
    /확인된\s*범위|자주\s*문의(?:되)?는\s*조건|볼\s*때\s*짚을\s*점|시즌\s*꽃재|조금씩\s*달라집니다/.test(
      text
    )
  ) {
    issues.push("brochure_voice");
  }
  ...
  const ok =
    ...
    !issues.includes("persona_voice_forbidden");
  ...
  return {
    ...
    fieldHits,
    issues,
    speakerArchetype: profile.archetype,
