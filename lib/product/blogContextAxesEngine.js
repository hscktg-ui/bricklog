...
function scoreResearchFactAnchoring(fullText = "", facts = []) {
...
function scoreResearchDepthAxis(pack, input = {}, gate = {}, v2Axis = {}) {
  ...
  const anchoring = scoreResearchFactAnchoring(full, facts);
  const meta = pack._meta || {};
...
function scoreSpeakerAlignmentAxis(pack, input = {}) {
  ...
  const score = clampScore(Math.round(persona.score * 0.55 + surface.score * 0.45));
  return {
    score,
    persona,
    surface,
...
export function buildBlogContextAxes(pack = {}, input = {}, gate = {}) {
  ...
  const brandScore = clampScore(
    v3?.brand ??
      v2Axis.scores?.brand ??
      (relevance != null ? 48 + relevance * 38 : 52)
  );
