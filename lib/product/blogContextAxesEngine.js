...
function scoreResearchFactAnchoring(fullText = "", facts = [], input = {}) {
  if (isBrandSpecificTopicInput(input)) {
...
function scoreResearchDepthAxis(pack, input = {}, gate = {}, v2Axis = {}) {
  ...
  if (isBrandSpecificTopicInput(input)) {
    const proper = scoreBrandProperNounAnchoring(full, input);
    score =
...
function scoreTopicAlignmentAxis(pack, input = {}, v2Axis = {}, v3 = null) {
  ...
  const properNouns = brandSpecific
    ? scoreBrandProperNounAnchoring(full, input)
    : null;
...
function scoreSpeakerAlignmentAxis(pack, input = {}) {
  ...
  if (isBrandSpecificTopicInput(input)) {
    const proper = scoreBrandProperNounAnchoring(getBlogFullText(pack), input);
...
export function buildBlogContextAxes(pack = {}, input = {}, gate = {}) {
  ...
  const brandScore = clampScore(
    v3?.brand ??
      v2Axis.scores?.brand ??
      (isBrandSpecificTopicInput(input)
        ? scoreBrandProperNounAnchoring(getBlogFullText(pack), input).score
        : relevance != null
          ? 48 + relevance * 38
          : 52)
  );
