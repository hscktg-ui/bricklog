...
function finishColumnistPack(pack, input = {}) {
  ...
  if (benchmark.hardFails?.includes("engine_spam")) return null;
...
- FORBIDDEN (instant fail): 「비교 기준」「대표 서비스」「방문·상담」「덜 헷갈릴까요」「목적별로 나눠」「매장·상담에서 확인」「왜 지금 는지」「현장 쇼룸 근처」「이 지역 브랜드」「근처 쇼룸」「공식 안내 기준」「로컬 매장 운영」「기준이 조금씩 보였」
...
export async function generateColumnistSovereignPack(input = {}, opts = {}) {
  ...
  try {
    ...
    if (!tierAccepts(parsed, input)) {
      ...
      raw = await callOpenAIChat(retry, { temperature: 0.52, maxTokens: 6800 });
