...
import { deepenLlmPackWithResearch } from "@/lib/golden/llmDeliveryPolish";
...
import { hasEngineSpamInPack, stripEngineSpamFromPack } from "@/lib/product/columnistEngineSpam";
import {
...
import {
  ...
  resolveColumnistRetryMaxTokens,
...
function stampColumnistFinishMeta(next, benchmark, input = {}, extra = {}) {
  return {
    ...next,
    _meta: {
      ...(next._meta || {}),
      ...
      visitReviewBenchmarkOk: benchmark.publishOk === true,
      llmGenerated: true,
...
export function tryFinishColumnistPack(pack, input = {}) {
  ...
  if (hasEngineSpamInPack(next)) {
    if (isColumnistFinishBenchRelaxed(input)) {
  ...
  if (!benchmark.publishOk) {
    if (
      ...
      return {
        ok: true,
        relaxed: true,
        pack: stampColumnistFinishMeta(next, benchmark, input, {
...
export async function generateColumnistSovereignPack(input = {}, opts = {}) {
  ...
  const maxTokens = resolveColumnistCompletionTokens(input, { fast });
  ...
  try {
    ...
    while (llmRounds < maxLlmRounds) {
      ...
      raw = await callOpenAIChat(attemptMessages, {
        ...
        maxTokens: llmRounds > 1 && fast ? resolveColumnistRetryMaxTokens(maxTokens, llmRounds) : maxTokens,
      });
    ...
    if (!columnistTierAccepts(parsed, input, { fast })) {
      ...
      if (bench.publishOk && !bench.hardFails?.length && parsed?.sections?.length) {
        const tier = resolveBlogLengthTier(input.blogLengthTier || DEFAULT_BLOG_LENGTH_TIER);
        let expanded = expandLlmPackPreservingBody(parsed, input, tier);
        if (!columnistTierAccepts(expanded, input, { fast })) {
          expanded = deepenLlmPackWithResearch(expanded || parsed, input);
        }
        if (expanded && columnistTierAccepts(expanded, input, { fast })) {
          parsed = expanded;
        }
      }
    }
    ...
    if (!finished && !fast) {
      ...
      raw = await callOpenAIChat(benchRetry, {
        temperature: 0.48,
        maxTokens: getChannelTokenBudget("blog", "columnistBenchRetry"),
      });
      parsed = parseLlmBlogResponse(raw, ctx);
      if (columnistTierAccepts(parsed, input, { fast })) {
        parsed = normalizeLlmVoiceForDelivery(parsed, input);
    ...
    if (!finished && fast && isCustomerTwoMinuteSlaMode()) {
      const finishRetry = buildColumnistSovereignMessages(input, ctx, opts);
      finishRetry[0].content +=
        "\n\n【2분 SLA 품질 재시도·1회】엔진 스팸·지역붙임·브로슈어 금지. 현장 포인트 2개 이상.";
      raw = await callOpenAIChat(finishRetry, {
        temperature: 0.5,
        maxTokens: resolveColumnistCompletionTokens(input, { fast: true }),
      });
      llmRounds += 1;
      parsed = parseLlmBlogResponse(raw, ctx);
      if (columnistTierAccepts(parsed, input, { fast })) {
...
export async function applyColumnistSovereignApiDelivery(result = {}, input = {}, opts = {}) {
  ...
  if (next.blogContent?.sections?.length) {
    next = {
      ...next,
      blogContent: applyKoreanOrthographyToBlogPack(
        scrubPlaceholderFromPack(next.blogContent),
        columnistInput
      ),
    };
  }
