...
export function pickIndustryBody(input = {}) {
  ...
  if (key === "education" && /학원|수업|회화|입시|과외|클래스|특강|내신|수학/.test(topicBlob)) {
    const mathFocus = /수학|내신|특강|중등|고등|방학/.test(topicBlob);
    return {
      opener: mathFocus
        ? "방학 특강을 알아볼 때는 브랜드보다 대상 학년·반 편성·수업 시간을 먼저 보게 됩니다. 처음 상담하는 분들은 현재 성적·목표·가능한 요일을 정리해 두면 추천이 빨라집니다."
        : "성인·학생 수업을 고를 때는 커리큘럼보다 시간대·소규모 여부·체험 수업을 먼저 보게 됩니다. 처음 상담하는 분들은 목표·현재 실력·가능한 요일을 정리해 두면 추천이 빨라집니다.",
      items: mathFocus
        ? `${bp}는 레벨 테스트 후 반 편성·특강 기간·수업 횟수를 안내하는 경우가 많습니다. 방학 특강은 조기 마감이 잦아, 일정이 확정되면 먼저 확인하는 편이 좋습니다.`
        : `${bp}는 레벨 테스트 후 반 편성·수업 횟수를 안내하는 경우가 많습니다. 직장인 반은 저녁 시간대 수요가 높아, 대기가 생길 수 있습니다.`,
      purpose: mathFocus
        ? "내신·수능·방학 특강 등 목표에 따라 수업 강도와 기간이 달라집니다. 체험 수업으로 분위기를 먼저 보는 편이 후회가 적습니다."
        : "회화·시험·취업 등 목표에 따라 수업 강도와 기간이 달라집니다. 체험 수업으로 분위기를 먼저 보는 편이 후회가 적습니다.",
...
export function buildEditorialTitle(input = {}, p = {}) {
...
function weaveResearchIntoParagraphs(paragraphs, input) {
  ...
  const out = paragraphs.map((p) => String(p || "").trim()).filter(Boolean);
  if (!out.length) return lines.slice(0, 3);
  for (let i = 0; i < lines.length; i += 1) {
    const idx = Math.min(out.length - 1, 1 + (i % Math.max(1, out.length - 1)));
    const line = lines[i];
    if (!line || out[idx]?.includes(line.slice(0, 12))) continue;
    out[idx] = `${out[idx]}\n\n${line}`.trim();
  }
  return out;
}
...
export function pickIndustryExtendedParagraphs(input = {}) {
  const brand = String(input.brandName || "").trim();
  const region = String(input.region || "").trim();
  const key = resolveBriclogIndustryKey(input);
  const topicBlob = [
  ...
  const byKey = {
    ...
    education: [
      /수학|내신|특강|방학/.test(topicBlob)
        ? "목표 학년·현재 성적·특강 기간을 상담 때 알려 주시면 반 편성 추천이 분명해집니다."
