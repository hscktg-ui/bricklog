...
function pickIndustryBody(input = {}) {
  ...
  const bucket = INDUSTRY_BODIES[key] || INDUSTRY_BODIES.default;
  return bucket[season] || bucket.default || INDUSTRY_BODIES.default.default;
}
...
function buildLocalContextLine(input = {}, p = {}) {
  ...
  const isFlower = resolveBriclogIndustryKey(input) === "flower";
  const season = detectSeason(input);
  const seasonTail =
    isFlower && season === "summer"
      ? "최근 여름 시즌에 어울리는 꽃들을 중심으로 꽃다발을 준비하고 있습니다."
      : `${seasonLabel(season)}에 맞는 안내를 준비하고 있습니다.`;
  return `${region}에 위치한 ${brand}에서도 ${seasonTail}`;
}
...
export function shouldUseEditorialQualityPath(input = {}) {
  ...
  if (
    key === "furniture" &&
    (hasUsableResearchFacts(input) || /전시|루체|매트리스|침대|쇼룸|가구/.test(topicBlob))
  ) {
    return false;
  }

  const seasonalInfo = /추천|시즌|여름|겨울|봄|가을|종류|고르|선택|꽃|메뉴|브런치/.test(topicBlob);

  if (isInformationalTopicInput(input)) {
    if (["flower", "cafe", "salon"].includes(key)) return true;
    return seasonalInfo;
  }

  const speaker = String(input.v4Speaker || "").trim();
  if (["brand_intro", "expert_info", "magazine", "local_blogger"].includes(speaker)) {
    if (key === "flower") return true;
    return seasonalInfo;
  }
  return false;
}
