...
const STORY_TARGETS_BY_INDUSTRY = {
  furniture: [
    {
      id: "dining_chair",
      match: /스트레스리스|stressless|다이닝체어|dining\s*chair|mint\s*lb|리클라이너\s*체어/i,
      label: "다이닝체어·리클라이너",
      emotionHook: "식사·작업을 한자리에서 오래 하다 보면 의자부터 불편해진다",
      visualKeywords: ["좌판 깊이", "등받이 각도", "팔걸이", "가죽 마감", "리클라인"],
      hashtagHints: [],
      problemOpenings: [
        "식사·작업을 한자리에서 오래 하다 보면 의자부터 불편해지는 경우가 많아요.",
        "다이닝체어는 사진만으로는 앉은 높이·등받이 지지가 감이 안 온다.",
      ],
      writerGuide:
        "침대·매트리스·전시 오피모 문구 금지. 앉은 체험·좌판·등받이·리클라인 장면만.",
      sceneLines: ({ p, brand, product }) => [
        `${p.regionBit}${brand} 쇼룸에서 ${product}에 앉아 등받이 기울기와 좌판 깊이를 비교해 봤어요.`,
        `팔걸이 높이와 좌판 쿠션 밀도를 바꿔 보니 거실·다이닝 동선과 맞는 조합이 달랐어요.`,
      ],
    },
    {
      id: "newlywed",
...
const VISIT_STORY_TARGET_IDS = new Set(["local_visit", "gift_moment", "brunch_mood", "pet_visit"]);
...
export function resolveStoryTarget(input = {}) {
  ...
  for (const target of pool) {
    ...
    if (!contract.visitToneAllowed && VISIT_STORY_TARGET_IDS.has(target.id)) continue;
    if (target.id === "exhibition" && isFurnitureChairProductTopic(input)) continue;
    if (target.match.test(blob)) {
      return { target, industryKey: key };
    }
  }

  if (!contract.visitToneAllowed) {
    const guide = pool.find((t) => /guide|info|seasonal|product/.test(t.id));
    if (guide) return { target: guide, industryKey: key };
  }

  const fallback = pool.find((t) => t.id === "local_visit");
  if (fallback && contract.visitToneAllowed) return { target: fallback, industryKey: key };
 ...
 */
export function buildStoryTargetSceneLines(input = {}, count = 3, channel = "blog") {
  const resolved = resolveStoryTarget(input);
  if (!resolved) return [];
  const ctx = storySceneContext(input);
  const { p, brand, region } = ctx;
  ...
  if (channel === "place") {
    ...
    if (offer) lines.push(`${brand} ${offer} 안내드립니다.`);
    if (period) lines.push(`기간: ${period}`);
    if (topic) lines.push(`${brand} ${topic} 관련 매장 소식입니다.`);
    lines.push(`${region}${brand ? ` ${brand}` : ""} — 예약·문의는 플레이스·전화로 확인해 주세요.`);
  ...
  if (channel === "instagram") {
    const { target } = resolved;
    const lines = [];
    if (target.emotionHook) lines.push(String(target.emotionHook).slice(0, 72));
    for (const kw of (target.visualKeywords || []).slice(0, 2)) {
      lines.push(`${kw} — ${brand || region}`.trim());
    }
    if (!lines.length) {
      lines.push(`${region}${brand ? ` ${brand}` : ""} · ${ctx.facet}`);
    }
  ...
  const contract = resolveWritingContract(input);
  const visitLineRe =
    /직접\s*(?:가|다녀|봤|확인)|들어서|진열대|비교해\s*봤|상담\s*(?:초반|에서)/;
  const filtered = contract.visitToneAllowed
    ? lines
    : lines.filter((line) => !visitLineRe.test(line));
  const seen = new Set();
  const out = [];
...
const CHANNEL_STORY_RULES = {
  place: [
    "플레이스: 업체(사장) 1인칭 공지 — shortNotice=핵심 안내 1문장, detailBody=운영·혜택·기간·예약.",
    "마켓 공지판 톤(저희·매장·안내드립니다). 고객 후기·방문기·「다녀왔/솔직 후기」 금지.",
...
export function buildStoryTargetChannelBrief(input = {}, channel = "place") {
  ...
  const { target } = resolved;
  const ctx = storySceneContext(input);
  const scenes = buildStoryTargetSceneLines(input, 2, channel);
  const rules = CHANNEL_STORY_RULES[channel] || [];
  const base = [
    `【STORY TARGET · ${STORY_TARGET_ENGINE_VERSION}】`,
    `타깃: 「${target.label}」`,
    channel === "place"
      ? `공지 각도: 운영·혜택·입고·이벤트·예약 — 업체(저희/매장) 시점.`
      : `감정·훅: ${target.emotionHook}`,
    channel === "instagram"
      ? `비주얼 키워드: ${target.visualKeywords.join(" · ")}`
      : "",
    scenes.length ? `톤 참고(복사 금지):\n- ${scenes.join("\n- ")}` : "",
    `${ctx.region}${ctx.brand ? ` ${ctx.brand}` : ""} · ${ctx.facet}`,
  ]
    .filter(Boolean)
    .join("\n");
