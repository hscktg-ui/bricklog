...
export function assertCompleteBlogPackForDelivery(pack, input = {}) {
  ...
  const effectiveMin = resolveEffectiveBlogLengthMin(pack, input, input.blogLengthTier, tier);
  ...
  const editorDelivered =
    isCustomerDeliveredEditorPack(pack) || isContentEvaluationEditorPass(pack);
  ...
  let lengthMin = effectiveMin;
  if (researchGrounded) {
    lengthMin = effectiveMin;
  } else if (isMissionCatalog || editorDelivered) {
    lengthMin = editorMin;
  } else if (isEditorialColumn) {
    lengthMin = editorialMin;
  }
  if (chars < lengthMin) reasons.push("length_tier_under");
  ...
  const editorExplainSkip =
    (isMissionCatalog && isMissionCatalogEvalPass(pack)) ||
    (editorDelivered && isContentEvaluationEditorPass(pack));
  if (outline.isOutline && !editorExplainSkip) {
    reasons.push("outline_only_output");
  }

  const explain = assessContentExplainabilityForPublish(input);
  if (!editorExplainSkip) {
    if (!explain.ok) {
  ...
  const thinMin =
    isMissionCatalog || editorDelivered
      ? Math.min(400, editorMin * 0.85)
