...
export function assertCompleteBlogPackForDelivery(pack, input = {}) {
  ...
  if (chars < tier.min) reasons.push("length_tier_under");
  ...
  if (outline.isOutline) reasons.push("outline_only_output");
  ...
  if (full.replace(/\s/g, "").length < Math.min(400, tier.min * 0.45)) {
