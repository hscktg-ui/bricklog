...
export function buildIndustryNarrativeParagraphs(p, input = {}) {
  ...
  const topicLead =
    topicLine && topicLine.length >= 4
      ? `${topicLine}을(를) 알아보는 분들`
      : lead;

  if (brandEditor) {
    blocks.push(
      `${season} ${facet} 관련 문의가 늘어나는 시기입니다. ${topicLine ? `${topicLine}과(와) 관련해 ` : ""}${topicLead}에게 저희 ${brand ? `${brand} ` : ""}이야기를 정리해 봤습니다.`,
      bodyTpl.opener,
    ...
    if (key === "salon" && topicLine) {
      blocks.push(
        `${topicLine}을(를) 고려하시면 ${items[0] || "컷"}·${items[1] || "염색"}·${items[2] || "펌"} 중에서 우선순위를 정해 상담하시면 좋습니다. ${regionBit}${brand}에서는 ${topicLine} 기준으로 스타일·관리 주기를 함께 안내합니다.`
      );
    }
    if (brand) {
      blocks.push(
    ...
    blocks.push(
      ...
      `${topicLine ? `${topicLine}과(와) 관련해 ` : ""}${regionBit}${brand}에 들어서니 ${items[0] || facet}부터 눈에 들어왔습니다. ${season} 분위기와 잘 어울리는 구성이었습니다.`,
