...
export function finishLocalBlogPackForBatch(pack, input = {}, options = {}) {
  ...
  let result = finishOneSeed(pack, input, ctx, batchMin);
  ...
  if (!batchBlogPassProxy(scored, batchMin) && scored.belief < BATCH_BELIEF_FLOOR) {
    ...
    const altInput = {
      ...input,
      v4Speaker: visitVoice ? "brand_intro" : "expert_info",
      contentPersona: visitVoice ? "brand_story" : "info_intro",
    ...
    if (altPack?.sections?.length) {
      const altCtx = { input: altInput, ...altInput };
