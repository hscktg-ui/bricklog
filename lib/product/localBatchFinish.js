...
import { stampBatchBlogAsChannelSource } from "@/lib/product/deriveChannelFromVerifiedBlog";
...
/** 고객 rescue — batch finish + 채널 파생 소스 스탬프 (tier 5600 패딩 금지) */
export function finishCustomerRescueBlogPack(pack, input = {}) {
  if (!pack?.sections?.length) return pack;
  const batchInput = { ...input, batchLocalFinish: true };
  let result = finishLocalBlogPackForBatch(pack, batchInput, {
    customerDelivery: true,
  });
  return stampBatchBlogAsChannelSource(result, batchInput);
}
