...
export const BATCH_BELIEF_FLOOR = BRICLOG_BASELINE_V2.humanBeliefMinScore - 15;
...
function repairBatchInformationYield(pack, input, ctx, batchMin) {
  ...
  if (scored.info.score < BATCH_INFO_FLOOR && scored.belief >= BATCH_BELIEF_FLOOR - 5) {
    ...
    if (
      ...
      rescueScore.belief >= BATCH_BELIEF_FLOOR - 5
    ) {
      next = rescued;
    }
  }
...
/** raw belief 70 미만 — batch belief boost → narrative fallback */
