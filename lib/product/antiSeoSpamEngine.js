...
/** 지역명 — 칼럼 톤 상한 (SEO 1~2회면 충분) */
export const ANTI_SEO_SPAM_MAX_REGION_REPEAT = 2;
...
export function softenTokenRepeats(
  ...
  const brand = String(opts.brandName || opts.brand || "").trim();
  const region = String(opts.region || "").trim();
  let n = 0;
  const re = buildTokenRepeatRegex(t);
  return String(text || "").replace(re, (match, ...args) => {
    ...
    if (
      ...
      brand.toLowerCase().startsWith(region.toLowerCase())
    ) {
      const rest = brand.slice(region.length);
      const after = String(text).slice(offset + match.length);
      if (rest && after.toLowerCase().startsWith(rest.toLowerCase())) {
        return match;
      }
    }
    n += 1;
...
export function scoreAntiSeoSpam(fullText, input = {}) {
  ...
  const overused = tokens
    .map(({ key, value, label }) => {
      ...
      const maxRepeat =
        key === "region" ? ANTI_SEO_SPAM_MAX_REGION_REPEAT : ANTI_SEO_SPAM_MAX_TOKEN_REPEAT;
      return count > maxRepeat ? { key, label, token: value, count, maxRepeat } : null;
    })
    .filter(Boolean);
