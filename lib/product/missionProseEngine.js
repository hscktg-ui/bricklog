...
export function buildMissionConclusionLine(p, input = {}, displayTopic = "", pack = null) {
  ...
  if (useInformationalConclusion) {
    ...
    const topic = displayTopic || topicRaw(input) || p.topicFacet || "이용";
    const byKey = {
      flower: `${p.regionBit}${p.brand} ${topic} — 종류·포장·픽업 시간은 매장 안내를 기준으로 확인하시면 됩니다.`,
      education: `${p.regionBit}${p.brand} ${topic} — 대상 학년·특강 일정·등록 안내는 상담 기준으로 확인하시면 됩니다.`,
      craft: `${p.regionBit}${p.brand} ${topic} — 소요 시간·난이도·예약 방법은 매장 안내를 기준으로 확인하시면 됩니다.`,
      pension: `${p.regionBit}${p.brand} ${topic} — 객실·할인·체크인 안내는 예약 문의로 확인하시면 됩니다.`,
      restaurant: `${p.regionBit}${p.brand} ${topic} — 메뉴·예약·방문 시간은 매장 안내를 기준으로 확인하시면 됩니다.`,
      construction: `${p.regionBit}${p.brand} ${topic} — 공사 범위·견적·일정은 상담 안내로 확인하시면 됩니다.`,
      salon: `${p.regionBit}${p.brand} ${topic} — 상담·시술·관리 방법은 매장 안내를 기준으로 확인하시면 됩니다.`,
      default: `${p.regionBit}${p.brand} ${topic} — ${flavor.visitReason} 관련 조건은 매장 안내를 기준으로 확인하시면 됩니다.`,
    };
