...
export function filterMissionExperienceParagraphs(paras = [], input = {}) {
  ...
  return paras.filter((p) => {
    ...
    // 정보형·브랜드 스토리 계약에서는 경험 마커 미달로 패드를 버리지 않음
    if (
      shouldUseVisitToneInMissionPads(input) &&
      isExperienceOpinionDefect(t, input)
    ) {
      return false;
    }
...
function buildInformationalMissionCatalog(p, input = {}, researchLines = []) {
  ...
  const byKey = {
    ...
    salon: [
      ...
      `원하는 색감 사진을 보여 드리니 손상·유지 기간을 먼저 상의해 주셨어요.`,
      `시술 후 거울로 톤 변화를 직접 확인하며 집에서 지킬 관리 포인트만 적어 두었어요.`,
      `대기 공간과 시술실 분위기가 달라서 편한 쪽 자리를 먼저 확인했어요.`,
    ],
    cafe: [
      ...
      `좌석·콘센트·조명 안내를 구역별로 비교해 두면 카공·브런치 선택이 수월합니다.`,
      `혼잡한 시간대를 피하니 메뉴와 자리 분위기를 천천히 볼 수 있었어요.`,
      `브런치 구성·가격·제공 시간은 요일마다 달라질 수 있어 메뉴판을 먼저 확인했어요.`,
      `${p.brand}에서 시즌 음료와 기본 메뉴를 나란히 보니 선택 기준이 정리됐어요.`,
    ],
    pet_cafe: [
...
export function buildMissionExperienceCatalog(p, input = {}, researchLines = []) {
  ...
  return filterMissionExperienceParagraphs(
    [
      buildHumanStoryProblemOpening(input),
      `${p.regionBit}그래서 ${p.brand} ${facet} 보러 직접 다녀왔어요.`,
      `${p.regionBit}${p.brand}에 들어가 ${koreanObjectParticle(facet)} 직접 봤어요.`,
...
function buildIndustryFieldPads(key, flavor, p, input = {}) {
  ...
  const universal = shouldUseVisitToneInMissionPads(input)
    ? [
        `${p.regionBit}${p.brand} ${flavor.spaceWord}에 들어가 ${koreanObjectParticle(subject)} 하나씩 비교해 봤어요.`,
  ...
  const byKey = {
    salon: [
      ...
      `염색 전 두피 상태를 보며 ${koreanObjectParticle(m2)} 먼저 짚어 주셔서 순서가 명확해졌어요.`,
      `원하는 색감과 ${koreanObjectParticle(m1)} 맞추려면 몇 가지 옵션을 비교해야 한다고 들었어요.`,
...
function buildServiceDeepenPads(p, flavor, input, seq) {
  ...
  const universal = [
    ...
    `${facet} 비교 시 ${m0}와 ${koreanObjectParticle(m1)} 나눠 보면 우선순위가 분명해집니다.`,
  ];
  const byKey = {
    cafe: [
