...
function buildConsumerTopicLengthPads(p, input = {}, count = 20) {
  const brand = String(input.brandName || p.brand || "브랜드").trim();
  ...
  const variants = [
    ...
    `${facet}를 비교할 때는 가격·조건·이용 절차·구성을 함께 정리해 두면 상담이 빨라집니다.`,
    `${region ? `${region} ` : ""}이용 전 영업 시간·주차·대기·예약 가능 여부를 확인하면 동선이 편합니다.`,
    `행사·혜택이 있다면 기간·대상·적용 조건을 안내 기준으로 메모해 두었어요.`,
    ...
    `${region ? `${region} 생활권에서 ` : ""}${brand} 이용 시 필요한 준비·주의 사항을 미리 보면 좋습니다.`,
    `${facet} 관련 FAQ는 상담 전에 궁금한 항목을 목록으로 정리해 가면 효율적입니다.`,
    `${brand}${region ? ` ${region}` : ""} — ${facet} 안내는 공식·매장 채널 기준으로 최종 확인하세요.`,
    `시즌·한정 구성은 재고·요일에 따라 달라질 수 있어 방문·문의 전에 확인하는 편이 좋습니다.`,
    `메뉴·옵션 비교 시 알레르기·원재료 표기를 함께 보면 선택이 수월합니다.`,
  ];
  const out = [];
  for (let i = 0; i < count; i += 1) {
    out.push(missionProseClean(variants[i % variants.length], input));
  }
  return out;
}
...
function buildServiceDeepenPads(p, flavor, input, seq) {
  ...
  const universal = [
    `${facet}를 고를 때 ${m0}·${m1}·이용 목적을 함께 보면 비교가 수월합니다.`,
...
function buildDynamicDeepenPad(p, flavor, input, seq) {
  ...
  if (isInformationalTopicInput(input)) {
    ...
    const infoPads = [
      `${subject}를 고를 때 ${m0}·${m1}·선물 목적을 함께 보면 비교가 수월합니다.`,
