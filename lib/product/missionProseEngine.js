...
export const MISSION_PROSE_ENGINE_VERSION = "v1.2";
...
export function buildMissionExperienceCatalog(p, input = {}, researchLines = []) {
  ...
  return filterMissionExperienceParagraphs(
    [
      ...
      `직원분이 ${moodA}·${moodB} 쪽을 먼저 짚어 주셔서 비교하기 수월했어요.`,
...
function buildIndustryFieldPads(key, flavor, p, input = {}) {
  ...
  const universal = [
    ...
    `처음엔 ${subject} 고를 때 ${flavor.visitReason} 기준만 보다가, 매장에서 보니 감이 왔어요.`,
    `두 번째로 비교해 보니 ${m0}·${m1} 차이가 눈에 들어왔어요.`,
  ...
  const byKey = {
    salon: [
      ...
      `${p.regionBit}에서 ${subject} 후보를 좁히려면 ${m0}·${m1}을 직접 보는 게 도움이 됐어요.`,
    ...
    default: [
      `${subject}를 볼 때 ${m0}·${m1}을 현장에서 직접 비교했어요.`,
...
function buildDynamicDeepenPad(p, flavor, input, seq) {
  ...
  const templates = [
    `${p.regionBit}${p.brand}에서 ${subject} 볼 때 ${m0}·${m1}을 ${seq + 1}번째로 다시 확인했어요.`,
    ...
    `${p.brand} ${flavor.spaceWord} — ${m0}·${m2Placeholder(flavor, seq)} 쪽을 현장에서 직접 봤어요.`,
  ];
...
function collectUsedParagraphKeys(pack) {
  ...
  for (const sec of pack.sections || []) {
    for (const para of String(sec.body || "").split(/\n\n+/)) {
      const t = para.trim();
      if (t.replace(/\s/g, "").length >= 12) used.add(paragraphKey(t));
    }
  }
  const conc = String(pack.conclusion || "").trim();
  if (conc.replace(/\s/g, "").length >= 12) used.add(paragraphKey(conc));
  return used;
}
...
export function deepenMissionProsePack(pack, minChars, input = {}, options = {}) {
  const { polishAfter = false, countChars = countBlogBodyCharsWithSpaces } = options;
  ...
  while (countChars(next) < minChars && guard < maxGuard) {
    ...
    if (!para) {
      for (let attempt = 0; attempt < 8; attempt += 1) {
        ...
        if (!usedKeys.has(dKey) && !isMissionChecklistPad(dynamic)) {
          para = dynamic;
          usedKeys.add(dKey);
          break;
        }
