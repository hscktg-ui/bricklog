...
function buildIndustryFieldPads(key, flavor, p, input = {}) {
  ...
  const universal = prefersVisitExperienceTone(input)
    ? [
...
function buildResearchFactOnlyLengthPadPool(p, input = {}, researchLines = []) {
  ...
  for (let slot = 0; slot < facts.length; slot += 1) {
    push(humanizeResearchFact(facts[slot], p, input, slot + 4, profile));
  }
...
export function deepenDensityFirstPack(pack, minChars, input = {}, options = {}) {
  ...
  while (countChars(next) < minChars && guard < maxGuard) {
    ...
    if (!para) break;
    if (PADDING_PATTERN_RES.some((re) => re.test(para)) || isMissionBrochurePad(para)) {
      guard += 1;
      continue;
    }
...
export function buildMissionConclusionLine(p, input = {}, displayTopic = "", pack = null) {
  ...
  if (isInformationalTopicInput(input)) {
    ...
    const topic = displayTopic || topicRaw(input) || p.topicFacet || "이용";
    const byKey = {
      flower: `${p.regionBit}${p.brand} ${topic} — 종류·포장·픽업 시간을 함께 보면 선택이 수월합니다. 궁금한 점은 매장 문의로 확인하시면 됩니다.`,
      education: `${p.regionBit}${p.brand} ${topic} — 대상 학년·특강 일정·등록 안내를 함께 보면 비교가 수월합니다. 궁금한 점은 상담으로 확인하시면 됩니다.`,
      craft: `${p.regionBit}${p.brand} ${topic} — 소요 시간·난이도·예약 방법을 함께 보면 선택이 수월합니다. 궁금한 점은 매장 문의로 확인하시면 됩니다.`,
      pension: `${p.regionBit}${p.brand} ${topic} — 객실·할인·체크인 안내를 함께 보면 예약이 수월합니다. 궁금한 점은 예약 문의로 확인하시면 됩니다.`,
      restaurant: `${p.regionBit}${p.brand} ${topic} — 메뉴·예약·방문 시간을 함께 보면 선택이 수월합니다. 궁금한 점은 매장 문의로 확인하시면 됩니다.`,
      construction: `${p.regionBit}${p.brand} ${topic} — 공사 범위·견적·일정을 함께 보면 상담이 수월합니다. 궁금한 점은 매장 문의로 확인하시면 됩니다.`,
      salon: `${p.regionBit}${p.brand} ${topic} — 상담·시술·관리 방법을 함께 보면 선택이 수월합니다. 궁금한 점은 매장 문의로 확인하시면 됩니다.`,
      default: `${p.regionBit}${p.brand} ${topic} — ${flavor.visitReason}에 맞는 ${flavor.productWord} 안내를 함께 보면 비교가 수월합니다. 궁금한 점은 매장 문의로 확인하시면 됩니다.`,
    };
