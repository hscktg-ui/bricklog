 ...
 */
function informationalSubjectLabel(subject = "") {
  const s = String(subject || "").trim();
  if (/라인업|전시|신제품|오픈|이벤트|프로모션/.test(s)) return "전시·모델";
  if (s.length > 12) return "이용";
  return s;
}

function buildInformationalFieldPads(key, flavor, p, input = {}) {
  const subject = informationalSubjectLabel(
    topicRaw(input) || p.topicRaw || p.topicFacet || "이용"
  );
  const m0 = flavor.moodWords?.[0] || "분위기";
  const m1 = flavor.moodWords?.[1] || "응대";
  const universal = [
    `${subject}를 고를 때 ${m0}·보관·목적을 함께 보면 기준이 분명해집니다.`,
    `${p.brand}에서 자주 문의되는 조건을 확인된 범위에서 정리했습니다.`,
  ];
  const byKey = {
    flower: [
