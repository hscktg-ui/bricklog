...
import { getIndustryFlavorForInput, isExhibitionTopic, isFurnitureIndustry } from "@/lib/product/industryContextEngine";
import {
...
export function isMissionChecklistPad(text = "") {
  ...
  if (!t) return false;
  return CHECKLIST_TEMPLATE_RES.some((re) => re.test(t));
}
...
function buildInformationalMissionCatalog(p, input = {}, researchLines = []) {
  ...
  const universal = [
    ...
    `${facet}를 고를 때 계절·목적별로 달라지는 기준을 먼저 정리해 두면 선택이 수월합니다.`,
    `색감·보관·유통 기간은 갈수록 우선순위가 바뀌므로, 이번 글에서는 확인 가능한 범위만 담았습니다.`,
    `${p.brand} 기준으로 ${facet} 관련 조건·구성을 중립적으로 정리했습니다.`,
    ...researchLines,
  ];

  const byKey = {
    flower: [
      `여름철에는 시원한 톤·굵은 꽃잎 종류가 선물·집들이에 자주 쓰입니다.`,
      `생일·축하·사과처럼 목적마다 추천 꽃 종류와 포장 스타일이 달라집니다.`,
      `직사광선·에어컨 바람에 따라 보관 일수가 달라지므로 배송·픽업 일정을 함께 고려합니다.`,
      `리본·카드 문구는 ${moodPairObject(moodA, moodB)}에 따라 인상이 크게 달라집니다.`,
      `시즌에 따라 재고·색감이 달라질 수 있어, 주문 전 가능 품목을 확인하는 것이 좋습니다.`,
    ],
    cafe: [
      `시즌 메뉴와 기본 메뉴 ${moodA}를 나눠 보면 선택 기준이 분명해집니다.`,
      `테이크아웃·매장 이용 ${moodB}는 메뉴 옆 안내에서 먼저 확인하는 편이 좋습니다.`,
    ],
    default: [
...
function buildInformationalFieldPads(key, flavor, p, input = {}) {
  ...
  const universal = [
    `${subject}를 고를 때 ${m0}·보관·목적을 함께 보면 기준이 분명해집니다.`,
    `${p.brand}에서 자주 문의되는 ${subject} 조건을 확인된 범위에서 정리했습니다.`,
  ];
  const byKey = {
    flower: [
      `여름철 ${subject}는 수분 보충·직사광선 회피가 관리의 첫 단계입니다.`,
      `선물·집들이·축하 등 목적별로 꽃 종류와 포장 톤이 달라집니다.`,
      `시즌 꽃재에 따라 색감·가격대가 달라질 수 있습니다.`,
      `배송·당일 픽업 가능 여부는 날짜와 재고에 따라 달라집니다.`,
      `${m0}와 ${m1} 조합은 메시지 카드 문구 길이에 따라 달라 보일 수 있습니다.`,
    ],
    default: [
...
function buildIndustryFieldPads(key, flavor, p, input = {}) {
  ...
  const byKey = {
    ...
    cafe: [
      ...
      `테이크아웃·매장 이용 ${m2}를 메뉴 옆 안내에서 확인했어요.`,
      `시즌 메뉴와 기본 메뉴 ${m0}를 나눠서 봤어요.`,
      `주문 후 나오는 시간·${m1} 안내를 직원분에게 들었어요.`,
      `좌석·콘센트·조명 ${m0}를 구역별로 비교해 봤어요.`,
    ],
    pet: [
...
export function buildMissionDeepenPadPool(p, input = {}) {
  ...
  const experience = buildMissionExperienceCatalog(p, input, []);
  const seen = new Set();
  const pool = [];
...
export function deepenMissionProsePack(pack, minChars, input = {}, options = {}) {
  ...
  while (countChars(next) < minChars && guard < maxGuard) {
    ...
    if (!para) {
      const moodA = flavor.moodWords?.[guard % 3] || "분위기";
      const moodB = flavor.moodWords?.[(guard + 1) % 3] || "응대";
      const topoff = missionProseClean(
        `${p.regionBit}${p.brand} 당일 상담 메모 ${guard + 1} — ${moodPairObject(moodA, moodB)} 포인트를 따로 적어 두었어요.`,
        input
      );
      const topoffKey = paragraphKey(topoff);
      if (topoff && topoffKey && !usedKeys.has(topoffKey)) {
        para = topoff;
        usedKeys.add(topoffKey);
      }
    }
    if (!para) break;
...
export function buildMissionConclusionLine(p, input = {}, displayTopic = "") {
  const { flavor } = getIndustryFlavorForInput(input);
  const topic = displayTopic || input.topic || p.topicFacet || "이용";
...
export function polishMissionProsePack(pack, input = {}) {
  ...
  sections = sections.filter((s) => String(s.body || "").replace(/\s/g, "").length >= 32);
