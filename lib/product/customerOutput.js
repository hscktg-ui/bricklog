...
/** 생성 실패·재시도 안내 — CTA 문구의 「조사」와 진행 중 조사를 구분 */
const BLOG_GENERATION_FAILURE_HINT_RE =
  /준비되지 않|올리지 못|다시 눌러|한도|로그인|인증|연결을 확인|끊겼|실패했|입력해 주세요|구체적으로/;

export function isBlogGenerationFailureHint(hint = "") {
  const h = String(hint || "").trim();
  return Boolean(h) && BLOG_GENERATION_FAILURE_HINT_RE.test(h);
}

/** 실패·보류·입력 안내 패널 톤 — soft여도 실패 문구는 녹색 금지 */
export function resolveBlogGenHintStatusClass(hint = "", soft = false) {
  if (isBlogGenerationFailureHint(hint)) return "warn";
  if (/아직 올리지 않|편집본을 한번 더|품질 기준/.test(String(hint || ""))) {
    return "neutral";
  }
  if (soft) return "ok";
  return "warn";
 ...
 */
export function resolveBlogGenHintFooter(hint = "", viewport = {}) {
  if (!hint || /로그인|인증/.test(hint)) return null;
  const { isMobile = false, isTablet = false } = viewport;
  if (isBlogGenerationFailureHint(hint)) {
    if (isMobile) return "아래 「조사 후 글 받기」를 다시 눌러 주세요.";
    if (isTablet) return "위 입력란의 「조사 후 글 받기」를 다시 눌러 주세요.";
    return "왼쪽 「조사 후 글 받기」를 다시 눌러 주세요.";
  }
  if (isMobile || isTablet) {
    return "입력 탭에서 「조사 후 글 받기」로 이어갈 수 있어요.";
  }
  return "왼쪽 폼에서 「조사 후 글 받기」로 이어갈 수 있어요.";
}

/** 빈 패널 힌트 제목 — withhold vs 로딩 vs 실패 구분 */
export function resolveBlogHintPanelTitle(hint = "", soft = false) {
  const h = String(hint || "").trim();
  if (isBlogGenerationFailureHint(h)) {
    if (/로그인|인증/.test(h)) return "로그인·인증을 확인해 주세요";
    if (/한도/.test(h)) return "이번 달 한도 안내";
    if (/연결|끊겼/.test(h)) return "조금 더 시간이 필요해요";
  ...
  if (soft) {
    if (/알아보는\s*중|브랜드·주제 조사|조사\s*중|보강\s*중/.test(h)) {
      return "브랜드·주제 조사 중";
    }
