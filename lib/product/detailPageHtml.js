/**
 * 골라보다 상세 HTML — 860px, 섹션별 레이아웃 (카드·표·인용)
    ...
    const cells = pair
      .map((b, j) => {
        ...
        return `<td${colspan} style="width:${pair.length === 1 ? "100%" : "50%"};vertical-align:top;padding:22px 20px;background:${T.wash};border:1px solid ${T.rule};">
          <p style="margin:0 0 10px;font-size:12px;letter-spacing:0.14em;font-weight:650;color:${accent};">${n}</p>
          <p style="margin:0;font-size:${T.body}px;line-height:1.7;color:${T.ink};word-break:keep-all;letter-spacing:-0.01em;">${esc(b)}</p>
        </td>`;
      })
      .join("");
...
function specSheet(rows, accent) {
  ...
  const body = list
    ...
    .join("");
  return `<table data-layout="spec-sheet" style="width:100%;border-collapse:collapse;font-size:${T.spec}px;line-height:1.6;margin-top:8px;border:1px solid ${T.rule};">${body}</table>`;
}
...
function featureList(bullets, accent) {
  ...
  return `<table data-layout="feature-list" style="width:100%;border-collapse:separate;border-spacing:0 10px;margin-top:8px;">${items
    .map((b, i) => {
      ...
      return `<tr><td style="width:48px;vertical-align:top;padding:4px 12px 4px 0;font-size:13px;letter-spacing:0.12em;font-weight:650;color:${accent};">${n}</td><td style="vertical-align:top;padding:2px 0;font-size:${T.body}px;line-height:1.75;color:${T.ink};word-break:keep-all;letter-spacing:-0.01em;">${esc(b)}</td></tr>`;
    })
    .join("")}</table>`;
}
...
function renderSection(section, photoSrc, accent, productName, caption) {
  ...
  const photoH = type === "scene" ? 520 : type === "feature" ? 480 : 440;
