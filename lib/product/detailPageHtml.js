...
function renderSection(section, photoSrc, accent, productName, caption) {
  ...
  if (type === "explain") {
    ...
    return wrapSection({
      type,
      layout: compare ? "reason-band" : "copy-block",
      bg: T.heroBg,
      extra: `;color:${T.heroFg}`,
      inner: `${kickerHtml(section.kicker, "rgba(247,244,239,0.62)")}${h2Html(section.title, T.heroFg)}${compare || leadHtml(section.body, "rgba(247,244,239,0.88)")}`,
    });
  }
...
export function wrapDetailPageImageStackHtml(images = [], pack = {}, mallId = "smartstore") {
  ...
  const imgs = (Array.isArray(images) ? images : [])
    ...
    .filter(Boolean)
    .join("");
  const body = `<article id="gollaboda-detail-page" data-deliverable="image-stack" data-engine="${esc(pack._meta?.engine || "")}" data-mall-ready="smartstore,coupang" style="width:${DETAIL_PAGE_WIDTH}px;margin:0 auto;background:#fff;">${imgs}</article>`;
