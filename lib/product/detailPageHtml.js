...
function imgFrame(src, alt, height, slot, caption, capColor = T.muted) {
  ...
  const cap = caption
    ...
    : "";
  if (!src) {
    return `<div data-photo-slot="${esc(slot || "")}" data-photo-empty="1" style="width:100%;height:${h}px;background:${T.rule};"></div>${cap}`;
  }
...
export function wrapSmartstoreHtml(bodyHtml, pack = {}) {
  ...
  const links = pairing.hrefs
    ...
    .join("");
  return `<!DOCTYPE html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>골라보다</title>${links}</head><body style="margin:0;background:${T.wash};font-family:${T.familyBody};">${bodyHtml}</body></html>`;
}
