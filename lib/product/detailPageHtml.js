  ...
  if (type === "cta") {
    ...
    return `<section id="detail-buy" data-section="cta" data-layout="cta-bar"${compositionAttr("cta")} style="padding:0;background:${T.paper};border-top:1px solid ${T.rule};">
      <div style="${padBox("cta")}">
        ${kickerHtml(section.kicker, T.muted)}${priceHtml(price || pack.price, T.ink)}${rest && !restIsEcho ? leadHtml(rest, T.muted) : ""}${specSheet(section.rows, accent)}${buyButton(section.ctaLabel || "구매하기")}
      </div>
    </section>`;
  }
