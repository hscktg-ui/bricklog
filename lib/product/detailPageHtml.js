...
function applyType(pack) {
  ...
  const palette = pack?._meta?.plan?.palette || pack?._meta?.style?.palette;
...
function imgFrame(src, alt, height, slot, caption, capColor = T.muted, extra = {}) {
  ...
  return `<div ${attrs} data-photo-crop="${esc(slot || "")}" style="width:100%;overflow:hidden;background:${T.wash};height:${h}px;padding:${inset};box-sizing:border-box;">
    <img src="${esc(src)}" alt="${esc(alt)}"${lazy} style="display:block;width:100%;height:100%;object-fit:cover;object-position:${esc(pos)};transform:scale(${zoom});transform-origin:${esc(pos)};" />
  </div>`;
}
...
function closeLine(label = "구매") {
  const line = String(label || "구매").replace(/하기$/, "");
  return `<p data-cta="close" style="margin:28px 0 0;font-size:15px;line-height:1.45;letter-spacing:0.04em;color:${T.muted};font-family:${T.familyBody};">${esc(line)}</p>`;
}
...
function compositionAttr(sectionOrType) {
  const type = typeof sectionOrType === "object" ? sectionOrType.type : sectionOrType;
  const id =
    (typeof sectionOrType === "object" &&
      (sectionOrType.composition || sectionOrType.art?.composition)) ||
    DETAIL_PAGE_COMPOSITIONS[type];
...
function wrapSection({ type, layout, bg, extra = "", photo, photoSlot, inner, padKind = "std", section }) {
  ...
  const ratio = Number(art.image_ratio || (photo ? 0.72 : 0.2));
  const purpose = section?.purpose || art.purpose || type;
  return `<section data-section="${esc(type)}" data-layout="${esc(layout)}" data-purpose="${esc(purpose)}" data-image-ratio="${esc(String(ratio))}" data-ui-elements="0"${compositionAttr(section || type)}${emptySlot} style="padding:0;background:${bg};">
    ${photo || ""}
...
function renderSection(section, photoSrc, accent, productName, pack = {}) {
  ...
  const photoH = section.art?.height
    ? Math.round(Number(section.art.height) * Number(section.art.image_ratio || 0.72))
    : dir.height || (type === "scene" ? 520 : type === "feature" ? 480 : 440);
  ...
  if (type === "hero") {
    ...
    const proof = section.proof
      ...
      : "";
    return `<section data-section="hero" data-layout="hero-banner" data-hero="plate" data-ranking-slot="hero-banner" data-purpose="${esc(section.purpose || "attention")}" data-image-ratio="${esc(String(section.art?.image_ratio || 0.82))}" data-ui-elements="0"${compositionAttr(section)} style="background:${T.paper};color:${T.ink};">
      ${heroPhoto}
  ...
  if (type === "cta") {
    ...
    const restIsEcho =
      rest &&
      rest.split("·").every((part) => {
        const t = part.trim();
        if (!t) return true;
        if (t === pack.target || t === pack.brandName) return true;
        if (pack.region && t.includes(pack.region)) return true;
        return false;
      });
    return `<section id="detail-buy" data-section="cta" data-layout="cta-bar" data-purpose="${esc(section.purpose || "closing")}" data-image-ratio="0.2" data-ui-elements="0"${compositionAttr(section)} style="padding:0;background:${T.paper};">
      <div style="${padBox("cta")}">
        ${kickerHtml(section.kicker, T.muted)}${priceHtml(price || pack.price, T.ink)}${rest && !restIsEcho ? leadHtml(rest, T.muted) : ""}${specSheet(section.rows, accent)}${closeLine(section.ctaLabel || "구매")}
      </div>
    </section>`;
  }
  ...
  if (type === "brand") {
    ...
    const bodyEchoesTitle =
      title && body && (body.includes(title.replace(/에서 출고$/, "")) || title.includes(body.replace(/합니다\.?$/, "")));
    return wrapSection({
      type,
      section,
      ...
      bg: T.paper,
      padKind: "brand",
      inner: `${kickerHtml(section.kicker, T.muted)}${h2Html(section.title)}${bodyEchoesTitle ? "" : leadHtml(section.body)}${faqHtml(section.faqs)}`,
    });
  }
...
export function renderDetailPageBodyHtml(pack, images = []) {
  ...
  const pageWidth = Number(pack.exportWidth || DETAIL_PAGE_WIDTH);
