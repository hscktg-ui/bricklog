...
function highlightBar(highlights, accent) {
  ...
  const chips = items
    ...
    .join("");
  return `<div data-section="highlights" data-highlights="${items.length}" style="display:flex;flex-wrap:wrap;padding:18px ${T.padX}px 6px;background:${T.wash};border-bottom:1px solid ${T.rule};">${chips}</div>`;
}
...
function pointsFive(bullets, accent) {
  ...
  const cells = items
    .map((item, i) => {
      ...
      const title = item.label || item.hint;
      const hint = item.label ? item.hint : "";
      return `<td style="width:${width}%;vertical-align:top;padding:20px 12px;background:${T.paper};border:1px solid ${T.rule};">
        <p style="margin:0 0 10px;font-size:11px;letter-spacing:0.16em;font-weight:650;color:${accent};font-family:${T.familyKicker};">${n}</p>
        <p style="margin:0;font-size:17px;line-height:1.32;letter-spacing:-0.03em;color:${T.ink};font-weight:650;word-break:keep-all;font-family:${T.familyDisplay};">${esc(title)}</p>
        ${hint ? `<p style="margin:8px 0 0;font-size:12px;line-height:1.4;color:${T.muted};word-break:keep-all;">${esc(hint)}</p>` : ""}
      </td>`;
    })
    .join("");
  return `<table data-layout="points-5" data-ranking-slot="5-points" style="width:100%;border-collapse:separate;border-spacing:8px;margin:4px -8px 0;"><tr>${cells}</tr></table>`;
}
...
export function wrapDetailPageImageStackHtml(images = [], pack = {}, mallId = "smartstore") {
  ...
  const imgs = (Array.isArray(images) ? images : [])
    ...
    .join("");
  const ranking = pack._meta?.ranking || {};
  const body = `<article id="gollaboda-detail-page" data-deliverable="image-stack" data-engine="${esc(pack._meta?.engine || "")}" data-ranking="${esc(ranking.source || "naver-shop-rank")}" data-list-sample="${esc(ranking.listSample || "creazy")}" data-mall-ready="smartstore,coupang" style="width:${DETAIL_PAGE_WIDTH}px;margin:0 auto;background:#fff;">${imgs}</article>`;
