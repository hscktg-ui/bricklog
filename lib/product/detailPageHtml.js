...
import { imageBriefLine } from "@/lib/product/detailPageImageBrief";
...
function highlightBar(highlights, accent) {
  ...
  const chips = items
    ...
    .join("");
  return `<div data-section="highlights" data-highlights="${items.length}" style="display:flex;flex-wrap:wrap;margin-top:18px;max-width:100%;overflow:hidden;">${chips}</div>`;
}
...
function consumerPhotoBrief(text) {
  return imageBriefLine(text)
    .replace(/모델컷\s*금지\.?/g, "")
    .replace(/새 문구 금지\.?/g, "")
    .replace(/포장 반복 금지\.?/g, "")
    .replace(/\s+/g, " ")
...
function imgFrame(src, alt, height, slot, caption, capColor = T.muted, extra = {}) {
  ...
  const brief = consumerPhotoBrief(extra.brief || dir.hint || "");
  ...
  if (!src) {
    ...
    const briefHtml = brief
      ? `<p data-photo-brief="1" style="margin:6px 0 0;font-size:16px;line-height:1.5;color:${T.muted};letter-spacing:-0.01em;word-break:keep-all;">${esc(brief)}</p>`
      : "";
    if (slot === "hero") {
    ...
    return `<div ${attrs} data-photo-empty="1" style="padding:16px ${T.padX}px;background:${T.wash};border-bottom:1px solid ${T.rule};"><p style="margin:0;font-size:13px;letter-spacing:0.08em;color:${T.muted};font-family:${T.familyKicker};">사진 ${shot}</p>${briefHtml}</div>`;
  }
