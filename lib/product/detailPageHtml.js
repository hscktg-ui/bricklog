...
function imgFrame(src, alt, height, slot, caption, capColor = T.muted, extra = {}) {
  ...
  const brief = extra.brief || dir.hint || "";
  ...
  if (!src) {
    ...
    const briefHtml = brief
      ? `<p data-photo-brief="1" style="margin:6px 0 0;font-size:14px;line-height:1.5;color:${T.muted};letter-spacing:-0.01em;word-break:keep-all;">${esc(brief)}</p>`
      : "";
    if (slot === "hero") {
    ...
    return `<div ${attrs} data-photo-empty="1" style="padding:16px ${T.padX}px;background:${T.wash};border-bottom:1px solid ${T.rule};"><p style="margin:0;font-size:12px;letter-spacing:0.14em;color:${T.muted};font-family:${T.familyKicker};">${esc(dir.plate || "")} ${shot}</p>${briefHtml}</div>`;
  }
