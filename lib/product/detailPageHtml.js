...
function specSheet(rows, accent) {
  ...
  const shown = filled.length ? filled : list;
  const body = shown
    .map(
      (r, i) =>
    ...
    .join("");
  const table = `<table data-layout="spec-sheet" style="width:100%;border-collapse:collapse;font-size:${T.spec}px;line-height:1.45;margin-top:8px;border:1px solid ${T.rule};font-family:${T.familySpec};">${body}</table>`;
  if (!filled.length || !missing.length) return table;
  return `${table}<p data-missing="${missing.length}" style="margin:14px 0 0;font-size:16px;line-height:1.55;color:${T.muted};word-break:keep-all;">${missing
