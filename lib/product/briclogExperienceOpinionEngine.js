...
const EXPERIENCE_OPINION_MARKERS = [
  {
    id: "observation",
    re: /느껴질|차이가|높은\s*편|쉬운\s*편|눈에\s*들어|눈에\s*띄|직접\s*확인|앉아\s*보면|한\s*다발|한\s*송이|비교해\s*보면|살보면|정리해\s*보면|들어가\s*보면|들어서면|분위기가|동선이/,
  },
  {
    id: "experience",
    re: /실제로|앉아\s*보면|앉아\s*보|직접|한\s*다발|한\s*송이|현장에서|쇼룸에서|맞춰\s*보면|이용해\s*보니|써\s*보니|다녀와\s*보니|받아\s*보니|가\s*보면/,
  },
  {
    id: "opinion",
    re: /생각보다|편안함|만족도|부담\s*없|무난|낫습니다|도움이\s*됩니다|좋은\s*편|편한\s*편|추천(?:드릴|할)|아쉬운\s*편|괜찮은\s*편/,
  },
];
...
const MAJOR_INFO_RE =
  /수국|해바라기|거베라|라넌큘러스|STRESSLESS|다이닝체어|좌판|등받이|만원|무인|24\s*시간|프랜차이즈|메뉴|예약|가격|시설|객실|수업|커리큘럼|시술|좌석|주차|영업|혜택|패키지|할인|오픈|리뉴얼|방문|상담|체험|수강|강좌|코스/;
