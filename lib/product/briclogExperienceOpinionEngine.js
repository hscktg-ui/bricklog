...
const EXPERIENCE_OPINION_MARKERS = [
  {
    id: "observation",
    re: /많이\s*선택|문의가\s*많|느껴질|차이가|높은\s*편|쉬운\s*편|잦습니다|늘어나는|경우가\s*많|자주\s*고르|선호하는\s*편|눈에\s*들어|눈에\s*띄/,
  },
  {
    id: "experience",
...
const FLOWER_EXPERIENCE = {
  수국: (brand, unmanned) => [
    "집들이 꽃다발을 찾는 분들은 수국을 많이 선택하는 편입니다.",
    "실제로 한 다발만으로도 풍성해 보이는 느낌이 있어, 선물용 만족도가 높은 편입니다.",
    unmanned
      ? `${brand}에서는 수국 조합 여름 꽃다발 문의가 실제로 잦은 편입니다.`
      : `${brand} 진열대에서 수국 톤을 나란히 보면 선물용 만족도 차이가 눈에 들어옵니다.`,
  ],
  해바라기: (brand, unmanned) => [
    "축하 꽃다발을 맞추는 분들은 해바라기를 많이 선택하는 편입니다.",
    "밝은 톤 덕분에 사진·인증샷에서도 잘 보이는 편이라, 생각보다 선물 만족도가 높습니다.",
    unmanned
      ? `24시간 무인이라 늦은 시간 축하 꽃다발을 찾는 분들이 실제로 해바라기 조합을 자주 고릅니다.`
      : `${brand}에서 주말·행사 시즌에는 해바라기 단품 수요가 눈에 띄게 늘어납니다.`,
  ],
  거베라: (brand, unmanned) => [
    "꽃을 처음 구매하는 분들은 거베라를 부담 없이 고르는 편입니다.",
    "관리 부담이 비교적 적어 실내에 두었을 때 색이 오래 가는 편이라, 첫 구매 만족도가 높습니다.",
    unmanned
      ? `만원대 꽃다발 라인에서 거베라 조합을 바로 맞출 수 있어, 첫 방문 때 생각보다 부담 없이 고르는 편입니다.`
      : `${brand}에서 거베라는 선물용으로 묻는 분이 많아, 포장 톤 비교가 먼저 이뤄집니다.`,
  ],
  라넌큘러스: (brand, unmanned) => [
    "기념일 선물 톤을 살리려는 분들은 라넌큘러스를 많이 선택하는 편입니다.",
    "실제로 라넌큘러스 한 송이만 넣어도 분위기가 살아나, 생각보다 소량 구성 만족도가 높습니다.",
    unmanned
      ? `무인 픽업이라 포장 톤만 정해 두면 실제로 라넌큘러스 조합을 빠르게 받을 수 있습니다.`
      : `${brand}에서는 라넌큘러스·안개꽃 믹스 톤을 여름철에 자주 찾습니다.`,
  ],
};
...
export function buildFlowerExperienceOpinionParagraphs(p, input = {}) {
  ...
  const lines = [
    "여름철 꽃 추천 글을 읽다 보면, 이름만 나열한 문장과 실제로 고를 때 도움이 되는 문장의 차이가 큽니다.",
  ];
...
export function buildChairExperienceOpinionParagraphs(p, input = {}, product) {
  ...
  return [
    ...
    `${topicLine}는 거실·식탁 겸용 공간에서 자세를 바꿀 때 리클라인 반응이 체감되는 편이라, 쇼룸에서 직접 앉아 보는 게 낫습니다.`,
    `${regionBit}${brand} 프랜차이즈 쇼룸에서는 전시 모델 구성이 지점마다 달라, 당일 기준으로 앉은 높이·등받이 지지를 확인하는 편이 좋습니다.`,
    `팔걸이와 좌판 쿠션 밀도 차이는 식탁 높이·팔꿈치 각도에 영향을 주므로, 실제 테이블 옆에서 앉아 보는 것이 선택 기준입니다.`,
    `가죽·패브릭 마감은 관리 방법과 내구성이 달라 보이므로, 거실 톤과 맞추기 전에 소재별 관리 조건을 함께 보는 편이 낫습니다.`,
