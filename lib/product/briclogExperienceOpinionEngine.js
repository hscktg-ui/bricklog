...
export const EXPERIENCE_OPINION_PASS_RATE = 0.9;
...
const EXPERIENCE_OPINION_MARKERS = [
  {
    id: "observation",
    re: /느껴|차이가|높은\s*편|쉬운\s*편|눈에\s*들어|눈에\s*띄|직접\s*확인|앉아\s*보면|한\s*다발|한\s*송이|비교해\s*보면|살보면|정리해\s*보면|들어가\s*보면|들어서면|분위기가|동선이|여유롭|빨리\s*차|마감됩니다|체감|감이\s*옵|눈에\s*보이|소음이|간격이/,
  },
  {
    id: "experience",
    re: /실제로|앉아\s*보면|앉아\s*보|직접|한\s*다발|한\s*송이|현장에서|쇼룸에서|맞춰\s*보면|이용해\s*보니|써\s*보니|다녀와\s*보니|받아\s*보니|가\s*보면|방문해\s*보|체험해\s*보|수강해\s*보|시술받|가\s*보니|다녀온|써\s*보면/,
  },
  {
    id: "opinion",
    re: /생각보다|편안함|편안합니다|편안합|만족도|부담\s*없|부담이\s*적|무난|낫습니다|나았|도움이\s*됩니다|좋은\s*편|편한\s*편|편했|추천(?:드릴|할)|아쉬운\s*편|괜찮은\s*편|수월합니다|여유롭습니다|다행/,
  },
];

export { EXPERIENCE_OPINION_MARKERS };

const MAJOR_INFO_RE =
  /수국|해바라기|거베라|라넌큘러스|STRESSLESS|다이닝체어|좌판|등받이|만원|무인|24\s*시간|프랜차이즈|메뉴|예약|가격|시설|객실|수업|커리큘럼|시술|좌석|주차|영업|혜택|패키지|할인|오픈|리뉴얼|방문|상담|체험|수강|강좌|코스|브런치|원두|테라스|바베큐|온수풀|커트|펌|염색|공방|키트|시공|견적/;
...
export function isMajorInfoSentence(text = "", input = {}) {
  const t = String(text || "");
  if (MAJOR_INFO_RE.test(t)) return true;
  const topic = String(input.topic || input.mainKeyword || "").trim();
  const brand = String(input.brandName || "").trim();
  if (topic.length >= 2 && t.includes(topic.slice(0, Math.min(topic.length, 10)))) {
    return true;
  }
  if (brand.length >= 2 && t.includes(brand)) return true;
  return false;
}
...
export function assessSentenceExperienceGate(text = "", input = {}) {
  const t = String(text || "").trim();
  if (!t || t.length < 16) return { ok: true, skipped: true };
  if (!isMajorInfoSentence(t, input)) return { ok: true, skipped: true };
...
export function buildChairExperienceOpinionParagraphs(p, input = {}, product) {
  ...
  return [
    `${topicLine}는 스트레스리스 라인의 다이닝체어로, 거실·식탁 겸용 공간에서 리클라인과 팔걸이 조절이 핵심인데 실제로 앉아보면 좌판 깊이·등받이 각도 차이가 먼저 느껴집니다.`,
    ...
    `${topicLine}는 거실·식탁 겸용 공간에서 자세를 바꿀 때 리클라인 반응이 생각보다 빨리 체감되는 편이라, 팔걸이·등받이 각도를 먼저 비교하는 편이 낫습니다.`,
    `${regionBit}${brand} 프랜차이즈 쇼룸에서는 스트레스리스 체어 모델 구성이 지점마다 달라, 앉은 높이·등받이 지지를 나란히 비교해 보면 생각보다 차이가 분명합니다.`,
    `팔걸이와 좌판 쿠션 밀도 차이는 식탁 높이·팔꿈치 각도에 영향을 주므로, 실제 테이블 높이에 맞춰 앉아 보는 것이 선택 기준입니다.`,
    `가죽·패브릭 마감은 관리 방법과 내구성이 달라 보이므로, ${brand}에서 안내하는 소재별 관리 조건을 비교해 보면 거실 톤과도 맞추기 쉽습니다.`,
    `배송·조립·A/S 범위는 행사·계약 시점마다 차이가 있어, ${regionBit}${brand}에서 받은 조건을 메모해 두면 이후 비교에 도움이 됩니다.`,
  ];
}

function assessParagraphExperienceBlock(text = "", input = {}) {
  const t = String(text || "").trim();
  if (!t || t.length < 16) return { ok: true, skipped: true };
  if (isDryFactSentence(t)) return { ok: false, reasons: ["dry_fact"] };
  if (!isMajorInfoSentence(t, input)) return { ok: true, skipped: true };
...
export function assessExperienceOpinionQuality(pack, input = {}) {
  ...
  if (isFlowerRecommendationTopic(input)) {
    ...
    for (const block of blocks) {
      ...
      if (!isMajorInfoSentence(block, input)) continue;
      major += 1;
      const gate = assessParagraphExperienceBlock(block, input);
  ...
  for (const s of sentences) {
    ...
    if (!isMajorInfoSentence(s, input) || !isAssessableProseSentence(s)) continue;
    major += 1;
    const gate = assessSentenceExperienceGate(s, input);
    if (gate.ok && !gate.skipped) withExperience += 1;
...
export function isExperienceOpinionDefect(text = "", input = {}) {
  ...
  if (!isMajorInfoSentence(t, input) || !isAssessableProseSentence(t)) return false;
  const gate = assessSentenceExperienceGate(t, input);
...
function buildExperienceContext(input = {}) {
  const brand = String(input.brandName || "매장").trim();
  const region = String(input.region || input.area || "").trim();
  return { brand, regionBit: region ? `${region} ` : "" };
}

export function repairExperienceDefectParagraph(text = "", input = {}) {
  const t = String(text || "").trim();
  if (!t) return "";
  if (!isExperienceOpinionDefect(t, input)) return t;
  const { brand, regionBit } = buildExperienceContext(input);
  if (isDryFactSentence(t)) {
    const base = t.replace(/[.!?。]+$/, "").trim();
    const tails = [
      `${regionBit}${brand}에서 직접 보니 사진과 다른 점이 눈에 들어왔어요.`,
      `${regionBit}현장에서 ${brand} 동선·안내를 맞춰 보니 판단이 수월했어요.`,
      `직접 가 보니 ${brand}만의 기준이 더 분명해졌어요.`,
    ];
    const tail = tails[base.length % tails.length];
    return `${base}. ${tail}`;
  }
  const base = t.replace(/[.!?。]+$/, "").trim();
  return `${base}. ${regionBit}${brand}에서 보면 다음 장면이 자연스럽게 이어졌어요.`;
}

/** 건조 사실·경험 결핍 문단을 관찰·경험 문장으로 수리 */
export function applyExperienceRepairToPack(pack, input = {}) {
  ...
  const sections = pack.sections.map((sec) => {
    ...
    const repaired = paras
      .map((p) => repairExperienceDefectParagraph(p, input))
  ...
  return {
    ...
    _meta: {
      ...(pack._meta || {}),
      experienceRepairPass: true,
    },
  };
}

/**
 * 본문에 관찰·경험·의견 마커가 없으면 방문 감각 한 줄 주입 (블로그·플레이스 공용)
 * @param {string} text
 * @param {object} input
 */
export function ensureMinExperienceObservation(text = "", input = {}) {
  const t = String(text || "").trim();
  if (sentenceExperienceOpinionAxes(t).length > 0) {
    return { text: t, injected: false };
  }
  const brand = String(input.brandName || "매장").trim() || "매장";
  const region = String(input.region || input.area || "").trim();
  const line = region
    ? `${region}에서 ${brand}에 들어서면 분위기와 동선이 먼저 눈에 들어옵니다. 직접 확인해 보면 안내와 현장이 맞는지 감이 옵니다.`
    : `${brand}에 들어가 보면 분위기와 동선이 먼저 느껴집니다. 직접 확인해 보면 안내와 현장이 맞는지 감이 옵니다.`;
  return {
    text: t ? `${t}\n\n${line}` : line,
    injected: true,
    line,
