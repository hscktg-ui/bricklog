...
import { resolveUnifiedDeliveryPassMin, UNIFIED_DELIVERY_PASS_MIN } from "@/lib/product/unifiedDeliveryPassMin";
import {
...
export const EDITOR_COLUMN_MIN_CHAR_RATIO = 0.52;
export const EDITOR_COLUMN_ABS_MIN_CHARS = 820;
...
export function isColumnistEditorGradePass(pack, input = {}) {
  const isColumnistDelivery =
    pack?._meta?.columnistSovereignLlm ||
    pack?._meta?.visitReviewBenchmarkOk ||
    pack?._meta?.generationMode === "columnist_sovereign" ||
    pack?._meta?.contentEvaluation?.columnistBenchmark === true;
  if (!isColumnistDelivery) return false;

  const bench =
    pack._meta?.visitReviewBenchmark || assessVisitReviewBenchmark(pack, input);
  const passMin = resolveVisitReviewPassMin({});
  if (bench.publishOk !== true || bench.score < passMin) return false;

  const full = getBlogFullText(pack);
  const woven = countConcreteFactsWovenInBody(full, input);
  const wovenMin = bench.score >= 90 ? 1 : 2;
  if (woven.woven >= wovenMin) return true;

  const facts = collectSubstantiveResearchFacts(input);
  const sections = pack.sections?.length || 0;
  if (facts.length >= 2 && bench.score >= 88 && sections >= 5) return true;

  return pack._meta?.visitReviewBenchmarkOk === true && bench.score >= resolveUnifiedDeliveryPassMin({});
}
export function isContentEvaluationEditorPass(pack, input = {}) {
  if (isColumnistEditorGradePass(pack, input)) return true;
...
export function assessProfessionalEditorDelivery(pack, input = {}) {
  ...
  const evalPass =
    ...
    isContentEvaluationEditorPass(pack, input) ||
