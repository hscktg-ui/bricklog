...
import { countConcreteFactsWovenInBody } from "@/lib/product/editorGradeResearchGate";
...
export function isColumnistEditorGradePass(pack, input = {}) {
  if (!pack?._meta?.columnistSovereignLlm && !pack?._meta?.visitReviewBenchmarkOk) {
    return false;
  }
  const bench =
    pack._meta?.visitReviewBenchmark || assessVisitReviewBenchmark(pack, input);
  const woven = countConcreteFactsWovenInBody(getBlogFullText(pack), input);
  return (
    bench.publishOk === true &&
    bench.score >= resolveVisitReviewPassMin({}) &&
    woven.woven >= 2
  );
}
