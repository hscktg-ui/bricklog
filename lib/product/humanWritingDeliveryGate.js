...
const VISIT_REVIEW_PERSONAS = new Set(["visit_review", "local_guide"]);

/**
 * 방문 후기 주제에 정보형·브랜드 소개 화자가 섞이면 가이드형 노출 위험
 * @param {object} input
 */
export function assessVisitSpeakerAlignment(input = {}) {
  if (!isVisitReviewTopicInput(input)) return { ok: true };
  const speaker = String(input.v4Speaker || "auto").trim();
  if (speaker === "auto") return { ok: true };
  const linked = getSpeakerPersonaFields(speaker);
  if (!linked) return { ok: true };
  const persona = linked.contentPersona;
  return {
    ok: VISIT_REVIEW_PERSONAS.has(persona),
    speaker,
    persona,
  };
}
...
export function assessHumanWritingDelivery(pack, input = {}, gate = {}) {
  ...
  const visitSpeaker = assessVisitSpeakerAlignment(input);
  if (!visitSpeaker.ok) {
    if (!reasons.includes("visit_speaker_mismatch")) {
      reasons.push("visit_speaker_mismatch");
    }
  }
