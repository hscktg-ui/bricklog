...
export function buildHumanStoryProblemOpening(input = {}) {
  ...
  const fromFlavor = (
    isInformationalTopicInput(input) ? flavor.infoProblemOpenings || flavor.problemOpenings : flavor.problemOpenings
  ).filter(Boolean);
  const pool =
    fromFlavor.length > 0
      ? fromFlavor
...
export function ensureHumanStoryOpeningBody(body, input = {}) {
  ...
  if (!text) return buildHumanStoryProblemOpening(input);
