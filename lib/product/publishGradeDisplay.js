...
export function resolvePublishGrade(input = {}) {
  ...
  if (status === "blocked" || score < 60) return PUBLISH_GRADE_C;
  if (status === "ready" || score >= 85) return PUBLISH_GRADE_A;
  if (status === "polishing" || score >= 70) return PUBLISH_GRADE_B;
...
export function buildManuscriptStatusFromPack(pack = {}) {
  ...
  return {
    readiness,
    grade: resolvePublishGrade({ publishScore, readiness }),
    publishScore,
  };
}
