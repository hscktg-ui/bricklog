...
const RICE_SRC = {
  ...
  pack: "/detail-sample/open-rice-hero.png",
...
export function buildRiceCanvasStory(pack = {}, photos = []) {
  ...
  const packShot = pickRicePhoto(photos, { hint: "open-rice-hero", slot: "hero", sample: RICE_SRC.pack, index: 0 });
  const label = pickRicePhoto(photos, { hint: "open-rice-observe", slot: "feature", sample: RICE_SRC.label, index: 2 });
...
export function buildBeansCanvasStory(pack = {}, photos = []) {
  ...
  const frames = [
    ...
    canvas({
      ...
      kicker: "원두",
      headline: "중배전",
      sub: "향은 좋은데, 집에서 어떻게 내려야 할지 모르겠다",
      ink: "#f3eadc",
      art: art({
        section: 2,
        purpose: "choose",
