...
export function buildRiceCanvasStory(pack = {}, photos = []) {
  ...
  const frames = [
    ...
    canvas({
      ...
      kicker: overlayFromPack(pack, "intent", "kicker", origin || "여주"),
      headline: overlayFromPack(pack, "intent", "headline", "여주 햅쌀"),
      sub: overlayFromPack(pack, "intent", "sub", [variety, process].filter((v) => v && !isNeedFact(v)).join(" · ") || "당일 도정"),
      ink: "#f6f1e6",
      art: art({
        section: 2,
        purpose: "info",
    ...
    canvas({
      ...
      kicker: "",
      headline: "",
      facts: overlayFromPack(pack, "explain", "facts", listRows),
      ink: "#f7f3ea",
      art: art({
        section: 3,
        purpose: "listing",
        message: listRows.map((r) => r[0]).join(" · "),
    ...
    canvas({
      ...
      kicker: "쌀알",
      headline: factOrNeed(pack.ingredient, "원재료") === needFact("원재료") ? "" : factOrNeed(pack.ingredient, "원재료"),
      ink: "#f3ebd8",
      art: art({
        section: 4,
        purpose: "ingredient",
...
export function buildBeansCanvasStory(pack = {}, photos = []) {
  ...
  const frames = [
    ...
    canvas({
      ...
      textPosition: "lower_left",
      kicker: "원두",
      headline: factOrNeed(pack.ingredient, "원재료"),
      ink: "#f3eadc",
      art: art({
        section: 4,
        purpose: "ingredient",
