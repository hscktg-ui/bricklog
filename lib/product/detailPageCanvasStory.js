...
import { mallBeatForSlot } from "@/lib/product/detailPageRankingPlaybook";
import { isDirectedDetailPack } from "@/lib/product/detailPageGrokDirector";
import {
  firstGlanceHeadline,
  resolveHookHeadline,
...
function overlayFromPack(pack, type, key, fallback) {
  if (type === "hero" && key === "headline") {
    const glance = resolveHookHeadline(pack, fallback);
    if (glance) return glance;
  }
  if (!isDirectedDetailPack(pack)) return fallback;
  ...
  if (key === "facts" && section.rows?.length) return section.rows;
  return fallback;
}
...
function listingFactsFromPack(pack, fallback = []) {
  const listing = buildDetailPageCategoryListing(pack);
  const skip = new Set(["cook", "ingredient", "keep", "brew"]);
  const rows = (listing.filled || [])
    .filter((s) => !skip.has(s.key))
    .slice(0, 6)
    .map((s) => [s.label, shortListingValue(s.label, s.value)])
    .filter((row) => row[0] && row[1]);
  return rows.length >= 3 ? rows : fallback;
}

function shortListingValue(label, value) {
  const v = String(value || "").replace(/\s+/g, " ").trim();
  if (v.length <= 14) return v;
  if (label === "포장" && /진공/.test(v)) return "진공";
  if (label === "도정" && /당일/.test(v)) return "당일 도정";
  if (label === "로스팅" && /중배전/.test(v)) return "중배전";
  return v.split(/[·.\n]/)[0].trim().slice(0, 14);
}

function canvas(partial) {
  const beat = partial.beat || mallBeatForSlot(partial.type) || partial.purpose || "";
  return {
    n: partial.n,
    type: partial.type,
    beat,
    purpose: partial.purpose || beat,
    composition: partial.composition,
    height: partial.height,
...
export function buildRiceCanvasStory(pack = {}, photos = []) {
  ...
  const variety = factOrNeed(pack.variety, "품종");
  const weight = factOrNeed(pack.weight || "10kg", "중량");
  const origin = factOrNeed(pack.origin || pack.region, "산지");
  const process = factOrNeed(pack.process || pack.milledAt, "도정");
  const packWay = factOrNeed(pack.pack, "포장");
  const listRows = listingFactsFromPack(pack, [
    ["산지", origin],
    ["햅쌀", "햅쌀"],
    ["도정", process],
    ["중량", weight],
  ...
  const hookKicker = overlayFromPack(pack, "hero", "kicker", "여주 햅쌀");
  const hookTitle = overlayFromPack(
    pack,
    "hero",
    "headline",
    firstGlanceHeadline(pack) || "햅쌀 · 진상 · 10kg"
  );
  const infoKicker = overlayFromPack(pack, "intent", "kicker", origin || "여주");
  const infoTitle = overlayFromPack(pack, "intent", "headline", "여주 햅쌀");
  const infoSub = overlayFromPack(pack, "intent", "sub", [variety, process].filter((v) => v && !isNeedFact(v)).join(" · ") || "당일 도정");
  const listingRows = overlayFromPack(pack, "explain", "facts", listRows);
  const ingredientKicker = overlayFromPack(pack, "observe", "kicker", "쌀알");
  const ingredientTitle = overlayFromPack(
    pack,
    "observe",
    "headline",
    factOrNeed(pack.ingredient, "원재료") === needFact("원재료") ? "" : factOrNeed(pack.ingredient, "원재료")
  );
  const materialKicker = overlayFromPack(pack, "usp", "kicker", "원료");
  const materialTitle = overlayFromPack(pack, "usp", "headline", "들판에서 온 햅쌀");
  const specFacts = overlayFromPack(pack, "spec", "facts", [
    ["중량", weight],
    ["품종", variety],
    ["옵션", options],
  ]);
  const packTitle = overlayFromPack(pack, "feature", "headline", weight);
  const shipTitle = overlayFromPack(pack, "scene", "headline", shipping);
  const shipFacts = overlayFromPack(pack, "scene", "facts", [
    ["출고", dispatch],
    ["도서산간", island],
  ]);
  const noticeKicker = overlayFromPack(pack, "cta", "kicker", pack.brandName || "우리쌀가게");
  const noticeTitle = overlayFromPack(pack, "cta", "headline", exchange);
  ...
  const frames = [
    canvas({
      ...
      art: art({
        section: 1,
        purpose: "hook",
        message: String(hookTitle).replace(/\n/g, " "),
    ...
    canvas({
      ...
      art: art({
        section: 2,
        purpose: "info",
        message: String(infoTitle).replace(/\n/g, " "),
    ...
    canvas({
      ...
      art: art({
        section: 3,
        purpose: "listing",
        message: listingRows.map((r) => r[0]).join(" · "),
    ...
    canvas({
      ...
      kicker: ingredientKicker,
      headline: ingredientTitle,
      ink: "#f3ebd8",
      art: art({
        section: 4,
        purpose: "ingredient",
    ...
    canvas({
      ...
      textPosition: "lower_left",
      kicker: materialKicker,
      headline: materialTitle,
      ink: "#f6f0e4",
      art: art({
        section: 5,
        purpose: "material",
        message: String(materialTitle).replace(/\n/g, " "),
    ...
    canvas({
      ...
      kicker: "",
      headline: overlayFromPack(pack, "spec", "headline", price),
      facts: specFacts,
      ink: "#efe6d6",
      paper: "#14110e",
      art: art({
        section: 6,
        purpose: "spec",
        message: overlayFromPack(pack, "spec", "headline", price),
...
export function buildBeansCanvasStory(pack = {}, photos = []) {
  ...
  const roast = factOrNeed(pack.process, "로스팅");
  const weight = factOrNeed(pack.weight || "200g", "중량");
  const origin = factOrNeed(pack.origin, "원산지");
  const packWay = factOrNeed(pack.pack, "포장");
  const options = factOrNeed(pack.options, "판매 옵션");
  const shipping = factOrNeed(pack.shipping, "배송비");
  const dispatch = factOrNeed(pack.dispatch, "출고 일정");
  ...
  const hookKicker = overlayFromPack(pack, "hero", "kicker", "하우스 블렌드");
  const hookTitle = overlayFromPack(
    pack,
    "hero",
    "headline",
    firstGlanceHeadline(pack) || "중배전 · 200g"
  );
  const infoKicker = overlayFromPack(pack, "intent", "kicker", "하우스 블렌드");
  const infoTitle = overlayFromPack(pack, "intent", "headline", roast || "중배전");
  const infoSub = overlayFromPack(pack, "intent", "sub", "주문 후 분쇄 가능");
  const listingRows = overlayFromPack(pack, "explain", "facts", listRows);
  const ingredientKicker = overlayFromPack(pack, "observe", "kicker", "원두");
  const ingredientTitle = overlayFromPack(pack, "observe", "headline", factOrNeed(pack.ingredient, "원재료"));
  const materialKicker = overlayFromPack(pack, "usp", "kicker", "원료");
  const materialTitle = overlayFromPack(pack, "usp", "headline", origin || "블렌드");
  const materialFacts = overlayFromPack(pack, "usp", "facts", [
    ["로스팅", roast],
    ["중량", weight],
  ]);
  const specFacts = overlayFromPack(pack, "spec", "facts", [
  ...
  const noticeKicker = overlayFromPack(pack, "cta", "kicker", pack.brandName || "골목카페");
  const noticeTitle = overlayFromPack(pack, "cta", "headline", exchange);
...
export function buildGenericCanvasStory(pack = {}, photos = []) {
  ...
  const frames = sections.map((section, i) => {
    ...
    return canvas({
      ...
      headline:
        i === 0
          ? firstGlanceHeadline(pack) || section.title || pack.productName || ""
          : section.title || pack.productName || "",
      sub: section.body || "",
      ...
      art: art({
        ...
        composition,
        imageCoverage: section.type === "explain" ? 0.52 : imageCoverage,
        background: "studio",
        camera: "85mm commercial",
        lighting: "soft directional",
        textPosition: composition === "editorial" ? "right_column" : "upper_left",
        headlineScale: i === 0 ? "XL" : section.type === "spec" ? "L" : i === 1 ? "L" : "M",
        photography: photoDir,
        height: 1200,
      }),
    });
...
function stampStory(pack, frames, category, photography) {
  ...
  return {
    ...
    sections: frames.map((f) => ({
      ...
      body: f.sub,
      bullets: f.type === "explain" ? (f.facts || []).map((row) => row.filter(Boolean).join(" — ")) : undefined,
...
export function applyCanvasCopyToPack(pack, story) {
  ...
  const directed = isDirectedDetailPack(pack);
  ...
  const sections = (pack.sections || []).map((s) => {
    ...
    return {
      ...
      title: directed
        ? s.title || next.title
        : ["intent", "explain", "scene"].includes(s.type)
          ? s.title
          : next.title || s.title,
      body: next.body || s.body,
      composition: s.composition || next.composition,
      purpose: next.purpose || s.purpose,
      art: next.art,
