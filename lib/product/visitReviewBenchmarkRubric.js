...
export const VISIT_REVIEW_BENCHMARK_VERSION = "visit-review-benchmark-v1";
...
function scoreResearchGrounded(pack, input = {}) {
  ...
  const facts = (input.researchFacts || [])
    ...
    .filter(Boolean);
  if (!facts.length) {
    return { score: 10, max: 15, woven: 0, total: 0, skipped: true };
  }
  let woven = 0;
  for (const fact of facts) {
    const tokens = String(fact)
  ...
  const ratio = woven / facts.length;
  return {
    score: Math.min(15, Math.round(6 + ratio * 9)),
    max: 15,
    woven,
    total: facts.length,
    ratio,
...
export function assessVisitReviewBenchmark(pack, input = {}, opts = {}) {
  ...
  return {
    ...
    passMin,
    grade:
      total >= 85 ? "A" : total >= 75 ? "B" : total >= 60 ? "C" : total >= 45 ? "D" : "F",
