...
export const DETAIL_PAGE_COMPETE_WINS = Object.freeze([
  {
    ...
    they: "드랩·크리에이지·후커블",
    theyDo: "모델컷·배경 합성",
    weWin: "컷별 상품 사진만 생성한다. 한글 상세페이지는 그리지 않는다. 올린 사진이 있으면 그걸 먼저 쓴다",
  ...
  const checks = DETAIL_PAGE_COMPETE_WINS.map((win) => {
    ...
    if (win.id === "first-glance") {
      ok =
        ...
        (html.includes('data-renderer="detail-canvas"') ||
          html.includes('data-layout="hero-banner"') ||
          html.includes('data-layout="hero-stack"') ||
          html.includes('data-layout="ad-canvas"')) &&
        html.includes("860px");
    }
