...
import { formatSqUserHint, resolveSqScoreFromPack } from "@/lib/product/sqvUserDisplay";
import { shouldShowInternalQualityScore } from "@/lib/copy/customerQualityDisplay";
import { BLOG_TARGET_BODY_CHARS } from "@/lib/constants";
...
function sqvHint(sqv, meta = {}) {
  if (!sqv || typeof sqv.score !== "number") return "";
  return formatSqUserHint(sqv, meta);
}
...
function goldenQualityHint(meta = {}) {
  ...
  if (typeof gate?.score === "number" && shouldShowInternalQualityScore()) {
...
export function resolvePublishReadiness(pack = {}) {
  ...
  const sqv =
    meta.sqv ||
    (typeof sqvResolved.score === "number"
      ? { score: sqvResolved.score, grade: sqvResolved.grade, publishReady: meta.publishReady }
      : null);
  ...
  if (meta.deliveryGrade === DELIVERY_GRADE.DRAFT) {
    const min = meta.lengthTierMin || BLOG_TARGET_BODY_CHARS;
    const voiceHint =
  ...
  if (meta.publishReady === true || sqv?.publishReady === true) {
    ...
    const tail = gHint || (sqvHint(sqv, meta) ? `${sqvHint(sqv, meta)} — ` : "");
    return {
      status: "ready",
  ...
  if (sqv && typeof sqv.score === "number") {
    if (sqv.score >= 76) {
      return {
        ...
        hint:
          prefix +
          (sqvHint(sqv, meta) || "복사는 가능하지만, 올리기 전에 한 번 더 읽어 보세요."),
        canCopy: true,
        sqvScore: sqv.score,
        sqvGrade: sqv.grade,
      };
    ...
    if (sqv.score >= 50) {
      return {
        status: "review",
        label: PUBLISH_REVIEW_LABEL,
        hint: prefix + (sqvHint(sqv, meta) || COMPLETION_READY_HINT),
        canCopy: true,
        sqvScore: sqv.score,
        sqvGrade: sqv.grade,
      };
...
export function resolvePublishGrade(input = {}) {
  ...
  const status = input.readiness?.status ?? input.status;
  const sqvGrade = input.sqvGrade;
  const editorGrade = input.professionalEditorGrade === true;
  const engineDelivered =
    input.publishReady === true ||
    input.contentQualityDelivered === true ||
    input.llmDeliveryPolish === true ||
    input.bGradeFloor === true ||
    input.deliveryRescue === true ||
    input.deliveryPreview === true;
  ...
  if (score < 60 && !editorGrade && !engineDelivered) {
    return PUBLISH_GRADE_C;
  }
...
export function buildManuscriptStatusLines(axes = []) {
  ...
  return [
    ...
    },
    {
      id: "speaker",
      label: "화자·톤",
      quality: axisQualityLabel(byId.speaker?.score),
    },
  ];
}
