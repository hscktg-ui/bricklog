...
function enrichSentenceWithExplainAxis(sentence = "", input = {}) {
  ...
  const brand = String(input.brandName || "").trim();
  const reason = brand
    ? `${brand}에서 실제로 비교해 보면 ${topic}를 고를 때 기준이 달라집니다.`
    : `${topic}를 고를 때는 현장에서 어떤 포인트가 먼저 보이는지가 기준이 됩니다.`;
  const base = t.replace(/[.!?。]+$/, "").trim();
  return `${base} — ${reason}`;
}

export function repairExplainDefectSentence(sentence = "", input = {}) {
  const t = String(sentence || "").trim();
  if (!t) return "";
  if (!isExplainEngineDefect(t, input)) return t;
  if (isHollowInfoSentence(t)) {
    const topic = String(input.topic || input.mainKeyword || "이 주제").trim();
    return enrichSentenceWithExplainAxis(`${topic}를 처음 정리할 때`, input);
  }
  return enrichSentenceWithExplainAxis(t, input);
}
  ...
  const sections = pack.sections.map((sec) => {
    ...
    const repaired = paras
      .map((para) => {
        ...
        if (sents.length <= 1) {
          return repairExplainDefectSentence(para, input);
        }
        return sents
          .map((s) => repairExplainDefectSentence(s, input))
          .filter(Boolean)
