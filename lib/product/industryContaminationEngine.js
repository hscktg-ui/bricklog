...
export function resolveLockedIndustryKey(input = {}) {
  ...
  const unmannedFlower =
    /무인|24\s*시간/.test(
      `${input.brandDescription || ""} ${(input.researchFacts || []).map((f) => f.fact || f).join(" ")}`
    ) && /꽃|플라워|flower/.test(`${input.industry || ""} ${input.topic || ""}`);
  if (unmannedFlower) return "unmanned_flower";
  return input.industryKey || input.industry || "default";
}
