...
export function buildQualitySignals(pack = {}, meta = {}) {
  ...
  return {
    ...
    publishReady: Boolean(m.publishReady ?? meta.publishReady),
    columnistReady: Boolean(
      m.columnVisitNorthStarOk ?? meta.columnVisitNorthStarOk
    ),
    writerFirstDelivery: Boolean(m.writerFirstDelivery ?? meta.writerFirstDelivery),
    elapsedMs: meta.elapsedMs ?? m.elapsedMs ?? null,
  };
}
