 ...
 *
 * 100점 만점 · 90점 미만 출력 금지 (eval-v2-2026-08)
...
export const CONTENT_EVAL_VERSION = "eval-v2-2026-08";
export const CONTENT_EVAL_PASS_SCORE = 90;

/** AGENTS.md · Brand Content OS 평가 축과 동일 합계 100 */
...
const INDUSTRY_FITNESS_MARKERS = {
  flower: { minNames: 3, names: true },
  unmanned_flower: { minNames: 3, names: true },
  cafe: /메뉴|브런치|원두|라떼|에스프레소|음료|디저트|분위기|인테리어|테라스|좌석|커피/,
  tea_cafe: /메뉴|티|차|원두|라떼|음료|디저트|분위기|인테리어|좌석/,
  furniture: /체험|비교|쇼룸|누워|앉아|직접|시공|견적|매트리스|소파|모션|체어|가구/,
  pension: /객실|숙소|바베큐|수영장|온수풀|조식|체크인|뷰|테라스|펜션|예약/,
  lodging: /객실|숙소|체크인|조식|뷰|예약|스테이|호텔|리조트/,
  restaurant: /메뉴|코스|예약|맛|좌석|분위기|요리|식사|주차|웨이팅/,
  salon: /시술|커트|펌|염색|예약|상담|스타일|케어|두피|클리닉/,
  academy: /수업|커리큘럼|수강|강좌|코스|레벨|상담|선생님|학원|교재/,
  craft: /체험|만들기|공방|클래스|재료|키트|수강|예약|워크숍/,
  shop: /제품|재고|픽업|배송|가격|구성|패키지|매장|구매|주문/,
  interior: /시공|견적|공간|인테리어|자재|동선|마감|상담|현장/,
};
...
function assessIndustryFitnessContract(pack, input = {}) {
  ...
  const contract = INDUSTRY_FITNESS_MARKERS[key];
  if (contract?.names) {
    const hits = FLOWER_NAMES.filter((re) => re.test(full)).length;
    if (hits < (contract.minNames || 3)) reasons.push("industry_flower_names_missing");
  } else if (contract instanceof RegExp) {
    if (!contract.test(full)) {
      reasons.push(`industry_${key}_markers_missing`);
    }
  }
...
export function assessContentEvaluation(pack, input = {}) {
  ...
  if (!truncated.ok) {
    hardReasons.push(...truncated.reasons.filter((r) => !hardReasons.includes(r)));
    breakdown.humanVoice = 0;
  }

  let humanBelief = null;
  if (isHumanBeliefEnforced()) {
    humanBelief = scoreHumanBelief(full, { input }, pack);
    const beliefSevere =
      !humanBelief.ok &&
    ...
    if (beliefSevere) {
      hardReasons.push("human_belief_low");
      breakdown.humanVoice = 0;
    } else if (!humanBelief.ok) {
      breakdown.humanVoice = Math.min(
        breakdown.humanVoice ?? CONTENT_EVAL_WEIGHTS.humanVoice,
        scoreBucket(CONTENT_EVAL_WEIGHTS.humanVoice, humanBelief.score / 100)
      );
    }
  }
  ...
  if (isBriclogExperienceOpinionEnforced()) {
    ...
    if (!experienceQuality.ok && (experienceQuality.rate ?? 0) < 0.55) {
      hardReasons.push("experience_opinion_low");
    }
  }
  if (isBriclogExplainV3Enforced()) {
    ...
    if (!explainQuality.ok && explainQuality.rate < 0.35) {
      hardReasons.push("explain_quality_low");
    }
    // 설명률이 낮아도 hollow·keyword 누출이 없으면 밀도는 하한 65%로 보존
    const densityExplainRatio =
      explainQuality.hollow === 0 && explainQuality.keywordLeaks === 0
        ? Math.max(explainQuality.rate, 0.65)
        : explainQuality.rate;
    breakdown.informationDensity = Math.min(
      breakdown.informationDensity ?? CONTENT_EVAL_WEIGHTS.informationDensity,
      scoreBucket(CONTENT_EVAL_WEIGHTS.informationDensity, densityExplainRatio)
    );
  }
