...
function editorV95PolishLines(input = {}) {
  ...
  const facet = topicWritingFacet(input) || "선택";
  const key = resolveBriclogIndustryKey(input);
  const flowerRec = /꽃|flower/i.test(`${input.industry || ""} ${input.topic || ""}`);
  if (flowerRec || key === "flower" || key === "unmanned_flower") {
    return [
      `${facet} 볼 때 꽃 이름·포장 톤·픽업 시간 순으로 보면 덜 헷갈려요.`,
      `직접 확인해 보니 색감과 줄기 상태 기준이 먼저 정리됐어요.`,
      `비교해 보니 가격보다 보관·배송 시간이 먼저 걸렸어요.`,
      `${brand}에서 고른 기준을 짧게 메모해 두었어요.`,
    ];
  }
  return [
...
export function polishEditorV95Delivery(pack, ctx = {}, input = {}) {
  ...
  const llmPack =
    /^llm_/.test(mode) ||
    Boolean(pack?._meta?.briclogWriterEngine || pack?._meta?.gpt55LlmBodyPreserved);
  if (isGpt55WriterDominant() && llmPack) {
    return stripCatalogContaminationFromBlogPack({
      ...next,
      _meta: {
        ...(next._meta || {}),
        editorV95CatalogInjectionSkipped: true,
      },
    });
  }
...
export function scoreEditorV95(pack, ctx = {}, input = {}) {
  ...
  const editorPass =
    (score >= 68 &&
      ...
      humanity.ok &&
      (belief.score >= 50 || (score >= 74 && belief.score >= 40)) &&
      !beliefHard) ||
