...
export function resolveIndustryCategoryKey(input = {}) {
  ...
  if (/화장품|세럼|크림|스킨케어|토너|앰플|코스메틱|뷰티|beauty/.test(blob) || /^(화장품|뷰티|beauty)/i.test(industryField)) {
    return "beauty";
  }
  if (/공기청정|블렌더|청소기|에어프라이|전기포트|소형가전|가전제품|appliance/.test(blob) || /^(가전|소형가전|appliance)/i.test(industryField)) {
    return "appliance";
  }
