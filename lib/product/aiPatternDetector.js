...
import { V4_AI_CLICHES } from "@/lib/quality/aiClicheConstants";
...
export function detectAiWritingPatterns(pack, input = {}) {
  ...
  const templateSpam = assessTemplateBoilerplateSpam(pack);
  if (!templateSpam.ok) {
    for (const issue of templateSpam.issues.slice(0, 4)) {
      hits.push({
  ...
  const block =
    ...
    uniqueTypes.has("template_boilerplate");
