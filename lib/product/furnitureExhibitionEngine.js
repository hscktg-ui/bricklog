...
import { isFieldReviewSpeaker, buildSpeakerAlignedTitle } from "@/lib/persona/speakerVoiceLock";
...
export function stripAntiSeoPlaceholderLeaks(text = "", input = {}) {
  ...
  if (ANTI_SEO_PLACEHOLDER_RES.some((rx) => rx.test(t))) {
    t = t
      ...
      .replace(re("생활권\\s*이곳", "g"), region || "현장")
...
export function rewriteFurnitureExhibitionHeading(heading, input = {}) {
  ...
  if (UNNATURAL_TITLE_HEADING_RES.some((rx) => rx.test(h))) {
    if (!isFieldReviewSpeaker(input)) {
      return buildSpeakerAlignedTitle(input, "brand_editor");
    }
...
export function applyFurnitureExhibitionPackPolish(pack, input = {}) {
  ...
  for (const sec of pack.sections) {
    ...
    if (narrative.length === 0 && isOpimoTopic(input) && isFieldReviewSpeaker(input) && !VISIT_SCENE_INLINE_RES.test(body)) {
  ...
  if (narrative.length < 2) {
    narrative.push({
      heading: isFieldReviewSpeaker(input)
        ? `${p.regionBit}${p.brand} 쇼룸, 전시 구성을 본 순서`
        : `${p.regionBit}${p.brand} 전시 구성·안내`,
      body: isFieldReviewSpeaker(input)
        ? `${p.regionBit}${p.brand} ${flavor.spaceWord}에서 전시대·체험 동선부터 확인했어요. 프레임·구성 차이는 사진보다 직접 보는 편이 빨라요.`
        : `${p.regionBit}${p.brand} ${flavor.spaceWord}에서 ${p.topicFacet || "전시"} 구성과 체험 포인트를 정리했습니다. 방문 전에 확인할 항목을 미리 보면 비교가 수월합니다.`,
    });
  }

  if (isExhibitionTopic(input) && narrative[0]) {
    const topicBit = OPIMO_TOPIC_INLINE_RES.test(`${input.topic || ""}`) ? "오피모 전시" : p.topicFacet || "전시";
    narrative[0].heading = isFieldReviewSpeaker(input)
      ? `${p.regionBit}${p.brand} ${topicBit}, 쇼룸에서 직접 본 점`
      : `${p.regionBit}${p.brand} ${topicBit} 안내`;
  }
  ...
  return {
    ...pack,
    title: !isFieldReviewSpeaker(input) && /직접|다녀|후기|솔직/.test(String(pack.title || ""))
      ? buildSpeakerAlignedTitle(input, "brand_editor")
      : pack.title,
    representativeTitle: !isFieldReviewSpeaker(input) && /직접|다녀|후기|솔직/.test(String(pack.representativeTitle || pack.title || ""))
      ? buildSpeakerAlignedTitle(input, "brand_editor")
...
export function buildFurnitureExhibitionWriterBrief(input = {}) {
  ...
  const field = isFieldReviewSpeaker(input);
  return [
    "【가구·전시 · FURNITURE EXHIBITION ENGINE】",
    field
      ? "제목·도입: 지역+브랜드+직접 다녀온 후기·전시 현장. 「~이 궁금하다면」「봐야 할 기준」형 부제 금지."
      : "제목·도입: 선택 화자(브랜드/정보형) 톤 유지. 「직접 다녀온 후기」「방문 후기」 표현 금지.",
    field
      ? "본문 비중: 전시대·쇼룸에서 본 구성·동선·체험·느낌(1인칭) 70% 이상."
      : "본문: 전시·구성·안내 정보 중심. 방문자 후기·체험 구어 톤 금지.",
    "예약·A/S·설치·배송·할인을 섹션으로 나열하지 말 것.",
