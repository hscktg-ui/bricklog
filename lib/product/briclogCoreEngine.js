...
import {
  assessCoreRulesDelivery,
  stampCoreRulesOnDelivery,
  stampCoreRulesOnInput,
} from "@/lib/product/briclogCoreRules";
...
const MANIFEST_LAYER_VERSIONS = Object.freeze({
  ...
  channel_quality: "v1",
  core1_human_writing: "core-rules-v1",
  core2_brand_memory: "core-rules-v1",
...
export const BRICLOG_PROCESS_MANIFEST = Object.freeze([
  ...
  { layer: "core1_human_writing", version: MANIFEST_LAYER_VERSIONS.core1_human_writing, role: "코어1 — 사람이 쓴 것 같은 글" },
  { layer: "core2_brand_memory", version: MANIFEST_LAYER_VERSIONS.core2_brand_memory, role: "코어2 — 브랜드 피드백 기억" },
...
export const BRICLOG_DEFENSIBILITY_PILLARS = Object.freeze([
  ...
  {
    id: "core1_human_writing",
    ko: "코어1 — 사람이 쓴 것 같은 잘 쓰인 글",
    en: "Core1 — human-readable, trusted prose (belief + SQV)",
  },
  {
    id: "core2_brand_memory",
    ko: "코어2 — 브랜드별 피드백·습관 기억",
    en: "Core2 — per-brand feedback memory on every generation",
  },
  {
    id: "channel_os",
...
export function assessCoreEngineDelivery(pack, input = {}, channel = "blog", ctx = {}) {
  ...
  const coreRules = assessCoreRulesDelivery(pack, input, channel);

  const composite =
    Math.round(
      brandOs.score * 0.38 +
        corePublishScore * 0.28 +
        (channelScore?.score ?? brandOs.score) * 0.18 +
        (coreRules.compositeScore ?? 0) * 0.16
    ) || brandOs.score;
...
export function stampCoreEngineDeliveryMeta(pack, input = {}, channel = "blog", ctx = {}) {
  ...
  const stamped = stampCoreRulesOnDelivery(pack, input, channel);
  ...
  return {
    ...
    _meta: {
      ...
      coreEngine: {
        ...
        coreRulesScore: assessment.coreRules?.compositeScore,
        core1Pass: assessment.coreRules?.core1?.ok,
        core2Applied: assessment.coreRules?.core2?.applied,
        integrityToken: deliveryIntegrityToken(stamped, input, assessment),
