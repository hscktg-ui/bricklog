...
function factLines(facts, ink, T, catalog = false) {
  ...
  if (catalog === "listing") {
    return facts
      .map(
        ([k, v], i) =>
          `<p data-role="list-line" style="margin:${i ? 22 : 0}px 0 0;font-size:28px;line-height:1.2;word-break:keep-all;color:${esc(ink)};font-family:${T.familyDisplay};">${esc(v)}${k && !String(v).includes(k) ? `<span style="display:block;margin-top:6px;font-size:13px;letter-spacing:0.02em;opacity:.55;font-family:${T.familyBody};">${esc(k)}</span>` : ""}</p>`
      )
      .join("");
  }
  if (catalog) {
...
function composeFrame(frame, T, eager) {
  ...
  if (c === "editorial" || c === "product_left") {
    ...
    layers = `${photoLayer(frame, eager)}
      ${veil("left")}
      <div style="position:absolute;top:72px;left:44px;width:300px;color:${esc(ink)};">${typeBlock(frame, T, `<div style="margin-top:8px;">${factLines(frame.facts, ink, T, "listing")}</div>`)}</div>`;
  ...
  const priceMark = /\d/.test(frame.headline) && /원/.test(frame.headline)
    ? ' data-price="1"'
    : "";
  const close = frame.type === "cta" || frame.beat === "notice" ? ' data-cta="close"' : "";
  const beat = frame.beat ? ` data-mall-beat="${esc(frame.beat)}"` : "";

  return `<section data-section="${esc(frame.type)}" data-canvas="${frame.n}" data-layout="ad-canvas" data-composition="${esc(c)}" data-purpose="${esc(frame.purpose)}"${beat} data-image-ratio="${esc(String(cov))}" data-image-coverage="${esc(String(cov))}" data-ui-elements="0" data-photo-direction="${esc(frame.photoDirection || "")}"${priceMark}${close} style="position:relative;width:${w}px;height:${h}px;overflow:hidden;background:${esc(paper)};margin:0;padding:0;">
    ${layers}
  </section>`;
}
