  ...
  const pos =
    frame.composition === "macro_crop"
      ...
      : frame.composition === "asymmetric"
        ? "70% 50%"
        : "center";
  const scale = frame.composition === "macro_crop" ? 1.08 : 1;
...
function typeBlock(frame, T, extra = "") {
  const kicker = frame.kicker
    ? `<p data-role="kicker" style="margin:0 0 14px;font-size:13px;line-height:1.45;letter-spacing:0.08em;font-weight:500;color:${esc(frame.ink)};font-family:${T.familyBody};">${esc(frame.kicker)}</p>`
    : "";
  const size =
...
function factLines(facts, ink, T) {
  if (!facts?.length) return "";
  return facts
    .map(
      ([k, v], i) =>
        `<p style="margin:${i ? 18 : 0}px 0 0;font-size:15px;line-height:1.55;word-break:keep-all;color:${esc(ink)};font-family:${T.familyBody};"><span style="display:block;font-size:12px;letter-spacing:0.06em;opacity:.62;margin-bottom:4px;">${esc(k)}</span>${esc(v)}</p>`
    )
    .join("");
}
...
function composeFrame(frame, T, eager) {
  ...
  if (c === "editorial" || c === "product_left") {
    layers = `${photoLayer(frame, eager)}
      <div style="position:absolute;top:0;right:0;bottom:0;width:42%;background:${esc(paper)};"></div>
      <div style="position:absolute;top:72px;right:40px;width:280px;color:${esc(ink)};">${typeBlock(frame, T, factLines(frame.facts, ink, T))}</div>`;
    ...
    layers = `<div style="position:absolute;inset:0;background:${esc(paper)};"></div>
      ${photoLayer(frame, eager, "inset:0", "opacity:.28;")}
      <div style="position:absolute;top:64px;left:48px;right:48px;color:${esc(ink)};">
        ${typeBlock(frame, T)}
        <div style="margin-top:36px;max-width:520px;">${factLines(frame.facts, ink, T)}</div>
      </div>`;
    ...
    layers = `${photoLayer(frame, eager)}
      ${veil("bottom")}
      <div style="position:absolute;left:48px;right:48px;bottom:56px;text-align:center;color:${esc(ink)};">${typeBlock(frame, T)}</div>`;
