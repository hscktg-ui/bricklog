...
export function shouldAllowMissionUiRescue(input = {}) {
  if (isLaunchPublishFirstMode()) return true;
...
export function isCustomerSafeChannelPack(pack, channel = "place") {
  ...
  if (pack._meta?.generationMode === "form_proxy") return false;
...
export function enforceCustomerChannelOutput(pack, channel, input = {}) {
  ...
  if (isLaunchPublishFirstMode()) {
    const cleaned =
      channel === "place"
        ? humanizePlaceNoticePack(pack, input)
        : channel === "instagram"
          ? pack
          : pack;
    return { ok: true, withheld: false, pack: cleaned, northStar: null };
  }
...
export function enforceCustomerBlogOutput(pack, input = {}) {
  ...
  if (
    ...
    (pack._meta?.missionProseFallback ||
      ...
      pack._meta?.generationMode === "form_proxy")
  ) {
    return {
      ok: false,
      withheld: true,
