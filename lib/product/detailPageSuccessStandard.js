...
export const DETAIL_PAGE_SUCCESS_VERSION = "gollaboda-success-v1";
export const DETAIL_PAGE_SUCCESS_PASS_SCORE = 80;
export const DETAIL_PAGE_SUCCESS_PANEL_MIN = 70;
...
export const DETAIL_PAGE_KOREA_FIRST = Object.freeze({
  ...
  notHow: ["AI 이미지 생성", "가짜 모델컷", "9몰 API"],
});
...
export const DETAIL_PAGE_SUCCESS_PHASES = Object.freeze([
  ...
  },
  {
    ...
    label: "검수",
    axis: "panel",
    meaning: "860px·카테고리 한글·영문 글꼴·약한 CTA·위계 — 디자이너 30인이 화면으로 본다",
    designerLenses: [
      "hierarchy",
      "type",
      "photo",
...
function explainScore(standard, measured) {
  const parts = [];
  if (standard?.rules?.explain) parts.push(78);
  else parts.push(42);
  if (standard?.rules?.observe_opinion) parts.push(82);
  else parts.push(48);
...
export function assessDetailPageSuccess(args = {}) {
  ...
  const panel = evaluateDetailPageDesignerPanel({ pack, html, photoCount });
