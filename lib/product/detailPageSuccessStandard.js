...
import { evaluateDetailPageMallMdPanel } from "@/lib/qa/detailPageMallMdPanel20";
import { inspectDetailPageFacts } from "@/lib/product/detailPageFactDossier";
...
export const DETAIL_PAGE_KOREA_FIRST = Object.freeze({
  ...
  notHow: ["가짜 모델컷", "9몰 API", "한글 상세페이지 통이미지", "원샷 생성"],
});
...
export const DETAIL_PAGE_SUCCESS_HARD_GATES = Object.freeze([
  ...
  {
    id: "planned_generation",
    label: "기획 먼저",
    fail: "한글 통이미지 one-shot. 기획 없이 이미지 모델에 상세페이지를 시키지 않는다",
  },
]);
...
function packFollowsPlannedGeneration(pack, html = "") {
  const pipe = pack?._meta?.pipeline;
  if (!pipe) return false;
  if (pipe.oneShot === true) return false;
  if (pipe.koreanOnPageNotInPhoto !== true) return false;
  if (String(html).includes('data-korean-in-image="1"')) return false;
  if (String(html).includes('data-pipeline="oneshot"')) return false;
  return true;
}
...
export function assessDetailPageSuccess(args = {}) {
  ...
  const mdPanel = evaluateDetailPageMallMdPanel({
    pack,
    html,
  ...
  if (!packFollowsPlannedGeneration(pack, html)) hard.push("planned_generation");
  ...
  return {
    ...
    mdPanel: {
      mean: mdPanel.summary.mean,
      passCount: mdPanel.summary.passCount,
      n: mdPanel.summary.n,
      hire: mdPanel.summary.hire,
      hireLabel: mdPanel.summary.hireLabel,
      vetoes: mdPanel.summary.vetoes,
    },
    imageConcept: inspectDetailPageImageConcept({
      photos: pack.photos || input.photos || [],
      html,
      input,
    }),
    measured: {
