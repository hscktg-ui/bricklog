...
export async function ensureServerResearchHydrated(input = {}) {
  ...
  const { ensureServerAxisResearch } = await import("@/lib/generation/serverAxisResearch");
  const axis = await ensureServerAxisResearch(input);
  if (axis.ok && axis.input?.v2ResearchReady) {
    return {
      ...axis.input,
      _meta: {
        ...(input._meta || {}),
        serverAxisResearch: true,
        masterRebuildVersion: MASTER_REBUILD_VERSION,
      },
    };
  }
