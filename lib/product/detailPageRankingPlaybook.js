 ...
 * 리스트(후커블·크리에이지·드랩·젠시)가 히트상품에서 가져가는 화면 순서만 쓴다.
 * 가짜 후기·모델컷·통이미지·9몰은 가져오지 않는다.
 */
export const DETAIL_PAGE_RANKING_PLAYBOOK_VERSION = "detail-ranking-playbook-v1";

/** 랭킹 상세·리스트 데모에서 반복되는 화면 순서. */
export const DETAIL_PAGE_RANKING_SEQUENCE = Object.freeze([
  { slot: "hero", they: "히어로 배너", we: "포장 앞면 + 상품명" },
  { slot: "intent", they: "고민 솔루션", we: "고를 때 막히는 점" },
  { slot: "explain", they: "핵심 소구점 5 Points", we: "막히는 점 / 먼저 볼 것" },
  { slot: "usp", they: "USP 카드", we: "짧은 이름 카드" },
  { slot: "observe", they: "제품 가까이·갤러리", we: "손에 쥐거나 가까이 컷" },
  { slot: "feature", they: "디테일·비교", we: "디테일 한 점" },
  { slot: "scene", they: "사용·조립 가이드", we: "꺼내 쓰는 장면" },
  { slot: "spec", they: "SPEC 표", we: "입력된 항목만 표" },
...
export const DETAIL_PAGE_LIST_TOOL_USE = Object.freeze([
  ...
  },
  {
    ...
    take: "히어로→막히는 점→포인트→USP→컷→장면→스펙 리듬",
    leave: "가짜 포토리뷰, 통이미지, 이벤트 애니메이션",
  },
  {
    id: "draph",
    they: "드랩아트",
...
export function formatRankingPlaybookForPrompt() {
  ...
  return [
    "스마트스토어 상위 상세 리듬(조사·리스트 대조). 화면 순서만 따른다.",
    seq,
    tools,
    "금지: 가짜 후기·별점·없는 인증·지금 바로 구매·모델컷·통이미지 카피.",
  ].join("\n");
}
