...
export function assessCompletionReadiness(pack, input = {}, gate = {}) {
  ...
  if (isBriclogMissionEnforced() && !visitTemplate.ok) {
    reasons.push("visit_review_template_contamination");
  }
  ...
  const grounded =
    hbMeta?.grounded ||
    scoreGroundedSpecificity(pack, { input, ...input }, mergedFacts);
  const factCount = mergedFacts.length;
  if (
  ...
  if (isBriclogMissionEnforced() && factCount >= 2 && !grounded.ok) {
    ...
    const onlyRatioLow =
      (grounded.issues || []).length === 1 &&
      grounded.issues[0] === "grounding_ratio_low";
    if (!(researchBuilt && grounded.factAnchors >= 2 && onlyRatioLow)) {
  ...
  if (isBriclogMissionEnforced()) {
    ...
    if (dl.total < DEEP_LEARNING_MIN_SCORE) {
      if (!(researchGroundedTrust && dl.total >= DEEP_LEARNING_MIN_SCORE - 18)) {
        reasons.push("deep_learning_below_min");
      }
    }
  }
...
export function isPreviewWithholdFailure(gate = {}, pack = null, input = {}) {
  if (pack?.sections?.length && isBriclogMissionEnforced()) {
    const full = getBlogFullText(pack);
    if (!scoreChecklistVoice(full, pack).ok) return true;
    if (!detectVisitReviewTemplateContamination(pack, input).ok) return true;
    if (!scoreHumanBelief(full, input, pack).ok) return true;
  }
  ...
  if (pack?.sections?.length) {
    ...
    if (substantiveSections < 3 && previewable < minPreviewable) return true;

    if (reasons.some((r) => PREVIEW_WITHHOLD_REASONS.has(r))) return true;
    if (reasons.some((r) => CRITICAL_PREVIEW_ONLY.has(r))) return true;
    return false;
  }
