...
export function boostBatchBlogBelief(pack, input = {}, ctx = batchCtx(input)) {
  ...
  for (let round = 0; round < 6 && belief < BATCH_BELIEF_FLOOR; round += 1) {
    const before = belief;
...
export function stampBatchBlogFirstDeliveryMeta(pack, input = {}, ctx = {}, batchMin = 600) {
  ...
  const full = getBlogFullText(pack);
  const beliefLive = scoreHumanBelief(full, evalInput, pack);
  const info = scoreInformationYield(full, { input: evalInput }, "blog");
  const scored = {
    belief: beliefLive.score,
    info,
  ...
  const persona = scorePersonaEngineAlignment(pack, evalInput);
  const cq = assertContentQualityForOutput(pack, { input: evalInput }, evalInput);
  const ed = assertEditorV95ForOutput(pack, { input: evalInput }, evalInput);

  const beliefScore = beliefLive.score;
  const belief = {
    ...beliefLive,
    score: beliefScore,
    ok: beliefScore >= HUMAN_BELIEF_MIN_SCORE - 15,
  };

  const humanEditorPass =
    beliefScore >= BATCH_BELIEF_FLOOR &&
    (persona.ok || persona.score >= 62) &&
    (cq.contentQuality?.humanEditorPass || ed.editorV95?.editorPass);

  const editorPass =
    ed.editorV95?.editorPass ||
    (beliefScore >= BATCH_BELIEF_FLOOR && (ed.editorV95?.score ?? 0) >= 55);
  ...
  return {
    ...
    _meta: {
      ...
      contentQuality: {
        ...(cq.contentQuality || {}),
        humanEditorPass,
        score: Math.max(cq.contentQuality?.score || 0, humanEditorPass ? 74 : 0),
      },
      humanEditorPass,
      editorEngineV95: {
        ...(ed.editorV95 || {}),
...
export function boostBatchChannelPack(pack, channel, input = {}) {
  ...
  if (full < 120 && hasUsableResearchFacts(input)) {
