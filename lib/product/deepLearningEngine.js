...
const INDUSTRY_SENSE = {
  ...
  craft: {
    required: /체험|클래스|준비물|난이도|완성품|예약|소요\s*시간/,
    forbidden: /매트리스|쇼룸|진료|학원\s*입시|급여\s*방법/,
  },
  education: {
    required: /특강|학년|반\s*편성|등록|내신|수업|상담/,
    forbidden: /매트리스|쇼룸|급여\s*방법|원재료\s*성분/,
  },
};
...
const CUSTOMER_SITUATIONS = {
  ...
  craft: [
    "원데이 클래스 난이도가 걱정된다",
    "준비물과 복장 규정을 미리 알고 싶다",
    "완성품 수령 시점이 궁금하다",
    "예약·취소 규정을 확인하고 싶다",
  ],
  education: [
    "방학 특강 일정이 기존 학원과 겹친다",
    "대상 학년·반 편성을 먼저 확인하고 싶다",
    "내신 대비 범위가 맞는지 상담이 필요하다",
  ],
  default: [
...
export function scoreDeepLearning(pack, input = {}) {
  ...
  const researchFacts = collectMergedResearchFacts(input);
