...
function applyResearchGroundedTrustPolish(pack, input = {}) {
  ...
  if (hasUsableResearchFacts(input) && bodyChars < targetMin) {
    const researchLines = buildResearchFactLines(input, 12);
  ...
  next = stripNearDuplicateSentencesGlobally(next, 0.64);
  next = ensureMinBlogSections(next, { input }, input, 3);
  if (resolvePersonaEngineProfile(input).archetype === "essay") {
    const essayMin = resolveEffectiveBlogLengthMin(next, input);
    if (countBlogBodyCharsWithSpaces(next) < essayMin) {
      next = deepenDensityFirstPack(next, essayMin, input, {
        polishAfter: false,
        seedOffset: 0,
        researchLines: [],
      });
...
export function finalizeContentQualityForDelivery(
  ...
  if (
    ...
    let next = pack;
    if (channel === "blog" && isBriclogMissionEnforced() && !pack._meta?.postLengthDeliveryPolish) {
      next = applyPostLengthDeliveryPolish(pack, input);
    }
    const sqv = next._meta?.sqv || computeContentQualityValue(next, input);
