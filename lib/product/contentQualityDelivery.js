...
function isLlmOriginatedPack(pack) {
  const mode = String(pack?._meta?.generationMode || "");
  return pack?._meta?.llmGenerated === true || mode === "llm" || mode.startsWith("llm_");
}
...
export function finalizeContentQualityForDelivery(pack, input = {}, channel = "blog") {
  ...
  const goldenPublishOk = resolveGoldenPublishOk(goldenGate);
  const publishReady =
