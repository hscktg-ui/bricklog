...
import { applyHumanColumnPolish, dedupeSectionHeadingsOnPack } from "@/lib/content/humanColumnPolishEngine";
...
import { enrichRegionDensity, scoreRegionDensity } from "@/lib/content/regionDensityEngine";
...
import { dedupeMissionPackGlobally } from "@/lib/llm/missionProseFallback";
...
function applyResearchGroundedTrustPolish(pack, input = {}) {
  if (!isResearchGroundedDeliveryPack(pack, input)) return pack;
  ...
  next = dedupeMissionPackGlobally(next);
  next = stripNearDuplicateSentencesGlobally(next, 0.64);
  next = stripAdjacentDupesOnPack(next, input);
  next = applyDuplicateKiller(next, { input, ...input }, "blog");
  ...
  for (let round = 0; round < 2; round += 1) {
    const dup = detectDuplicateKillerIssues(getBlogFullText(next));
    if (dup.ok && !hasDuplicateSentences(getBlogFullText(next), 16)) break;
    next = dedupeMissionPackGlobally(next);
    next = stripNearDuplicateSentencesGlobally(next, 0.64);
    next = applyDuplicateKiller(next, { input, ...input }, "blog");
    next = stripGlobalExactDuplicateSentences(next);
  ...
  if (hasUsableResearchFacts(input) && bodyChars < targetMin) {
    const researchLines =
      profile.archetype === "essay" ? [] : buildResearchFactLines(input, 12);
  ...
  next = ensureResearchGroundedPackStructure(next, input);
  next = dedupeMissionPackGlobally(next);
  next = stripNearDuplicateSentencesGlobally(next, 0.64);
  next = stripGlobalExactDuplicateSentences(next);
  next = ensureResearchGroundedPackStructure(next, input);
  ...
  if (!regionScore.ok && !regionScore.skipped && regionScore.tooSparse) {
    next = enrichRegionDensity(next, regionCtx, "blog");
    next = weaveTopicDominanceIntoPack(next, { input, ...input });
  }
  const targetMinFinal = resolveEffectiveBlogLengthMin(next, input);
  if (hasUsableResearchFacts(input) && countBlogBodyCharsWithSpaces(next) < targetMinFinal) {
    next = ensureResearchGroundedPackStructure(next, input);
