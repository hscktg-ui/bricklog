...
import { applyKoreanHumanPioneerPass } from "@/lib/product/koreanHumanPioneerEngine";
...
export function finalizeContentQualityForDelivery(
  ...
  if (
    ...
    if (channel === "blog" && (hasUsableResearchFacts(input) || next._meta?.researchGroundedTrustPolish)) {
      next = finalizeResearchGroundedGateRepair(next, input);
    }
