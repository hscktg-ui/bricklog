...
function withCoreEngineDeliveryMeta(pack, input, channel, ctx = {}) {
  ...
  try {
    return stampCoreEngineDeliveryMeta(pack, input, channel, ctx);
  } catch {
    return pack;
  }
}
...
function finalizeMissionCatalogHumanityPass(pack, input = {}) {
  ...
  if (!isFieldReviewSpeaker(input) && !industryColumn) {
    ...
    next = repairThinSectionsAfterVoiceLock(next, input);
  } else if (isFieldReviewSpeaker(input)) {
    const title = String(next.title || next.representativeTitle || "").trim();
    if (!/직접\s*보/g.test(title)) {
      const topicLine = String(input.topic || input.mainKeyword || "").trim();
      const brand = String(input.brandName || "").trim();
...
function finalizeMissionCatalogDelivery(pack, input = {}, inbound, inboundChars) {
  ...
  if (isBriclogResetQualityEnforced()) {
    ...
    if (
      ...
      contentEvaluation?.hardFail !== true
    ) {
  ...
  const resetBlocksPublish =
    resetQualityGate?.hardFail === true ||
  ...
  const publishReady =
    ...
    !resetBlocksPublish &&
    (!isBriclogResetQualityEnforced() || customerPerspective.pass);
  ...
  return withCoreEngineDeliveryMeta(
    {
      ...
      _meta: {
        ...
        sqv: { ...sqv, publishReady: finalReady },
...
export function finalizeContentQualityForDelivery(
  ...
  if (
    ...
    const sqv = pack._meta?.sqv || computeContentQualityValue(pack, input);
    return withCoreEngineDeliveryMeta(
      {
        ...pack,
        _meta: {
  ...
  if (channel === "blog" && isMissionCatalogDeliveryPack(pack, input)) {
    return withCoreEngineDeliveryMeta(
      finalizeMissionCatalogDelivery(pack, input, inbound, inboundChars),
      input,
      channel,
      { missionCatalog: true },
    );
  ...
  if (channel === "blog") {
    ...
    const llmWriterPack =
      isLlmOriginatedPack(next) || isWriterEngineExpandedPack(next);
    const llmExpandTarget = llmDeliveryExpandFloor(input, inbound);
    ...
    if (isLlmOriginatedPack(next) || isWriterEngineExpandedPack(next)) {
      next = stripCatalogContaminationFromBlogPack(next);
