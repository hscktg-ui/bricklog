...
export function finalizeContentQualityForDelivery(
  ...
  if (channel === "blog" && isBriclogResetQualityEnforced()) {
    ...
    const evalResult = evaluateReviseAndGateOutput(next, input, { allowRevise: true });
    ...
    if (evalResult.outputAllowed === false || resetQualityGate.shouldWithhold) {
      publishReady = false;
    }
  ...
  return {
    ...
    _meta: {
      ...
      resetQualityWithheld: resetQualityGate?.shouldWithhold || undefined,
