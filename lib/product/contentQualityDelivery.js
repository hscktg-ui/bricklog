...
/** GPT-5.6 원고 — Mission·Experience 카탈로그 패딩 금지 */
...
function applyResearchGroundedFinalGatePolish(pack, input = {}, gateCtx = {}) {
  ...
  if (isBriclogResetQualityEnforced()) {
    ...
    if (
      contentEvaluation &&
      (!contentEvaluation.pass || contentEvaluation.hardFail) &&
      !next._meta?.paragraphSafeEdit
    ) {
      const revised = applyParagraphSafeEdit(next, input, contentEvaluation);
      next = revised;
      contentEvaluation = assessContentEvaluation(next, input);
      resetQualityGate = assessBriclogResetQualityGate(next, input);
      evalOutputAllowed = contentEvaluation.pass === true && !resetQualityGate.shouldWithhold;
      next = {
        ...next,
        _meta: {
          ...(next._meta || {}),
