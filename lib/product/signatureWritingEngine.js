...
function signatureTopicLabel(ctx = {}) {
  const raw = String(ctx.topic || ctx.mainKeyword || "").trim();
  const facet = topicWritingFacet(ctx) || raw;
  return String(facet || raw)
    .replace(/\s*관련\s*$/i, "")
    .trim();
}
...
export function rewriteSignatureHeading(heading, ctx = {}) {
  ...
  if (topic) return `왜 ${koreanObjectParticle(topic)} 찾게 되는가`;
  return "왜 지금 이 주제를 찾게 되는가";
}
