...
const CHECKLIST_WHY_HEADING_RE =
  /찾게\s*되는가|찾게\s*되었는가|^왜\s+.+\s*찾게|검색하게\s*되는|왜\s+지금\s+검색/;

/** 가이드·체크리스트 톤 없는 자연 소제목 */
export function buildNaturalConcernHeading(ctx = {}) {
  if (isFlowerRecommendationTopic(ctx)) {
    return buildFlowerRecommendationSectionHeadings(ctx)[0];
  }
  const topic = signatureTopicLabel(ctx);
  const brand = String(ctx.brandName || "").trim();
  const region = String(ctx.region || "").trim();
  const profile = resolvePersonaEngineProfile(ctx);
  if (profile.archetype !== "field_review") {
    if (brand && topic) {
      return region
        ? `${topic}, ${brand} 안내에서 짚을 포인트`
        : `${brand} ${topic} — 비교 기준`;
    }
    if (topic) return `${topic} — 확인할 항목`;
    return "비교할 때 막히는 지점";
  }
  if (brand && topic) {
    return region
      ? `${topic}, ${region} ${brand}에서 직접 본 이야기`
      : `${brand}에서 ${topic} — 직접 본 이야기`;
  }
  if (topic) return `${topic} — 직접 가보니 달랐던 점`;
  return "비교할 때 막히는 지점";
}
