 ...
 */
import { fixBrandJosa, fixBrokenCompoundJosa } from "@/lib/korean/josaFix";
...
export function topicCapAltPhrase(input = {}) {
  ...
  return facet && facet.length >= 2 && facet !== "이용" ? facet : "매장 안내";
}
...
export function sanitizeMissionSentence(text = "", input = {}) {
  let s = stripMissionOutputMeta(String(text || "").trim()).slice(0, 4000);
  if (!s) return "";
  ...
  try {
    ...
    s = s.replace(/(확인해\s*){2,}/g, "확인해 ");
    s = s.replace(/구성\s+구성/g, "구성");
    s = s.replace(/컬러를에/g, "컬러에");
    s = s.replace(/추천를/g, "추천을");
    s = s.replace(/\s*\([^)]*기준\)\s*/g, " ");
    s = s.replace(/✔\s*/g, "");
    s = s.replace(/이번\s*전시\s*을/g, "전시 일정을");
    s = s.replace(/메뉴\s*기준/g, "선택 기준");
    s = s.replace(/입력·공개\s*맥락[^.\n]*\.?/g, "");
    s = s.replace(/조건\s*조건/g, "조건");
    s = s.replace(/모집를/g, "모집을");
    s = s.replace(/커리큘럼와/g, "커리큘럼과");
    s = s.replace(/([가-힣]{2,})\s*모집를/g, "$1 모집을");
    s = s.replace(/특강\s*모집를/g, "특강 모집을");
    s = s.replace(/착와감/g, "착좌감");
    s = s.replace(/동선와\s*재질을/g, "동선과 재질");
    s = s.replace(/비교해\s*보니\s*기준이\s*보였어요/g, "");
    s = s.replace(/정리해\s*보았어요/g, "");
    s = s.replace(/하나씩\s*비교해\s*봤어요/g, "");
    s = s.replace(/\s+\./g, ".");
    const brand =
      String(input?.brandName || deriveTopicWritingContext(input).brand || "").trim().slice(0, 48);
    s = fixBrandJosa(s, brand);
    s = fixBrokenCompoundJosa(s);
  } catch {
...
export function isMissionOutputDefectSentence(text = "", input = {}) {
  ...
  if (/입력·공개\s*맥락/.test(t)) return true;
  if (/응대을|이용를|쇼룸를|매장를/.test(t)) return true;
  ...
  if (isDisplayBodyForbidden(t, input)) return true;

  if (isInformationalTopicInput(input)) {
    if (/당일\s*상담\s*메모|누워보니|직접\s*다녀온|솔직\s*후기|향와|질감와|성분와|확인해\s*확인해/.test(t)) {
      return true;
    }
  }
...
export function applyMissionOutputSanitizerPack(pack, input = {}, options = {}) {
  ...
  const minKeep = pack?._meta?.researchGroundedHumanPack ? 4 : 2;
  let safeSections = sections.filter((s) => String(s.body || "").replace(/\s/g, "").length >= 20);
  if (safeSections.length < minKeep) {
    safeSections = sections.filter((s) => String(s.body || "").replace(/\s/g, "").length >= 12);
  }
  if (safeSections.length < minKeep) {
    safeSections = sections.filter((s) => String(s.body || "").trim());
  }
