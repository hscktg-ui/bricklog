...
} from "@/lib/product/naturalVoiceDelivery";
...
export function assessHumanColumnContract(pack, input = {}) {
  ...
  const defaultEffectiveMin =
    catalogPack || editorPass || pack?._meta?.contentQualityDelivered
      ...
      : tier.min;
  const customerDeliveryMin = resolveCustomerDeliveryBlogMinChars(tierKey, tier);
  const naturalVoiceFirst = shouldPreferNaturalnessOverDensity(pack, input);
  const effectiveMin = naturalVoiceFirst
    ? resolveNaturalVoiceDeliveryMinChars(tierKey, tier)
    : localBatchFinish
      ? resolveLocalBatchBlogMinChars(tierKey, tier)
      : missionPublishReady
        ? Math.min(defaultEffectiveMin, customerDeliveryMin)
        : defaultEffectiveMin;
