...
export function assessHumanColumnContract(pack, input = {}) {
  ...
  const effectiveMin = catalogPack ? resolveMissionCatalogMinChars(input) : tier.min;
  ...
  const beliefOk =
    catalogEvalPass || (belief.ok && belief.score >= HUMAN_BELIEF_MIN_SCORE);
  const narrativeOk =
    catalogEvalPass || (narrative.ok && (narrative.score || 0) >= 72);
  ...
  if (!humanWriting.humanReady && !catalogEvalPass) {
    reasons.push(
...
export function contractToDeliveryGrade(contract, meta = {}) {
  const catalogPass =
    meta.forcedMissionProseRoute &&
    meta.contentEvaluation?.pass === true;
  if (!contract?.tierMet && !(catalogPass && contract?.chars >= (contract?.tierMin || 0))) {
    return DELIVERY_GRADE.DRAFT;
  }
  if (!contract?.humanVoiceMet && !catalogPass) return DELIVERY_GRADE.DRAFT;
  const rescue =
    meta.deliveryRescue || meta.missionProseFallback || meta.draftFallback;
  if (rescue || catalogPass) return DELIVERY_GRADE.HUMAN;
  if (
