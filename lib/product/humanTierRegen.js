...
export function needsWriterEnginePass(pack, input = {}) {
  ...
  if (needsHumanColumnProsePass(pack, input)) return true;
...
export function buildHumanColumnRegenNote(pack, input = {}) {
  ...
  const prose = scoreHumanColumnProseContamination(pack, input);
  const reasons = [...(contract.reasons || [])];
  if (!prose.ok) reasons.push("catalog_prose_contamination");
  const unique = [...new Set(reasons)].slice(0, 6).join(", ");
  const lengthNote = buildHumanTierRegenNote(pack, input);
  if (!unique) return lengthNote;
  return `【칼럼 계약 재작성】미달: ${unique}. 카탈로그·확인포인트 문장 금지, 질문형 소제목·서사형 본문. ${lengthNote}`;
}
