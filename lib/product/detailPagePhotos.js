 ...
 * 역할(packshot/detail/usage…)이 있는 자산이다. 익명 image[]가 아니다.
 */
import { isDetailPageAssetRole } from "@/lib/product/detailPageAssets";

export const DETAIL_PAGE_MAX_PHOTOS = 20;

/** 셀러가 고를 때 보는 컷 순서. */
export const DETAIL_PAGE_PHOTO_DIRECTION = {
  hero: { shot: "포장 앞면", hint: "세로 · 전체가 보이게", height: 760, plate: "01", objectPosition: "50% 46%", zoom: 1 },
  ...
  observe: { shot: "창으로 원물", hint: "포장 창·원물이 보이게. 손·모델 없음", height: 640, plate: "05", objectPosition: "50% 86%", zoom: 1.72 },
  feature: { shot: "라벨 한 점", hint: "같은 포장 글자가 읽히게. 다른 SKU로 바꾸지 않음", height: 400, plate: "06", objectPosition: "50% 34%", zoom: 1.48 },
...
export function normalizeDetailPagePhoto(item) {
  ...
  return {
    ...
    role: isDetailPageAssetRole(item.role) ? item.role : undefined,
  };
}
...
export function assignDetailPagePhotos(sections = [], images = []) {
  ...
  for (const photo of list) {
    if (
      photo.slot &&
      slots.some((s) => s.type === photo.slot) &&
      !byType[photo.slot] &&
      !usedSrc.has(photo.src)
    ) {
  ...
  for (const slot of slots) {
    ...
    while (i < unused.length && usedSrc.has(unused[i].src)) i += 1;
    if (i >= unused.length) break;
    byType[slot.type] = unused[i].src;
    usedSrc.add(unused[i].src);
    i += 1;
  }
  return {
    byType,
    leftovers: list.map((p) => p.src).filter((src) => !usedSrc.has(src)),
