...
export async function applyWriterEngineIfNeeded(pack, input = {}, hooks = {}) {
  if (
    ...
    !hooks.forceWriterEngine &&
    (pack?._meta?.columnistSovereignLlm || pack?._meta?.visitReviewSovereignLlm)
  ) {
    return pack;
  }
  if (pack?.sections?.length && isColumnistSovereignEligible(input, pack)) {
    const sovereign = await upgradePackViaColumnistSovereign(pack, input);
    if (sovereign?.sections?.length) return sovereign;
  }
  if (pack?.sections?.length && isVisitReviewSovereignEligible(input, pack)) {
    const sovereign = await upgradeVisitReviewPackViaSovereign(pack, input);
    if (sovereign?.sections?.length) return sovereign;
  }
  ...
  const shouldRun =
    ...
    (maxQuality || needsWriterEnginePass(pack, input));
...
export async function runBriclogWriterEngine(pack, ctx = {}, input = {}) {
  ...
  if (
    isVisitReviewSovereignEligible(input, next) &&
    needsVisitReviewSovereignUpgrade(next, input)
  ) {
    const sovereign = await upgradeVisitReviewPackViaSovereign(next, input);
    if (sovereign?.sections?.length) {
      return {
        pack: sovereign,
        expanded: false,
        rewritten: true,
        voicePolished: false,
        judged: false,
        grade: assessDeliveryGrade(sovereign, input),
        engineVersion: WRITER_ENGINE_VERSION,
  ...
  if (
    ...
    !shouldUseGpt55LightDelivery(next, input)
  ) {
  ...
  if (shouldUseGpt55LightDelivery(next, input)) {
