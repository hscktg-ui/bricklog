...
function isEditorialQualityFastPassPack(pack) {
  return Boolean(
    pack?._meta?.editorialQualityStandard ||
      pack?._meta?.editorialQualityReshape ||
...
function assessEditorialQualityFastPass(pack, input = {}) {
  if (!isEditorialQualityFastPassPack(pack)) {
    return null;
  }
  const full = getBlogFullText(pack);
...
export function stripContentGateViolationsFromPack(pack, input = {}) {
  ...
  if (pack?._meta?.researchGroundedTrustPolish) return pack;
