...
export function buildFlowerNarrativeParagraphs(p, input = {}) {
  ...
  if (brand) {
    const visit =
      unmanned
        ? `최근 꽃 선물 때문에 알아보다가 ${regionBit}${brand}를 들르게 됐습니다. 24시간 무인으로 운영되는 곳이라 늦은 시간에도 부담 없이 방문할 수 있었고, 어떤 꽃들이 여름에 많이 활용되는지 직접 살펴볼 수 있었습니다.`
        : `최근 꽃 선물 때문에 알아보다가 ${regionBit}${brand}를 들르게 됐습니다. 진열대에서 여름에 많이 찾는 꽃 구성을 직접 살펴볼 수 있었습니다.`;
    blocks.push(visit);
  }

  blocks.push(
    "수국은 한 다발만으로도 풍성해 보이는 느낌이 있어서 집들이나 개업 선물로 자주 활용됩니다. 공간에 두었을 때 존재감이 큰 편이라, 작은 꽃다발보다 조금 더 특별한 느낌을 주고 싶을 때 잘 어울립니다.",
    "해바라기도 여름철에 진열대에서 눈에 잘 들어오는 꽃입니다. 노란 색감 자체가 밝고 시원한 분위기를 만들어주기 때문에 생일이나 축하 꽃다발을 준비할 때 많이 찾게 됩니다. 실제로 해바라기가 들어간 꽃다발은 사진으로 찍었을 때도 색감이 잘 살아나는 편입니다."
  );
  ...
  const gerberaTail =
    unmanned && brand
      ...
      : "";
  blocks.push(
    `꽃을 자주 사보지 않은 분들은 어떤 꽃이 오래 가는지부터 고민하게 됩니다. 그럴 때는 거베라를 한번 고려해 볼 만한데, 실제로 관리 부담이 비교적 적은 편이라 처음 구매하는 분들도 부담 없이 고를 수 있습니다.${gerberaTail}`,
  ...
  if (brand && unmanned) {
    blocks.push(
      `${regionBit}${brand}는 24시간 무인 운영이라 늦은 시간에도 꽃을 구매할 수 있다는 점이 인상적이었습니다. 키오스크에서 조합을 고른 뒤 픽업함에서 바로 받을 수 있어, 짧은 일정에도 부담 없이 들를 수 있었습니다.`,
      `${regionBit}${brand}에서 여름철 꽃을 고를 예정이라면 수국, 해바라기, 거베라, 라넌큘러스 정도는 한 번 비교해 보시는 것을 추천드립니다. 꽃은 결국 취향의 영역이지만, 어떤 상황에 어떤 꽃이 어울리는지만 알아도 선택이 훨씬 쉬워지는 것 같습니다.`
    );
  } else if (brand) {
    blocks.push(
      `${regionBit}${brand}에서 여름철 꽃을 고를 때는 진열대 색감과 줄기 상태를 먼저 보면 이후 선택이 편합니다. 같은 꽃이라도 당일 입고 구성에 따라 톤 차이가 있을 수 있어, 방문 시간대도 함께 고려해 보면 좋습니다.`
    );
  }
...
export function buildFlowerRecommendationEditorialPack(input = {}) {
  ...
  let pack = {
    title: `${p.regionBit}${p.brand}, ${facet} 직접 보고 정리해봤습니다`
      .replace(/\s+/g, " ")
      .trim(),
    representativeTitle: `${p.regionBit}${p.brand}, ${facet} 직접 보고 정리해봤습니다`
      .replace(/\s+/g, " ")
      .trim(),
    sections,
    conclusion:
      paras[paras.length - 1] ||
      `${p.regionBit}${p.brand}에서 ${facet}를 고를 때는 수국·해바라기·거베라·라넌큘러스를 한 번 비교해 보시면 좋습니다.`,
    hashtags: [],
    _meta: {
