...
import { stripUnverifiedClaimsFromPack } from "@/lib/product/briclogFactFirstEngine";
import { EDITORIAL_QUALITY_VERSION } from "@/lib/product/editorialQualityStandard";
import {
...
const TOPIC_TWIST_PREFIX_RE =
  /^(?:예약·상담|방문\s*전\s*체크|여름\s*시즌|가을\s*시즌|신규\s*오픈\s*안내|시즌\s*프로모션|대표\s*메뉴·서비스|이벤트\s*안내|고객\s*후기)\s*·\s*/;

/** 페르소나 twist(예약·상담 ·) 제거 — occasion 본문·제목에 전체 topic 반복 방지 */
export function resolveFlowerEditorialTopicLine(input = {}) {
  const raw = String(input.topic || topicRaw(input) || "").trim();
  if (!raw) return "꽃다발 예약·배송";
  const stripped = raw.replace(TOPIC_TWIST_PREFIX_RE, "").trim();
  if (stripped.length >= 6) return stripped;
  const facet = topicWritingFacet(input);
  if (facet && facet.length >= 4 && !TOPIC_TWIST_PREFIX_RE.test(facet)) return facet;
  return raw;
}
...
export function buildFlowerRecommendationEditorialPack(input = {}) {
  ...
  const brandEditor = !isFieldReviewSpeaker(input);
  const facet = resolveFlowerEditorialTopicLine(input) || p.topicFacet || "여름철 꽃 추천";
  ...
  if (countBlogBodyCharsWithSpaces(pack) < Math.round(tier.min * 0.45)) {
    ...
    if (tail) {
      const existing = String(tail.body || "");
      const extra = paras
        .slice(-2)
        .filter((para) => para && !existing.includes(para.slice(0, Math.min(48, para.length))))
        .join("\n\n");
      if (extra) {
  ...
  pack = stripGlobalExactDuplicateSentences(pack);
  pack = stripUnverifiedClaimsFromPack(pack, input);
  return {
    ...pack,
    _meta: {
      ...(pack._meta || {}),
...
/** long·medium tier — occasion 서사만으로 분량 보강 (mission 카탈로그·humanColumn 금지) */
export function expandFlowerEditorialPackToTier(pack, input = {}) {
  if (!pack?.sections?.length) return pack;
  const tier = resolveBlogLengthTier(input.blogLengthTier || "medium");
  const target = Math.round(tier.min * 0.72);
  let next = pack;
  let chars = countBlogBodyCharsWithSpaces(next);
  if (chars >= target) return next;

  const p = deriveTopicWritingContext(input);
  const paras = buildFlowerNarrativeParagraphs(p, input).filter(Boolean);
  let paraIdx = 0;

  for (let round = 0; round < 24 && chars < target && paraIdx < paras.length; round += 1) {
    const sections = [...(next.sections || [])];
    const tailIdx = sections.length - 1;
    const tail = sections[tailIdx];
    const chunk = paras[paraIdx % paras.length];
    paraIdx += 1;
    if (!chunk || String(tail?.body || "").includes(chunk.slice(0, Math.min(40, chunk.length)))) {
      continue;
    }
    sections[tailIdx] = {
      ...tail,
      body: `${String(tail?.body || "").trim()}\n\n${chunk}`.trim(),
    };
    next = { ...next, sections };
    chars = countBlogBodyCharsWithSpaces(next);
  }

  if (chars < target && paras.length >= 2) {
    const extraSections = paras
      .slice(paraIdx)
      .filter((para) => !getBlogFullText(next).includes(para.slice(0, Math.min(36, para.length))))
      .slice(0, 2)
      .map((body, i) => ({
        heading: i === 0 ? "예약·배송 참고" : "마무리",
        body,
      }));
    if (extraSections.length) {
      next = {
        ...next,
        sections: [...(next.sections || []), ...extraSections],
      };
      chars = countBlogBodyCharsWithSpaces(next);
    }
  }
  ...
  return {
    ...
    _meta: {
      ...(next._meta || {}),
      flowerEditorialLengthExpand: true,
      flowerEditorialLengthChars: chars,
      flowerEditorialLengthTarget: target,
    },
  };
}
