...
import {
  applyDirectedSample,
  loadDirectedDetailSample,
} from "@/lib/product/detailPageDirectedSample";
import { applyVendorMethodToPack } from "@/lib/product/detailPageVendorMethod";
...
export function detailPageSampleShots(id = DETAIL_PAGE_PUBLIC_SAMPLE_ID) {
  ...
  const extra =
    resolved === "open-rice"
      ? [
          { slot: "hero", file: "open-rice-canvas-pack.png", role: "packshot" },
          { slot: "observe", file: "open-rice-canvas-macro.png", role: "grain" },
          { slot: "feature", file: "open-rice-observe.png", role: "label" },
      ...
      : [
          { slot: "hero", file: "open-beans-hero.png", role: "packshot" },
          { slot: "observe", file: "open-beans-canvas-macro.png", role: "bean" },
          { slot: "feature", file: "open-beans-observe.png", role: "label" },
...
export function buildDetailPagePublicSample(id = DETAIL_PAGE_PUBLIC_SAMPLE_ID) {
  ...
  const fallback = buildDetailPageFallbackPack({
    ...(example || { presetId: DETAIL_PAGE_PUBLIC_SAMPLE_ID }),
    photos: shots,
  });
  const directed = loadDirectedDetailSample(resolved);
  const draft = applyVendorMethodToPack(
    directed ? applyDirectedSample(fallback, directed) : fallback,
    { ...(example || {}), photos: shots }
  );
