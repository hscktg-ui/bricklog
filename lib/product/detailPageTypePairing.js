...
export function resolveDetailPageTypePairing(input = {}) {
  if (input.stylePreset) {
    ...
    if (preset?.pairing && DETAIL_PAGE_TYPE_PAIRINGS[preset.pairing]) {
      return DETAIL_PAGE_TYPE_PAIRINGS[preset.pairing];
    }
  }
...
export function makeDetailPageTypeBox(pairing) {
  ...
  const surface =
    p.mood === "harvest"
      ? { paper: "#f3eee4", wash: "#e8e0d2", rule: "#d8cfc0" }
      : p.mood === "warm-editorial"
        ? { paper: "#f6f1ea", wash: "#efe8dc", rule: "#e0d6c8" }
        : {};
  return {
    ...DETAIL_PAGE_TYPE,
    ...surface,
