...
const FLOWS = {
  grocery: {
    ...
    label: "양곡·쌀",
    analysis:
      "화면은 후킹→정보→나열→재료→원료→스펙→패키지→배송→필수정보. 나열 칸의 항목만 산지 → 품종·등급(있을 때만) → 햅쌀 → 도정 → 중량 → 포장.",
  ...
  beauty: {
    ...
    label: "화장품",
    analysis:
      "상위 화장품 상세는 제형 → 사용감 → 성분(있는 것만) → 용량 → 쓰는 순서 순으로 고른다.",
    doNotInvent: ["임상", "인증", "효능 수치", "후기"],
    intentKicker: "",
    chooseTitle: "바르기 전에 보는 칸",
    uspTitle: "제형에서 대조하는 점",
    sceneTitle: "바르는 순서",
    observeTitle: "제형을 보면",
    slots: [
      { key: "form", label: "제형", material: true, match: /세럼|크림|앰플|토너|에센스|제형/ },
      { key: "feel", label: "사용감", match: /수분|흡수|산뜻|보습|사용감/ },
      { key: "ingredient", label: "성분", match: /성분|나이아신|히알루론|세라마이드/ },
      {
        key: "volume",
        label: "용량",
        material: true,
        match: /\d+\s*ml/i,
        infer: (input) => {
          const m = String(input.productName || "").match(/(\d+\s*ml)/i);
          return m ? m[1].replace(/\s+/g, "") : "";
        },
      },
      { key: "use", label: "쓰는 순서", match: /아침|저녁|바르/ },
    ],
  },
  appliance: {
    id: "appliance",
    label: "소형가전",
    analysis:
      "상위 소형가전 상세는 하는 일 → 크기 → 소음·전력(있는 것만) → 관리 → 구성 순으로 고른다.",
    doNotInvent: ["인증", "실험 수치", "1위"],
    intentKicker: "",
    chooseTitle: "고를 때 이 칸부터",
    uspTitle: "쓰기 전에 대조하는 점",
    sceneTitle: "놓고 쓰는 순서",
    observeTitle: "만져보면",
    slots: [
      { key: "job", label: "하는 일", material: true, match: /청정|분쇄|청소|튀김|가열/ },
      { key: "size", label: "크기", match: /크기|가로|세로|mm|cm/ },
      { key: "power", label: "전력", match: /W|와트|전력/ },
      { key: "noise", label: "소음", match: /dB|소음/ },
      { key: "care", label: "관리", match: /세척|필터|관리/ },
    ],
  },
  cafe: {
    id: "cafe",
    label: "원두·카페",
    analysis:
      "화면은 후킹→정보→나열→재료→원료→스펙→패키지→배송→필수정보. 나열 칸의 항목만 원산지·블렌드 → 로스팅 → 분쇄 → 중량 → 추출.",
