...
const FLOWS = {
  grocery: {
    ...
    slots: [
      ...
      { key: "variety", label: "품종", match: /추청|고시히카리|삼광|진상|아끼바레|품종/ },
      { key: "grade", label: "등급", match: /특등|1등급|2등급|등급/ },
      {
        key: "harvest",
        label: "햅쌀",
        match: /햅쌀|생산년도|20(2[0-9]|3[0-9])년/,
        infer: (input) => (/햅쌀/.test(`${input.productName || ""}`) ? "햅쌀로 표기" : ""),
      },
...
export function buildDetailPageCategoryListing(input = {}) {
  ...
  return {
    ...
    explainKicker: flow.explainKicker || "핵심 소구점",
    explainTitle: flow.explainTitle || "이 카테고리에서 먼저 보는 칸",
...
export function formatCategoryFlowForPrompt(input = {}) {
  ...
  return [
    ...
    "intent bullets = 고르는 순서의 짧은 이름(라벨 — 값). explain bullets = 핵심 소구점(카테고리 칸 3~5). usp bullets = 소재·재료 항목.",
