...
export function assessIndustryResearchContract(input = {}, organized = {}) {
  ...
  if (key === "cafe" || key === "tea_cafe" || key === "pet_cafe") {
    const topicBlob = `${input.topic || ""} ${input.mainKeyword || ""} ${input.includePhrases || ""}`;
    const merged = `${corpus}\n${topicBlob}`;
    hits.menu = /메뉴|라떼|브런치|음료|디저트|원두|에스프레소|티|차|다실|보이차/.test(merged);
    hits.atmosphere = /분위기|인테리어|좌석|테라스|조명|무드|다실/.test(merged);
    hits.visitReason = /방문|찾|이유|추천|한잔|모임|시즌|가을|여름|봄|겨울/.test(merged);
...
export function assessBrandResearchPresence(input = {}) {
  ...
  const memoryHits = [
    input.brandWiki?.philosophy,
    input.brandWiki?.brandStory,
    input.brandContentBrief,
  ...
  const hits = [
    ...new Set([...facts.map((f) => f.value), ...memoryHits]),
  ].filter(Boolean);
  const ok =
    featureHits.length >= 1 ||
    memoryHits.length >= 1 ||
    String(input.brandDescription || "").trim().length >= 8;
  return {
    ok,
    hits,
    required: 2,
    reasons: ok ? [] : ["brand_facts_missing"],
    brandMemoryLoaded: memoryHits.length > 0,
  };
}
