...
function observeLine(feature, brandName) {
  ...
  if (looksLikeClause(f)) {
    return `현장에서 보면 ${f}. ${brand} 그 기준으로 설명을 줄입니다.`;
  }
  return `현장에서 ${f}${eulReul(f)} 직접 확인해 보면, ${brand} 쪽 설명이 짧아집니다.`;
}
...
function padBody(text, min, extra) {
  ...
  if (countDetailPageChars(t) < min) {
    const tail = "입력된 사실 바깥의 숫자는 보태지 않았습니다.";
    if (!t.includes(tail)) t = `${t} ${tail}`.trim();
  }
  return t;
}
...
export function injectDetailPageMustCopy(pack, input = {}) {
  ...
  if (unusedHighlights.length) {
    ...
    usp.bullets = [
      ...(usp.bullets || []),
      ...unusedHighlights.map((h) => `${h} — 입력하신 강조 문구입니다.`),
    ].slice(0, 8);
  }
...
export function buildDetailPageFallbackPack(input = {}) {
  ...
  const intent =
    n.searchIntent ||
    `${n.target}가 ${n.productName}${eulReul(n.productName)} 고를 때 스펙만 보다가 막히는 지점`;
  const explainLead = looksLikeClause(feats[0])
    ? `${feats[0]} — 이게 현장에서 어떻게 느껴지는지가 먼저입니다.`
    : `${feats[0]}가 현장에서 어떻게 느껴지는지가 먼저입니다.`;
  ...
  const byType = {
    hero: {
      ...
      body: padBody(
        `${n.target}가 ${n.productName}${eulReul(n.productName)} 고를 때, 먼저 보면 좋은 기준만 정리했습니다. 광고 문장 대신 ${n.brandName}에서 실제로 확인하는 지점입니다.`,
        70,
        "목록으로 외우게 만들지 않고, 고르는 순서만 남깁니다."
      ),
    },
    intent: {
      type: "intent",
      ...
      body: padBody(
        `${intent}. 그 막힘이 남는 이유는 특징을 나열해도 고르는 순서가 안 보이기 때문입니다. ${brand} 확인 가능한 기준부터 맞춰 두고, 없는 후기나 인증은 적지 않습니다.`,
        110,
        `${n.target}가 다음에 무엇을 보면 되는지만 적습니다.`
      ),
    },
    problem: {
      type: "intent",
    ...
    explain: {
      ...
      body: padBody(
        `${product} 목록으로 외우는 상품이 아닙니다. ${explainLead} ${n.target} 입장에서 그 쓰임이 맞는지가, 스펙표보다 먼저입니다.`,
        120,
        "이유를 문장으로 풀어 두고, 특징만 한 줄로 끝내지 않습니다."
      ),
    },
    usp: {
      type: "usp",
      ...
      title: "반복해서 확인하는 지점",
      body: padBody(
        `${n.target}가 반복해서 확인하는 기준입니다. 아래는 입력된 특징을 고르는 순서로 풀어 쓴 것입니다.`,
        70,
        "없는 장점을 보태지 않았습니다."
      ),
      bullets: feats.slice(0, 5).map((f) => {
    ...
    observe: {
      ...
      title: "직접 보면 달라지는 점",
      body: padBody(
        `${observeLine(feats[0], n.brandName)} 종이 위 문장보다, 그 확인이 끝나면 설명이 짧아집니다. 없는 별점이나 가상 후기는 넣지 않습니다.`,
        100,
    ...
    feature: {
      ...
      body: padBody(
        `${observeLine(feats[0], n.brandName)} 아래는 입력된 특징을 현장에서 어떻게 보는지입니다.`,
        90,
        "입력에 없는 성능을 보태서 설명하지 않습니다."
      ),
    ...
    scene: {
      ...
      body: padBody(
        n.region
          ? `${n.region}에서 ${n.productName}${eulReul(n.productName)} 고를 때는 당장의 필요와 오래 쓰는 쓰임을 같이 묻습니다. ${n.target}가 그 둘을 한 번에 맞춰 보려는 때가 많습니다.`
          : `${n.productName}${eulReul(n.productName)} 고를 때는 당장의 필요와 오래 쓰는 쓰임을 같이 묻습니다. ${n.target}가 그 둘을 한 번에 맞춰 보려는 때가 많습니다.`,
        90,
    ...
    brand: {
      ...
      body: padBody(
        n.brandDescription ||
          `${brand} ${n.productName}${eulReul(n.productName)} 크게 외치지 않습니다. 고르는 기준을 먼저 맞춰 두고, 확인된 사실만 본문에 남깁니다.`,
        80,
        "브랜드 이름은 맥락 안에만 둡니다."
      ),
    },
    notice: {
      ...
      body:
        [n.hours && `운영 ${n.hours}`, n.phone && `문의 ${n.phone}`, n.address]
          .filter(Boolean)
          .join(" · ") ||
        "운영 시간과 문의는 브랜드에 적힌 그대로입니다. 이 페이지에 없는 약속은 적지 않았습니다.",
    },
    cta: {
      type: "cta",
      kicker: "다음에",
      ...
      body: padBody(
        n.phone
          ? `궁금한 점은 ${n.phone}으로 물어보면 됩니다. 서두르라는 안내는 하지 않습니다.`
          : `${brand} ${n.productName} 기준으로 먼저 맞춰 보면 됩니다. 서두르라는 안내는 하지 않습니다.`,
        50,
        "방문이나 구매를 재촉하지 않습니다."
      ),
    },
