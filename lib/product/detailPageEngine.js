...
export function buildDetailPageFallbackPack(input = {}) {
  ...
  const byType = {
    ...
    observe: {
      ...
      body: padBody(
        `${observeLine(feats[0], n.brandName)} 종이 위 문장보다, 그 확인이 끝나면 설명이 짧아집니다. 지어낸 후기나 가상 후기는 넣지 않습니다. ${n.target}${iGa(n.target)} 화면에서 다시 묻는 지점만 남깁니다.`,
        170,
...
export function stampDetailPagePack(pack, input, mode) {
  ...
  withMeta._meta.success = assessDetailPageSuccess({
    pack: withMeta,
    input,
    photoCount: Number(input.imageCount || 0),
  });
