...
export function buildDetailPageFallbackPack(input = {}) {
  ...
  const byType = {
    hero: {
      ...
      body: padBody(
        `${n.target}가 ${n.productName}${eulReul(n.productName)} 고를 때, 스펙표만 보다가 멈추는 지점부터 적었습니다. ${product} 광고 문장 대신, ${n.brandName}에서 실제로 대조하는 기준만 화면에 남깁니다. 고르는 순서가 보이면 설명은 짧아집니다.`,
        120,
        n.highlights[1]
    ...
    intent: {
      ...
      body: padBody(
        `${intent}. 특징을 나열해도 막히는 이유는, 다음에 무엇을 보면 되는지 순서가 없기 때문입니다. ${brand} 확인 가능한 항목만 맞춰 두고, 없는 후기·인증·가격은 화면에 올리지 않습니다. ${n.target}가 비교할 때 다시 묻는 지점만 남깁니다.`,
        180,
    ...
    usp: {
      ...
      title: "반복해서 확인하는 지점",
      body: padBody(
        `${n.target}가 화면에서 다시 돌아오는 기준입니다. 아래는 입력된 특징을 고르는 순서로 풀어 쓴 것입니다. 없는 장점은 보태지 않았습니다.`,
        70,
    ...
    observe: {
      ...
      body: padBody(
        `${observeLine(feats[0], n.brandName)} 종이 위 문장보다, 그 확인이 끝나면 설명이 짧아집니다. 없는 별점이나 가상 후기는 넣지 않습니다. ${n.target}가 화면에서 다시 묻는 지점만 남깁니다.`,
        170,
    ...
    scene: {
      ...
      body: padBody(
        n.region
          ? `${n.region}에서 ${n.productName}${eulReul(n.productName)} 고를 때는 당장의 필요와 오래 쓰는 쓰임을 같이 묻습니다. ${n.target}가 그 둘을 한 번에 맞춰 보려는 때가 많습니다. 그 장면을 기준으로 설명을 맞춰 둡니다.`
          : `${n.productName}${eulReul(n.productName)} 고를 때는 당장의 필요와 오래 쓰는 쓰임을 같이 묻습니다. ${n.target}가 그 둘을 한 번에 맞춰 보려는 때가 많습니다. 그 장면을 기준으로 설명을 맞춰 둡니다.`,
        150,
    ...
    cta: {
      ...
      body: padBody(
        n.phone
          ? `궁금한 점은 ${n.phone}으로 물어보면 됩니다. 서두르라는 안내는 하지 않습니다. 고르는 순서가 보이면, 그다음 한 걸음은 ${n.target}가 정하면 됩니다.`
          : `${brand} ${n.productName} 기준으로 먼저 맞춰 보면 됩니다. 서두르라는 안내는 하지 않습니다. 고르는 순서가 보이면, 그다음 한 걸음은 ${n.target}가 정하면 됩니다.`,
        80,
        "방문이나 구매를 재촉하지 않습니다."
      ),
    },
