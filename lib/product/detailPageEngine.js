...
function buyerQuestions(n, listing) {
  const intent = cleanLine(n.searchIntent);
  const qs = [];
  if (intent) qs.push(clipHeadline(intent.replace(/[다요]$/, "") + (intent.includes("?") ? "" : ""), 24));
  if (listing.key === "grocery") {
    qs.push("도정은 언제 했나");
    qs.push("산지는 어디인가");
  } else if (listing.key === "cafe") {
    qs.push("집에서 어떻게 내리나");
    qs.push("분쇄는 어떻게 고르나");
  } else {
    qs.push("고를 때 무엇을 보나");
    qs.push("쓰는 순서는 어떻게 되나");
  }
...
export function buildDetailPageFallbackPack(input = {}) {
  ...
  const byType = {
    ...
    observe: {
      ...
      imageBrief:
        listing.key === "grocery"
          ? "쌀알 매크로. 포장 반복 금지. 원물 질감만."
          : "상품 디테일 매크로. 같은 포장 정면 반복 금지.",
    ...
    feature: {
      ...
      imageBrief: "패키지 확대. 로고·한글 라벨 원본 유지. 새 문구 금지.",
