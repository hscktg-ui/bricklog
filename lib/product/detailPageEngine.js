...
export function buildDetailPageFallbackPack(input = {}) {
  ...
  const byType = {
    ...
    intent: {
      type: "intent",
      kicker: listing.problemKicker || "고민",
      title: n.searchIntent || listing.chooseTitle || "고를 때 이 칸부터",
      body: listing.firstLook ? `${listing.firstLook}부터.` : "",
      bullets: [],
    },
    problem: {
      type: "intent",
    ...
    explain: {
      type: "explain",
      kicker: listing.explainKicker || "핵심 소구점",
      title: listing.explainTitle || "이 카테고리에서 먼저 보는 칸",
      body: "",
      bullets:
...
export function stampDetailPagePack(pack, input, mode) {
  ...
  const draftPipe = buildDetailPagePipeline({ ...input, photos }, cleaned, "");
  const withMeta = {
    ...cleaned,
