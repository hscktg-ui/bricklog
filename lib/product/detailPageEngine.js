...
import { pickDetailPageCommerceFields } from "@/lib/product/detailPageCommerceFields";
import {
...
import { renderDetailPageFromCanvas } from "@/lib/product/detailPageCanvasRenderer";
import {
...
import { attachDetailPageImageBriefs } from "@/lib/product/detailPageImageBrief";
import { inspectDetailPageImageConcept } from "@/lib/product/detailPageImageConcept";
import { resolveDetailPageStylePreset } from "@/lib/product/detailPageStylePreset";
import {
  inspectDetailPageWebUi,
  scoreDetailPageVisualCritic,
} from "@/lib/qa/detailPageWebUiGuard";
import { evaluateDetailPageAiDevPanel } from "@/lib/qa/detailPageAiDevPanel50";
import {
  scoreDetailPageCommerce,
  refineDetailPagePackForCommerce,
} from "@/lib/product/detailPageCommerceCritique";
import { stampGrokArtOnPack } from "@/lib/product/detailPageGrokDirector";
import {
  applyVendorMethodToPack,
  firstGlanceHeadline,
} from "@/lib/product/detailPageVendorMethod";
...
export function normalizeDetailPageInput(raw = {}) {
  ...
  const sourceChannel = normalizeSourceChannel(merged.sourceChannel);
  const continuityCopy = cleanLine(
    merged.continuityCopy || merged.channelContinuity || merged.continuity
  );
  const detailPageAstro = buildDetailPageScopedAstro({
    sourceChannel,
    continuityCopy,
  ...
  const commerce = pickDetailPageCommerceFields(merged);
...
function observeLine(feature, brandName, listing) {
  const f = cleanLine(feature);
  if (listing?.key === "grocery") {
    return "포장 창으로 쌀알만 보면 윤이 바로 눈에 들어온다.";
  }
  if (listing?.key === "cafe") {
    return "봉투를 열면 볶은 향이 바로 눈에 들어온다.";
  }
  if (!f) {
...
function buildDetailPageScopedAstro(input = {}) {
  const sourceChannel = normalizeSourceChannel(
    input.sourceChannel || input.channelSource || input.source
  );
  const continuity = clipBody(
    oneLine([
      input.continuityCopy,
      input.brandFeedbackBrief,
      Array.isArray(input.researchFacts) ? input.researchFacts[0] : "",
      input.searchIntent,
    ]),
    80
  );
  const anchorTypes =
    sourceChannel === "blog"
      ? ["intent", "scene", "cta"]
      : sourceChannel === "place" || sourceChannel === "instagram"
        ? ["intent", "cta"]
        : continuity
          ? ["intent", "cta"]
          : [];
  return {
    enabled: anchorTypes.length > 0,
    sourceChannel,
    continuity,
    anchorTypes,
  };
...
function summarizeDetailPageAstro(astro = {}) {
  return {
    enabled: astro.enabled === true,
    sourceChannel: normalizeSourceChannel(astro.sourceChannel),
    continuity: clipBody(astro.continuity || "", 80),
    anchorTypes: Array.isArray(astro.anchorTypes)
      ? astro.anchorTypes.filter(Boolean).slice(0, 4)
      : [],
  };
}

function llmBufferReasons(pack = {}) {
  const reasons = [];
  const standard = pack?._meta?.standard;
  if (standard && !standard.ok && standard.reasons?.includes("fake_review")) {
    reasons.push("fake_review");
  }
  if (pack?._meta?.mdPanel?.hire === false) reasons.push("md_panel");
  if (pack?._meta?.aiDevPanel?.hire === false) reasons.push("ai_dev_panel");
  return reasons;
}
...
export function injectDetailPageMustCopy(pack, input = {}) {
  ...
  if (unusedHighlights.length) {
    ...
    const extra = unusedHighlights.filter((h) => {
      const blob = (usp.bullets || []).join(" ");
      if (blob.includes(h)) return false;
      const stripped = String(h)
        .replace(/\s*(수확|재배|생산)\s*$/g, "")
        .trim();
      if (stripped && stripped !== String(h).trim() && blob.includes(stripped)) {
        return false;
      }
      return true;
    });
    usp.bullets = [...(usp.bullets || []), ...extra].slice(0, 6);
  }
...
function sceneBodyForListing(listing, n) {
  if (listing.key === "grocery") {
    return oneLine([
      n.pack || /진공/.test(`${n.features || ""}`)
        ? "진공 포장을 뜯고 바로 밥을 짓습니다."
        : "개봉한 뒤 바로 밥을 짓습니다.",
    ]);
  }
  if (listing.key === "cafe") {
    return oneLine([
      /분쇄/.test(`${n.features || ""}${n.highlights || ""}`)
        ? "주문한 굵기로 받아서 바로 내립니다."
        : "받은 원두를 바로 내립니다.",
    ]);
  }
  if (listing.key === "furniture") {
    return oneLine([`${n.productName}${eulReul(n.productName)} 놓고 바로 씁니다.`]);
  }
  return oneLine([
    n.region
      ? `${n.region}에서 ${n.productName}${eulReul(n.productName)} 꺼내 씁니다.`
      : `${n.productName}${eulReul(n.productName)} 꺼내 씁니다.`,
  ]);
}
...
function compactFaqs(items) {
  const list = (items || []).filter((item) => item?.q && item?.a);
  const filled = list.filter((item) => !isNeedFact(item.a));
  if (filled.length) return filled.slice(0, 6);
  const required = list.filter(
    (item) => isNeedFact(item.a) && /출고|배송비/.test(item.q)
  );
  return (required.length ? required : list.slice(0, 1)).slice(0, 4);
}

function keepEvidenceBullets(list) {
  return (list || []).some((b) => /\s+[—–]\s+/.test(String(b || "")));
}

const SHOPPER_REQUIRED_SPEC = new Set(["가격", "중량", "원산지", "상품명"]);

function compactSpecRows(rows) {
  const list = (rows || []).filter((r) => r?.[0] && r?.[1]);
  const filled = list.filter((r) => !isNeedFact(r[1]));
  const missingRequired = list.filter(
    (r) => isNeedFact(r[1]) && SHOPPER_REQUIRED_SPEC.has(r[0])
  );
  return [...filled, ...missingRequired].slice(0, 8);
}
...
export function buildDetailPageFallbackPack(input = {}) {
  ...
  const specRows = compactSpecRows(dossier.specRows);
  ...
  const byType = {
    hero: {
      type: "hero",
      kicker: listing.label || "",
      title: clipHeadline(firstGlanceHeadline(n) || n.productName),
      heading: firstGlanceHeadline(n) || n.productName,
      proof: clipBody(proof, 80),
      body: clipBody(
        [n.target, dossier.values.price].filter(Boolean).join(" · "),
        80
      ),
      ctaLabel: "구매하기",
      sourceFactIds: dossier.strongest ? [dossier.strongest.id] : [],
    ...
    usp: {
      ...
      title: clipHeadline(listing.uspTitle || "표기와 대조하는 점"),
      body: "",
    ...
    observe: {
      ...
      title: clipHeadline(listing.observeTitle),
      body: clipBody(observeLine(feats[0], n.brandName, listing), 80),
      imageBrief:
        listing.key === "grocery"
    ...
    feature: {
      type: "feature",
      kicker: "",
      title: clipHeadline(listing.key === "cafe" ? "봉투에서 읽는 줄" : "라벨에서 읽는 줄"),
      body: clipBody(
        [dossier.values.process, dossier.values.pack].filter(Boolean).length
          ? "포장 라벨에서 가공 시점과 포장 방식을 읽습니다."
          : needFact("생산·가공 방식"),
        80
      ),
    ...
    scene: {
      ...
      body: clipBody(sceneBodyForListing(listing, n), 80),
      imageBrief:
        listing.key === "grocery"
    ...
    spec: {
      type: "spec",
      kicker: listing.specKicker ?? "표기",
      title: clipHeadline(listing.specTitle || ""),
      rows: specRows,
    },
    brand: {
      type: "brand",
      kicker: "",
      title: clipHeadline(
        dossier.values.producer
          ? `${dossier.values.producer}에서 출고`
          : `${n.brandName}에서 출고`
      ),
      body: clipBody(
        dossier.values.producer
          ? `${dossier.values.producer}에서 출고합니다.`
          : needFact("생산자 또는 제조자"),
        80
      ),
      sourceFactIds: ["origin", "producer"].filter((id) => dossier.values[id]),
    },
    notice: {
      type: "notice",
      kicker: "출고",
      ...
      body: clipBody(
        [
          dossier.values.dispatch,
          dossier.values.shipping,
          n.hours && `운영 ${n.hours}`,
          n.phone && `문의 ${n.phone}`,
        ]
          .filter(Boolean)
          .join(" · ") || "출고·배송비·교환은 판매 자료가 있는 항목만 적습니다.",
        80
      ),
      faqs: compactFaqs(
        [
          {
            q: "출고는 언제인가?",
            a: dossier.values.dispatch || dossier.values.shipping || needFact("출고 일정"),
          },
          {
            q: "배송비는 얼마인가?",
            a: dossier.values.shipping || needFact("배송비"),
          },
          {
            q: "파손·품질 문제는?",
            a: dossier.values.exchange || needFact("교환·환불 기준"),
          },
          {
            q: "도서산간 배송은?",
            a: dossier.values.islandShipping,
          },
        ].filter((item) => item.a)
      ),
    },
  ...
  const sections = attachDetailPageImageBriefs(
    plan.sections
      .map((frame) => {
        const s = byType[frame.id];
        if (!s) return null;
        return {
          ...s,
...
function sanitizeSection(raw, index) {
  ...
  const bullets = Array.isArray(raw.bullets)
    ? keepEvidenceBullets(raw.bullets) || !isEssayBulletList(raw.bullets)
  ...
  return {
    ...
    imageBrief:
      raw.imageBrief && typeof raw.imageBrief === "object"
        ? raw.imageBrief
        : cleanLine(raw.imageBrief).slice(0, 160),
...
export function parseDetailPageLlmPack(raw, input) {
  ...
  const n = normalizeDetailPageInput(input);
  const sections = attachDetailPageImageBriefs(
    (parsed.sections || []).map((s, i) => sanitizeSection(s, i)).filter(Boolean),
    n
  );
...
export function stampDetailPagePack(pack, input, mode) {
  ...
  let working = applyVendorMethodToPack(refineDetailPagePackForCommerce(cleaned), {
    ...input,
    photos,
  });
  const draftPipe = buildDetailPagePipeline({ ...input, photos }, working, "");
  const storyOrder = (draftPipe.plan?.sections || []).map((s) => s.id);
  if (storyOrder.length) {
    working = {
      ...working,
      sections: sortSectionsToRanking(working.sections, storyOrder).map((s) => {
        const frame = (draftPipe.plan?.sections || []).find((f) => f.id === s.type);
        return frame
          ? { ...s, purpose: frame.purpose, composition: frame.composition, art: frame.art }
          : s;
      }),
    };
    cleaned.sections = working.sections;
  }
  const withMeta = {
    ...working,
    ...cleaned,
    ...
    target: input.target || cleaned.target || "",
    photos,
    _meta: {
      ...
      director: pack._meta?.director,
    },
  };
  const commerceFields = pickDetailPageCommerceFields(input);
  for (const [key, value] of Object.entries(commerceFields)) {
    if (value) withMeta[key] = value;
  }
  const painted = renderDetailPageFromCanvas(withMeta, photos);
  Object.assign(withMeta, painted.pack);
  withMeta._meta = {
    ...(painted.pack._meta || withMeta._meta),
  ...
  withMeta._meta.plan = {
    ...summarizeDetailPagePlan(pipeline.plan),
    customOrder: true,
    strategy: pipeline.plan?.strategy || "",
    preset: pipeline.plan?.preset || "",
    palette: pipeline.plan?.palette || null,
  };
  withMeta._meta.style = resolveDetailPageStylePreset({
    ...input,
    category: pipeline.plan?.category,
  });
  withMeta._meta.astro = summarizeDetailPageAstro(
    input.detailPageAstro || withMeta.detailPageAstro || {}
  );
  ...
  const finalHtml = renderDetailPageBodyHtml(withMeta, photos);
  withMeta._meta.webUi = inspectDetailPageWebUi(finalHtml);
  withMeta._meta.critic = scoreDetailPageVisualCritic(finalHtml, {
    preset: withMeta._meta.style?.id,
    photos: photos.length,
    uniqueCompositions:
      new Set((withMeta.sections || []).map((s) => s.composition).filter(Boolean)).size >= 4,
  });
  withMeta._meta.imageConcept = inspectDetailPageImageConcept({
    photos,
    html: finalHtml,
    input,
  });
  const aiDevPanel = evaluateDetailPageAiDevPanel({
    pack: withMeta,
    html: finalHtml,
  });
  withMeta._meta.aiDevPanel = {
    mean: aiDevPanel.summary.mean,
    passCount: aiDevPanel.summary.passCount,
    n: aiDevPanel.summary.n,
    hire: aiDevPanel.summary.hire,
    hireLabel: aiDevPanel.summary.hireLabel,
    topIssues: aiDevPanel.summary.topIssues,
    byLens: aiDevPanel.summary.byLens,
  };
  ...
  withMeta._meta.mdPanel = withMeta._meta.success.mdPanel;
...
function compactGptUserPayload(n) {
  ...
  const commerce = pickDetailPageCommerceFields(n);
  const filledCommerce = Object.fromEntries(
    Object.entries(commerce).filter(([, v]) => v)
  );
  if (Object.keys(filledCommerce).length) payload.commerce = filledCommerce;
  if (n.photoCaptions?.length) payload.photoCaptions = n.photoCaptions;
  if (n.imageCount) payload.imageCount = n.imageCount;
  if (n.sourceChannel) payload.sourceChannel = n.sourceChannel;
  if (n.detailPageAstro?.enabled) {
    payload.astro = {
      enabled: true,
      sourceChannel: n.detailPageAstro.sourceChannel || "",
      continuity: n.detailPageAstro.continuity || "",
      anchorTypes: n.detailPageAstro.anchorTypes || [],
    };
  }
  const plan = buildDetailPagePlan(n, n.photos);
  payload.plan = {
...
async function finishDetailPagePack(pack, input, mode, options = {}) {
  let working = pack;
  if (mode === "llm" || mode === "directed") {
    working = (await stampGrokArtOnPack(pack, input)).pack;
  }
  ...
  const shot = await generateDetailPageShots(
    { ...input, photos: photosIn },
    {
      ...
      generateMissing: options.allowImages !== false,
      photos: photosIn,
  ...
  const stamped = stampDetailPagePack(working, nextInput, mode);
...
export async function generateDetailPagePack(rawInput = {}, options = {}) {
  ...
  try {
    ...
    if (parsed?.sections?.length >= 3) {
      ...
      const bufferReasons = llmBufferReasons(probe);
      if (bufferReasons.length) {
        const buffered = await finishDetailPagePack(
          injectDetailPageMustCopy(fallback, input),
          input,
          "fallback",
          options
        ...
        buffered.pack._meta = {
          ...(buffered.pack._meta || {}),
          llmBuffered: true,
          llmBufferReasons: bufferReasons,
        };
        return buffered;
      }
