...
function compactFaqs(items) {
  ...
  const missing = list.filter((item) => isNeedFact(item.a));
  if (!missing.length) return filled;
  const combined = {
    q: filled.length ? "아직 없는 안내" : "배송·교환은 어떻게 되나?",
    a: missing.map((item) => `${item.q.replace(/\?$/, "")} ${item.a}`).join(" · "),
  };
  return [...filled, combined].slice(0, 6);
}
...
function compactSpecRows(rows) {
  ...
  const missing = list.filter((r) => isNeedFact(r[1]));
  if (!missing.length) return filled;
  const price = missing.filter((r) => r[0] === "가격" || String(r[1]).includes("가격"));
  const rest = missing.filter((r) => r[0] !== "가격" && !String(r[1]).includes("[자료 필요: 가격]"));
  const out = filled.length ? [...filled, ...price] : missing.slice(0, 3);
  if (filled.length && rest.length) {
    out.push(["아직 없는 항목", rest.map((r) => r[1]).join(" · ")]);
  }
