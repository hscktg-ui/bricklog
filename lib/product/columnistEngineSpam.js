...
/** 가구·정보형 — 엔진 금지 문구가 포함된 문장만 제거 */
export function stripEngineSpamFromPack(pack) {
  if (!pack?.sections?.length) return pack;
  const stripSentence = (text = "") =>
    String(text || "")
      .split(/(?<=[.!?…])\s+/)
      .filter((s) => s.trim() && !hasEngineSpamInText(s))
      .join(" ")
      .trim();
  const sections = pack.sections.map((sec) => ({
    ...sec,
    body: stripSentence(sec.body),
  }));
  return {
    ...pack,
    sections: sections.filter((s) => String(s.body || "").trim().length > 20),
    conclusion: stripSentence(pack.conclusion),
  };
}
