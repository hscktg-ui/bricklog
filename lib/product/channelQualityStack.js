...
export function computeChannelContentQualityValue(pack, channel, input = {}) {
  ...
  const delivered =
    pack._meta?.channelPackFinished === true ||
    pack._meta?.deliveryChannelFinish === true ||
    pack._meta?.aGradeChannelPass === true ||
    pack._meta?.contentQualityDelivered === true;

  if (delivered && (first.displayReady || scored.humanEditorPass)) {
    score = Math.max(score, B_GRADE_MIN_SCORE);
  }
  if (pack._meta?.aGradeChannelPass || pack._meta?.professionalEditorGrade) {
    score = Math.max(score, A_GRADE_MIN_SCORE);
  }
  if (publishReady && score < B_GRADE_MIN_SCORE) {
  ...
  return {
    ...
    publishReady: publishReady || score >= B_GRADE_MIN_SCORE,
    breakdown,
    reasons,
    channel,
    humanEditorPass: scored.humanEditorPass,
