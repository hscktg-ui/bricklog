/**
 * place · instagram — 블로그와 동일 품질 축 (채널 형식에 맞게 완화)
 */
import { getChannelFullText, isChannelPackDeliverable } from "@/lib/content/channelPack";
import { splitKoreanSentences } from "@/lib/content/v2AxisSentencePrune";
import { B_GRADE_MIN_SCORE } from "@/lib/product/bGradeDeliveryEngine";
import { A_GRADE_MIN_SCORE } from "@/lib/product/aGradeDeliveryEngine";
import { stampCoreRulesOnDelivery } from "@/lib/product/briclogCoreRules";

function softenChannelRecallEndings(text = "") {
  const recallRe = /(?:더라구요|더라고요|거든요|었어요)/g;
  const alt = ["해요", "했습니다", "네요", "어요", "입니다", "해 주세요"];
  let recallSeen = 0;
  return String(text || "").replace(recallRe, (match) => {
    recallSeen += 1;
    if (recallSeen <= 2) return match;
    return alt[(recallSeen - 3) % alt.length];
  });
}

function applyChannelRecallSoftenPass(pack, channel = "instagram") {
  if (channel !== "instagram" || !pack) return pack;
  const recallRe = /(?:더라구요|더라고요|거든요|었어요)/g;
  const alt = ["해요", "했습니다", "네요", "어요", "입니다", "해 주세요"];
  let recallSeen = 0;
  const softenField = (text) =>
    String(text || "").replace(recallRe, (match) => {
      recallSeen += 1;
      if (recallSeen <= 2) return match;
      return alt[(recallSeen - 3) % alt.length];
    });

  const hook = softenField(pack.hook);
  const body = softenField(pack.body);
  const ending = softenField(pack.ending);
  const lineBreakBody = softenField(pack.lineBreakBody || pack.body);
  return {
    ...pack,
    hook,
    body,
    ending,
    lineBreakBody,
    legacyBody: lineBreakBody,
  };
}
import {
  detectForbiddenIntro,
  FORBIDDEN_INTRO_PATTERNS,
} from "@/lib/product/editorIntroRules";
import {
  ANTI_SEO_SPAM_MAX_TOKEN_REPEAT,
  ANTI_SEO_SPAM_PRONOUNS,
  resolveAntiSeoTopicPronouns,
  softenTokenRepeats,
  scoreAntiSeoSpam,
} from "@/lib/product/antiSeoSpamEngine";
import {
  detectIndustryCommonSenseViolations,
  detectRegionCommonSenseViolations,
  detectSentenceStructureOveruse,
  detectForcedEntityStacking,
  scoreSentenceLengthVariety,
} from "@/lib/product/humanityCommonSenseEngine";
import {
  detectSeoSentenceSmell,
  detectDuplicateContentViolations,
} from "@/lib/product/contentQualityEngine";
import {
  scoreHumanBelief,
  HUMAN_BELIEF_MIN_SCORE,
} from "@/lib/product/humanBeliefEngine";
import {
  scoreChannelPersonaAlignment,
  applyChannelPersonaMetaPass,
} from "@/lib/persona/personaEngineProfile";
import {
  applyChannelMarketerPack,
  detectChannelMarketerIssues,
} from "@/lib/content/channelMarketerEngine";
import {
  isMechanicalListingTitle,
  rewriteMechanicalTitle,
} from "@/lib/content/humanTitleEngine";
import { isBriclogMissionEnforced } from "@/lib/product/missionFlags";
import { applyLocalEditorBeliefPassToText } from "@/lib/content/humanBeliefGate";
import { applyKoreanOrthographyToChannelPack } from "@/lib/korean/koreanOrthographyEngine";
import { applyChannelUnifiedProsePass } from "@/lib/content/channelUnifiedProseEngine";
import { applyDuplicateKiller } from "@/lib/content/duplicateKillerEngine";
import { expandPackByInformation } from "@/lib/content/informationExpansionEngine";
import { applyChannelStoryGate } from "@/lib/content/channelStoryEngine";
import { scoreSmartPlaceVoice } from "@/lib/channel/smartPlaceVoiceProfile";
import { detectPlaceReviewLeak } from "@/lib/channel/smartPlaceNoticeGuard";
import { applyPlaceExpertPanel } from "@/lib/channel/placeExpertPanel";
import { enrichChannelInput, detectChannelTemplateLeak } from "@/lib/content/channelBrandResolve";
import { humanizePlaceNoticePack } from "@/lib/content/placeNoticeHumanize";
import {
  buildResearchGroundedInstagramPack,
  buildResearchGroundedPlacePack,
} from "@/lib/content/researchGroundedHumanPack";
import { applyInstagramExpertPanel, strengthenInstagramCaption } from "@/lib/channel/instagramExpertPanel";
import { applyChannelEditorWriterDeliveryPass } from "@/lib/product/editorWriterDeliveryPass";
import { applyDiagnosticScoreTrust } from "@/lib/product/diagnosticScoreTrust";
import { humanizeInstaCaptionPack } from "@/lib/content/instaCaptionHumanize";
import {
  applyRegionBrandMashRepairToChannelPack,
  assessPackRegionBrandMash,
} from "@/lib/content/regionBrandMashRepair";
import { stripChannelTemplateBoilerplate } from "@/lib/content/templateBoilerplateEngine";
import {
  applyChannelSovereignTrimPass,
  softenChannelEntityRepeatsInText,
  stripForbiddenIntroText,
} from "@/lib/product/channelSovereignTrim";
import {
  finalizeWriterFirstChannelDelivery,
  shouldUseWriterFirstChannelPostProcess,
} from "@/lib/product/channelWriterFirst";
import { assessChannelVisitNorthStar } from "@/lib/product/channelVisitNorthStar";

function dedupeInstaLineBreakBody(pack) {
  return humanizeInstaCaptionPack(pack);
}

/** 인스타 본문 — 줄바꿈·분량 (full text와 lineBreakBody 불일치 보정) */
function ensureInstagramLineBreakBody(pack, stackCtx, input) {
  const bodyKey = pack?.lineBreakBody ? "lineBreakBody" : "body";
  let body = String(pack[bodyKey] || "").trim();
  let bodyLen = body.replace(/\s/g, "").length;
  let lineCount = body.split(/\n+/).filter((l) => l.trim()).length;
  let next = pack;

  if (bodyLen < 200 || lineCount < 3) {
    for (let round = 0; round < 3 && (bodyLen < 200 || lineCount < 3); round += 1) {
      next = expandPackByInformation(next, stackCtx, input, {
        channel: "instagram",
        minChars: 240,
        salvageForce: true,
      });
      body = String(next[bodyKey] || "").trim();
      bodyLen = body.replace(/\s/g, "").length;
      lineCount = body.split(/\n+/).filter((l) => l.trim()).length;
    }
  }

  next = strengthenInstagramCaption(next, { ...stackCtx, input });

  if (lineCount < 2 && bodyLen >= 20) {
    const parts = body.split(/(?<=[.!?…])\s+/).filter((p) => p.trim());
    if (parts.length >= 2) {
      const mid = Math.ceil(parts.length / 2);
      body = `${parts.slice(0, mid).join(" ")}\n\n${parts.slice(mid).join(" ")}`.trim();
    } else if (body.includes(". ")) {
      body = body.replace(/\.(\s+)/, ".\n\n");
    } else {
      const mid = Math.floor(body.length / 2);
      const space = body.indexOf(" ", Math.max(20, mid - 20));
      if (space > 20) {
        body = `${body.slice(0, space).trim()}\n\n${body.slice(space).trim()}`;
      }
    }
    next = { ...next, [bodyKey]: body, body: next.body || body };
  }

  return next;
}

export const CHANNEL_QUALITY_VERSION = "v1";

const CHANNEL_BLOG_TONE_RE =
  /(이번\s*글|결론적으로|정리하면|소제목|서론|본론|마무리|알아보시다\s*보면)/;

function stripForbiddenIntroTextLocal(text = "") {
  return stripForbiddenIntroText(text);
}

function softenEntityRepeatsInText(text = "", input = {}) {
  return softenChannelEntityRepeatsInText(text, input);
}

function polishTextField(text, input = {}) {
  if (!text?.trim()) return text;
  return softenEntityRepeatsInText(stripForbiddenIntroTextLocal(text), input);
}

/**
 * 채널 필드 — 도입 클리셰·억지 반복 완화
 */
export function applyChannelEditorPolishPass(pack, channel = "place", input = {}) {
  if (!pack || !isBriclogMissionEnforced()) return pack;

  let base = applyChannelSovereignTrimPass(pack, channel, input);

  if (channel === "place") {
    let title = String(base.title || "").trim();
    if (title && isMechanicalListingTitle(title, {}, input)) {
      title = rewriteMechanicalTitle(title, {}, input, input.contentPerspective || "brand");
    }
    return {
      ...base,
      title: title || base.title,
      shortNotice: base.shortNotice
        ? polishTextField(base.shortNotice, input)
        : base.shortNotice,
      detailBody: base.detailBody
        ? polishTextField(base.detailBody, input)
        : base.detailBody,
    };
  }

  if (channel === "instagram") {
    const bodyKey = base.lineBreakBody ? "lineBreakBody" : "body";
    return {
      ...base,
      hook: base.hook ? polishTextField(base.hook, input) : base.hook,
      [bodyKey]: base[bodyKey]
        ? polishTextField(base[bodyKey], input)
        : base[bodyKey],
      ending: base.ending ? polishTextField(base.ending, input) : base.ending,
    };
  }

  return base;
}

function scoreChannelLineVariety(fullText = "") {
  const lines = String(fullText || "")
    .split(/\n+/)
    .map((l) => l.trim())
    .filter((l) => l.replace(/\s/g, "").length >= 12);
  if (lines.length < 3) {
    return { ok: false, score: 45, short: 0, medium: 0, long: 0, kinds: 1 };
  }
  let short = 0;
  let medium = 0;
  let long = 0;
  for (const line of lines) {
    const n = line.replace(/\s/g, "").length;
    if (n < 36) short += 1;
    else if (n <= 88) medium += 1;
    else long += 1;
  }
  const kinds = [short > 0, medium > 0, long > 0].filter(Boolean).length;
  const hasMediumLine = lines.some((l) => l.replace(/\s/g, "").length >= 28);
  const ok =
    kinds >= 2 ||
    (lines.length >= 4 && medium + long >= 1) ||
    (lines.length >= 5 && hasMediumLine);
  return { ok, score: ok ? 72 + kinds * 8 : 48, short, medium, long, kinds };
}

export function scoreChannelHumanityCommonSense(pack, channel = "place", input = {}) {
  if (!isBriclogMissionEnforced()) {
    return { ok: true, score: 100, humanRead: true, issues: [], checks: {} };
  }
  const full = getChannelFullText(pack, channel);
  const structure = detectSentenceStructureOveruse(full);
  const variety =
    channel === "place" || channel === "instagram"
      ? scoreChannelLineVariety(full)
      : scoreSentenceLengthVariety(full);
  const industry = detectIndustryCommonSenseViolations(full, input);
  const region = detectRegionCommonSenseViolations(full, input);
  const stack = detectForcedEntityStacking(
    { title: pack.title || pack.hook, sections: [{ body: full }] },
    {},
    input
  );

  const issues = [
    ...structure.issues,
    ...industry.issues,
    ...region.issues,
    ...stack.issues,
  ];
  if (!variety.ok) issues.push({ type: "sentence_length_flat" });

  let score = 86;
  if (!structure.ok) score -= 14;
  if (!variety.ok) score -= 8;
  if (!industry.ok) score -= 24;
  if (!region.ok) score -= 16;
  if (!stack.ok) score -= 18;
  score = Math.max(0, Math.min(100, score));

  const structureOk =
    structure.ok ||
    ((channel === "place" || channel === "instagram") && variety.ok);
  const hardFail = !industry.ok || !region.ok || !stack.ok || !structureOk;
  const humanRead = score >= 65 && !hardFail;

  return {
    ok: humanRead,
    score,
    humanRead,
    issues,
    checks: { structure, variety, industry, region, stack },
    channel,
  };
}

export function scoreChannelEditorQuality(pack, channel = "place", input = {}) {
  if (!isBriclogMissionEnforced()) {
    return { ok: true, score: 100, editorPass: true, issues: [] };
  }
  const full = getChannelFullText(pack, channel);
  const introForbidden = detectForbiddenIntro(
    channel === "instagram"
      ? String(pack.hook || "").slice(0, 200)
      : String(pack.shortNotice || pack.detailBody || "").slice(0, 200)
  );
  const structure = detectSentenceStructureOveruse(full);
  const seo = scoreAntiSeoSpam(full, input);
  const blogTone = CHANNEL_BLOG_TONE_RE.test(full);

  const issues = [];
  if (!introForbidden.ok) issues.push({ type: "channel_intro_cliche" });
  if (!structure.ok) issues.push({ type: "channel_structure_repeat" });
  if (!seo.ok) issues.push({ type: "channel_seo_smell" });
  if (blogTone) issues.push({ type: "channel_blog_tone_leak" });

  let score = 88;
  if (!introForbidden.ok) score -= 18;
  if (!structure.ok) score -= 12;
  if (!seo.ok) score -= 14;
  if (blogTone) score -= 16;
  score = Math.max(0, Math.min(100, score));

  const editorPass = score >= 62 && !blogTone && introForbidden.ok;

  return { ok: editorPass, score, editorPass, issues, channel };
}

export function scoreChannelSpecialQuality(pack, channel = "place", input = {}) {
  const reasons = [];
  if (channel === "place") {
    const title = String(pack?.title || "").trim();
    const shortNotice = String(pack?.shortNotice || "").trim();
    const detailBody = String(pack?.detailBody || "").trim();
    const detailLen = detailBody.replace(/\s/g, "").length;
    if (title.length < 6) reasons.push("place_title_too_short");
    if (shortNotice.length < 12) reasons.push("place_notice_too_short");
    if (detailLen < 80) reasons.push("place_detail_too_short");
    if (detailLen > 620) reasons.push("place_detail_too_long");
    if (CHANNEL_BLOG_TONE_RE.test(`${title}\n${shortNotice}\n${detailBody}`)) {
      reasons.push("place_blog_tone");
    }
    if (detectPlaceReviewLeak(`${title}\n${shortNotice}\n${detailBody}`)) {
      reasons.push("place_review_tone");
    }
    const voice = scoreSmartPlaceVoice(`${title}\n${shortNotice}\n${detailBody}`);
    if (!voice.ok) {
      if (voice.blogLeakHits > 0) reasons.push("place_review_leak");
      if (voice.ownerHits < 2) reasons.push("place_missing_owner_voice");
    }
  } else if (channel === "instagram") {
    const body = String(pack?.lineBreakBody || pack?.body || "").trim();
    const bodyLen = body.replace(/\s/g, "").length;
    const lineCount = body.split(/\n+/).filter((l) => l.trim()).length;
    const hashtagCount = Array.isArray(pack?.hashtags) ? pack.hashtags.length : 0;
    const minLen =
      String(input.instaBodyLength || "medium").toLowerCase() === "short"
        ? 100
        : String(input.instaBodyLength || "").toLowerCase() === "long"
          ? 260
          : 180;
    if (bodyLen < minLen) reasons.push("insta_body_too_short");
    if (lineCount < 3) reasons.push("insta_line_break_thin");
    const topicBlob = `${input.topic || ""} ${input.mainKeyword || ""} ${input.main || ""}`.trim();
    const topicTokens = topicBlob.split(/\s+/).filter((t) => t.length >= 2).slice(0, 4);
    if (
      topicTokens.length &&
      !topicTokens.some((t) => body.replace(/\s/g, "").includes(t.replace(/\s/g, "")))
    ) {
      reasons.push("insta_topic_mismatch");
    }
    if (hashtagCount < 3) reasons.push("insta_hashtag_low");
    if (hashtagCount > 14) reasons.push("insta_hashtag_over");
    if (CHANNEL_BLOG_TONE_RE.test(body)) reasons.push("insta_blog_tone");
  }
  return { ok: reasons.length === 0, reasons, channel };
}

/** place·instagram 첫 배달 belief floor — 블로그 파생 시 완화 */
export const CHANNEL_FIRST_DELIVERY_BELIEF_OFFSET = 22;
export const CHANNEL_RESEARCH_SOFT_BELIEF_MARGIN = 18;

function isTrustedChannelResearchPack(pack = {}) {
  return Boolean(
    pack._meta?.researchGroundedChannelPack ||
    pack._meta?.channelNorthStarPack ||
    pack._meta?.derivedFromVerifiedBlog ||
    pack._meta?.derivedFromBlog
  );
}

function resolveChannelBeliefScore(pack = {}, belief = {}) {
  return Math.max(
    Number(belief?.score ?? 0),
    Number(pack._meta?.humanBeliefScore ?? 0),
    Number(pack._meta?.humanBelief?.score ?? 0)
  );
}

export function resolveChannelFirstDeliveryBeliefFloor(channel, pack = {}, cq = {}) {
  const base =
    channel === "place" || channel === "instagram"
      ? HUMAN_BELIEF_MIN_SCORE - CHANNEL_FIRST_DELIVERY_BELIEF_OFFSET
      : HUMAN_BELIEF_MIN_SCORE - 10;
  if (
    (pack._meta?.derivedFromVerifiedBlog ||
      pack._meta?.channelNorthStarPack ||
      pack._meta?.derivedFromBlog) &&
    cq?.humanEditorPass
  ) {
    return Math.max(58, base - 6);
  }
  return base;
}

export function scoreChannelContentQuality(pack, channel = "place", ctx = {}, input = {}) {
  if (!isBriclogMissionEnforced()) {
    return {
      ok: true,
      score: 100,
      humanEditorPass: true,
      issues: [],
      checks: {},
    };
  }

  const evalInput = input || ctx.input || ctx;
  const full = getChannelFullText(pack, channel);
  const humanity = scoreChannelHumanityCommonSense(pack, channel, evalInput);
  const editor = scoreChannelEditorQuality(pack, channel, evalInput);
  const persona = scoreChannelPersonaAlignment(pack, evalInput, channel);
  const marketer = detectChannelMarketerIssues(pack, channel, ctx, evalInput);
  const special = scoreChannelSpecialQuality(pack, channel, evalInput);
  const industry = detectIndustryCommonSenseViolations(full, evalInput);
  const region = detectRegionCommonSenseViolations(full, evalInput);
  const seo = detectSeoSentenceSmell(full);
  const dup = detectDuplicateContentViolations(full);
  const belief = scoreHumanBelief(full, evalInput, pack);

  const issues = [
    ...humanity.issues,
    ...editor.issues,
    ...industry.issues,
    ...region.issues,
    ...seo.issues,
    ...dup.issues,
    ...(marketer.ok ? [] : marketer.issues.slice(0, 4)),
    ...(persona.ok ? [] : persona.issues.slice(0, 3)),
    ...(special.ok ? [] : special.reasons.map((r) => ({ type: r }))),
    ...(belief.ok ? [] : [{ type: "human_belief" }]),
  ];

  let score = Math.round(
    humanity.score * 0.35 +
      editor.score * 0.2 +
      (belief.score || 0) * 0.25 +
      (persona.score || 0) * 0.2
  );
  if (!special.ok) score -= 8;
  if (!marketer.ok) score -= 6;
  score = Math.max(0, Math.min(100, score));

  const dupHard =
    channel === "place" || channel === "instagram"
      ? dup.issues?.some((i) => i.type === "exact_sentence_repeat")
      : !dup.ok;

  const hardFail =
    !industry.ok ||
    !region.ok ||
    !seo.ok ||
    dupHard ||
    CHANNEL_BLOG_TONE_RE.test(full);

  const beliefFloor = resolveChannelFirstDeliveryBeliefFloor(channel, pack, {
    humanEditorPass:
      score >= 60 &&
      !hardFail &&
      humanity.humanRead &&
      editor.editorPass &&
      persona.score >= 52,
  });

  const humanEditorPass =
    score >= 60 &&
    !hardFail &&
    humanity.humanRead &&
    editor.editorPass &&
    belief.score >= beliefFloor &&
    persona.score >= 52;

  return {
    ok: humanEditorPass,
    score,
    humanEditorPass,
    humanReadLikely: humanity.humanRead,
    issues,
    checks: { humanity, editor, persona, marketer, special, belief, seo, dup },
    channel,
  };
}

export function applyChannelContentQualityMetaPass(
  pack,
  channel = "place",
  ctx = {},
  input = {}
) {
  const scored = scoreChannelContentQuality(pack, channel, ctx, input);
  return {
    ...pack,
    _meta: {
      ...(pack._meta || {}),
      contentQuality: scored,
      humanEditorPass: scored.humanEditorPass,
      channelEditor: scored.checks?.editor,
      editorV95Pass: scored.checks?.editor?.editorPass,
      humanityCommonSense: scored.checks?.humanity,
      channelQualityStack: CHANNEL_QUALITY_VERSION,
    },
  };
}

/**
 * @param {"place"|"instagram"} channel
 */
export function assessChannelFirstDeliveryQuality(pack, channel = "place", input = {}) {
  if (!pack) {
    return { ok: false, displayReady: false, reasons: ["empty_pack"], channel };
  }

  if (!isBriclogMissionEnforced()) {
    return { ok: true, displayReady: true, reasons: [], channel };
  }

  if (pack._meta?.channelNorthStarPack || pack._meta?.derivedFromVerifiedBlog) {
    const north =
      pack._meta?.channelVisitNorthStar ||
      assessChannelVisitNorthStar(pack, channel, input);
    const full = getChannelFullText(pack, channel);
    if (
      north.publishOk &&
      !CHANNEL_BLOG_TONE_RE.test(full) &&
      isChannelPackDeliverable(channel, pack)
    ) {
      return {
        ok: true,
        displayReady: true,
        reasons: [],
        channel,
        northStarFastPass: true,
        scores: {
          contentQuality: pack._meta?.contentQuality?.score,
          persona: pack._meta?.personaEngineAlignment?.score,
          belief: resolveChannelBeliefScore(pack, pack._meta?.humanBelief),
        },
      };
    }
  }

  const cq =
    pack._meta?.contentQuality ||
    scoreChannelContentQuality(pack, channel, { input }, input);
  const persona =
    pack._meta?.personaEngineAlignment ||
    scoreChannelPersonaAlignment(pack, input, channel);
  const belief = pack._meta?.humanBelief || {
    score: pack._meta?.humanBeliefScore,
    ok: pack._meta?.humanBelief?.ok,
  };
  const editor = pack._meta?.channelEditor || scoreChannelEditorQuality(pack, channel, input);
  const special =
    pack._meta?.channelSpecialQuality || scoreChannelSpecialQuality(pack, channel, input);
  const marketer = pack._meta?.channelMarketerGate;

  const reasons = [];
  if (!cq?.humanEditorPass) reasons.push("first_delivery_channel_editor");
  if (!editor?.editorPass) reasons.push("first_delivery_channel_intro");
  const personaFloor = channel === "place" || channel === "instagram" ? 64 : 70;
  if (!persona?.ok && (persona?.score ?? 0) < personaFloor) {
    reasons.push("first_delivery_persona");
  }
  const beliefFloor = resolveChannelFirstDeliveryBeliefFloor(channel, pack, cq);
  let beliefScore = resolveChannelBeliefScore(pack, belief);
  if (beliefScore <= 0) {
    const rescored = scoreHumanBelief(getChannelFullText(pack, channel), input, pack);
    beliefScore = Number(rescored?.score ?? 0);
  }
  if (beliefScore < beliefFloor) {
    reasons.push("first_delivery_human_belief");
  }
  if (special?.ok === false) reasons.push("first_delivery_channel_format");
  if (marketer?.ok === false) reasons.push("first_delivery_channel_marketer");

  let displayReady = reasons.length === 0;
  const fromBlog =
    input.sourceChannel === "blog" ||
    pack._meta?.sourceChannel === "blog" ||
    pack._meta?.derivedFromBlog;
  if (
    !displayReady &&
    fromBlog &&
    cq?.humanEditorPass &&
    belief?.ok !== false &&
    !CHANNEL_BLOG_TONE_RE.test(getChannelFullText(pack, channel))
  ) {
    displayReady = !reasons.some((r) =>
      /industry|region|seo|human_belief/.test(r)
    );
  }

  if (
    !displayReady &&
    isTrustedChannelResearchPack(pack) &&
    !CHANNEL_BLOG_TONE_RE.test(getChannelFullText(pack, channel))
  ) {
    const cqScore = cq?.score ?? 0;
    const textLen = getChannelFullText(pack, channel).replace(/\s/g, "").length;
    const minChars = channel === "instagram" ? 120 : 90;
    const softOk =
      textLen >= minChars &&
      beliefScore >= beliefFloor - CHANNEL_RESEARCH_SOFT_BELIEF_MARGIN &&
      (editor?.editorPass || cq?.humanEditorPass || cqScore >= 48) &&
      (persona?.ok || (persona?.score ?? 0) >= personaFloor - 12);
    if (softOk) {
      displayReady = true;
    }
  }

  return {
    ok: displayReady,
    displayReady,
    reasons,
    channel,
    scores: {
      contentQuality: cq?.score,
      persona: persona?.score,
      belief: beliefScore,
    },
  };
}

export const SQV_CHANNEL_VERSION = "v3-editor-channel";

function channelGradeFromScore(score) {
  if (score >= A_GRADE_MIN_SCORE) return "A";
  if (score >= B_GRADE_MIN_SCORE) return "B";
  if (score >= 64) return "C";
  if (score >= 50) return "D";
  return "F";
}

/** @param {object} pack @param {"place"|"instagram"} channel @param {object} [input] */
export function computeChannelContentQualityValue(pack, channel, input = {}) {
  if (!pack || !isChannelPackDeliverable(channel, pack)) {
    return {
      version: SQV_CHANNEL_VERSION,
      score: 0,
      grade: "F",
      publishReady: false,
      breakdown: {},
      reasons: ["empty_pack"],
      channel,
    };
  }

  if (!isBriclogMissionEnforced()) {
    return {
      version: SQV_CHANNEL_VERSION,
      score: 100,
      grade: "A",
      publishReady: true,
      breakdown: { skipped: true },
      reasons: [],
      channel,
    };
  }

  const scored = scoreChannelContentQuality(pack, channel, { input }, input);
  const first = assessChannelFirstDeliveryQuality(pack, channel, input);
  const breakdown = {
    humanity: scored.checks?.humanity?.score,
    editor: scored.checks?.editor?.score,
    belief: scored.checks?.belief?.score,
    persona: scored.checks?.persona?.score,
    special: scored.checks?.special?.ok ? 100 : 58,
  };

  const issueReasons = (scored.issues || [])
    .map((issue) => issue?.type || issue)
    .filter(Boolean);
  const reasons = [...new Set([...issueReasons, ...(first.reasons || [])])].slice(
    0,
    12
  );

  const publishReady =
    first.displayReady === true &&
    scored.humanEditorPass === true &&
    pack._meta?.outputWithheld !== true;

  let score = scored.score;

  const batchLocalFinish =
    pack._meta?.batchLocalChannelFinish === true ||
    pack._meta?.localBatchFinish === true;
  if (batchLocalFinish) {
    const full = getChannelFullText(pack, channel);
    const charLen = full.replace(/\s/g, "").length;
    const beliefScore =
      pack._meta?.humanBelief?.score ?? scored.checks?.belief?.score ?? 0;
    if (charLen >= 95 && beliefScore >= 44 && score < 50) {
      score = 50;
    }
  }

  const sqvBase = {
    version: SQV_CHANNEL_VERSION,
    score,
    grade: channelGradeFromScore(score),
    publishReady,
    breakdown,
    reasons,
    channel,
    humanEditorPass: scored.humanEditorPass,
    displayReady: first.displayReady,
  };

  return applyDiagnosticScoreTrust(sqvBase, pack, input, channel);
}

export function stampChannelContentQualityValue(pack, channel, input = {}) {
  if (!pack || (channel !== "place" && channel !== "instagram")) return pack;

  const sqv = computeChannelContentQualityValue(pack, channel, input);
  const priorPublishReady = pack._meta?.publishReady;

  return {
    ...pack,
    _meta: {
      ...(pack._meta || {}),
      sqv,
      contentQualityValue: sqv.score,
      channelQualityValue: sqv.score,
      publishReady:
        typeof priorPublishReady === "boolean"
          ? priorPublishReady
          : sqv.publishReady,
    },
  };
}

export function assertChannelContentQualityValueStamped(pack, channel, label = channel) {
  const sqv = pack?._meta?.sqv;
  if (!sqv || typeof sqv.score !== "number" || !sqv.grade) {
    throw new Error(`channel_sqv_missing:${label}`);
  }
  if (!String(sqv.version || "").startsWith("v")) {
    throw new Error(`channel_sqv_version_missing:${label}`);
  }
  if (pack._meta?.contentQualityValue !== sqv.score) {
    throw new Error(`channel_contentQualityValue_mismatch:${label}`);
  }
  return sqv;
}

function isNorthStarDerivedChannelPack(pack = {}) {
  return Boolean(pack?._meta?.channelNorthStarPack || pack?._meta?.derivedFromVerifiedBlog);
}

/** 배치·prod 공통 — story gate 생략, belief·분량 보강만 (조사 본문 과삭제 방지) */
function finishChannelPackLight(channel, pack, ctx = {}, finishFlags = {}) {
  if (!pack) return pack;
  const input = enrichChannelInput(ctx.input || ctx);
  const stackCtx = { input, ...ctx, batchLocalFinish: Boolean(finishFlags.batchLocalChannelFinish) };
  const northStarChannel = isNorthStarDerivedChannelPack(pack);

  if (
    northStarChannel ||
    (!finishFlags.batchLocalChannelFinish &&
      shouldUseWriterFirstChannelPostProcess(input, pack, channel))
  ) {
    let next = channel === "instagram" ? dedupeInstaLineBreakBody(pack) : pack;
    next = finalizeWriterFirstChannelDelivery(next, channel, input);
    if (channel === "place" && !northStarChannel) {
      next = humanizePlaceNoticePack(next, input);
    }
    const beliefFinal = scoreHumanBelief(getChannelFullText(next, channel), input, next);
    const channelNorthStar = assessChannelVisitNorthStar(next, channel, input);
    next = {
      ...next,
      _meta: {
        ...(next._meta || {}),
        humanBeliefScore: beliefFinal.score,
        humanBelief: beliefFinal,
        channelVisitNorthStar: channelNorthStar,
        channelVisitNorthStarOk: channelNorthStar.publishOk,
        channelFirstDelivery: assessChannelFirstDeliveryQuality(next, channel, input),
        channelPackFinished: true,
        ...finishFlags,
      },
    };
    return stampCoreRulesOnDelivery(
      stampChannelContentQualityValue(next, channel, input),
      input,
      channel
    );
  }

  let next = channel === "instagram" ? dedupeInstaLineBreakBody(pack) : pack;
  const minChars = channel === "instagram" ? 240 : 180;
  let textLen = getChannelFullText(next, channel).replace(/\s/g, "").length;
  if (textLen < minChars) {
    next = expandPackByInformation(next, stackCtx, input, {
      channel,
      minChars,
      salvageForce: true,
    });
  }
  if (channel === "instagram") {
    next = ensureInstagramLineBreakBody(next, stackCtx, input);
  }

  next = applyChannelEditorWriterDeliveryPass(next, channel, input, stackCtx);

  next = applyChannelUnifiedProsePass(next, channel, input);

  next = applyChannelQualityStack(next, channel, stackCtx);

  if (channel === "place") {
    next = {
      ...next,
      shortNotice: next.shortNotice
        ? applyLocalEditorBeliefPassToText(next.shortNotice)
        : next.shortNotice,
      detailBody: next.detailBody
        ? applyLocalEditorBeliefPassToText(next.detailBody)
        : next.detailBody,
    };
  } else if (channel === "instagram") {
    const bodyKey = next.lineBreakBody ? "lineBreakBody" : "body";
    next = dedupeInstaLineBreakBody({
      ...next,
      hook: next.hook ? applyLocalEditorBeliefPassToText(next.hook) : next.hook,
      [bodyKey]: next[bodyKey]
        ? applyLocalEditorBeliefPassToText(next[bodyKey])
        : next[bodyKey],
      ending: next.ending
        ? applyLocalEditorBeliefPassToText(next.ending)
        : next.ending,
    });
  }

  const belief = scoreHumanBelief(getChannelFullText(next, channel), input, next);
  if (channel === "place") {
    next = applyPlaceExpertPanel(next, stackCtx);
  } else if (channel === "instagram") {
    if (finishFlags.batchLocalChannelFinish) {
      next = ensureInstagramLineBreakBody(next, stackCtx, input);
      const bodyKey = next.lineBreakBody ? "lineBreakBody" : "body";
      next = dedupeInstaLineBreakBody({
        ...next,
        hook: next.hook ? applyLocalEditorBeliefPassToText(next.hook) : next.hook,
        [bodyKey]: next[bodyKey]
          ? applyLocalEditorBeliefPassToText(next[bodyKey])
          : next[bodyKey],
        ending: next.ending
          ? applyLocalEditorBeliefPassToText(next.ending)
          : next.ending,
      });
    } else {
      next = applyInstagramExpertPanel(next, stackCtx);
    }
  }
  next = applyKoreanOrthographyToChannelPack(next, channel, input);
  next = applyChannelSovereignTrimPass(next, channel, input);
  if (channel === "place") {
    next = humanizePlaceNoticePack(next, input);
  }
  next = applyChannelContentQualityMetaPass(next, channel, stackCtx, input);
  const beliefFinal = scoreHumanBelief(getChannelFullText(next, channel), input, next);
  const channelNorthStar = assessChannelVisitNorthStar(next, channel, input);
  next = {
    ...next,
    _meta: {
      ...(next._meta || {}),
      humanBeliefScore: beliefFinal.score,
      humanBelief: beliefFinal,
      channelFirstDelivery: assessChannelFirstDeliveryQuality(next, channel, input),
      channelVisitNorthStar: channelNorthStar,
      channelVisitNorthStarOk: channelNorthStar.publishOk,
      channelPackFinished: true,
      ...finishFlags,
    },
  };
  return stampCoreRulesOnDelivery(
    stampChannelContentQualityValue(next, channel, input),
    input,
    channel
  );
}

/** 배치·크로스채널 — story gate가 조사 본문을 과삭제하지 않도록 경량 finish */
export function finishChannelPackForBatch(channel, pack, ctx = {}) {
  return finishChannelPackLight(channel, pack, ctx, { batchLocalChannelFinish: true });
}

/** prod API·fallback — 배치와 동일 경량 finish (story gate 2회 적용 방지) */
export function finishChannelPackForDelivery(channel, pack, ctx = {}) {
  return finishChannelPackLight(channel, pack, ctx, { deliveryChannelFinish: true });
}

export function resolveChannelPassOutputAfterFinish(
  pack,
  channel = "place",
  input = {},
  basePass = false
) {
  if (!isBriclogMissionEnforced()) return Boolean(basePass);
  const first = assessChannelFirstDeliveryQuality(pack, channel, input);
  return (
    Boolean(basePass) &&
    first.displayReady &&
    pack?._meta?.humanEditorPass !== false &&
    pack?._meta?.personaAligned !== false
  );
}

/**
 * 블로그 파생·LLM 채널 공통 — humanity finish 확장
 */
export function applyChannelQualityStack(pack, channel = "place", ctx = {}) {
  const input = enrichChannelInput(ctx.input || ctx);
  if (!pack) return pack;

  const skipMarketer =
    pack?._meta?.channelNorthStarPack ||
    pack?._meta?.researchGroundedChannelPack ||
    pack?._meta?.derivedFromVerifiedBlog ||
    pack?._meta?.launchPublishFirst;

  let next = applyChannelEditorPolishPass(pack, channel, input);
  next = applyChannelHumanityCommonSenseMetaPass(next, channel, input);
  if (!skipMarketer) {
    next = applyChannelMarketerPack(next, channel, { ...ctx, input }, input);
  }
  next = applyChannelPersonaMetaPass(next, input, channel);
  next = applyChannelContentQualityMetaPass(next, channel, ctx, input);

  const full = getChannelFullText(next, channel);
  const belief = scoreHumanBelief(full, input, next);

  return {
    ...next,
    _meta: {
      ...(next._meta || {}),
      humanBeliefScore: belief.score,
      humanBelief: belief,
      channelQualityStack: CHANNEL_QUALITY_VERSION,
    },
  };
}

function applyChannelHumanityCommonSenseMetaPass(pack, channel, input) {
  const sense = scoreChannelHumanityCommonSense(pack, channel, input);
  return {
    ...pack,
    _meta: {
      ...(pack._meta || {}),
      humanityCommonSense: sense,
      humanReadLikely: sense.humanRead,
    },
  };
}

/**
 * 파생·로컬 파이프라인 출구
 */
export function finishChannelPack(channel, pack, ctx = {}) {
  if (!pack) return pack;
  if (pack._meta?.channelPackFinished) return pack;
  const input = enrichChannelInput(ctx.input || ctx);
  const insights = ctx.insights || pack._meta?.blogInsights;
  const stackCtx = { input, ...ctx, insights: insights || ctx.insights };

  let next =
    channel === "instagram"
      ? dedupeInstaLineBreakBody(pack)
      : applyDuplicateKiller(pack, stackCtx, channel);

  if (channel === "place") {
    const detailLen = String(next.detailBody || "").replace(/\s/g, "").length;
    const noticeLen = String(next.shortNotice || "").replace(/\s/g, "").length;
    if (detailLen < 120 || noticeLen < 12) {
      next = expandPackByInformation(next, stackCtx, input, {
        channel: "place",
        minChars: 180,
      });
    }
    const brand = String(input.brandName || "").trim();
    const region = String(input.region || "").trim();
    if (String(next.shortNotice || "").replace(/\s/g, "").length < 12 && brand) {
      next = {
        ...next,
        shortNotice: `${region ? `${region} ` : ""}${brand} 매장 소식 — 문의·예약 안내`.slice(
          0,
          120
        ),
      };
    }
  } else if (channel === "instagram") {
    const bodyLen = String(next.lineBreakBody || next.body || "").replace(/\s/g, "")
      .length;
    if (bodyLen < 200) {
      next = expandPackByInformation(next, stackCtx, input, {
        channel: "instagram",
        minChars: 240,
        salvageForce: true,
      });
    }
  }

  next = applyChannelQualityStack(next, channel, stackCtx);
  next = applyChannelUnifiedProsePass(next, channel, input);
  if (isBriclogMissionEnforced()) {
    next = applyChannelStoryGate(next, channel, stackCtx);
  }
  next = applyChannelRecallSoftenPass(next, channel);
  next =
    channel === "instagram"
      ? dedupeInstaLineBreakBody(next)
      : applyDuplicateKiller(next, stackCtx, channel);

  if (channel === "place") {
    next = {
      ...next,
      shortNotice: next.shortNotice
        ? applyLocalEditorBeliefPassToText(next.shortNotice)
        : next.shortNotice,
      detailBody: next.detailBody
        ? applyLocalEditorBeliefPassToText(next.detailBody)
        : next.detailBody,
    };
  } else if (channel === "instagram") {
    const bodyKey = next.lineBreakBody ? "lineBreakBody" : "body";
    next = {
      ...next,
      hook: next.hook ? applyLocalEditorBeliefPassToText(next.hook) : next.hook,
      [bodyKey]: next[bodyKey]
        ? applyLocalEditorBeliefPassToText(next[bodyKey])
        : next[bodyKey],
      ending: next.ending
        ? applyLocalEditorBeliefPassToText(next.ending)
        : next.ending,
    };
  }

  const belief = scoreHumanBelief(getChannelFullText(next, channel), input, next);

  if (channel === "place") {
    next = applyPlaceExpertPanel(next, stackCtx);
  } else if (channel === "instagram") {
    next = applyInstagramExpertPanel(next, stackCtx);
  }

  next = applyChannelSovereignTrimPass(next, channel, input);

  const templateLeak = detectChannelTemplateLeak(next, channel, input);
  const channelNorthStar = assessChannelVisitNorthStar(next, channel, input);
  if (!templateLeak.ok || !channelNorthStar.ok) {
    const rescued =
      channel === "place"
        ? buildResearchGroundedPlacePack(input)
        : buildResearchGroundedInstagramPack(input, input.instaTone || "emotional");
    if (rescued?.title || rescued?.hook || rescued?.shortNotice) {
      next = {
        ...rescued,
        _meta: {
          ...(rescued._meta || {}),
          ...(next._meta || {}),
          channelDeliveryRescue: true,
          channelTemplateLeak: templateLeak.issues,
        },
      };
      if (channel === "place") {
        next = humanizePlaceNoticePack(next, input);
      }
    }
  }

  next = applyChannelContentQualityMetaPass(next, channel, stackCtx, input);
  const beliefFinal = scoreHumanBelief(getChannelFullText(next, channel), input, next);
  const channelNorthStarFinal = assessChannelVisitNorthStar(next, channel, input);
  next = {
    ...next,
    _meta: {
      ...(next._meta || {}),
      humanBeliefScore: beliefFinal.score,
      humanBelief: beliefFinal,
      channelFirstDelivery: assessChannelFirstDeliveryQuality(next, channel, input),
      channelPackFinished: true,
      sourceChannel: ctx.sourceChannel || pack._meta?.sourceChannel || "blog",
      derivedFromBlog: Boolean(ctx.sourceChannel === "blog" || pack._meta?.derivedFrom),
      channelVisitNorthStar: channelNorthStarFinal,
      channelVisitNorthStarOk: channelNorthStarFinal.publishOk,
    },
  };

  return stampCoreRulesOnDelivery(
    stampChannelContentQualityValue(next, channel, input),
    input,
    channel
  );
}
