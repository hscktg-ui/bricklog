...
export function tryWriterFirstOrchestratorDelivery(pack, input = {}) {
  ...
  let light = finalizeGpt55BlogPackForUi(pack, input);
  if (!light?.sections?.length) return null;

  const north = assessColumnVisitNorthStar(light, input);
  if (!north.publishOk || !north.spam.ok) return null;
  ...
  if (light._meta?.outputWithheld || light._meta?.publishReady !== true) {
    light = {
      ...light,
      _meta: {
        ...light._meta,
