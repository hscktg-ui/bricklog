/**
 * GPT-5.6 Light Delivery — 조사 → Writer(GPT) → 경량 점검 → 발행 전 화자·최종 마감
...
/** GPT-5.6 LLM 원고 — Max Quality 시 풀 humanity·에디터 경로 */
...
/** GPT-5.6 LLM 원고 — Max Quality·미달 시 Writer·풀 에디터 항상 실행 */
export function shouldSkipWriterEngineForGpt55(pack, input = {}) {
  ...
  return contract.ok && contract.humanVoiceMet;
}
...
/**
 * UI 송출 — finalizeBlogPackForUi 대체 (GPT-5.6 LLM)
 */
export function finalizeGpt55BlogPackForUi(pack, input = {}) {
  ...
  if (
    ...
    let next = stripGlobalExactDuplicateSentences(pack);
    next = applyDisplayBodyGuardPack(next, input);
    ...
    return {
      ...
      _meta: {
        ...
        columnistFastUi: true,
        gpt55LightDeliveryVersion: GPT55_LIGHT_DELIVERY_VERSION,
        generationMode: next._meta?.generationMode || "columnist_sovereign",
      },
    };
  }
  const inbound = pack;
  ...
  if (isBriclogMasterRebuildEnforced()) {
    next = applyMasterRebuildPostWritePass(next, input, { force: true });
  }
  next = applyGpt55VoiceFinalPass(next, input, { force: true });
  next = guardPackAgainstShrink(inbound, next, { stage: "gpt55LightDelivery" });
  next = isWriterFirstDeliveryEnabled()
    ? finalizeWriterFirstBlogDelivery(next, input)
    : finalizeContentQualityForDelivery(next, input, "blog", {
...
/**
 * 화면 표시 — ensureBlogDisplayPack 대체 (GPT-5.6 LLM)
 */
export function ensureGpt55BlogDisplayPack(blog, input = {}) {
  ...
  pack = isWriterFirstDeliveryEnabled()
    ? finalizeWriterFirstBlogDelivery(pack, input)
    : finalizeContentQualityForDelivery(pack, input, "blog", {
        afterWriterEngine: Boolean(pack._meta?.briclogWriterEngine),
      });
  return {
    ...pack,
...
/** Heavy v2 gate 실패 시 — writer-first light path로 칼럼 복붙 가능 여부 재판정 */
export function tryWriterFirstOrchestratorDelivery(pack, input = {}) {
  if (!isWriterFirstDeliveryEnabled() || !pack?.sections?.length) return null;
  if (
  ...
  const canEscape =
    ...
    chars >= 400 &&
    north.readability.hitCount >= 2;
  if (!canEscape) return null;

  let light = applyGpt55SalvageLightPass(pack, input);
  if (!light?.sections?.length) light = pack;

  const readAloud = light._meta?.readAloudHumanGate;
  const engineSpam = readAloud?.hardReasons?.includes("engine_spam_draft");
  if (engineSpam || light._meta?.regionBrandMashOk === false) return null;
  ...
  return {
    ...
    _meta: {
      ...
      publishReady: true,
      columnVisitNorthStarOk: north.publishOk,
      writerFirstDelivery: true,
      writerFirstOrchestratorEscape: true,
      passOutput: true,
      columnistDeliveryOk: true,
      generationMode: light._meta?.generationMode || "llm_gpt55",
    },
  };
}
