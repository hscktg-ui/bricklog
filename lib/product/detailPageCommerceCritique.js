...
export function scoreDetailPageCommerce(args = {}) {
  ...
  const hasTable =
    (html.includes("<table") && html.includes("spec-sheet")) ||
    html.includes('data-composition="specification"');
  const hasFaq =
    html.includes("<dl") ||
    html.includes('data-layout="faq"') ||
    (html.includes("도서산간") && html.includes("교환"));
  const hasCta =
    html.includes('data-cta="buy"') ||
    html.includes('data-cta="close"') ||
