...
/** humanBeliefEngine SSOT와 동기 — import 순환 방지용 상수 */
export const CORE1_BELIEF_FLOOR_BLOG = 75;
export const CORE1_BELIEF_FLOOR_CHANNEL = 63;
...
export function stampCoreRulesOnInput(input = {}) {
  if (!input || !isBriclogCoreRulesEnforced()) return input;
  ...
  const mergedFeedbackHints = [
    ...
    ...(Array.isArray(input.brandMemory?.feedbackIntents)
      ? input.brandMemory.feedbackIntents
      : []),
  ].filter(Boolean);
  const uniqHints = [...new Set(mergedFeedbackHints)].slice(0, 12);

  const memoryForbidden = String(
    input.brandMemory?.forbiddenWords || input.brandMemory?.excludePhrases || ""
  ).trim();
  const excludeMerged = [input.excludePhrases, memoryForbidden]
    .filter(Boolean)
    .join(", ")
    .trim();
  ...
  return {
    ...
    feedbackHints: uniqHints.length ? uniqHints : input.feedbackHints,
    excludePhrases: excludeMerged || input.excludePhrases,
    _coreRulesInput: {
      core2,
