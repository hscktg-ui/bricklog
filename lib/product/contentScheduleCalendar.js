 ...
 * @param {unknown[]} archiveItems
 */
export function buildScheduleHistory(memoryItems = [], archiveItems = []) {
  return mergeDraftHistoryItems(memoryItems, archiveItems).map(
...
export function buildScheduleTips(input = {}) {
  ...
  /** @type {{ id: string; kind: 'gap' | 'season' | 'rhythm'; tone: 'info' | 'warn' | 'accent'; title: string; body: string }[]} */
  const tips = [];
...
export function buildContentScheduleView(input = {}) {
  ...
  const history = buildScheduleHistory(input.memoryItems || [], archiveItems);
