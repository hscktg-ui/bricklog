 ...
 * Research First V2: 조사 dossier(STEP1–8) → 본문(STEP9) → 평가(STEP10–12)
 * @see briclogResearchFirstPipeline.js
 */
...
import { applyBriclogDeleteEngine } from "@/lib/product/briclogDeleteEngine";
...
export const EVALUATE_FIRST_PIPELINE_VERSION = "eval-first-v2";
...
export function evaluateReviseAndGateOutput(pack, input = {}, opts = {}) {
  ...
  const forcedMission =
    opts.forcedMissionProseRoute === true || pack?._meta?.forcedMissionProseRoute === true;
  if (!forcedMission) {
    next = runIndustryPipelineSanitize(next, input);
    next = injectBrandFactsIntoPack(next, input);
  }
  if (isBriclogMasterRebuildEnforced() && !forcedMission) {
    next = applyBriclogDeleteEngine(next, input);
  ...
  next = applyQualityAug2026Finish(next, input, {
    forceSafeEdit: !evaluation.pass,
  });
  next = stampQualityAug2026Stack(next, input);
  evaluation = assessContentEvaluation(next, input);
  const outputAllowed =
    resolveDeliveryAllowed(next, evaluation) ||
    (isBriclogAlwaysDeliverEnabled() && Boolean(next?.sections?.length));
