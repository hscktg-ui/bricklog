...
export function formatSqGradeShort(grade, score) {
  ...
  if (!shouldShowInternalQualityScore()) return label;
  return `글값 ${g} · ${label}`;
}
...
export function formatSqUserHint(sqv = {}, meta = {}) {
  ...
  const customer = !shouldShowInternalQualityScore();

  if (customer) {
    if (meta.publishReady === true || sqv.publishReady === true) {
      return tips.length ? tips[0] : "복사해 올려도 됩니다";
    }
    if (completionLow) {
      return tips.length
        ? `분량·구성을 맞추는 중입니다. ${tips[0]}`
        : "분량·구성을 맞추는 중입니다";
    }
    if (score >= B_GRADE_MIN_SCORE) {
      return tips.length ? tips.join(" · ") : "한 번 읽고 올려 주세요";
    }
    if (score >= 50) {
      return tips.length ? tips[0] : "사실·톤만 확인해 주세요";
    }
    return "「다시 받기」 또는 직접 다듬어 주세요";
  }
