...
import { calibrateSqToBGradeMinimum } from "@/lib/product/bGradeDeliveryEngine";
import { calibrateSqToAGradeMinimum } from "@/lib/product/aGradeDeliveryEngine";
import { calibrateResearchHeavySqMinimum } from "@/lib/content/researchHeavyDeliveryEngine";
import {
...
const WEIGHTS = {
  ...
  humanBelief: 0.18,
  explainability: 0.14,
  completeDelivery: 0.22,
  speakerSurface: 0.14,
  brandSpecificity: 0.1,
};
...
export function computeContentQualityValue(pack, input = {}) {
  ...
  const brandSpecific = isResearchProperNounTopic(input)
    ? scoreResearchProperNounAnchoring(fullText, input)
    : null;
  ...
  let brandSpecificityScore = brandSpecific?.score ?? 100;

  const editorBoost =
    (missionEvalPass || editorEvalPass || editorEligible) && complete.ok;
  if (editorBoost) {
    ...
    brandSpecificityScore = Math.max(
      brandSpecificityScore,
      brandSpecific?.productRatio >= 0.45 ? 82 : 76
    );
  ...
  const brandWeight =
    brandSpecific?.brandSpecific || isResearchHeavyTopicInput(input)
      ? WEIGHTS.brandSpecificity
      : 0;
  const weightScale = brandWeight > 0 ? 1 - brandWeight : 1;
  const scaled = (key) => WEIGHTS[key] * weightScale;
  ...
  const raw = {
    ...
    score: Math.max(
      ...
      Math.min(
        100,
        Math.round(
          personaScore * scaled("persona") +
            beliefScore * scaled("humanBelief") +
            explainScore * scaled("explainability") +
            completeScore * scaled("completeDelivery") +
            speakerScore * scaled("speakerSurface") +
            brandSpecificityScore * brandWeight
        )
      )
    ),
    breakdown: {
      ...
      brandSpecificity: brandSpecificityScore,
    },
  ...
  if (brandSpecific?.brandSpecific && brandSpecificityScore < 64 && !editorBoost) {
    reasons.push("brand_proper_noun_weak");
  }
  ...
  const editorCalibrated = calibrateSqvForProfessionalEditor(sqv, pack, input);
  const aCalibrated = calibrateSqToAGradeMinimum(editorCalibrated, pack, input);
  const bCalibrated = calibrateSqToBGradeMinimum(aCalibrated, pack, input);
  return applyDiagnosticScoreTrust(
    calibrateResearchHeavySqMinimum(bCalibrated, pack, input),
    pack,
    input,
    "blog"
  );
