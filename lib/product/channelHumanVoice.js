...
/** @typedef {'blog' | 'place' | 'instagram' | 'insta' | 'detailPage'} ChannelVoiceId */
...
const VOICES = {
  ...
  place: {
    ...
    loadingSteps: [
      { sketch: "place", text: "브랜드 담당자 공지 쓰는 중…" },
  ...
  detailPage: {
    ...
    resultLine: "Pretendard 860px 상세 리듬으로 맞췄어요",
    loadingSteps: [
      { sketch: "map", text: "상품 기준 맞추는 중…" },
      { sketch: "write", text: "상세 섹션 짜는 중…" },
      { sketch: "check", text: "글꼴·붙여넣기 마무리 중…" },
    ],
    completeMessage: "상세페이지 디자인이 준비됐어요 — HTML을 복사해 붙이세요",
    trustPrefix: "상품 상세를 직접 짠 것처럼",
    sidebarDesc: "고르는 기준이 보이는 860px 상세",
  },
};
...
export function normalizeChannelVoiceId(channel = "blog") {
  ...
  if (ch === "detailpage" || ch === "detail-page" || ch === "detail") return "detailPage";
...
export const BRICLOG_MAIN_PROMISE =
  "채널마다 칼럼니스트·브랜드 담당자·마케터·상세 디자인 — 보통 1~2분이면 편집본이 나옵니다.";
