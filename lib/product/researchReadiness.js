...
/** 첫 노출 — 일반 주제 최소 조사 팩트 수 (MIN_INFORMATION_UNITS=20 기준) */
export const MIN_RESEARCH_FACTS_FOR_FIRST_WRITE = 10;
...
const INTERNAL_LEAK_RES = [
  ...
  /referenceerror|syntaxerror|unexpected token/i,
  /is not defined/i,
  /industry_furniture|research_first_blocked/i,
];
...
/** input 단일 인자 — v2Axis·research 병합 SSOT */
export function resolveResearchContextFromInput(input = {}) {
  return {
    parsed: input.v2AxisParsed || input.v2Parsed || {},
    research:
      input.research ||
      input.v2Research ||
      input.researchResult ||
      input.v2AxisResearch ||
      {},
  };
}

export function collectMergedResearchFactsFromInput(input = {}) {
  const { parsed, research } = resolveResearchContextFromInput(input);
...
export function evaluateResearchWriteGate(input = {}, parsed = {}, research = {}) {
  const resolved = materializeVerifiedGenerationAxes(input);
  const brand = String(resolved.brandName || "").trim();
  const region = String(resolved.region || "").trim();
  const topic =
    String(resolved.topic || "").trim() ||
    String(resolved.mainKeyword || "").trim();

  if (!hasFilledBlogAxes(resolved)) {
    return {
      ok: false,
...
export function resolveMinResearchFactsForWrite(input = {}, parsed = {}, research = {}) {
  ...
  const tierMin =
    tier === "long" ? 14 : tier === "medium" ? 12 : MIN_RESEARCH_FACTS_FOR_FIRST_WRITE;
  if (hasAnyOnlineResearchSignal(input, parsed, research)) {
    return Math.max(1, tierMin - 4);
  }
  if (hasCompletedResearchStep(input)) return Math.max(1, tierMin - 2);
