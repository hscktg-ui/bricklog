...
function distributeInstagramLineEmojis(text, pool, budget) {
  const lines = String(text || "").split("\n");
  let used = countEmoji(lines.join("\n"));
  const out = lines.map((line, i) => {
    if (used >= budget.max) return enforceOneEmojiPerLine(stripEmojiSpam(line));
    const trimmed = String(line || "").trim();
    if (!trimmed || countEmoji(trimmed)) return enforceOneEmojiPerLine(stripEmojiSpam(line));
    if (used < budget.min || (i < 4 && used < budget.max)) {
      used += 1;
      return prefixEmojiOnce(trimmed, pool[(i + used) % pool.length] || CHECK_EMOJI);
    }
    return enforceOneEmojiPerLine(stripEmojiSpam(line));
  });
  return out.join("\n");
...
function applyInstagramEmoji(pack, level, pool, rules) {
  ...
  if (ending && used < rules.max) {
    ending = prefixEmojiOnce(ending, pool[1] || "📍");
    used += 1;
  }

  hook = enforceOneEmojiPerLine(stripEmojiSpam(hook));
  body = distributeInstagramLineEmojis(body, pool, {
    min: Math.max(0, rules.min - countEmoji([hook, ending].filter(Boolean).join("\n"))),
    max: rules.max,
  });
  ending = enforceOneEmojiPerLine(stripEmojiSpam(ending));

  let lineBreakBody = trimTextToEmojiBudget(
    [hook, body, ending].filter(Boolean).join("\n\n"),
    rules.min,
    rules.max
  );
  if (countEmoji(lineBreakBody) < rules.min) {
    const parts = lineBreakBody.split("\n\n");
    hook = parts[0] || hook;
    body = parts.slice(1, -1).join("\n\n") || body;
    ending = parts.length > 1 ? parts[parts.length - 1] : ending;
    body = distributeInstagramLineEmojis(body, pool, rules);
    lineBreakBody = trimTextToEmojiBudget(
      [hook, body, ending].filter(Boolean).join("\n\n"),
      rules.min,
      rules.max
    );
