...
/** 공백 포함 본문 글자 수 기준 — V20 tier (V2: 기본 약 4,000자) */
export const BLOG_LENGTH_TIER_OPTIONS = [
  { value: "short", label: "기본 글", hint: "약 4,000자 · 사람이 쓴 블로그" },
  { value: "medium", label: "중간 글", hint: "약 5,600~6,400자" },
  { value: "long", label: "긴 글", hint: "약 7,600~10,000자" },
];
...
/** 제품 기본 — V20 short tier (target 4,000자) */
export const DEFAULT_BLOG_LENGTH_TIER = "short";

export function resolveBlogLengthTier(tier) {
  const key = tier || DEFAULT_BLOG_LENGTH_TIER;
  return BLOG_LENGTH_TARGETS[key] || BLOG_LENGTH_TARGETS[DEFAULT_BLOG_LENGTH_TIER];
}

/** @param {"short"|"medium"|"long"} tier */
export function getBlogLengthTierLabel(tier) {
  const key = tier || DEFAULT_BLOG_LENGTH_TIER;
  return (
    BLOG_LENGTH_TIER_OPTIONS.find((o) => o.value === key)?.label || "기본 글"
  );
}

/** @deprecated prefer resolveBlogLengthTier — default short tier */
export const BLOG_MIN_BODY_CHARS = BLOG_LENGTH_TARGETS.short.min;
export const BLOG_TARGET_BODY_CHARS = BLOG_LENGTH_TARGETS.short.target;
export const BLOG_MAX_BODY_CHARS = BLOG_LENGTH_TARGETS.short.max;
...
export const GENERATION_MIN_OVERLAY_MS = 400;
import {
...
export const DEFAULT_BLOG_INPUT = {
  ...
  /** 사용자가 결과 화면에서 지정하는 이번 톤 (브랜드 습관보다 우선 반영) */
  toneRequest: "",
  researchEnabled: true,
  researchTypes: ["latest", "local", "keyword", "trend"],
  researchQuery: "",
};
