...
function makeSample(seed) {
  ...
  const channels = buildLandingChannelPacks(
    { ...seed, industry: industryForFeaturedSeed(seed) },
    blogPack
  );
...
export const DEMO_FLOW_STEPS = [
  ...
  { title: "4. 운영 글 받기", hint: "이야기 · 플레이스 · 인스타 초안" },
  { title: "5. 상품 화면 · 이어가기", hint: "상세에서 붙일 화면, 다음 주는 운영 계획" },
];
...
export const WORKFLOW_STEPS = [
  ...
  },
  {
    n: "05",
    title: "브릭로그 상세",
    desc: "상품명·사진을 넣으면 스마트스토어·쿠팡에 붙일 화면이 나갑니다.",
  },
  {
    n: "06",
    title: "다음 주 계획",
...
export const WHY_BRICLOG = [
  ...
  {
    title: "운영 글, 그리고 상세",
    desc: "한 주제로 이야기·플레이스·인스타. 상품은 고르는 화면으로 따로 짭니다.",
  },
  {
    title: "월·주 운영 계획",
...
export function getChannelSnippetsFromSample(sample) {
  ...
  return {
    ...
    detail: "상품명·사진을 넣으면 스마트스토어·쿠팡에 붙일 화면",
