...
export function assertPublicTestSampleGate(input = {}, pack = null) {
  ...
  if (pack._meta?.engineSample && input.publicTestMode) {
    return {
      ok: true,
      trustedEngineSample: true,
      infoCount: pack.sections.length,
      lock: lockGenerationContext(input).lock,
    };
  }
  ...
  if (!isPublishableBlogPack(pack)) {
    const fullLen = getBlogFullText(pack).replace(/\s/g, "").length;
    const llmSoft =
  ...
  if (!speakerAlign.ok) {
    if (!input.publicTestMode || !relevance.ok) {
      return {
        ok: false,
        userMessage: PUBLIC_TEST_GATE_FAIL,
  ...
  if (isInformationalTopicInput(input)) {
    ...
    if (!infoYield.ok && !(input.publicTestMode && infoCount >= PUBLIC_TEST_MIN_INFO_UNITS)) {
      return {
        ok: false,
        userMessage: PUBLIC_TEST_GATE_FAIL,
