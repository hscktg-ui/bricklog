...
export function buildBriclogContextScore(input = {}, pack = {}, gate = {}) {
  ...
  const axes = [
    ...
    },
    {
      ...
      score: clampScore(
        v3?.topic ??
          v2?.product ??
          (topicLabel ? 58 + relevance * 34 : 52 + relevance * 36)
      ),
    },
    {
      id: "trust",
  ...
  if (reflection.season) {
    axes.push({
      ...
      score: clampScore(56 + relevance * 36),
    });
  }
  if (reflection.speaker) {
    axes.push({
      id: "speaker",
      label: `화자 · ${reflection.speaker}`,
      score: clampScore(54 + relevance * 38),
    });
  }
...
export function buildChannelWorkspaceContextScore(
  ...
  const axes = [
    {
      id: "brand",
      label: "브랜드 맥락",
      score: clampScore(52 + relevance * 38),
    },
    {
      id: "region",
      label: "지역 정합",
      score: clampScore(50 + relevance * 36),
    },
    {
      id: "topic",
      label: topic ? `주제 · ${topic.slice(0, 12)}` : "주제 답변",
      score: clampScore(54 + relevance * 34),
    },
    {
      id: "trust",
      label: "신뢰·근거",
      score: clampScore(
        (typeof sqv.score === "number" ? sqv.score : 58) * 0.92
      ),
    },
  ];
  ...
  return {
    ...
    checks: {
      relevancePct: Math.round(relevance * 100),
      infoUnits: input.researchFactCount ?? input.researchFacts?.length ?? 0,
      grounded: meta.researchGroundedHumanPack === true,
    },
