 ...
 */
export const BRICLOG_DEFAULTS_VERSION = "briclog-defaults-v2";
...
export const BRICLOG_TIMING_DEFAULTS = Object.freeze({
  ...
  /** Async job — poll·TTL (Vercel run 300s · 생성 130–200s 여유) */
  asyncPollIntervalMs: 2_000,
  asyncJobTtlMs: 240_000,
  asyncStartTimeoutMs: 15_000,
  /** run 트리거 — 서버 생성 완료까지 연결 유지 (abort 금지) */
  asyncRunAwaitTimeoutMs: 310_000,
  /** 블로그 생성 고객 체감 SLA */
...
/** Async poll 상한 — 생성 200s+ · after() worker 여유 */
...
export function getAsyncRunAwaitTimeoutMs() {
  return readEnvNumber(
    "BRICLOG_ASYNC_RUN_AWAIT_TIMEOUT_MS",
    BRICLOG_TIMING_DEFAULTS.asyncRunAwaitTimeoutMs
  );
}
