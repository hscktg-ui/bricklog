...
const BUDGETS = Object.freeze({
  blog: Object.freeze({
    columnistFast: 2800,
    columnistSlow: 5200,
    columnistFastDossier: 2600,
    columnistSlowDossier: 3600,
...
/** dossier·축 조사 완료 — 펼치기 전용 토큰 (비용↓) */
export function hasColumnistDossierReady(input = {}) {
  const d = input.researchFirstDossier;
  if (d?.writable && (d.organized?.lines?.length >= 2 || d.factCount >= 2)) {
    return true;
  }
  return Boolean(
    input.v2ResearchReady &&
      input.v2PreWriteVerified &&
      (input.researchFactCount >= 2 || input.researchFacts?.length >= 2)
  );
}
...
export function resolveColumnistCompletionTokens(input = {}, opts = {}) {
  ...
  const dossierReady = hasColumnistDossierReady(input);
  if (fast && dossierReady) return getChannelTokenBudget("blog", "columnistFastDossier");
  if (!fast && dossierReady) return getChannelTokenBudget("blog", "columnistSlowDossier");
