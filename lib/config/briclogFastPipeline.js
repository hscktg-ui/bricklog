...
export function getBlogClientFetchTimeoutMs() {
  ...
  const customerMargin = isCustomerTwoMinuteSlaMode() ? 30_000 : 45_000;
...
export function getBlogGenerationProbeTimeoutMs() {
  ...
  if (Number.isFinite(n) && n > 0) return n;
  return getCustomerBlogSlaMs() + 45_000;
}
...
/** 2분 SLA — fast 실패 시 API 재시도 (빈 응답·finish reject 완화, slow fallback 없음) */
