...
export function getBlogClientFetchTimeoutMs() {
  if (isLaunchPublishFirstMode()) return LAUNCH_PUBLISH_CLIENT_FETCH_MS;
...
/** 2분 SLA — 칼럼니스트 1회 토큰 상한 */
export function getColumnistFastMaxTokens() {
  if (!isColumnistFastDeliveryEnabled()) return 6500;
  if (isCustomerTwoMinuteSlaMode()) {
    const n = Number(process.env.BRICLOG_COLUMNIST_FAST_MAX_TOKENS);
...
export function getColumnistMaxLlmRounds(input = {}) {
  ...
  if (isCustomerTwoMinuteSlaMode() || isColumnistFastDeliveryEnabled()) return 2;
  return 3;
}

/** 2분 SLA — fast 실패 시 API 1회 재시도 (클라이언트 3회 폴링 대신) */
...
/** 2분 SLA — fast+재시도 실패 후 slow columnist 1회 (성공률↑, 비용↑) */
export function shouldUseColumnistSlaSlowFallback() {
  if (process.env.BRICLOG_COLUMNIST_SLA_SLOW_FALLBACK === "false") return false;
  return isCustomerTwoMinuteSlaMode();
}
...
export function getBlogWriteMaxTokens(tier = "medium") {
  ...
  if (usesMaxCompletionTokens(OPENAI_WRITER_MODEL)) {
    const headroom = tier === "short" ? 4000 : tier === "long" ? 4800 : 4400;
