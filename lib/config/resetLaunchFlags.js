 ...
 */
import { isBriclogFreeLaunchMode } from "@/lib/billing/freeLaunchMode";
...
export function getPublicResetLaunchFlags() {
  ...
  const researchFirst =
    process.env.BRICLOG_RESEARCH_FIRST === "false"
      ? false
      : process.env.BRICLOG_RESEARCH_FIRST === "true" || qualityReset;
  const brandContentOS =
    process.env.BRICLOG_BRAND_CONTENT_OS === "false"
      ? false
      : process.env.BRICLOG_BRAND_CONTENT_OS === "true" || qualityReset;
  const experienceOpinion =
    process.env.BRICLOG_EXPERIENCE_OPINION === "false"
      ? false
      : process.env.BRICLOG_EXPERIENCE_OPINION === "true" || qualityReset;
  return {
    qualityReset,
    researchFirst,
    brandContentOS,
    experienceOpinion,
    productIdentity: "Brand Content OS",
    userValue: "이번 달 운영 계획이 생겼다",
    contentOsKpi: {
      planning: 30,
      research: 30,
      explain: 20,
    ...
    freeLaunch: isBriclogFreeLaunchMode(),
