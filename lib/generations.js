...
/** generations NOT NULL text columns — null/undefined → '' */
...
export async function saveGeneration(userId, data) {
  const { data: row, error } = await supabase
    ...
    .insert({
      user_id: userId,
      business_type: generationText(data.business_type),
      region: generationText(data.region),
      main_keyword: generationText(data.main_keyword),
      sub_keywords: generationText(data.sub_keywords),
      purpose: generationText(data.purpose),
      tone: generationText(data.tone),
      blog: generationText(data.blog),
      place: generationText(data.place),
      instagram: generationText(data.instagram),
      hashtags: generationText(data.hashtags),
      image_prompt: generationText(data.image_prompt),
      brand_id: data.brand_id || null,
      full_copy_text: generationText(data.full_copy_text),
    })
    .select()
    .single();
...
export async function fetchGenerations(userId, { sinceIso, brandId } = {}) {
  let q = supabase
    .from("generations")
    .select(
      "id, business_type, region, main_keyword, purpose, tone, created_at, brand_id"
    )
    .eq("user_id", userId)
...
/** 스케줄 캘린더 — 채널별 생성 이력 */
export async function fetchGenerationsForSchedule(
  userId,
  { sinceIso, brandId } = {}
) {
  let q = supabase
    .from("generations")
    .select("id, brand_id, main_keyword, region, blog, place, instagram, created_at")
    .eq("user_id", userId)
    .order("created_at", { ascending: false });
