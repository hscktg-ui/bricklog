...
/**
 * 롤백용 — merge 없이 규칙 전체 교체
 * @param {Record<string, object>} rulesByKey keyed by rule file name
 */
export async function replaceGlobalEngineRules(rulesByKey = {}, db = null) {
  const client = db || createServiceSupabase();
  if (!client) return { ok: false, reason: "no_service_role" };

  const replaced = [];
  for (const [ruleKey, rules] of Object.entries(rulesByKey)) {
    if (!RULE_KEYS.includes(ruleKey) || !rules) continue;
    const next = {
      ...rules,
      updatedAt: new Date().toISOString(),
    };
    const { error } = await client.from("global_engine_rules").upsert(
      {
        rule_key: ruleKey,
        rules: next,
        version: next.version || 1,
    ...
    if (error) {
      if (isMissingGlobalEngineRulesTable(error)) {
        return { ok: false, reason: "tables_missing" };
      }
      throw error;
    }
    replaced.push(ruleKey);
  }
