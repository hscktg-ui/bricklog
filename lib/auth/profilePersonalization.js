...
export function profileNeedsSetup(profile) {
  ...
  if (profileSetupInSkipCooldown(profile)) return false;
...
export function defaultMenuFromProfile(profile) {
  ...
  if (use === "place") return "plan";
  if (use === "instagram") return "plan";
  if (use === "image_copy" || use === "brand_ops" || use === "multi_brand") {
    return "plan";
  }
  if (use === "blog") return "plan";
  return "plan";
}
