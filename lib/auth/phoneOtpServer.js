...
import { assertPhoneAvailableForSignup } from "@/lib/auth/checkPhoneServer";
...
async function invalidatePreviousOtps(phoneNormalized) {
  if (useDevOtpStore()) {
    devOtpInvalidateActive(phoneNormalized);
    return;
  }
...
async function insertOtpRow(norm, code) {
  await invalidatePreviousOtps(norm.e164);
...
/**
 * @param {string} phoneRaw
 * @param {{ excludeUserId?: string | null, signupStrict?: boolean }} [opts]
 */
export async function sendPhoneOtp(phoneRaw, opts = {}) {
  const { excludeUserId = null, signupStrict = false } = opts;
  ...
  const phoneAvail = await assertPhoneAvailableForSignup(phoneRaw, excludeUserId, {
    signupStrict,
  });
  if (!phoneAvail.ok) {
    return { ok: false, message: phoneAvail.message, code: phoneAvail.code };
  }
 ...
 * @param {{ excludeUserId?: string | null, signupStrict?: boolean }} [opts]
 */
export async function verifyPhoneOtp(phoneRaw, code, opts = {}) {
  const { excludeUserId = null, signupStrict = false } = opts;
  const norm = normalizePhoneInput(phoneRaw);
  if (!norm.ok) return { ok: false, message: norm.message };
...
export function isMissingPhoneOtpTable(error) {
  ...
  return (
    error?.code === "42P01" ||
    error?.code === "PGRST205" ||
    (/phone_otp_verifications/i.test(msg) &&
