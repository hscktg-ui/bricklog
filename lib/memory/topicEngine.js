...
export function buildTopicRecommendations(ctx = {}) {
  ...
  const recentClean = uniq(recentTopics.map((t) => String(t || "").trim()).filter((t) => t.length >= 2));
  const deepenTopics = recentClean.slice(0, 2).map((t) => `같은 주제 깊게: ${t}`);
  const refreshTopic = `시즌 새로고침: ${season} ${brandName || "브랜드"} 이야기`;

  const weekTopics = uniq([
    ...deepenTopics,
    refreshTopic,
    ...base,
    ...recentClean.map((t) => `이어쓰기: ${t}`),
  ]).slice(0, 6);
  ...
  const week = weekTopics.map((topic) => {
    let axis = "fresh";
    if (topic.startsWith("같은 주제 깊게:")) axis = "deepen";
    else if (topic.startsWith("시즌 새로고침:") || topic.includes("시즌")) axis = "refresh";
    else if (topic.startsWith("이어쓰기:")) axis = "deepen";
    return {
      topic,
      ...
      reason:
        axis === "deepen"
          ? "기존 주제를 한 층 더 풀어 주제 일관성(검색 신뢰)을 쌓습니다"
          : axis === "refresh"
            ? "시즌·시의성으로 같은 브랜드 축을 새로고침합니다"
            : "이번 주 새 축으로 운영합니다",
    };
  });
  ...
  return {
    ...
    week,
    month: monthTopics.map((topic) => ({
      topic,
      channels: ["blog"],
      axis: topic.startsWith("같은 주제 깊게:") || topic.startsWith("이어쓰기:")
        ? "deepen"
        : topic.includes("시즌") || topic.startsWith("시즌 새로고침:")
          ? "refresh"
          : "fresh",
    })),
    byChannel: {
