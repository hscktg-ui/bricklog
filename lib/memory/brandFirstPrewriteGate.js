...
import { stampCoreRulesOnInput } from "@/lib/product/briclogCoreRules";
import {
...
import { loadBrandTopicMemory } from "@/lib/memory/brandTopicMemory";
...
export async function prepareBrandFirstInput({
  ...
  if (!isBrandFirstEngineEnabled()) {
    return { ok: true, input: stampCoreRulesOnInput(input), personalization: null };
  }

  const nextInput = { ...input };
  const hasBrandScope = Boolean(nextInput.brandId || nextInput.brandMemory);
  if (!hasBrandScope) {
    return { ok: true, input: stampCoreRulesOnInput(nextInput), personalization: null };
  }
  ...
  if (nextInput.brandId) {
    nextInput.topicMemoryLearned = await loadBrandTopicMemory(
      supabase,
      userId,
      nextInput.brandId
    );
  ...
  return {
    ok: true,
    input: stampCoreRulesOnInput(nextInput),
    personalization: {
      ...personalization,
