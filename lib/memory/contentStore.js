...
/**
 * DB content_item → 편집기용 구조 복원
 * @param {"blog"|"place"|"instagram"|"image"} channel
 * @param {object} item
 */
export function pipelineContentFromMemoryItem(channel, item) {
  if (!item) return null;
  const pi = item.prompt_input || item.promptInput || {};
  if (pi.structured_content && typeof pi.structured_content === "object") {
    return pi.structured_content;
  }
  const fc = String(item.full_content || item.fullContent || "").trim();
  if (fc.startsWith("{")) {
    try {
  ...
  if (channel === "blog") {
    return {
      title: item.title || "블로그",
      sections: [{ heading: "", body: fc }],
      conclusion: "",
      hashtags: [],
    };
  }
  if (channel === "place") {
    return { title: item.title || "플레이스", body: fc, shortNotice: fc };
  }
  if (channel === "instagram") {
    return { hook: item.title || "", body: fc, lineBreakBody: fc, hashtags: [] };
  }
  return null;
}
