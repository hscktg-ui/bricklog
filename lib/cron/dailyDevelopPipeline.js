...
import { runWeeklySelfEvolutionAnalysis } from "@/lib/evolution/selfEvolutionCore";
...
export async function runDailyDevelopPipeline(options = {}) {
  ...
  const insightAutoApprove = await autoApprovePendingInsights(db);
  const selfEvolution = await runWeeklySelfEvolutionAnalysis(db);
  ...
  const learning = {
    ...
    insightsAutoApproved: insightAutoApprove.approved ?? 0,
    insightsDeferred: insightAutoApprove.deferred ?? 0,
    insightsOk: insightResult.ok ?? false,
    selfEvolution: selfEvolution.ok ? selfEvolution.metrics : null,
    selfEvolutionImprovements: selfEvolution.improvements || [],
