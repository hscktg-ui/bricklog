...
import {
  ...
  rollbackAutoApprovalsIfQualityDropped,
...
import { loadLatestCrossChannelBatchSummary } from "@/lib/ops/batchSummaryStore";
...
export async function runNightlyEvolutionPipeline(options = {}) {
  ...
  appendNightlyEvolutionEvent(
    ...
    approved.ok !== false
      ? `인사이트 자동 승인 — 승인 ${approved.approved ?? 0}건 · 보류 ${approved.deferred ?? 0}건`
      : "인사이트 자동 승인을 진행하지 못했습니다.",
    {
      step: "insights_approve",
      approved: approved.approved ?? 0,
      deferred: approved.deferred ?? 0,
      sampleSize: approved.metrics?.sampleSize ?? null,
    }
  );
  ...
  if (!options.skipTraining) {
    ...
    try {
      ...
      if (quality.skipped) {
        ...
        if (db && approved?.approved > 0) {
          const rollback = await rollbackAutoApprovalsIfQualityDropped(db, approved, quality);
          if (rollback.rolledBack > 0) {
            appendNightlyEvolutionEvent("progress", "품질 급락으로 자동 승인 규칙을 롤백했습니다.", {
              step: "insights_rollback",
              rolledBack: rollback.rolledBack,
              avgScore: rollback.avgScore,
            });
          }
        }
      } else {
  ...
  const batchSummary = await loadLatestCrossChannelBatchSummary(db);
