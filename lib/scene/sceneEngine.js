...
export function pickScene(ctx, flavor) {
  ...
  return {
    industryKey: key,
    sceneId: scene.id,
    heading: fillTemplate(scene.heading, ctx),
    opener: fillTemplate(pick(scene.opener, subSeed), ctx),
    experienceLines: (scene.experience || []).map((l, i) =>
      fillTemplate(l, subSeed + i)
    ),
    bridgeLines: (scene.bridge || []).map((l, i) =>
