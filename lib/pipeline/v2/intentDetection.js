...
export function detectContentIntent(profile = {}, ctx = {}) {
  ...
  const informational = isInformationalTopicInput({
    topic: profile.topic,
    mainKeyword: profile.mainKeyword || profile.topic,
    includePhrases: profile.includeList?.join(","),
  });

  if (informational) {
    if (/비교|vs|차이/.test(blob)) intent = "compare";
    else if (
      /가이드|알아두|방법|팁|고르는\s*법|종류|계절|여름|가을|봄|겨울|시즌/.test(blob)
    ) {
      intent = "guide";
    } else if (/상품|메뉴|구성|제품/.test(blob)) intent = "product_intro";
    else intent = "info";
  } else if (/체험|후기|다녀|방문해\s*보|솔직/.test(blob)) intent = "visit_review";
