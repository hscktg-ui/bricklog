...
const CROSS_INDUSTRY_MARKERS = {
  flower: [
    /꽃다발|생화|플라워|화병|꽃\s*리본|포장\s*리본|꽃집|무인꽃|기념일\s*꽃/,
    /향기(?:와|를)?\s*컬러|향기\s*톤|여름\s*꽃|여름철\s*꽃/,
    /리본(?:·|과)?\s*카드|꽃\s*특징|컬러\s*톤|꽃\s*보관/,
  ],
  ...
  hospital: [
    ...
    /(?:^|[\s·])클리닉(?:[\s·,]|$)/,
    /(?:^|[\s·])환자(?:[\s·,]|$)/,
    /(?<![가-힣])의사(?:가|는|를|와|의|선|님|진)/,
  ],
  furniture: [/소파|매트리스|모션\s*베드|모션베드|헤드\s*각도|인테리어\s*쇼룸|가구\s*배송|스트레스리스|stressless|리클라이너|쇼파|붙박이|체어|다이닝/],
  carwash: [/세차|왁스|코팅|차량\s*관리/],
  academy: [/수업|강사|입시|학원|교육\s*과정/],
  education: [/수업|강사|입시|학원|교육\s*과정/],
  craft: [/원데이|체험\s*클래스|도자기|공방|핸드메이드/],
  restaurant: [/맛집|메뉴판|예약\s*식당|코스\s*요리/],
  salon: [/헤어\s*펌|펌\s*시술|염색|네일|헤어\s*디자인/],
...
const COMPATIBLE_INDUSTRY = {
  ...
  pet: ["snack", "pet_cafe"],
  pet_cafe: ["pet", "cafe"],
  cafe: ["pet_cafe", "tea_cafe"],
  academy: ["education"],
  education: ["academy"],
};

function normalizeLockedKey(lockedKey) {
  return LOCK_KEY_ALIASES[lockedKey] || lockedKey;
}
...
export function detectIndustryCrossContamination(text, lockedKey) {
  const t = String(text || "");
  const locked = normalizeLockedKey(lockedKey || "default");
  const violations = [];

  // 업종 미확정(default)에서는 교차 오염 hard-fail 금지 — 주제 자체 키워드를 타업종으로 오판
  if (!locked || locked === "default") {
    return { ok: true, lockedKey: locked || "default", violations: [] };
  }

  for (const key of ALL_INDUSTRY_KEYS) {
    if (key === locked) continue;
    if (normalizeLockedKey(key) === locked) continue;
...
const INDUSTRY_INTRUSION_PHRASES = {
  ...
  academy: [/색감·보관|성분·보관|알레르기·원재료|첨가물·알레르기|선물·반려/],
  education: [/색감·보관|성분·보관|알레르기·원재료|첨가물·알레르기|선물·반려/],
  craft: [/색감·보관|성분·보관|알레르기·원재료|첨가물·알레르기|선물·반려|급여\s*방법/],
  pension: [/색감·보관|성분·보관|알레르기·원재료|염색|펌\s*시술/],
  construction: [/색감·보관|성분·보관|알레르기·원재료|염색|펌\s*시술/],
  salon: [/알레르기·원재료|성분·보관·선물|첨가물·알레르기|급여\s*방법/],
  marketing: [/색감·보관|성분·보관|알레르기·원재료|염색|펌\s*시술/],
