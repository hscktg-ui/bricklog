...
const PATTERNS = [
  ...
  { id: "testimonial", re: /testimonial|실구매자 후기|별점|testimonial cards/i, weight: 20 },
  { id: "saas_hero", re: /data-layout="website-hero"|generic website hero|Get started|무료로 시작/i, weight: 18 },
  { id: "rounded_card", re: /rounded card|data-ui="card"|box-shadow:\s*0 20px/i, weight: 16 },
  { id: "feature_grid", re: /feature grid|data-layout="feature-grid"/i, weight: 16 },
  { id: "accordion", re: /<details\b|accordion/i, weight: 16 },
  { id: "dashboard", re: /dashboard|browser interface/i, weight: 18 },
];
...
export function scoreDetailPageVisualCritic(html = "", extra = {}) {
  ...
  const source = String(html || "");
  const canvas = source.includes('data-renderer="detail-canvas"');
  const imageDominant =
    canvas ||
    ((source.match(/data-photo-slot=/g) || []).length >= 2 &&
      source.includes("data-image-ratio="));
  const productFocus = canvas
    ? 92
    : source.includes('data-deliverable="mall-image"')
      ? 90
      : 40;
  return {
    version: DETAIL_PAGE_WEB_UI_GUARD_VERSION,
    ...
    purchase_desire: canvas ? 86 : 62,
    visual_quality: imageDominant ? 90 : 62,
    composition_quality: source.includes("data-composition=") ? 88 : 50,
