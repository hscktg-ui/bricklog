...
export function scoreCanvasFrame(frame, html = "") {
  ...
  const infoCanvas = frame.composition === "specification" && (frame.facts || []).length >= 3;
  ...
  let purchase_desire = clamp(
    (hasPhoto ? 50 : 16) +
      (["hero", "close", "table", "cook", "package", "origin", "texture", "mill", "detail", "choose", "roast", "label", "grind", "brand"].includes(frame.purpose) ? 28 : 16) +
      (cov >= 0.7 ? 12 : 6)
  );
  if (infoCanvas) {
