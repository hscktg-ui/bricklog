 ...
 * HTML 마커만으로 출고하지 않는다. 페이지 이미지(860 PNG)를 본 뒤에 hire.
 */
...
export const DETAIL_PAGE_DESIGNER_PANEL_VERSION = "gollaboda-designer-30-v3";
...
/** @param {object} pack @param {string} [html] @param {number} [photoCount] @param {object} [screenshots] */
export function measureDetailPageForDesigners(pack, html = "", photoCount = 0, screenshots) {
  ...
  const canvas = htmlStr.includes('data-renderer="detail-canvas"');
  const directed = (htmlStr.match(/data-photo-direction=/g) || []).length;
  const firstGlance = htmlStr.includes('data-visual="first-glance"');
  const mallReady =
    htmlStr.includes("data-mall-ready") || htmlStr.includes("data-mall=");
  const rushCta = /서두르세|지금\s*바로|바로 담기|지금 구매/.test(text);
  const hierarchy = clamp(
    (htmlStr.includes("font-size:38px") || canvas ? 28 : 8) +
      (types.has("hero") ? 24 : 0) +
      (headline.length >= 8 && headline.length <= 48 ? 22 : 8) +
      (htmlStr.includes('data-hero="canvas"') || htmlStr.includes('data-hero="plate"')
        ? 16
        : htmlStr.includes("letter-spacing:0.16em")
          ? 10
          : 6) +
      (firstGlance ? 8 : 0) +
      (directed >= 3 ? 1 : 0) +
  ...
  const type = clamp(
    (/pretendard|noto serif|noto sans|ibm plex|gowun|hahmlet|song myung|gothic a1|nanum|jua|playfair|fraunces|cormorant|outfit|instrument serif|dm sans|fonts\.googleapis/i.test(
      ...
      (htmlStr.includes("line-height:1.55") ||
      htmlStr.includes("line-height:1.7") ||
      htmlStr.includes("line-height:1.9")
        ? 16
        : 8) +
      (firstGlance ? 3 : 0)
  );
  ...
  const filledShots = (htmlStr.match(/<img /g) || []).length;
  const briefs = (htmlStr.match(/data-photo-brief=/g) || []).length;
  const photo = clamp(
    filledShots >= 3 && directed >= 3 && firstGlance
      ? 99
      : filledShots >= 1 && directed >= 3 && (briefs >= 2 || emptyWells >= 2) && firstGlance
        ? 91
        : filledShots >= 3
          ? 86
          : photoCount >= 3
            ? 78 + (directed >= 3 ? 6 : 0)
            : filledShots === 2 || photoCount === 2
              ? 64
              : filledShots === 1 || photoCount === 1
                ? 52
  ...
  const intent = clamp(
    (types.has("intent") ? 40 : 8) +
      ...
      (types.has("observe") ? 13 : 0) +
      (padHits > 4 ? -16 : 0)
  );
  const trust = HARD_SELL.test(text)
    ? 32
    : padHits === 0
      ? 99
      : clamp(86 - Math.min(20, padHits * 3));
  const cta = clamp(
    (types.has("cta") ? 40 : 10) +
  ...
  const commercial = clamp(
    (htmlStr.includes("860px") ? 34 : 0) +
      ...
      (htmlStr.includes('data-deliverable="mall-image"') ? 16 : 4) +
      (htmlStr.includes("data-ui-elements=\"0\"") ? 10 : 0) +
      (!/<button\b|role="button"/.test(htmlStr) ? 8 : 0) +
      (htmlStr.includes('data-ui="ad-canvas"') || htmlStr.includes('data-ui="section-layouts"')
        ? 8
        : 4) +
      (mallReady ? 6 : 0) +
      (firstGlance ? 1 : 0)
  );
  const bodyParas = (htmlStr.match(/data-role="lead"/g) || []).length;
  const visualLayouts = canvas
    ? Math.min(8, (htmlStr.match(/data-composition="/g) || []).length)
    : [
        'data-layout="usp-cards"',
        'data-layout="usp-rows"',
        'data-layout="spec-sheet"',
        'data-layout="problem-band"',
        'data-layout="choose-steps"',
        'data-layout="points-5"',
        'data-layout="compare-pair"',
        'data-layout="reason-band"',
        'data-layout="brand-dark"',
        'data-layout="brand-quiet"',
        'data-layout="observe-quote"',
        'data-layout="feature-list"',
        'data-layout="faq"',
        'data-layout="cta-bar"',
      ].filter((token) => htmlStr.includes(token)).length;
  const density = clamp(
    visualLayouts * 12 +
      (bodyParas <= 8 ? 22 : bodyParas <= 14 ? 10 : 0) +
      (uniqueness >= 70 ? 16 : uniqueness >= 50 ? 8 : 4) +
      (pack?._meta?.densityOk ? 8 : 0) +
      (pack?._meta?.visualOk ? 8 : 0) +
      (padHits > 3 ? -20 : 0)
  );
  const color = clamp(
    (/#9a3412|#5c4033|#3f6b4a|#03a94d|#16120e|#14110e/.test(htmlStr) ? 36 : 16) +
      (htmlStr.includes('data-hero="canvas"') ||
      htmlStr.includes('data-hero="plate"') ||
      htmlStr.includes('data-nameplate="1"')
        ? 22
        : htmlStr.includes("border-radius:999px")
          ? 10
          : 10) +
...
function emptyWellsNote(m) {
  if (m.slotCount >= 3) {
    return "컷 칸은 잡혀 있습니다. 빈 칸은 상품 사진을 생성해야 완성입니다.";
  }
...
function axisNote(designer, m) {
  ...
  if (lens === "density" && (m.bodyParas > 14 || m.visualLayouts < 3)) {
    return "정보가 표·카드·순서칸이 아니라 본문 문장으로 늘었습니다.";
  }
  ...
  return "고르는 화면이 붙습니다. 패딩·하드셀만 없으면 출고선입니다.";
}
...
/**
 * @param {{ pack: object, html?: string, photoCount?: number, screenshots?: object }} input
 */
export function evaluateDetailPageDesignerPanel(input) {
  const pack = input?.pack || {};
  const html = String(input?.html || "");
  const photoCount = Number(input?.photoCount || 0);
  const measured = measureDetailPageForDesigners(
    pack,
    html,
    photoCount,
    input?.screenshots
  ...
  const issues = [];
  if (!measured.lookedAtImage) issues.push("페이지 이미지를 디자이너가 안 봄");
  if (measured.uniqueness < 60) issues.push("같은 마침 문장 반복");
  if (measured.photo < 80) issues.push("사진 리듬 부족");
  if (measured.hierarchy < 70) issues.push("첫 화면 위계");
  if (measured.density < 60) issues.push("정보 밀도보다 글자 수");
  if (measured.visualLayouts < 3) issues.push("리스트가 화면 모듈이 아님");
  const hire =
    mean >= 90 &&
    measured.uniqueness >= 58 &&
    measured.commercial >= 70 &&
    measured.lookedAtImage;
  return {
