/**
 * 골라보다 결과물 — 스마트스토어·쿠팡 상세 디자이너 30인 패널
...
export function measureDetailPageForDesigners(pack, html = "", photoCount = 0) {
  ...
  const type = clamp(
    (/pretendard/i.test(htmlStr) ? 30 : 0) +
      (htmlStr.includes("860px") ? 28 : 0) +
      (htmlStr.includes("word-break:keep-all") ? 22 : 8) +
      (htmlStr.includes("line-height:1.9") ? 16 : 8)
  );
  const photo = clamp(photoCount <= 0 ? 38 : photoCount === 1 ? 52 : photoCount === 2 ? 64 : photoCount >= 3 ? 78 : 70);
  const intent = clamp(
