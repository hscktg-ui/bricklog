...
export function measureDetailPageForDesigners(pack, html = "", photoCount = 0, screenshots) {
  ...
  const commercial = clamp(
    (htmlStr.includes("860px") ? 34 : 0) +
      ...
      (htmlStr.includes('data-layout="problem-band"') ||
      htmlStr.includes('data-layout="choose-steps"')
        ? 4
        : 0) +
      (htmlStr.includes('data-layout="usp-rows"') || htmlStr.includes('data-layout="usp-cards"') ? 3 : 0) +
      (htmlStr.includes('data-layout="points-5"') ||
      htmlStr.includes('data-layout="reason-band"') ||
      htmlStr.includes('data-layout="compare-pair"')
        ? 2
        : 0) +
      (mallReady ? 6 : 0) +
      (firstGlance ? 1 : 0)
  ...
  const color = clamp(
    (/#9a3412|#5c4033|#3f6b4a|#03a94d/.test(htmlStr) ? 36 : 16) +
      (htmlStr.includes('data-hero="plate"') || htmlStr.includes('data-nameplate="1"')
        ? 22
