...
import { buildFormBlogProxy } from "@/lib/contentPipeline";
...
export async function resolvePersonaBlogPack(rawInput = {}, persona = {}) {
  ...
  const input = applyV2PersonaToInput(
  ...
  if (prep.ok) {
    ...
    if (pack?.sections?.length) {
      return { pack, mode: "search_enriched_draft", qualityScore: null };
    }
  }

  return {
    pack: buildFormBlogProxy(input),
    mode: "form_proxy",
    qualityScore: null,
  };
}
