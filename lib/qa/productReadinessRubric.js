  ...
  return [
    {
      ...
      score: Math.min(
        ...
        round(
          7.5 +
            slaRatio * 2.5 +
            (s.engineOpsOk && Number(s.channelSlaPassCount) === 0 ? 1.5 : 0)
        )
      ),
      note:
        Number(s.channelSlaPassCount) > 0
          ? `채널 SLA ${slaPass}/${slaTotal}`
          : s.engineOpsOk
            ? "엔진 정상 · SLA 아티팩트 갱신 대기"
            : `채널 SLA ${slaPass}/${slaTotal}`,
    },
    {
      id: "quality_engine",
    ...
    },
    {
      ...
      label: "인증·온보딩",
      max: 4,
      score: s.smsOtpTable ? 4 : 3.5,
    },
    {
      id: "billing",
      label: "결제·요금",
      max: 4,
      // 베타: 무료 런치면 결제 미연결을 감점하지 않음
      score: s.pgCheckoutReady
        ? 3.5
        : s.betaFreeLaunch || s.billingFreeLaunch || s.inicisReview
          ? 4
          : 2.5,
      note: s.pgCheckoutReady
        ? `${s.pgProviderLabel || "PG"} 연결`
        : s.betaFreeLaunch || s.billingFreeLaunch
          ? "베타 무료 런치 — 결제 후순위"
          : s.inicisReview
            ? "베타 무료 · PG 심사 중"
            : "베타 무료·PG 미연결",
...
function scoreUser(s) {
  ...
  return [
    ...
    },
    {
      ...
      max: 8,
      score:
        s.publishReadyRate != null
          ? round(4 + Math.min(1, Number(s.publishReadyRate) / 0.7) * 4)
          : s.alwaysCompleteDelivery
            ? 8
            : 6,
      note:
        s.publishReadyPercent != null
          ? `publish-ready ${s.publishReadyPercent}%`
          : undefined,
    },
    {
      id: "speed",
...
function speedScore(s) {
  ...
  const targetMs = Number(s.customerSlaMs) || 30_000;
  if (!ms) {
    // 측정 아티팩트 부재 시 엔진 정상이면 베타 기본 점수
    return s.engineOpsOk ? 5.5 : 4;
  }
  if (ms <= targetMs * 1.2) return 6;
