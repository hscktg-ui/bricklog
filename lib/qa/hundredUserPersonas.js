...
/** 업종 행 ↔ 데모 풀 인덱스 (row만으로 뽑으면 멍멍살롱×숙박 같은 불일치 발생) */
const INDUSTRY_BRAND_SEED = {
  cafe: QUICK_DEMO_POOL[0],
  flower: QUICK_DEMO_POOL[3],
  medical: QUICK_DEMO_POOL[5],
  beauty: QUICK_DEMO_POOL[1],
  academy: QUICK_DEMO_POOL[2],
  restaurant: QUICK_DEMO_POOL[6],
  retail: QUICK_DEMO_POOL[14] || QUICK_DEMO_POOL[12],
  fitness: QUICK_DEMO_POOL[7],
  professional: {
    brandName: "해신기획",
    region: "파주",
    topic: "블로그 마케팅 상담 후기",
    mainKeyword: "파주 블로그 마케팅",
  },
  lodging: QUICK_DEMO_POOL[4],
};

function seedBrand(rowIndex) {
  const industryId = INDUSTRIES[rowIndex]?.id || "cafe";
  const seed =
    INDUSTRY_BRAND_SEED[industryId] ||
    QUICK_DEMO_POOL[rowIndex % QUICK_DEMO_POOL.length];
  return {
...
function buildPersona(row, col, index) {
  ...
  const device =
    journey.device === "mobile" ||
    journey.device === "tablet" ||
    journey.device === "desktop"
      ? journey.device
