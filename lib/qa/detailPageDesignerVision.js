...
import { isGrokConfigured } from "@/lib/llm/grokClient";
import { reviewDetailPageWithGrok } from "@/lib/product/detailPageGrokDirector";
...
const SYSTEM = [
  ...
  "읽는 순서는 후킹>정보>나열>재료>원료>스펙>패키지>배송>필수정보. 웹 랜딩이 아니다.",
  "출고 OK: 깨진 칸 없음, 첫눈에 상품 사진, 글자 벽이 아님, 가짜 후기·모델컷·지금 바로 구매 없음. 점수는 90 이상이어야 출고.",
  "감점하면 안 되는 것: 가격·혜택·강한 CTA 없음(약한 안내가 맞다).",
  "감점하는 것: 웹 관리자 카드 UI, FAQ 아코디언, 같은 항목을 카드로 두 번 반복.",
...
export async function reviewDetailPageDesignerImage(args = {}) {
  ...
  if (isGrokConfigured()) {
    try {
      const grok = await reviewDetailPageWithGrok({
        heroDataUrl: hero,
        midDataUrl: mid,
