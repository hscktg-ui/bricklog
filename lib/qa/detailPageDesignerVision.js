...
export const DETAIL_PAGE_DESIGNER_VISION_VERSION = "gollaboda-designer-vision-v1";
...
const SYSTEM = [
  ...
  "출고 OK: 깨진 칸 없음, 첫눈에 상품 사진, 글자 벽이 아님, 가짜 후기·모델컷·지금 바로 구매 없음.",
  "감점하면 안 되는 것: 가격·혜택·강한 CTA 없음(약한 안내가 맞다), 없는 연출컷·밥 클로즈업 요구, 막히는 점부터 시작하는 카피.",
...
export async function reviewDetailPageDesignerImage(args = {}) {
  ...
  const issues = Array.isArray(parsed.issues)
    ...
    : [];
  const ok =
    score >= 70 && parsed.isImageNotEssay !== false && parsed.firstGlance !== false;
