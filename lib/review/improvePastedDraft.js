...
사용자가 직접 쓴 캡션을 저장·공감에 맞게 다듬습니다.
- 첫 줄 Hook에 주제/키워드를 자연스럽게 넣습니다.
- 이후 1~2문장마다 줄바꿈, 본문 200~480자(공백 제외).
- 블로그 설명문·체크리스트·검색창 같은 표현은 피합니다.
- 사실은 원문만 유지, 과장·AI 클리셰를 줄입니다.
- 업종에 맞는 이모지 3~6개, 줄당 1개 이하.
...
${text.slice(0, 12000)}${
  ...
  try {
    ...
    let { pack, text: improvedText } = formatImprovedOutput(channel, parsed);
    if (channel === "instagram") {
      pack = strengthenInstagramCaption(pack, params);
      improvedText = formatInstaFullCopy(pack);
    }
