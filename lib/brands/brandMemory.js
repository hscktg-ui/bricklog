...
function resolveBrandTopicSeed(brand) {
  const preview = brand?.recentContent?.blog?.preview;
  if (preview?.trim()) return String(preview).trim().slice(0, 120);
  const kw = brand?.mainKeyword?.trim();
  if (kw) return kw;
  if (brand?.brandName?.trim()) return `${brand.brandName.trim()} 소식`;
  return "";
}
...
export function brandMemoryToFormInput(brand) {
  ...
  const forbidden = [
    ...
    .filter(Boolean)
    .join(", ");
  const topicSeed = resolveBrandTopicSeed(brand);
  ...
  return {
    ...
    topic: topicSeed,
    mainKeyword: brand.mainKeyword || brand.preferredKeywords?.split(",")?.[0] || topicSeed,
    ...
    writingSkillLevel:
      brand.writingSkillLevel || DEFAULT_BLOG_INPUT.writingSkillLevel,
    blogLengthTier:
      brand.blogLengthTier || DEFAULT_BLOG_INPUT.blogLengthTier,
  };
}
...
export function mergeBrandFromForm(brand, formInput) {
  return {
    ...
    mainKeyword: formInput.mainKeyword || formInput.topic,
