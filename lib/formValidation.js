...
export function validateForm(values, options = {}) {
  const axes = options.brandHooks
    ? resolveBlogFormAxes(values, options.brandHooks)
    : values;
  const errors = {};

  if (!axes.region?.trim() || axes.region.trim().length < 2) {
    errors.region = "지역을 입력해 주세요";
  }

  const topic = axes.topic?.trim();
  const brand = axes.brandName?.trim();
  const main = axes.mainKeyword?.trim();
  const hasStory = Boolean(topic || main);

  if (!brand) {
    errors.brandName = "브랜드명을 입력해 주세요";
  } else if (isPlaceholderBrandName(brand)) {
    errors.brandName = "매장·브랜드 이름을 바꿔 입력해 주세요";
  }
  if (!hasStory) {
...
export function isFormValid(values, options = {}) {
  return Object.keys(validateForm(values, options)).length === 0;
}
